...
public class GUIBank extends GuiScreen {
    ...
    public void sendCoinsMoved() {
        if (this.coinsMoved != 0) {
            ...
            packet.writeInts(this.tile.getPos().getX(), this.tile.getPos().getY(), this.tile.getPos().getZ(), this.coinsMoved, 0, 0);
    ...
    public void playsoundCoin() {
        ...
        if (player != null) {
            player.world.playSound(player.posX, player.posY, player.posZ, Sounds.grap_money, SoundCategory.PLAYERS, 0.3F, 0.9F + player.getRNG().nextFloat() / 5.0F, false);
        }
    }
