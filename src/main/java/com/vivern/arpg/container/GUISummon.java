...
public class GUISummon extends GuiScreen {
            ...
            for (int i = 0; i < 99; i++) {
                if (filtersTAG.hasKey("" + i)) {
                    ...
                    if (filter.hasKey("mode") && filter.hasKey("data") && filter.hasKey("inverted")) {
                        FilterDraw draw = new FilterDraw(filter.getInteger("mode"), i, filter.getString("data"), false, filter.getBoolean("inverted"));
        ...
        if (wand.hasTagCompound() && wand.getTagCompound().hasKey(filterType)) {
            ...
            for (int ix = 0; ix < 99; ix++) {
                if (filtersTAG.hasKey("" + ix)) {
                    ...
                    if (filter.hasKey("mode") && filter.hasKey("data") && filter.hasKey("inverted")) {
                        FilterDraw draw = new FilterDraw(filter.getInteger("mode"), ix, filter.getString("data"), true, filter.getBoolean("inverted"));
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (my > 0 && my <= 16 && mx > 23 && mx <= 39) {
            ...
            if (max < 9) {
                ...
                this.listMinion.add(new FilterDraw(this.filterModeSelecMinion, max, this.filterMinionEdit.getText(), false, this.filterModeInvertedMinion));
            }
            ...
            if (maxx < 9) {
                ...
                PacketHandler.NETWORK.sendToServer(packet2);
                this.listHostil.add(new FilterDraw(this.filterModeSelecHostil, maxx, this.filterHostilEdit.getText(), true, this.filterModeInvertedHostil));
            }
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        if (!this.filterMinionEdit.isFocused() && !this.filterHostilEdit.isFocused() && (keyCode == 1 || this.mc.gameSettings.keyBindInventory.isActiveAndMatches(keyCode))) {
