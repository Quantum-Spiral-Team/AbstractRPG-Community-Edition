...
public class GUIResearchTable extends GuiContainer {
                    ...
                    if (fullAmounts > 0) {
                        ...
                        for (int ixxx = 1; ixxx <= 12; ixxx++) {
                            ...
                            if (amount > 0) {
                                ...
                                if (shardType != null) {
                                    this.fontRenderer.drawString(amount + "%", mouseX + 20, mouseY + 3 + var23, ColorConverters.RGBtoDecimal(shardType.colorR, shardType.colorG, shardType.colorB));
                                }
        ...
        if (this.specialization == 2) {
            ...
            if (puzzle != null && puzzleRender != null) {
                ...
                if (puzzle.movingOrbs > 0) {
                    ...
                    drawModalRectWithCustomSizedTexture2(xI, yI, orbsStart, 0.0F, orbsOne, orbsOne, Phenomenons.tfr_sprites_sizeX / 3.0F, Phenomenons.tfr_sprites_sizeY / 3.0F);
                ...
                if (puzzle.splitOrbs > 0) {
                    ...
                    int yI = j + 114;
                    this.mc.getTextureManager().bindTexture(Phenomenons.tfr_sprites);
                    drawModalRectWithCustomSizedTexture2(xI, yI, orbsStart + orbsOne + orbsOne, 0.0F, orbsOne, orbsOne, Phenomenons.tfr_sprites_sizeX / 3.0F, Phenomenons.tfr_sprites_sizeY / 3.0F);
        ...
        if (this.specialization == 3) {
            ...
            if (this.tile.blanks != null) {
                ...
                for (int bl = (int) blMin; bl < blMax; bl++) {
                    ...
                    this.drawTexturedModalRectFromTo(i + 86 + (int) (xPos * 52.0F), j + 65, 0, 0, 75, MathHelper.clamp(absFromX, 0, 52), MathHelper.clamp(absToX, 0, 52));
                }
            }
            ...
            if (this.tile.blanks != null) {
                ...
                for (int bl = (int) blMin; bl < blMax; bl++) {
                    ...
                    if (blank != null) {
                        ...
                        this.tile.blanks[bl].renderInGui(this, i + 87 + (int) (xPos * 52.0F), j + 65, MathHelper.clamp(absFromX, 0, blank.sizeX), MathHelper.clamp(absToX, 0, blank.sizeX), 0, blank.sizeY);
                    }
                }
            }
        }
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (this.specialization == 2 && puzzle != null && puzzleRender != null && mouseButton == 0) {
                ...
                for (int bx = 2048; bx > 0; bx >>>= 1) {
                    if ((inerflowBit & bx) > 0) {
                        ...
                        if (mouseX > minx && mouseY > 10 && mouseX < maxx && mouseY < 24) {
                            this.puzzleCommand = new TerraformingPlayerCommand(TerraformingPlayerCommand.TRPlayerCommandType.SPLIT, puzzle.selected.phenomenon.posX, puzzle.selected.phenomenon.posY, idElement);
            ...
            if (splitNoPressed && mouseX > 10 && mouseY > 10 && mouseX < 210 && mouseY < 185) {
                ...
                if (puzzle.selected != null && puzzle.selected.phenomenon != null && (puzzle.movingOrbs > 0 || puzzle.canMove(puzzle.selected.phenomenon.posX, puzzle.selected.phenomenon.posY))) {
                ...
                if (puzzle.checkBounds(cellPos[0], cellPos[1]) && !moveClicked) {
                    ...
                    if (cell.orb != 0 && (cell.surfaceTerrain != null || cell.surfaceAtmosphere != null || cell.surfaceCreature != null)) {
                        ...
                        if (relativeMouseXx <= 10.0F && relativeMouseXx >= -10.0F && relativeMouseYx <= 10.0F && relativeMouseYx >= -10.0F) {
                            this.puzzleCommand = new TerraformingPlayerCommand(TerraformingPlayerCommand.TRPlayerCommandType.GET_ORB, cellPos[0], cellPos[1], 0);
                            ;
                            boardSelect = false;
                        }
                    }
                    ...
                    if (boardSelect) {
                        this.puzzleCommand = new TerraformingPlayerCommand(TerraformingPlayerCommand.TRPlayerCommandType.BOARD_SELECT, cellPos[0], cellPos[1], 0);
                        ;
                    }
                }
            }
            ...
            if (mouseX > 235 && mouseY > 174 && mouseX < 249 && mouseY < 188) {
                ...
                packet.writeInts(this.tile.getPos().getX(), this.tile.getPos().getY(), this.tile.getPos().getZ(), mouseX, mouseY, mouseButton);
    ...
    public void drawEnergyBar(int shardTypeId, float percentFilled, int x, int y, float percentIdeal) {
        ...
        if (shardTypeId != 0 && percentFilled > 0.0F) {
            this.drawTexturedModalRect(x, Math.round(y + 196.0F * (1.0F - percentFilled)), 6 * shardTypeId, Math.round(196.0F * (1.0F - percentFilled)), 6, Math.round(196.0F * percentFilled));
        }
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
        ...
        if (this.specialization == 2 && this.dragAndDropInventoryCell >= 0 && this.dragAndDropInventoryCell < 12 && puzzle != null && puzzleRender != null) {
            ...
            for (int e = 1; e <= 12; e++) {
                if (puzzle.inventory[e] > 0) {
                    if (invI == this.dragAndDropInventoryCell) {
                        this.puzzleCommand = new TerraformingPlayerCommand(TerraformingPlayerCommand.TRPlayerCommandType.PLACE_FROM_INVENTORY, pos[0], pos[1], e);
                        ;
                        break;
                    }
    ...
    public void drawGraph(WriteGraph graph, int x, int y) {
        ...
        for (Vec2i vertex : graph.vertexes) {
            drawModalRectWithCustomSizedTexture2(vertex.getX() + x - 3.75F, vertex.getY() + y - 3.75F, 113.0F, ((float) AnimationTimer.tick / 10 + i) % 11 * 7.5F, 7.5F, 7.5F, 128.0F, 128.0F);
            if (i % 2 == 0) {
                i += 7;
            } else if (i % 3 == 0) {
                i += 2;
