        ...
        if (this.tileinv instanceof TileCrystallizer) {
            ...
            if (f1 != null) {
                ...
                for (int ii = 0; ii < 64; ii += 16) {
                    ...
                    if (interpamount >= 16) {
                        RenderFluidHelper.drawTexturedFluidRect(i + xPoss, j + (hmin - 16), wi, 16, 1.0F, f1.getFluid(), f1, 0.0, (float) vec.x, (float) vec.y, (float) vec.z);
                        polygonheight = hmin - 16 - displace;
                    }

                    if (interpamount >= 32) {
                        RenderFluidHelper.drawTexturedFluidRect(i + xPoss, j + (hmin - 32), wi, 16, 1.0F, f1.getFluid(), f1, 0.0, (float) vec.x, (float) vec.y, (float) vec.z);
                        polygonheight = hmin - 32 - displace;
                    }

                    if (interpamount >= 48) {
                        RenderFluidHelper.drawTexturedFluidRect(i + xPoss, j + (hmin - 48), wi, 16, 1.0F, f1.getFluid(), f1, 0.0, (float) vec.x, (float) vec.y, (float) vec.z);
                        polygonheight = hmin - 48 - displace;
                    }

                    RenderFluidHelper.drawTexturedFluidRect(i + xPoss, j + displace, wi, polygonheight, polygonheight / 16.0F, f1.getFluid(), f1, 0.0, (float) vec.x, (float) vec.y, (float) vec.z);
                }
            }
