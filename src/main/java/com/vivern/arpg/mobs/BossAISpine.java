...
public class BossAISpine extends EntityAIBase {
    ...
    public BossAISpine(EntityCreature entity, int frequency, float radius, float speed, float radiusVar, float yOffset, float resetDistance, float attackChance, float friction, boolean checkInBlock) {
        this.entity = entity;
    ...
    public void updateTask() {
        if (this.entity.ticksExisted % 20 == 0) {
            double agrr = 50.0;
            AxisAlignedBB agraabb = new AxisAlignedBB(this.entity.posX - agrr, this.entity.posY - agrr, this.entity.posZ - agrr, this.entity.posX + agrr, this.entity.posY + agrr, this.entity.posZ + agrr);
        ...
        if (this.entity.getAttackTarget() != null) {
                ...
                if (this.entity.getRNG().nextFloat() > this.attackChance) {
                    if (this.yOffset == 0.0F) {
                        ...
                        if (this.checkInBlock) {
                            double radr = 0.1;
                            AxisAlignedBB aabb = new AxisAlignedBB(this.vec.x - radr, this.vec.y - radr, this.vec.z - radr, this.vec.x + radr, this.vec.y + radr, this.vec.z + radr);
                            if (this.entity.world.collidesWithAnyBlock(aabb)) {
                                this.delay = 0;
                            }
                        }
                    } else {
                        this.vec = GetMOP.findRandPosNearPoint(this.entity.getAttackTarget().posX, this.entity.getAttackTarget().posY + this.yOffset, this.entity.getAttackTarget().posZ, this.radius + this.entity.getRNG().nextFloat() * this.radiusVar);
                        if (this.checkInBlock) {
                            double radr = 0.1;
                            AxisAlignedBB aabb = new AxisAlignedBB(this.vec.x - radr, this.vec.y - radr, this.vec.z - radr, this.vec.x + radr, this.vec.y + radr, this.vec.z + radr);
                            if (this.entity.world.collidesWithAnyBlock(aabb)) {
            ...
            SuperKnockback.applyMove(this.entity, -this.speed, this.vec.x, this.vec.y + this.entity.getAttackTarget().height / 2.0F, this.vec.z);
        }
    }
