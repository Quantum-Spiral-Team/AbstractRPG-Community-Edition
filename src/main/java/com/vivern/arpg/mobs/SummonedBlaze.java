...
public class SummonedBlaze extends EntitySummoned {
    ...
    public void expelling() {
        for (int ss = 0; ss < 10; ss++) {
            GUNParticle bigsmoke = new GUNParticle(largesmoke, 0.4F + (float) this.rand.nextGaussian() / 20.0F, 0.0F, 12, 80, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 23.0F, (float) this.rand.nextGaussian() / 27.0F, (float) this.rand.nextGaussian() / 23.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
    ...
    public void spawnBetwParticle(Vec3d from, Vec3d to) {
        if (from.lengthSquared() > 1.0E-6 && to.lengthSquared() > 1.0E-6) {
            BetweenParticle laser = new BetweenParticle(this.world, texturelaser, 0.35F, 240, 1.0F, 1.0F, 1.0F, 0.15F, from.distanceTo(to), 5, 0.3F, 4.0F, from, to);
            ...
            for (int ss = 0; ss < 5; ss++) {
                GUNParticle bigsmoke = new GUNParticle(largesmoke, 0.4F + (float) this.rand.nextGaussian() / 20.0F, 0.0F, 12, 80, this.world, to.x, to.y, to.z, (float) this.rand.nextGaussian() / 23.0F, (float) this.rand.nextGaussian() / 27.0F, (float) this.rand.nextGaussian() / 23.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
                this.world.spawnEntity(bigsmoke);
            }

            for (int ss = 0; ss < 8; ss++) {
                GUNParticle fire = new GUNParticle(sparkle, 0.08F + (float) this.rand.nextGaussian() / 30.0F, 0.01F, 10, 240, this.world, to.x, to.y, to.z, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F + 0.15F, (float) this.rand.nextGaussian() / 14.0F, 1.0F, 0.8F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, 0);
    ...
    public void onUpdate() {
        ...
        if (attackTarg != null) {
            ...
            if (this.firedelay < 1) {
                ...
                double range = 0.5;
                AxisAlignedBB aabb = new AxisAlignedBB(vec.x - range, vec.y - range, vec.z - range, vec.x + range, vec.y + range, vec.z + range);
                List<EntityLivingBase> list = this.world.getEntitiesWithinAABB(EntityLivingBase.class, aabb);
                this.spawnBetwParticle(pos1, vec);
                this.world.playSound(this.posX, this.posY, this.posZ, Sounds.fire, SoundCategory.AMBIENT, 0.9F, 0.95F + this.rand.nextFloat() / 10.0F, false);
        ...
        if (this.followPoint != null) {
            ...
            this.motionZ /= 2.0;
            if (this.owner != null) {
                SuperKnockback.applyMove(this, ColorConverters.InBorder(-0.4F, -0.1F, -this.getDistance(this.owner) / 5.0F), this.followPoint.x, this.followPoint.y, this.followPoint.z);
            }
        }
        ...
        GUNParticle fire2 = new GUNParticle(flame, 0.2F + (float) this.rand.nextGaussian() / 15.0F, -0.009F, 10 + this.rand.nextInt(5), 240, this.world, this.posX, this.posY + 1.0, this.posZ, 0.0F, -0.15F, 0.0F, 1.0F, 0.8F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, this.rand.nextInt(100) - 50);
