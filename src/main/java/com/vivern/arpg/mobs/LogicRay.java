...
public class LogicRay {
    ...
    public static Vec3d[] offsets = new Vec3d[]{new Vec3d(1.0, 0.0, 1.0), new Vec3d(-1.0, 0.0, 1.0), new Vec3d(1.0, 0.0, -1.0), new Vec3d(-1.0, 0.0, -1.0), new Vec3d(1.0, 1.0, 0.0), new Vec3d(-1.0, 1.0, 0.0), new Vec3d(0.0, 1.0, 1.0), new Vec3d(0.0, 1.0, -1.0), new Vec3d(1.0, -1.0, 0.0), new Vec3d(-1.0, -1.0, 0.0), new Vec3d(0.0, -1.0, 1.0), new Vec3d(0.0, -1.0, -1.0), new Vec3d(1.0, 0.0, 0.0), new Vec3d(-1.0, 0.0, 0.0), new Vec3d(0.0, 0.0, 1.0), new Vec3d(0.0, 0.0, -1.0), new Vec3d(0.0, 1.0, 0.0), new Vec3d(0.0, -1.0, 0.0)};
    ...
    public void checkShortRay(World world, Entity target, Predicate<? super Entity> successEntity, Predicate<? super Entity> filterEntityToIgnore, boolean stopOnLiquid) {
        ...
        AxisAlignedBB cube = new AxisAlignedBB(impact.x - this.raySize, impact.y - this.raySize, impact.z - this.raySize, impact.x + this.raySize, impact.y + this.raySize, impact.z + this.raySize);
    ...
    public Vec3d getForwardPosition() {
        if (this.targetEntity != null) {
            return new Vec3d(this.targetEntity.posX, this.targetEntity.posY + this.targetEntity.height / 2.0F, this.targetEntity.posZ);
        } else {
    ...
    public boolean checkOffPos(World world, Vec3d offpos, boolean stopOnLiquid, Predicate<? super Entity> filterEntityToIgnore) {
        ...
        Vec3d target;
        if (this.targetEntity != null) {
            target = new Vec3d(this.targetEntity.posX, this.targetEntity.posY + this.targetEntity.height / 2.0F, this.targetEntity.posZ);
        } else {
            if (this.nextRay == null) {
                return false;
            }
    ...
    public boolean noBlockImpact(World world, Vec3d impact) {
        return !world.checkBlockCollision(new AxisAlignedBB(impact.x - this.raySize, impact.y - this.raySize, impact.z - this.raySize, impact.x + this.raySize, impact.y + this.raySize, impact.z + this.raySize));
    }
    ...
    public void debug(World world) {
        ...
        BetweenParticle laser = new BetweenParticle(world, this.debugtexture, 0.05F, 240, 0.0F, 0.0F, 1.0F, 0.17F, thisp.distanceTo(target), 2, 0.3F, 2.0F, thisp, target);
