        ...
        if (attackTarg != null && this.entity.getNavigator().noPath() && this.cooldownTimer <= 0 && (!this.useMaxBlocks || this.entity.var1 > 0)) {
            if (Math.abs(this.entity.posY - attackTarg.posY) <= 1.5) {
                ...
                for (EnumFacing face : EnumFacing.HORIZONTALS) {
                    ...
                    if (maxDist > distt && this.checknoInList(offpos) && this.entity.world.getBlockState(offpos).getBlock().isReplaceable(this.entity.world, offpos)) {
                        maxDist = distt;
                        finalpos = offpos;
                    }
                }
                ...
                if (finalpos != null) {
                    ...
                    this.entity.playSound(this.block.getBlock().getSoundType().getPlaceSound(), 1.0F, 0.9F + this.entity.getRNG().nextFloat() / 5.0F);
                ...
                if (finalpos != null) {
                    this.entity.world.setBlockState(finalpos, this.block);
                    this.entity.playSound(this.block.getBlock().getSoundType().getPlaceSound(), 1.0F, 0.9F + this.entity.getRNG().nextFloat() / 5.0F);
                    if (this.checknoInList(finalpos.down()) && this.entity.world.getBlockState(finalpos.down()).getBlock().isReplaceable(this.entity.world, finalpos.down())) {
                        this.entity.world.setBlockState(finalpos.down(), this.block);
                    }

                    boolean succx = this.entity.getNavigator().tryMoveToXYZ(finalpos.getX(), finalpos.getY() + 1.1, finalpos.getZ(), 1.0);
                    this.cooldownTimer = this.maxCooldown;
                    if (succx) {
                        this.cooldownTimer *= 2;
                    }
