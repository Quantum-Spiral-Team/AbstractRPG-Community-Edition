...
public class EntityAICloudAttack extends EntityAIBase {
    ...
    public EntityAICloudAttack(EntityCreature entity, int frequency, float radius, float damage, float knockback, byte handleStatusUpdateID, @Nullable Potion effect, int potionDur, int potionPower, float potionChance) {
        this.entity = entity;
    ...
    public EntityAICloudAttack(EntityCreature entity, int frequency, float radius, float damage, float knockback, byte handleStatusUpdateID, @Nullable Potion effect) {
        this.entity = entity;
        this.frequency = frequency;
        this.radius = radius;
        this.damage = damage;
    ...
    public void updateTask() {
        if (this.entity.ticksExisted % this.frequency == 0) {
            ...
            if (attackTarg != null && !this.entity.world.isRemote) {
                ...
                AxisAlignedBB axisalignedbb = new AxisAlignedBB(vec.x - this.radius, vec.y - this.radius, vec.z - this.radius, vec.x + this.radius, vec.y + this.radius, vec.z + this.radius);
                ...
                if (!list.isEmpty()) {
                    for (EntityLivingBase entitylivingbase : list) {
                        if (entitylivingbase == attackTarg || entitylivingbase instanceof EntityPlayer) {
                            ...
                            SuperKnockback.applyKnockback(entitylivingbase, this.knockback, this.entity.posX, this.entity.posY, this.entity.posZ);
