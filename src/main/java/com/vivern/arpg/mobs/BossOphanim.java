...
public class BossOphanim extends AbstractBoss implements ISynchronizedEntity, IMultipartMob {
    ...
    public void readEntityFromNBT(NBTTagCompound compound) {
        if (compound.hasKey("healthCircle1") && compound.hasKey("healthCircle2") && compound.hasKey("healthCircle3") && compound.hasKey("healthCircle4")) {
            this.circlesHealth = new float[]{compound.getFloat("healthCircle1"), compound.getFloat("healthCircle2"), compound.getFloat("healthCircle3"), compound.getFloat("healthCircle4")};
        }
    ...
    public void onUpdate() {
        ...
        if (!this.world.isRemote) {
            for (EntityPlayer entityPlayer : this.world.getPlayers(EntityPlayer.class, player -> this.getDistanceSq(player) < 16384.0 && player.posY > 270.0)) {
        ...
        if (this.isEntityAlive() && this.laserTimer > 0) {
            ...
            if (this.getAttackTarget() != null) {
                this.onUpdateLaserRotation(this.getAttackTarget().posX, this.getAttackTarget().posY + this.getAttackTarget().height / 2.0F, this.getAttackTarget().posZ);
            }
                ...
                if (this.world.isRemote) {
                    ...
                    for (Entity entity : list) {
                        if (Team.checkIsOpponent(this, entity)) {
                            float size = GetMOP.getFromTo((float) (this.maxlazerTime - this.laserTimer), 20.0F, 25.0F) - GetMOP.getFromTo((float) (this.maxlazerTime - this.laserTimer), 85.0F, 100.0F);
                            Weapons.dealDamage(new WeaponDamage(null, this, null, false, false, vec3d, WeaponDamage.laser), strong ? size * 25.0F : size * 20.0F, this, entity, true, 0.1F, this.posX, this.posY + this.getEyeHeight(), this.posZ);
                            if (strong) {
        ...
        if (!this.world.isRemote) {
            ...
            if (this.guards != null) {
                ...
                for (int ix = 0; ix < 3; ix++) {
                    ...
                    if (guard.isEntityAlive()) {
                        ...
                        Vec3d posvec = this.getVectorForRotation(this.rotationPitch / 4.0F, this.renderYawOffset - (ix - 1) * 40).scale(6.0).add(this.getPositionVector());
                        double friction = 0.9;
                    ...
                    if (this.lightningCooldown <= 0) {
                        ...
                        if (this.lightningsTime <= this.lightningsPoses.length * 5) {
                            ...
                            if (this.lightningsTime % 5 == 0) {
                                ...
                                if (target < this.lightningsPoses.length && target >= 0) {
                                    Vec3d vec = this.lightningsPoses[target];
                                    LightningStrike proj = new LightningStrike(this.world, vec.x, vec.y, vec.z, this, this.lightningsPosesTargets[target]);
                                    proj.setPosition(vec.x, vec.y, vec.z);
                                    proj.damage = 23.0F;
                                    this.world.spawnEntity(proj);
                                }
            ...
            if (this.laserCooldown <= 0) {
                if (this.ticksExisted % 11 == 0 && (this.circlesAttackCooldown > 0 || this.circlesCharges <= 0) && !this.isDashing && this.getAttackTarget() != null && this.getEntitySenses().canSee(this.getAttackTarget())) {
            ...
            if (this.dashCooldown <= 0) {
                if (this.getAttackTarget() != null && this.laserTimer <= 0 && (this.circlesAttackCooldown > 0 || this.circlesCharges <= 0) && !this.isAIDisabled() && (!this.getEntitySenses().canSee(this.getAttackTarget()) || this.ticksExisted % 60 == 0)) {
                    this.isDashing = true;
                    this.dashVector = new Vec3d(this.getAttackTarget().posX - this.posX, this.getAttackTarget().posY - this.posY, this.getAttackTarget().posZ - this.posZ).normalize().scale(0.6);
                    this.dashTime = 0;
                    if (this.circlesHealth[0] <= 0.0F && this.circlesHealth[1] <= 0.0F && this.circlesHealth[2] <= 0.0F && this.circlesHealth[3] <= 0.0F) {
    ...
    public void spawnBetwParticle(Vec3d from, Vec3d to, float pitch, float yaw, float prevpitch, float prevyaw) {
        if (from.lengthSquared() > 1.0E-6 && to.lengthSquared() > 1.0E-6) {
            if (this.ticksExisted % 2 == 0) {
                ...
                this.world.playSound((from.x + to.x) / 2.0, (from.y + to.y) / 2.0, (from.z + to.z) / 2.0, Sounds.ophanim_beam, SoundCategory.HOSTILE, 2.3F - soundsize * 1.5F, 1.0F - soundsize * 0.65F, false);
            }

            float size = GetMOP.getFromTo((float) (this.maxlazerTime - this.laserTimer), 20.0F, 25.0F) - GetMOP.getFromTo((float) (this.maxlazerTime - this.laserTimer), 85.0F, 100.0F);
            EntityStreamLaserP laser = new EntityStreamLaserP(this.world, generic_beam2, size, 240, 1.0F, 1.0F, 1.0F, 0.15F, from.distanceTo(to), 3, 0.5F, 0.0F, pitch, yaw, this.ticksExisted);
            ...
            laser.model.next = new LaserModel.LaserModelCircles(new ResourceLocation("arpg:textures/circle2.png"), 240, 50.0, 12, 10, size + 0.1F, size * 0.9F, color1, color1);
            ...
            for (int ss = 0; ss < 3; ss++) {
                ...
                GUNParticle part = new GUNParticle(star2, scl, -0.009F, lt, 240, this.world, to.x, to.y, to.z, (float) this.rand.nextGaussian() / 15.0F, (float) this.rand.nextGaussian() / 15.0F, (float) this.rand.nextGaussian() / 15.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
    ...
    public Vec3d findLightningPosNearEntity(Entity entity, float radius) {
        ...
        for (int yy = (int) entity.posY; yy <= entity.posY + 20.0; yy++) {
            ...
            if (this.world.getBlockState(pos).getCollisionBoundingBox(this.world, pos) == null && this.world.getBlockState(pos.up()).getCollisionBoundingBox(this.world, pos.up()) != null) {
                height = yy;
                break;
            }
        }

        return new Vec3d(entity.posX + (this.rand.nextFloat() - 0.5) * 2.0 * radius, height, entity.posZ + (this.rand.nextFloat() - 0.5) * 2.0 * radius);
    }
    ...
    public void handleStatusUpdate(byte id) {
        ...
        if (id == 8) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.ophanim_circle, SoundCategory.HOSTILE, 3.7F, 0.95F + this.rand.nextFloat() / 10.0F, false);
        }

        if (id == 9) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.ophanim_dash, SoundCategory.HOSTILE, 4.5F, 0.95F + this.rand.nextFloat() / 10.0F, false);
        ...
        if (id == 7) {
            ...
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.ophanim_beam_start, SoundCategory.HOSTILE, 3.8F, 0.95F + this.rand.nextFloat() / 10.0F, false);
        }

        if (id == 10) {
            this.hasShield = true;
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.forcefield_trigger, SoundCategory.HOSTILE, 1.4F, 0.8F + this.rand.nextFloat() / 4.0F, false);
        }

        if (id == 11) {
            this.hasShield = false;
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.forcefield_impact, SoundCategory.HOSTILE, 1.9F, 0.8F + this.rand.nextFloat() / 4.0F, false);
        }
    ...
    public class EntityAIOphanim extends EntityAIBase {
        ...
        public void updateTask() {
            ...
            if (entitylivingbase != null) {
                ...
                SuperKnockback.applyMove(this.entity, this.movingDirection, entitylivingbase.posX, entitylivingbase.posY, entitylivingbase.posZ);
                ...
                SuperKnockback.applyMove(this.entity, this.movingDirection / 2.5F, this.entity.posX + this.dislocDirectionX, this.entity.posY + this.dislocDirectionY, this.entity.posZ + this.dislocDirectionZ);
