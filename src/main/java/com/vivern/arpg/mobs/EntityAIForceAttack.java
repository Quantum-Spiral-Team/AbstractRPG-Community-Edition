...
public class EntityAIForceAttack extends EntityAIBase {
    ...
    public EntityAIForceAttack(AbstractMob entity, float damage, int maxCooldown, float radiusHigher, float radiusLower, float power, float upMovement, float forcedistance, float sizestep) {
        this.entity = entity;
        ...
        if (attackTarg != null && this.attackTimer <= 0) {
            ...
            if (dist < this.radiusHigher && dist > this.radiusLower) {
                if (this.useRocketRotations) {
                    for (Entity base : GetMOP.entityMopRayTrace(Entity.class, this.forcedistance, 1.0F, this.entity, this.sizestep, this.entity.rotationPitch * -5.0F, -this.entity.rotationYaw)) {
                        if (!this.isWaterhammer || base.isInWater()) {
                            ...
                            if (this.damage > 0.0F) {
                                ...
                                base.attackEntityFrom(DamageSource.causeMobDamage(this.entity), (float) this.entity.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue());
                            }
                    ...
                    for (EntityLivingBase basex : GetMOP.mopRayTrace(this.forcedistance, 1.0F, this.entity, this.sizestep, this.sizestep)) {
                        if (!this.isWaterhammer || basex.isInWater()) {
                            ...
                            if (this.damage > 0.0F) {
                                ...
                                basex.attackEntityFrom(DamageSource.causeMobDamage(this.entity), (float) this.entity.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue());
                            }
