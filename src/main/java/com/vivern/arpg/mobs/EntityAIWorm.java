...
public class EntityAIWorm extends EntityAIBase {
    ...
    public EntityAIWorm(AbstractMob entity, int frequency, float radius, float radiusVar, float yOffset, float resetDistance, float attackChance, float friction, boolean checkInBlock, int solidblockChecks) {
        this.entity = entity;
    ...
    public List<AbstractMob> getSegments() {
        ...
        AxisAlignedBB bb = new AxisAlignedBB(cenVec3d.x - radiu, cenVec3d.y - radiu, cenVec3d.z - radiu, cenVec3d.x + radiu, cenVec3d.y + radiu, cenVec3d.z + radiu);
                ...
                if (this.entity.getRNG().nextFloat() > this.attackChance) {
                    if (this.checkInBlock) {
                        ...
                        for (int i = 0; i < this.solidblockChecks; i++) {
                            this.vec = GetMOP.findRandPosNearPoint(followPoint.x, followPoint.y + this.yOffset, followPoint.z, this.radius + this.entity.getRNG().nextFloat() * this.radiusVar);
                        ...
                        if (!suc) {
                            this.vec = this.lastSolidBlock == null ? new Vec3d(this.entity.posX, MathHelper.clamp(this.entity.posY - 40.0, 1.0, 255.0), this.entity.posZ) : this.lastSolidBlock;
                            this.speed = (float) this.entity.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).getAttributeValue();
                        }
                    } else {
                        this.vec = GetMOP.findRandPosNearPoint(followPoint.x, followPoint.y + this.yOffset, followPoint.z, this.radius + this.entity.getRNG().nextFloat() * this.radiusVar);
