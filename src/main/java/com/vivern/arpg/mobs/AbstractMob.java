...
public abstract class AbstractMob extends EntityMob implements IMultitexture {
    ...
    public boolean canFreezingImmobilizeMob(PotionEffect effect) {
        ...
        float entityPower = this.getMaxHealth() * 0.025F + (this.width + this.height) / 2.5F + (float) this.getEntityAttribute(SharedMonsterAttributes.KNOCKBACK_RESISTANCE).getAttributeValue();
                    ...
                    if (Math.abs(xx - this.posX) <= radius) {
                        ...
                        if (Math.abs(yy - this.posY) <= radius) {
                            ...
                            if (Math.abs(zz - this.posZ) <= radius) {
                                ...
                                if ("thermalexpansion:device".equals(state.getBlock().getRegistryName().toString()) && state.getBlock().getMetaFromState(state) == 11) {
    ...
    public void tame(EntityPlayer player) {
        if (!this.world.isRemote) {
            ...
            if (!playerteam.isEmpty()) {
                ...
                player.sendMessage(new TextComponentString("Unable to tame a creature without being in a team. Create or join team, using scoreboard commands (/scoreboard teams add _teamName_) (/scoreboard teams join _teamName_)"));
            }
        }
    }

    public int getMobDifficulty() {
        return this.defaultteam != null && this.world != null && this.world.getDifficulty() != null && this.defaultteam.equals(this.team) ? this.world.getDifficulty().getId() : 2;
    }
