...
public class EntityAITeasing<T extends EntityLiving> extends EntityAIBase {
        ...
        if (this.entity.getAttackTarget() != null) {
            if (this.targetEntity == null) {
                if (this.entity.ticksExisted % this.probablity == 0) {
                    AxisAlignedBB aabb = new AxisAlignedBB(this.entity.posX - this.raduis, this.entity.posY - this.raduis, this.entity.posZ - this.raduis, this.entity.posX + this.raduis, this.entity.posY + this.raduis, this.entity.posZ + this.raduis);

                    for (T targ : this.entity.world.getEntitiesWithinAABB(this.classTarget, aabb)) {
                        if (targ.getAttackTarget() != this.entity.getAttackTarget() && targ.getDistanceSq(this.entity.getAttackTarget()) <= this.distToAttackTargetSq && this.entity.getNavigator().tryMoveToEntityLiving(targ, 1.0)) {
    ...
    public void checkAndPerformAttack(EntityLivingBase enemy, double distToEnemySqr) {
        ...
        if (distToEnemySqr <= d0) {
            ...
            Weapons.dealDamage(null, 1.0F, this.entity.getAttackTarget(), enemy, true, 0.2F, this.entity.posX, this.entity.posY - 0.4, this.entity.posZ);
            EntityAIRush.setEnable(this.entity, true);
