...
public class EntityAIJumpBeastAttack extends EntityAIBase {
    ...
    public EntityAIJumpBeastAttack(AbstractMob entity, int maxCooldown, float castRadius, float damageRadius, float upMovement, float forwardMovement, int triggeredAnimation, float blockBreakingHardness, boolean isAdvanced) {
        this.entity = entity;
        ...
        if (attackTarg != null) {
            if (this.startedFlying) {
                ...
                if (this.entity.motionY < 0.0) {
                    SuperKnockback.applyMove(this.entity, -0.015F, this.targetPosition.x, this.targetPosition.y, this.targetPosition.z);
                }
                ...
                if (dist < this.radius) {
                    ...
                    SuperKnockback.setMove(this.entity, (float) (-dist * this.forwardMovement), attackTarg.posX, attackTarg.posY, attackTarg.posZ);
    ...
    public void sweep() {
        ...
        AxisAlignedBB aabb = new AxisAlignedBB(vec.x - this.damageRadius, vec.y - 1.0, vec.z - this.damageRadius, vec.x + this.damageRadius, vec.y + 0.5, vec.z + this.damageRadius);
        List<Entity> list = world.getEntitiesWithinAABBExcludingEntity(this.entity, aabb);
        if (this.soundOnFall != null) {
            world.playSound(null, this.entity.posX, this.entity.posY, this.entity.posZ, this.soundOnFall, SoundCategory.HOSTILE, 1.3F, 0.9F + this.entity.getRNG().nextFloat() / 5.0F);
        }
