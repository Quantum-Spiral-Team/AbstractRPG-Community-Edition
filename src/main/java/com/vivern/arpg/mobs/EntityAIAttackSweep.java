...
public class EntityAIAttackSweep extends AbstractMob.AbstractAI {
    ...
    public EntityAIAttackSweep(AbstractMob entity, int maxcooldown, float radius, float endradius, float length, int maxreflectTime, boolean canReflect, float damageToReflection, float maxdamageBlocked, SoundEvent reflectSound) {
        this.entity = entity;
    ...
    public float onEntityAttacked(DamageSource source, float amount) {
        if (this.canReflect && this.actualReflectTime > 0 && amount > this.damageToReflection) {
            ...
            if (this.reflectSound != null) {
                this.entity.world.playSound(null, this.entity.posX, this.entity.posY, this.entity.posZ, this.reflectSound, SoundCategory.HOSTILE, this.reflectSoundVolume, 1.1F + this.entity.getRNG().nextFloat() / 5.0F);
            }

            if (source.getTrueSource() != null) {
                SuperKnockback.applyKnockback(this.entity, this.knockbackOnReflect, source.getTrueSource().posX, source.getTrueSource().posY, source.getTrueSource().posZ);
            }
            ...
            if (this.attackstarted) {
                ...
                if (this.actualReflectTime == 1) {
                    if (this.swoshSound != null) {
                        this.entity.world.playSound(null, this.entity.posX, this.entity.posY, this.entity.posZ, this.swoshSound, SoundCategory.HOSTILE, this.swoshSoundVolume, 0.8F + this.entity.getRNG().nextFloat() / 5.0F);
                    }
    ...
    public void sweep() {
        ...
        Vec3d vec = this.useYawOffset ? GetMOP.rotatedPosRayTrace(this.length, 1.0F, this.entity, this.radius, this.radius, this.entity.rotationPitch, this.entity.renderYawOffset) : GetMOP.posRayTrace(this.length, 1.0F, this.entity, this.radius, this.radius);
        ...
        if (this.hitSound != null) {
            world.playSound(null, this.entity.posX, this.entity.posY, this.entity.posZ, this.hitSound, SoundCategory.HOSTILE, this.hitSoundVolume, 0.9F + this.entity.getRNG().nextFloat() / 5.0F);
        }
    ...
    public void breakBlocks(Vec3d center, int radius) {
        if (this.entity.world.isAreaLoaded(this.entity.getPosition().add(-16, -16, -16), this.entity.getPosition().add(16, 16, 16))) {
            for (BlockPos blockpos : GetMOP.getPosesInsideSphere((int) center.x, (int) center.y, (int) center.z, radius)) {
                if (!this.avoidDownbreak || this.entity.getAttackTarget() != null && blockpos.getY() >= this.entity.getAttackTarget().posY + 1.0 || blockpos.getY() >= this.entity.posY + 0.5) {
                    ...
                    if (Weapons.easyBreakBlockFor(this.entity.world, this.blockBreakingHardness, blockpos, state)) {
                        if (!state.getBlock().hasTileEntity(state) && !(this.entity.getRNG().nextFloat() < 0.6F)) {
                            EntityFallingBlock entityfallingblock = new EntityFallingBlock(this.entity.world, blockpos.getX() + 0.5, blockpos.getY() + 0.2, blockpos.getZ() + 0.5, state);
                            this.entity.world.spawnEntity(entityfallingblock);
                            SuperKnockback.applyKnockback(entityfallingblock, 1.5F, this.entity.posX, this.entity.posY - 0.4, this.entity.posZ);
