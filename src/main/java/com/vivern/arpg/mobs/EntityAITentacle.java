...
public class EntityAITentacle extends EntityAIBase {
    ...
    public EntityAITentacle(AbstractMob mob, boolean useStrikes, float strafingDistance, float strafingAmplitude, float tremor, int maxStrikeCooldown, int strikeCooldownAmplitude, float strikeAcceleration) {
        this.entity = mob;
    ...
    public void updateTask() {
        ...
        if (entitylivingbase != null) {
            ...
            double p1 = entitylivingbase.posY + entitylivingbase.height / 2.0F + this.randomTremorDirection.y * lookTremor - (this.entity.posY + this.entity.getEyeHeight());
            ...
            if (this.strikeCooldown > 15 && this.strikeTime >= 60.0F) {
                if (this.stayPos == null) {
                    this.stayPos = new Vec3d(rand.nextGaussian(), rand.nextGaussian(), rand.nextGaussian()).normalize().scale(this.strafingDistance - this.strafingAmplitude / 2.0F + rand.nextFloat() * this.strafingAmplitude);
                    ...
                    SuperKnockback.applyMove(this.entity, -speed, this.stayPos.x + entitylivingbase.posX, this.stayPos.y + entitylivingbase.posY + entitylivingbase.height / 2.0F, this.stayPos.z + entitylivingbase.posZ);
    ...
    public void breakBlocks(Random rand, int radius) {
        ...
        for (BlockPos pos : GetMOP.getPosesInsideSphere(poss.getX(), poss.getY(), poss.getZ(), radius)) {
            ...
            if (Weapons.easyBreakBlockFor(this.entity.world, this.hardnessBreaks, pos, blockState) && !blockState.getMaterial().isLiquid() && blockState.getMaterial() != Material.AIR) {
