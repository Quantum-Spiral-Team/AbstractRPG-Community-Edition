...
public class SmokeDemon extends AbstractMob {
    ...
    public SmokeDemon(World world) {
        ...
        this.registerLOOT(new MobDrop[]{new MobDrop(ItemsRegister.BRASS_NUGGET, 0.5F, 0, 1, 2, 3), new MobDrop(ItemsRegister.BRASS_INGOT, 0.05F, 0, 1, 1, 1)});
    ...
    public void onUpdate() {
        ...
        if (this.world.isRemote && this.getRNG().nextFloat() > 0.4F) {
            GUNParticle bigsmoke = new GUNParticle(this.largesmoke, 0.65F, -2.0E-4F, 35, -1, this.world, this.posX, this.posY + this.height / 2.0F, this.posZ, (float) this.rand.nextGaussian() / 35.0F, (float) this.rand.nextGaussian() / 35.0F, (float) this.rand.nextGaussian() / 35.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
