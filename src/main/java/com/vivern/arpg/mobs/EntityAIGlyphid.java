...
public class EntityAIGlyphid extends EntityAIBase {
    ...
    public void updateTask() {
        ...
        if (attackTarg != null) {
            if (this.path != null) {
                this.moveOnPath(this.entity, -this.entity.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).getAttributeValue(), this.path.getX() + 0.5, this.path.getY() + (1.0 - this.entity.height) / 2.0, this.path.getZ() + 0.5);
            }
    ...
    public void recalcPathToTarget(World world, EntityLivingBase target, int reachDist, int exploreCount) {
        AxisAlignedBB axisalignedbb = new AxisAlignedBB(target.posX - reachDist, target.posY - reachDist, target.posZ - reachDist, target.posX + reachDist, target.posY + reachDist, target.posZ + reachDist);
                        ...
                        if ((point.exploredWays & 1 << face.getIndex()) == 0) {
                            ...
                            if (this.entity.checkPosition(world, next)) {
                                ...
                                for (AbstractGlyphid glyphid : listGlyphids) {
                                    if (glyphid.isAiCompatible(this) && (glyphid.currentCost > next.cost || glyphid.pathDepecated) && MathHelper.floor(glyphid.posX) == next.getX() && MathHelper.floor(glyphid.posY + glyphid.height / 2.0F) == next.getY() && MathHelper.floor(glyphid.posZ) == next.getZ()) {
    ...
    public static PathPointGlyphid alreadyHas(List<PathPointGlyphid> list, PathPointGlyphid point) {
        for (PathPointGlyphid pointin : list) {
            if (pointin.getX() == point.getX() && pointin.getY() == point.getY() && pointin.getZ() == point.getZ()) {
                return pointin;
            }
        }
    ...
    public static class PathPointGlyphid extends BlockPos {
        ...
        public PathPointGlyphid offset(EnumFacing facing, int n) {
            return n == 0 ? this : new PathPointGlyphid(this.getX() + facing.getXOffset() * n, this.getY() + facing.getYOffset() * n, this.getZ() + facing.getZOffset() * n, this.cost + n);
        }
