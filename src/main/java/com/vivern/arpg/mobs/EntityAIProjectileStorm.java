...
public class EntityAIProjectileStorm extends EntityAIBase {
    ...
    public EntityAIProjectileStorm(AbstractMob entity, double distance, int maxCooldown, int maxDelay, float radius, int minCount, int maxCount, int height, int heightRandAdd, boolean selfCast, byte specialAttackId, byte sendId) {
        this.entity = entity;
        ...
        if (attackTarg != null) {
            ...
            if (this.delay > this.maxDelay && this.vector != null) {
                ...
                for (int c = 0; c < count; c++) {
                    BlockPos randpos = new BlockPos(this.vector.x + (rand.nextFloat() - 0.5F) * this.radius * 2.0F, this.vector.y, this.vector.z + (rand.nextFloat() - 0.5F) * this.radius * 2.0F);
                    int hh = this.height + rand.nextInt(this.heightRandAdd);

                    for (int yy = 0; yy <= hh; yy++) {
                        randpos = randpos.up();
                        if (yy >= heightmin && (yy == hh || this.entity.world.getBlockState(randpos).getCollisionBoundingBox(this.entity.world, randpos) != null)) {
                            this.entity.specialAttack(randpos.getX() + 0.5, randpos.getY() - 0.5, randpos.getZ() + 0.5, this.specialAttackId);
                            break;
                        }
                    }
                }
            ...
            if (this.attackTimer <= 0) {
                ...
                if (distSq <= this.distanceSq) {
                    ...
                    for (int i = 0; i < 5; i++) {
                        ...
                        int counter = 0;
                        AxisAlignedBB aabb = new AxisAlignedBB(pos.x - this.radius, pos.y - this.radius, pos.z - this.radius, pos.x + this.radius, pos.y + this.radius, pos.z + this.radius);
