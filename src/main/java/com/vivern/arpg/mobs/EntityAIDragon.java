...
public class EntityAIDragon extends AbstractMob.AbstractAI {
    ...
    public EntityAIDragon(AbstractMob mob, int attackCooldown, float maxShootDistance, int attackDelay, boolean shoot, float strafingDistance, float strafingAmplitude, boolean canWalk) {
        this.entity = mob;
    ...
    public void updateTask() {
        ...
        if (entitylivingbase != null) {
            ...
            if (this.MODE != 1) {
                ...
                if (this.rushTIME > 160 && this.rand.nextFloat() < 0.01 || this.HITS >= 8 || this.noPathTime > 30 || d0 > this.followFlyDistSq || this.entity.fallDistance >= 2.0F && !this.onGround(this.entity.world, this.entity.posX, this.entity.posY, this.entity.posZ)) {
                    this.resetRush();
                    this.entity.setNoGravity(true);
                    this.HITS = 0;
                    this.MODE = 2;
                ...
                if (this.strafeVec != null) {
                    ...
                    this.rushTIME++;
                    this.entity.getLookHelper().setLookPosition(this.entity.posX + this.strafeVec.x, this.entity.posY - 8.0, this.entity.posZ + this.strafeVec.z, 30.0F, 30.0F);
                    if (this.rushTIME == 5) {
                        this.entity.shoot();
                    }
                ...
                if (this.entity.ticksExisted % 4 == 0) {
                    ...
                    if (this.stayTIME > 130 || this.HITS > 8) {
                        if (this.onGround(this.entity.world, this.entity.posX, this.entity.posY, this.entity.posZ) && this.canWalk) {
                            ...
                            this.entity.world.setEntityState(this.entity, (byte) (1 + this.sendEntStateAdd));
                            this.entity.var1 = this.MODE;
                        } else if (cansee && (this.onGround(this.entity.world, entitylivingbase.posX, entitylivingbase.posY, entitylivingbase.posZ) || !this.canWalk) && this.rand.nextFloat() < 0.5) {
                            this.stayTIME /= 2;
                            this.HITS /= 2;
            ...
            if (this.MODE == 2) {
                if (this.firstAgred) {
                    ...
                    Vec3d randVec = this.nextFlyPointRel.add(this.rand.nextFloat() < 0.5 ? this.rand.nextFloat() - 1.5F : this.rand.nextFloat() + 0.5F, this.rand.nextFloat() < 0.5 ? this.rand.nextFloat() - 1.5F : this.rand.nextFloat() + 0.5F, this.rand.nextFloat() < 0.5 ? this.rand.nextFloat() - 1.5F : this.rand.nextFloat() + 0.5F);
    ...
    public static Vec3d RotatedPosRayTrace(EntityLivingBase attacktarget, double blockReachDistance, float partialTicks, EntityLivingBase entity, double size, double step, float rotationPitch, float rotationYaw) {
        Vec3d vec3d = entity.getPositionEyes(partialTicks);
        Vec3d vec3d1 = GetMOP.pitchYawToVec3D(rotationPitch, rotationYaw);
        Vec3d vec3d2 = vec3d.add(vec3d1.x * blockReachDistance, vec3d1.y * blockReachDistance, vec3d1.z * blockReachDistance);
        RayTraceResult raytraceresult = entity.world.rayTraceBlocks(vec3d, vec3d2, attacktarget == null || !attacktarget.isInWater(), true, false);
    ...
    public void updateRush() {
        ...
        double d0 = this.entity.getDistanceSq(entitylivingbase.posX, entitylivingbase.getEntityBoundingBox().minY, entitylivingbase.posZ);
        this.delayCounter--;
        if ((this.longMemory || this.entity.getEntitySenses().canSee(entitylivingbase)) && this.delayCounter <= 0 && (this.targetX == 0.0 && this.targetY == 0.0 && this.targetZ == 0.0 || entitylivingbase.getDistanceSq(this.targetX, this.targetY, this.targetZ) >= 1.0 || this.entity.getRNG().nextFloat() < 0.05F)) {
    ...
    public boolean shouldExecuteRush() {
        ...
        if (entitylivingbase == null) {
            ...
            if (!this.canPenalize) {
                ...
                return this.path != null || this.getAttackReachSqr(entitylivingbase) >= this.entity.getDistanceSq(entitylivingbase.posX, entitylivingbase.getEntityBoundingBox().minY, entitylivingbase.posZ);
    ...
    public boolean shouldContinueRush() {
        ...
        if (entitylivingbase == null) {
            ...
            return this.entity.isWithinHomeDistanceFromPosition(new BlockPos(entitylivingbase)) && (!(entitylivingbase instanceof EntityPlayer) || !((EntityPlayer) entitylivingbase).isSpectator() && !((EntityPlayer) entitylivingbase).isCreative());
        }
    }
