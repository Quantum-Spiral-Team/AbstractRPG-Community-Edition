...
public class EntityAIFloatingSkeleton extends EntityAIBase {
    ...
    public EntityAIFloatingSkeleton(AbstractMob mob, int attackCooldown, float maxShootDistance, int attackDelay, boolean shoot, float strafingDistance, float strafingAmplitude, float levitateHeight, float levitatePower, boolean canFlyup, boolean triggerAnim3) {
        this.entity = mob;
    ...
    public boolean tryMove(double x, double y, double z) {
        if (this.isNotColliding(this.entity.world, this.entity.getEntityBoundingBox().offset(x, y, z), this.entity)) {
            ...
            RayTraceResult res = this.entity.world.rayTraceBlocks(vec, vec.add(0.0, -this.levitateHeight - this.levitateStair, 0.0), this.entity.getAttackTarget() == null || !this.entity.getAttackTarget().isInWater());
    ...
    public static Vec3d RotatedPosRayTrace(EntityLivingBase attacktarget, double blockReachDistance, float partialTicks, EntityLivingBase entity, double size, double step, float rotationPitch, float rotationYaw, boolean levitateOverMobs) {
