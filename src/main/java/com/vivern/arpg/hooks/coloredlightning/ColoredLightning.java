...
public class ColoredLightning {
    ...
    public static boolean calculateColorsInRangeTest(World world, BlockPos centerpos) {
        ...
        for (BlockPos updateRelativePos : updateRangeCircle) {
            ...
            if (coloredChunk != null) {
                int light = Math.round(15.0F - (float) Math.sqrt(updateRelativePos.getX() * updateRelativePos.getX() + updateRelativePos.getY() * updateRelativePos.getY() + updateRelativePos.getZ() * updateRelativePos.getZ()));
                int inChunkX = blockToInchunkCoords(pos.getX());
                int inChunkZ = blockToInchunkCoords(pos.getZ());
            ...
            for (BlockPos updateRelativePos : updateRange) {
                ...
                if (coloredChunk != null) {
                    ...
                    if (colorOfTheBlock != null) {
                        coloredChunk.setColorsAt(inChunkX, pos.getY(), inChunkZ, bakeColor(colorOfTheBlock.redSat, colorOfTheBlock.redPower), bakeColor(colorOfTheBlock.greenSat, colorOfTheBlock.greenPower), bakeColor(colorOfTheBlock.blueSat, colorOfTheBlock.bluePower));
                    } else {
