...
public class TileItemCharger extends TileEntityLockableLoot implements ITickable {
    ...
    public TileItemCharger() {
        this.electricStorage = new EnergyStorage(ItemAccumulator.LEAD_ACID_CAPACITY, ItemAccumulator.LEAD_ACID_THROUGHPUT, ItemAccumulator.LEAD_ACID_THROUGHPUT, 0);
    }
    ...
    public void update() {
        if (!this.world.isRemote) {
            ...
            if (send && (Math.abs(this.electricStorage.getEnergyStored() - this.prevEnergy) > this.electricStorage.getMaxEnergyStored() / 68.0F || this.electricStorage.getEnergyStored() != this.prevEnergy && (this.electricStorage.getEnergyStored() == 0 || this.electricStorage.getEnergyStored() == this.electricStorage.getMaxEnergyStored()))) {
    ...
    public void read(NBTTagCompound compound) {
        ...
        if (compound.hasKey("rf")) {
            this.electricStorage = new EnergyStorage(ItemAccumulator.LEAD_ACID_CAPACITY, ItemAccumulator.LEAD_ACID_THROUGHPUT, ItemAccumulator.LEAD_ACID_THROUGHPUT, compound.getInteger("rf"));
        }
        ...
        switch (id) {
            ...
            case 1:
                if (this.stacks.get(0).getItem() instanceof IEnergyItem) {
                    return Math.round(100.0F * NBTHelper.GetNBTint(this.stacks.get(0), "rf") / ((IEnergyItem) this.stacks.get(0).getItem()).getMaxEnergyStored(this.stacks.get(0)));
                }
            default:
                return 0;
        }
    ...
    public void setField(int id, int value) {
        switch (id) {
            case 0:
                this.electricStorage = new EnergyStorage(ItemAccumulator.LEAD_ACID_CAPACITY, ItemAccumulator.LEAD_ACID_THROUGHPUT, ItemAccumulator.LEAD_ACID_THROUGHPUT, value);
        }
    }
