...
public class TileCombinationLock extends TileEntity implements TileEntityClicked {
    ...
    public static SeaElement[] all = new SeaElement[]{cell, seaweed, stone, worm, coral, shell, seaplant, sponge, fish, jellyfish, shark, starfish, anemone, seafeather, lamprey, turtle, seahorse, coralfish, toxicfish, trilobite, crustacean, seaurchin, chalk, megafish, sand, mollusk, octopus, pearl, neuron, island, palm, aspid, seasnake, amphibian, tryton, sugarcane, coconut, mind, mermaid, flyfish, stingray, ctenophore, ship, kraken, lightning, siren, temple, staff, blade, trident, symbioticweapon, conger, metal, gold, gems, chest, treasure, town, piracy, cannon, cactus, rum};
    ...
    public void mouseClick(int mouseX, int mouseY, int mouseButton) {
        ...
        if (mouseY > 59 && mouseY < 80 && mouseX > 118 && mouseX < 139 && this.selected1 != null && this.selected2 != null) {
            ...
            if (el != null) {
                ...
                this.result = el;
                if (el == this.question) {
                    this.world.playSound(null, this.pos, Sounds.sealock_win, SoundCategory.BLOCKS, 0.5F, 0.85F + rand.nextFloat() / 4.0F);
                } else {
                    this.world.playSound(null, this.pos, Sounds.alchemic_craft, SoundCategory.BLOCKS, 0.4F, 0.85F + rand.nextFloat() / 4.0F);
                }

                SeaLock.trySendPacketUpdate(this.world, this.getPos(), this, 8);
            } else {
                this.world.playSound(null, this.pos, Sounds.item_misc_e, SoundCategory.BLOCKS, 0.4F, 0.85F + rand.nextFloat() / 4.0F);
        ...
        if (mouseY > 132 && mouseY < 165 && mouseX > 16 && mouseX < 241) {
            ...
            if (framenumber < this.unlocked.size()) {
                ...
                this.select(el);
                this.world.playSound(null, this.pos, Sounds.item_misc_d, SoundCategory.BLOCKS, 0.4F, 0.85F + rand.nextFloat() / 4.0F);
                SeaLock.trySendPacketUpdate(this.world, this.getPos(), this, 8);
            }
        }
