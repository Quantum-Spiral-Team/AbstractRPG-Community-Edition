    ...
    public boolean addNewVialItem(ItemStack stack, int posx, int posz, boolean rotated) {
        ...
        for (int i = 0; i < 4; i++) {
            if (this.vials[i] == null) {
                ...
                AxisAlignedBB vialaabb = emptyvial ? ShardType.emptyVialBoundingBox.offset(posx / 16.0F, 0.0, posz / 16.0F) : shardType.vialBoundingBox.offset(posx / 16.0F, 0.0, posz / 16.0F);
