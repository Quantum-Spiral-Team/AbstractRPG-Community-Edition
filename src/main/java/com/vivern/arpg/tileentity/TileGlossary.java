        ...
        for (int x = -radiusXZ; x <= radiusXZ; x++) {
            for (int z = -radiusXZ; z <= radiusXZ; z++) {
                for (int y = -yDown; y <= yUp; y++) {
                    BlockPos watchPos = new BlockPos(this.pos.getX() + x, this.pos.getY() + y, this.pos.getZ() + z);
    ...
    public void startAnimation(Random rand, @Nullable BlockPos casePos) {
        ...
        if (casePos != null) {
            ...
            for (int j2 = 0; j2 < 3; j2++) {
                this.world.spawnParticle(EnumParticleTypes.ENCHANTMENT_TABLE, this.pos.getX() + 0.5, this.pos.getY() + 1.8, this.pos.getZ() + 0.5, i + rand.nextFloat() - 0.5, k - rand.nextFloat() - 1.0F, j + rand.nextFloat() - 0.5, new int[0]);
            }
        }
    }
