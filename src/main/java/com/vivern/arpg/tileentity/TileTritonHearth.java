...
public class TileTritonHearth extends TileEntity implements IMagicVision, ITickable, IVialElementsAccepter, IManaBuffer, ITileEntitySynchronized {
    ...
    public void onClient(double... args) {
        if (args.length == 1 && args[0] == 1.0) {
            for (int i = 0; i < 10; i++) {
                ShardType.WATER.spawnNativeParticle(this.world, 1.0F, this.getPos().getX() + 0.5, this.getPos().getY() + 0.9, this.getPos().getZ() + 0.5, GetMOP.rand.nextGaussian() / 22.0, GetMOP.rand.nextGaussian() / 20.0 + 0.07F, GetMOP.rand.nextGaussian() / 22.0, true);
            }
        }
    }
        ...
        if (!this.world.isRemote) {
            if (this.hasMaterials) {
                ...
                if (this.workTicks > (wet ? workTicksMax : workTicksMax * 1.5F)) {
                    ...
                    this.getWorld().spawnEntity(new EntityItem(this.getWorld(), this.getPos().getX() + 0.5, this.getPos().getY() + 0.9, this.getPos().getZ() + 0.5, new ItemStack(ItemsRegister.AQUATIC_INGOT)));
                    ITileEntitySynchronized.sendSynchronize(this, 48.0, 1.0);
                    this.world.playSound(null, this.getPos(), Sounds.triton_hearth_end, SoundCategory.BLOCKS, 0.6F, 0.9F + GetMOP.rand.nextFloat() * 0.2F);
                } else {
                    this.workTicks++;
                }
            ...
            if (list.size() == 1) {
                ...
                if (entityItem != null && entityItem.getItem() != null) {
                    ...
                    if (is) {
                        ...
                        this.world.playSound(null, this.getPos(), Sounds.triton_hearth_start, SoundCategory.BLOCKS, 0.6F, 0.95F + GetMOP.rand.nextFloat() * 0.1F);
