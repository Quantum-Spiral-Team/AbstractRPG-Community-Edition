...
public class TileSpellForge extends TileEntityLockableLoot implements IMagicVision, ITickable, ISpellcastListener, IVialElementsAccepter, ITileEntitySynchronized {
    ...
    public float getElementEnergy(ShardType shardType) {
        return Minecraft.getMinecraft().player != null && Minecraft.getMinecraft().player.getHeldItemMainhand().getItem() == ItemsRegister.SPELL_ROLL ? 0.0F : this.elementsCollected[shardType.id - 1];
    }
        ...
        if (iBlockStateUp.getCollisionBoundingBox(this.world, posup) == null) {
            ...
            if (aabb != null) {
                ...
                for (int i = 0; i < imax; i++) {
                    ...
                    this.world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, x, y, z, 0.0, 0.15 + rand.nextGaussian() / 20.0, 0.0, new int[]{Block.getStateId(iBlockState)});
                }
        ...
        if (args.length == 1) {
            if (args[0] == 0.0) {
                ...
                for (int x = -radius; x <= radius; x++) {
                    for (int z = -radius; z <= radius; z++) {
                        if (rand.nextFloat() > Math.sqrt(x * x + z * z) / radius && (x != 0 || z != 0)) {
                            ...
                            while (y >= this.pos.getY() - 2 && !this.spawnParticleOnBlockTop(new BlockPos(this.pos.getX() + x, y, this.pos.getZ() + z))) {
                                y--;
                            }
                        }
                    }
                ...
                if (this.currentForging != null) {
                    ...
                    if (shardType != null) {
                        for (int p = 0; p < 16.0F + Debugger.floats[3]; p++) {
                            shardType.spawnNativeParticle(this.world, 1.0F, this.pos.getX() + 0.5, this.pos.getY() + 1.1, this.pos.getZ() + 0.5, rand.nextGaussian() / 19.0, rand.nextGaussian() / 19.0 + 0.07F, rand.nextGaussian() / 19.0, true);
                        }

                        AnimatedGParticle anim = new AnimatedGParticle(forge_hit, 0.6F, 0.0F, 28, 240, this.world, this.pos.getX() + 0.5, this.pos.getY() + 1.01, this.pos.getZ() + 0.5, 0.0F, 0.0F, 0.0F, shardType.colorR, shardType.colorG, shardType.colorB, true, rand.nextInt(360));
                        anim.framecount = 28;
                        ...
                        GUNParticle part1 = new GUNParticle(b ? forge_hit_a : forge_hit_b, 0.3F + rand.nextFloat() * 0.1F, 0.0F, 5, 240, this.world, this.pos.getX() + 0.5, this.pos.getY() + 1.1, this.pos.getZ() + 0.5, 0.0F, 0.0F, 0.0F, shardType.colorR, shardType.colorG, shardType.colorB, true, rand.nextInt(91) - 45);
                        part1.alphaGlowing = true;
                        this.world.spawnEntity(part1);
                        GUNParticle part2 = new GUNParticle(b ? forge_hit_b : forge_hit_a, 0.18F + rand.nextFloat() * 0.08F, 0.0F, 5, 240, this.world, this.pos.getX() + 0.5, this.pos.getY() + 1.1, this.pos.getZ() + 0.5, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, rand.nextInt(91) - 45);
                ...
                if (args[0] == 4.0) {
                    ...
                    ShardType[] shardTypes = new ShardType[]{this.currentForging.recipe.getRandomWeightedShardType(rand), this.currentForging.recipe.getRandomWeightedShardType(rand), this.currentForging.recipe.getRandomWeightedShardType(rand)};
                    ...
                    for (int i = 0; i < amount; i++) {
                        ...
                        GUNParticle part = new GUNParticle(star, scl, 0.0F, lt, 220, this.world, this.pos.getX() + 0.5, this.pos.getY() + 1.25, this.pos.getZ() + 0.5, 0.0F, 0.0F, 0.0F, r, g, b, false, 0);
    ...
    public boolean onSpellcast(double fromX, double fromY, double fromZ, ItemStack scroll, Spell[] spells, Object caster) {
        if (this.currentForging != null && this.currentForging.recipe != null && this.currentForging.canAcceptSpells && this.currentForging.recipe.spells != null && this.currentForging.recipe.spells.length == spells.length) {
    ...
    public boolean tryDestroyFloor() {
        ...
        if (this.world.isAirBlock(this.pos.down())) {
            for (int ix = 0; ix < this.getSizeInventory(); ix++) {
                ...
                if (!stack.isEmpty()) {
                    EntityItem entityItem = new EntityItem(this.getWorld(), this.getPos().getX() + 0.5, this.getPos().getY() + 1.5, this.getPos().getZ() + 0.5, stack);
        ...
        if (this.tryDestroyFloor()) {
            ...
            if (this.currentForging != null && this.currentForging.recipe != null) {
                ...
                if (this.currentForging.recipe.catalyst != null) {
                    canContinue = false;
                    float bounds = 0.3F;
                    AxisAlignedBB aabb = new AxisAlignedBB(this.pos.getX() + bounds, this.pos.getY() + 1, this.pos.getZ() + bounds, this.pos.getX() + 1.0F - bounds, this.pos.getY() + 1.6F, this.pos.getZ() + 1.0F - bounds);
                ...
                if (canContinue) {
                    if (!firstHit) {
                        this.currentForging.hammerCompleteness = this.currentForging.hammerCompleteness + (1.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, spellHammer) * 0.2F);
    ...
    public void completeCraft(SpellForgeRecipe recipe) {
        if (recipe.tryCraft(this, null, true)) {
            this.getWorld().spawnEntity(new EntityItem(this.getWorld(), this.getPos().getX() + 0.5, this.getPos().getY() + 1.5, this.getPos().getZ() + 0.5, recipe.craftresult.createStack()));
            this.currentForging = null;
            PacketHandler.trySendPacketUpdate(this.world, this.pos, this, 64.0);
    ...
    public void destabilize() {
        if (this.currentForging != null) {
            if (!this.world.isRemote) {
                ...
                this.world.playSound(null, this.getPos(), Sounds.spell_forge_fail, SoundCategory.BLOCKS, 1.4F, 0.9F + rand.nextFloat() * 0.2F);
        ...
        if (!lightning) {
            if (this.currentForging != null) {
                ...
                if (shardType != null) {
                    ...
                    Vec3d poss = new Vec3d(this.getPos().getX() + 0.5, this.getPos().getY() + 1.25, this.getPos().getZ() + 0.5);
            ...
            if (shardType != null) {
                ParticleFastSummon.coloredLightning(this.world, 0.035F, 10 + rand.nextInt(10), 240, shardType.colorR, shardType.colorG, shardType.colorB, 7, poss, pos3, poss.squareDistanceTo(pos3) > 2.0 ? 0.5F : 0.25F, 0.2F, ParticleFastSummon.rand);
            }
        }
    }
    ...
    public static class Forging {
        ...
        public void update(TileSpellForge tileEntity) {
            if (this.fullComplete) {
                if (this.ticks == 0) {
                    tileEntity.world.playSound(null, tileEntity.getPos(), this.hitsNeed >= 3.0F ? Sounds.spell_forge_complete : Sounds.spell_forge_smallcomplete, SoundCategory.BLOCKS, 1.8F, 1.0F);
                }
            ...
            if (tileEntity.world.isRemote && this.recipe != null) {
                ...
                if (this.canAcceptSpells && this.lastClientShardType != null) {
                    ...
                    if (AnimationTimer.normaltick % 2 == 0) {
                        AnimatedGParticle anim = new AnimatedGParticle(TileSpellForge.forge_absorption, 0.6F + TileSpellForge.rand.nextFloat() * 0.35F, 0.0F, 28, 240, tileEntity.world, tileEntity.pos.getX() + 0.5, tileEntity.pos.getY() + 1.1, tileEntity.pos.getZ() + 0.5, 0.0F, 0.0F, 0.0F, this.absorbingRed, this.absorbingGreen, this.absorbingBlue, true, TileSpellForge.rand.nextInt(360));
                        anim.framecount = 25;
                        anim.alphaGlowing = true;
                        anim.randomDeath = false;
                    ...
                    Vec3d pos = new Vec3d(tileEntity.pos.getX() + 0.5, tileEntity.pos.getY() + 1.1, tileEntity.pos.getZ() + 0.5);
                    Vec3d randPos = pos.add(TileSpellForge.rand.nextGaussian() * 2.0, TileSpellForge.rand.nextGaussian() * 1.5 + 0.5, TileSpellForge.rand.nextGaussian() * 2.0);
                    Vec3d motion = pos.subtract(randPos).scale(Debugger.floats[0] + 0.1);
                    this.lastClientShardType.spawnNativeParticle(tileEntity.world, 0.8F, randPos.x, randPos.y, randPos.z, motion.x, motion.y, motion.z, true);
                }
                ...
                if (this.lastClientShardType != null && TileSpellForge.rand.nextFloat() < 0.5F) {
                    ...
                    Vec3d pos1 = new Vec3d(tileEntity.pos.getX() + 0.5 + face.getXOffset() * 0.5625 + face2.getXOffset() * offset, tileEntity.pos.getY() + 0.875F, tileEntity.pos.getZ() + 0.5 + face.getZOffset() * 0.5625 + face2.getZOffset() * offset);
                    Vec3d pos2 = new Vec3d(tileEntity.pos.getX() + 0.5 + face.getXOffset() * 2.5 + face2.getXOffset() * offset * 3.0 + TileSpellForge.rand.nextGaussian(), tileEntity.pos.getY() - 2, tileEntity.pos.getZ() + 0.5 + face.getZOffset() * 2.5 + face2.getZOffset() * offset * 3.0 + TileSpellForge.rand.nextGaussian());
                    ...
                    ParticleFastSummon.coloredLightning(tileEntity.world, 0.03F, 4, 240, this.lastClientShardType.colorR, this.lastClientShardType.colorG, this.lastClientShardType.colorB, 5, pos1, pos3, pos1.squareDistanceTo(pos3) > 2.0 ? 0.5F : 0.25F, 0.1F, ParticleFastSummon.rand);
                }

                if (this.fullComplete) {
                    if (AnimationTimer.normaltick % 8 == 0) {
                        Vec3d pos1 = new Vec3d(tileEntity.pos.getX() + 0.5, tileEntity.pos.getY() + 1.25F, tileEntity.pos.getZ() + 0.5);
                        ShardType typecomplete = this.recipe.getRandomWeightedShardType(TileSpellForge.rand);
                        if (typecomplete != null) {
                            ParticleFastSummon.round(tileEntity.world, pos1, 3.0F + TileSpellForge.rand.nextFloat() * 2.0F, 0.0F, 7 + TileSpellForge.rand.nextInt(5), 240, typecomplete.colorR, typecomplete.colorG, typecomplete.colorB, 1, true);
                            ...
                            GUNParticle part1 = new GUNParticle(TileSpellForge.star3, 0.1F, 0.0F, 15, 240, tileEntity.world, pos1.x, pos1.y, pos1.z, 0.0F, 0.0F, 0.0F, r, g, b, true, 0);
                            part1.tracker = tss;
                            part1.alphaGlowing = true;
                    ...
                    if (AnimationTimer.normaltick % 11 == 0) {
                        ...
                        if (typecomplete != null) {
                            GUNParticle part2 = new GUNParticle(TileSpellForge.flare_1, 0.2F, 0.0F, 60, 210, tileEntity.world, pos1.x, pos1.y, pos1.z, 0.0F, 0.0F, 0.0F, typecomplete.colorR, typecomplete.colorG, typecomplete.colorB, true, TileSpellForge.rand.nextInt(31) - 15);
                            part2.tracker = ptvr;
                            part2.alphaGlowing = true;
        ...
        public boolean trySetFullComplete(TileSpellForge tileEntity) {
            if (this.recipe != null) {
                ...
                float bounds = 0.3F;
                AxisAlignedBB aabb = new AxisAlignedBB(pos.getX() + bounds, pos.getY() + 1, pos.getZ() + bounds, pos.getX() + 1.0F - bounds, pos.getY() + 1.6F, pos.getZ() + 1.0F - bounds);
