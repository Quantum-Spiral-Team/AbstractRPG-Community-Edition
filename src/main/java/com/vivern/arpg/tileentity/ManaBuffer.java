...
public class ManaBuffer {
    ...
    public static ParticleTracker.TrackerTurbulence pt_turbulence = new ParticleTracker.TrackerTurbulence(noisegenerator3dX, noisegenerator3dY, noisegenerator3dZ, 1.0F, 0.04F);
    public static Random rand = new Random();

    public ManaBuffer(@Nullable TileEntity tileEntity, IManaBuffer iBuffer, float manaMaximum, int initialFieldRadius, float initialAttraction, float initialFlowSpeed) {
                                ...
                                if (tile instanceof IManaBuffer) {
                                    ...
                                    if (anotherAttraction < attraction) {
                                        ...
                                        if (passwordAccessed) {
                                            ...
                                            float manaNeed = Math.min(this.getManaStorageSize() - this.getManaStored(), (this.initialFlowSpeed + modifiers.speed) * TICKRATE);
                                            if (manaNeed == 0.0F) {
                                                return;
                                            }
    ...
    public static void spawnFlowParticles(World world, AxisAlignedBB from, BlockPos posto, float mana, int count, float size) {
        ...
        ParticleTracker.TrackerFollowStaticPoint tfsp = new ParticleTracker.TrackerFollowStaticPoint(new Vec3d(posto.getX() + 0.5, posto.getY() + 0.5, posto.getZ() + 0.5), true, 0.4F, 0.001F, 0.02F);
        ...
        for (int i = 0; i < count; i++) {
            ...
            GUNParticle spelll = new GUNParticle(tex, scale, 0.0F, livetime, 240, world, aabb.minX + (aabb.maxX - aabb.minX) * rand.nextFloat(), aabb.minY + (aabb.maxY - aabb.minY) * rand.nextFloat(), aabb.minZ + (aabb.maxZ - aabb.minZ) * rand.nextFloat(), 0.0F, 0.0F, 0.0F, rand.nextFloat() * 0.5F, 0.5F + rand.nextFloat() * 0.5F, 1.0F, true, 0);
            spelll.alpha = 0.0F;
