...
public class TileRunicMirror extends TileEntity implements ITickable, IVialElementsAccepter, IMagicVision, ITileEntitySynchronized {
    ...
    public void rotateMirror(BlockPos targetPos) {
        Vec3d thispos = new Vec3d(this.getPos().getX() + 0.5, this.getPos().getY() + 0.5, this.getPos().getZ() + 0.5);
        ...
        double anglePitch = GetMOP.getAngleBetweenVectors(0.0, 1.0, 0.0, targpos.x - thispos.x, targpos.y - thispos.y, targpos.z - thispos.z) - 90.0;
    ...
    public void update() {
        ...
        if (this.world.getWorldTime() % ManaBuffer.TICKRATE == 0L) {
            ...
            if (this.elementCollected > 0.0F) {
                ...
                Vec3d start = new Vec3d(this.pos.getX() + 0.5, this.pos.getY() + 1, this.pos.getZ() + 0.5);
                Vec3d end = start.add(GetMOP.pitchYawToVec3D(this.rendrotationPitch, this.rendrotationYaw).scale(20.0));
                RayTraceResult result = fixedRayTraceBlocks(this.world, this.getPos(), null, 0.8, 0.8, false, start, end, this.elementType != ShardType.WATER, true, false);
                if (result != null) {
                    if (!this.world.isRemote) {
                        ...
                        if (this.elementType != null && GetMOP.rand.nextFloat() < 0.2) {
                            this.elementType.spawnNativeParticle(this.world, 1.2F, result.hitVec.x, result.hitVec.y, result.hitVec.z, (GetMOP.rand.nextFloat() - 0.5) * 0.2, (GetMOP.rand.nextFloat() - 0.5) * 0.2, (GetMOP.rand.nextFloat() - 0.5) * 0.2, true);
                        }
                    }
                }
            }
    ...
    public void spawnParticleAtMirror() {
        if (this.elementType != null) {
            ...
            this.elementType.spawnNativeParticle(this.world, this.elementType == ShardType.VOID ? 1.0F : 1.6F, this.pos.getX() + 0.5 + vecV.x + vecH.x + this.clientBeamLook.x * 0.25, this.pos.getY() + 1 + vecV.y + vecH.y + this.clientBeamLook.y * 0.25, this.pos.getZ() + 0.5 + vecV.z + vecH.z + this.clientBeamLook.z * 0.25, this.clientBeamLook.x / 5.0, this.clientBeamLook.y / 5.0, this.clientBeamLook.z / 5.0, true);
    ...
    public static RayTraceResult fixedRayTraceBlocks(World world, BlockPos posToIgnore, Entity shooter, double size, double raystep, boolean checkTeam, Vec3d start, Vec3d end, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock) {
    ...
    @Nullable
    public static RayTraceResult rayTraceBlocks(World world, BlockPos posToIgnore, Vec3d vec31, Vec3d vec32, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock) {
        if (Double.isNaN(vec31.x) || Double.isNaN(vec31.y) || Double.isNaN(vec31.z)) {
            ...
            while (k1-- >= 0) {
                ...
                if ((blockposDown.getX() != posToIgnore.getX() || blockposDown.getY() != posToIgnore.getY() || blockposDown.getZ() != posToIgnore.getZ()) && blockDown == BlocksRegister.RUNIC_MIRROR) {
                ...
                if ((blockpos.getX() != posToIgnore.getX() || blockpos.getY() != posToIgnore.getY() || blockpos.getZ() != posToIgnore.getZ()) && block1 == BlocksRegister.RUNIC_MIRROR) {
    ...
    protected static RayTraceResult rayTrace(BlockPos tilepos, BlockPos pos, Vec3d start, Vec3d end, AxisAlignedBB boundingBox) {
        ...
        return raytraceresult == null ? null : new RayTraceResult(raytraceresult.hitVec.add(pos.getX(), pos.getY(), pos.getZ()), raytraceresult.sideHit, tilepos);
    }
