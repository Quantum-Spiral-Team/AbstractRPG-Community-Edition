...
public class TileMonsterSpawner extends TileEntity implements ITickable {
    ...
    public TileMonsterSpawner initSpawner(int baseMaxDelay, float maxDelayMultiplier, float delayKillAdd, int spawnCountMin, int spawnCountMax, float spawnRange, float spawnHeight, float activeRange, int maxMobsNearby) {
    ...
    public void update() {
        ...
        if (this.spawnDelay <= 0 && this.isActive) {
            ...
            for (Entity entity : this.world.loadedEntityList) {
                if (entity.getDistanceSq(this.pos) < this.nearCheckDistance * this.nearCheckDistance && entity instanceof AbstractMob && ((AbstractMob) entity).spawner != null) {
                    mobsSpawned++;
                }
            }
        ...
        if (this.spawnDelay % 6 == 0) {
            this.isActive = this.world.isAnyPlayerWithinRangeAt(this.pos.getX() + 0.5, this.pos.getY() + 0.5, this.pos.getZ() + 0.5, this.activeRange);
        }
    }
        ...
        if (nbttagcompound != null) {
            ...
            if (nbttagcompound.hasKey("Pos", 6)) {
                ...
                if (j >= 1) {
                    ...
                    d0 = this.pos.getX() + (this.world.rand.nextDouble() - this.world.rand.nextDouble()) * this.spawnRange + 0.5;
                }
                ...
                if (j >= 3) {
                    ...
                    d2 = this.pos.getZ() + (this.world.rand.nextDouble() - this.world.rand.nextDouble()) * this.spawnRange + 0.5;
                }
            } else {
                d0 = this.pos.getX() + (this.world.rand.nextDouble() - this.world.rand.nextDouble()) * this.spawnRange + 0.5;
                d1 = this.pos.getY();
            ...
            if (landGround) {
                d1 = getLandHeight(this.world, new BlockPos(d0, d1 + this.world.rand.nextInt((int) this.spawnHeight), d2), (int) this.spawnHeight * 2);
            } else {
    ...
    public static int getLandHeight(World world, BlockPos pos, int range) {
        ...
        for (int yx = pos.getY(); yx < pos.getY() + range; yx++) {
            if (chunk.getBlockState(pos.getX(), yx, pos.getZ()).isTopSolid() && !chunk.getBlockState(pos.getX(), yx + 1, pos.getZ()).getMaterial().blocksMovement()) {
                return yx + 1;
            }
        }
