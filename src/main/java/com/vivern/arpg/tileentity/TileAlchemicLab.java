            ...
            if (this.brewProgress < this.ticksToRecipe) {
                ...
                for (AlchemicLabRecipe recipe : AlchemicLabRecipesRegister.allRecipes) {
                    if (this.brewProgress >= recipe.ticksToBrew && this.getManaBuffer().getManaStored() >= recipe.manacost && recipe.tryCraft(this)) {
                        this.world.playSound(null, this.getPos(), Sounds.alchemic_craft, SoundCategory.BLOCKS, 1.0F, 0.9F + rand.nextFloat() / 5.0F);
        ...
        if (this.ticksExisted % 15 == 0) {
            ...
            if (this.world.isRemote) {
                if (this.tank1.getFluid() != null) {
                    ...
                    GUNParticle spelll = new GUNParticle(texbubble, 0.05F, -8.0E-4F, 40, 80, this.world, this.getPos().getX() + 0.0625F + rand.nextFloat() * 0.875F, this.getPos().getY() + 0.1875F, this.getPos().getZ() + 0.0625F + rand.nextFloat() * 0.875F, 0.0F, 0.0F, 0.0F, R, G, B, true, 0);
                    spelll.scaleTickAdding = 0.001F;
                    this.world.spawnEntity(spelll);
                }

                if (this.started && rand.nextFloat() < 0.7) {
                    this.world.spawnParticle(EnumParticleTypes.CLOUD, this.getPos().getX() + 0.5, this.getPos().getY() + 1.3, this.getPos().getZ() + 0.5, 0.0, 0.1, 0.0, new int[0]);
                }
            }
        }
    }
