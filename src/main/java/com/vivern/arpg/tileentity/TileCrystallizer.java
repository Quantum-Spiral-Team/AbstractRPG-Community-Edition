...
public class TileCrystallizer extends TileEntityLockable implements IManaBuffer, ISidedInventory, IFillDrain, ITickable {
    ...
    public static String transformName(String name) {
        if (OreDicHelper.DUSTCHARCOAL.equals(name)) {
            ...
            return !OreDicHelper.DUSTCERTUSQUARTZ.equals(name) && !OreDicHelper.DUSTNETHERQUARTZ.equals(name) && !OreDicHelper.DUSTSILICONDIOXIDE.equals(name) ? name : OreDicHelper.DUSTQUARTZ;
        }
    }
    ...
    public static boolean collidesWithSoftBlock(World world, BlockPos pos) {
        return isNonGermeticBlock(world, 3.0F, pos.up()) || isNonGermeticBlock(world, 3.0F, pos.down()) || isNonGermeticBlock(world, 3.0F, pos.west()) || isNonGermeticBlock(world, 3.0F, pos.south()) || isNonGermeticBlock(world, 3.0F, pos.north()) || isNonGermeticBlock(world, 3.0F, pos.east());
    }
    ...
    public static boolean isNonGermeticBlock(World world, float hardnessBreaks, BlockPos pos) {
        ...
        if (block.getBlock() == BlocksRegister.FLUID_HYDROTHERMAL) {
            ...
            return blockhard >= 0.0F && (Math.max(blockhard, blockres) <= hardnessBreaks || block.getBlock().isReplaceable(world, pos) || !Block.FULL_BLOCK_AABB.equals(block.getBoundingBox(world, pos)));
        }
    }
