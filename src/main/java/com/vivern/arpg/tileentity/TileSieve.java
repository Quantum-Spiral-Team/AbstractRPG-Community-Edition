    ...
    public void update() {
        ...
        if (this.sievePower > 0) {
            ...
            if (!this.world.isRemote) {
                if (this.input == null) {
                    AxisAlignedBB aabb = new AxisAlignedBB(this.pos.getX(), this.pos.getY() + 0.5, this.pos.getZ(), this.pos.getX() + 1, this.pos.getY() + 1.1F, this.pos.getZ() + 1);
                        ...
                        for (int i = 0; i < this.currentRecipe.output.size(); i++) {
                            ...
                            if (GetMOP.rand.nextFloat() < this.currentRecipe.chances[i]) {
                                ...
                                if (!OreDicHelper.isMissing(out.getItem())) {
                                    this.world.spawnEntity(new EntityItem(this.world, this.pos.getX() + 0.5, this.pos.getY() + 0.17F, this.pos.getZ() + 0.5, out));
                                }
                            }
                        }
                    ...
                    if (this.currentRecipe != null && GetMOP.rand.nextFloat() < 0.05F + limitedPower * 0.001F) {
                        this.world.playSound(null, this.getPos(), this.currentRecipe.sound, SoundCategory.BLOCKS, 0.5F, 0.9F + GetMOP.rand.nextFloat() * 0.2F);
                    }
                }
