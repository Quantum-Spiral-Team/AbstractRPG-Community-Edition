...
public class TileSplitter extends TileEntity implements ITickable, IMagicVision {
    ...
    public void onPlayerUseBeaker(ItemStack beaker, EntityPlayer playerIn) {
        ...
        if (fluidlast.isEmpty && this.dissolvedElementsSumm >= 8.0F) {
            ...
            for (int i = 0; i < laysAmount / 3.0F; i++) {
                ...
                if (randslot >= 0) {
                    spawnTentacleParticle(this.world, new Vec3d(this.pos.getX() + 0.5, this.pos.getY() + 0.75, this.pos.getZ() + 0.5), new Vec3d(rand.nextFloat() - 0.5, 2.0, rand.nextFloat() - 0.5).normalize(), playerIn, ShardType.byId(randslot + 1), 6.0F, false);
                }
            }
    ...
    public static void spawnTentacleParticle(World world, Vec3d start, Vec3d rotationStart, EntityPlayer player, ShardType shardType, float vectorLength, boolean inverse) {
        spawnTentacleParticle(world, start, rotationStart, player, EnumHand.MAIN_HAND, 0.2, -0.1, shardType, vectorLength, inverse);
    }

    public static void spawnTentacleParticle(World world, Vec3d start, Vec3d rotationStart, EntityPlayer player, @Nullable EnumHand hand, double shoulders, double yoffset, ShardType shardType, float vectorLength, boolean inverse) {
        ...
        ParticleTentacle particleTentacle = inverse ? new ParticleTentacle(shardType != ShardType.VOID ? effectTex : effectTexVoid, 0.1875F + rand.nextFloat() * 0.04F, 0.1F, 0.0F, 13 + rand.nextInt(8), 240, world, dynamic, fixed, 0.0F, 0.0F, 0.0F, shardType.colorR, shardType.colorG, shardType.colorB, true, 15, (1.0F + rand.nextFloat() * 0.25F) * vectorLength) : new ParticleTentacle(shardType != ShardType.VOID ? effectTex : effectTexVoid, 0.1875F + rand.nextFloat() * 0.04F, 0.1F, 0.0F, 13 + rand.nextInt(8), 240, world, fixed, dynamic, 0.0F, 0.0F, 0.0F, shardType.colorR, shardType.colorG, shardType.colorB, true, 15, (1.0F + rand.nextFloat() * 0.25F) * vectorLength);
    ...
    public void update() {
        ...
        if (!this.world.isRemote) {
            if (this.ticksExisted % 15 == 0) {
                ...
                AxisAlignedBB aabb = new AxisAlignedBB(this.pos.getX() + Splitter.boundOffset, this.pos.getY() + 0.3125, this.pos.getZ() + Splitter.boundOffset, this.pos.getX() + zxoffset, this.pos.getY() + 0.85, this.pos.getZ() + zxoffset);
