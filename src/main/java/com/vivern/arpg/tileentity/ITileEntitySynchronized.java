...
import com.vivern.arpg.network.packet.PacketTileEntityToClients;
import com.vivern.arpg.network.packet.PacketTileEntityToServer;
import net.minecraft.entity.EntityLivingBase;
import net.minecraft.tileentity.TileEntity;

public interface ITileEntitySynchronized {
    ...
    static void sendSynchronize(TileEntity thisentity, double distance, double... args) {
        if (!thisentity.getWorld().isRemote) {
            ...
            packet.writeArgs(thisentity.getPos().getX(), thisentity.getPos().getY(), thisentity.getPos().getZ(), args);
            PacketHandler.sendToAllAround(packet, thisentity.getWorld(), thisentity.getPos().getX(), thisentity.getPos().getY(), thisentity.getPos().getZ(), distance);
        }
    }
