...
public class TileAbsorptionTotem extends TileEntity implements IManaBuffer, ITickable, IMagicVision {
    ...
    public void update() {
        ...
        if ((this.world.getWorldTime() + 2L) % ManaBuffer.TICKRATE == 0L && this.getManaBuffer().getManaStored() < this.getManaBuffer().getManaStorageSize()) {
                ...
                for (EntityPlayer player : list) {
                    ...
                    if (Mana.getMana(player) > manaNeed) {
                        if (!this.world.isRemote) {
                            ...
                            double distModifier = MathHelper.clamp(1.0 - Math.sqrt(Minecraft.getMinecraft().getRenderViewEntity().getDistanceSq(this.pos)) / 64.0, 0.0, 1.0);
                            float sizeMultiplier = 2.0F + 5.0F * (float) (1.0 - distModifier);
                            ManaBuffer.spawnFlowParticles(this.world, player.getEntityBoundingBox(), this.pos, manaNeed, (int) (ManaBuffer.TICKRATE * distModifier / 2.0), sizeMultiplier);
                        }
