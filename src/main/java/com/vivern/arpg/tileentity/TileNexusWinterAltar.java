...
public class TileNexusWinterAltar extends TileNexus implements IVialElementsAccepter, IMagicVision, ITileEntitySynchronized {
    ...
    public void update() {
        ...
        if (this.world.isRemote && this.invasionStarted) {
            if (this.invasionTime % 20 == 0) {
                ParticleFastSummon.round2(this.world, circle_aurora, new Vec3d(this.pos.getX() + 0.5, this.pos.getY() + 1.05, this.pos.getZ() + 0.5), 0.2F, 1.2F, 20, 220, 1.0F, 1.0F, 1.0F, 2);
            }

            if (this.rand.nextFloat() < 0.25F) {
                ShardType.COLD.spawnNativeParticle(this.world, 1.0F, this.pos.getX() + 0.5, this.pos.getY() + 1.05, this.pos.getZ() + 0.5, this.rand.nextGaussian() / 19.0, this.rand.nextGaussian() / 19.0 + 0.07F, this.rand.nextGaussian() / 19.0, true);
            }
        }
        ...
        if (!this.world.isRemote && this.invasionStarted) {
            if (this.resourceItem > 0) {
                ...
                if (this.resourceComplete >= 100) {
                    this.world.spawnEntity(new EntityItem(this.world, this.pos.getX() + 0.5, this.pos.getY() + 1.5, this.pos.getZ() + 0.5, new ItemStack(this.resourceItem == 1 ? ItemsRegister.NORTHERN_INGOT : ItemsRegister.NORTHERN_SPHERE)));
                    this.resourceComplete = 0;
                    this.resourceItem = 0;
                    this.world.playSound(null, this.getPos(), Sounds.magic_k, SoundCategory.BLOCKS, 1.0F, 0.8F + this.rand.nextFloat() * 0.2F);
                }
            }
            ...
            if (this.invasionTime % 30 == 0 && this.resourceItem == 0) {
                ...
                if (can > 0) {
                    ...
                    if (shouldRemoveIngots == 0 && shouldRemoveFragments == 0 && shouldRemoveCircles == 0) {
                        ...
                        this.world.playSound(null, this.getPos(), Sounds.winter_altar_craft, SoundCategory.BLOCKS, 1.0F, 0.9F + this.rand.nextFloat() * 0.2F);
                ...
                for (Entity entityx : GetMOP.getEntitiesInAABBof(this.world, this.pos.up(), 1.3, null)) {
                    if (entityx instanceof EntityItem) {
                        ...
                        if (itemxx == ItemsRegister.WEATHER_FRAGMENTS || itemxx == ItemsRegister.ICE_CIRCLE_FILLED || OreDicHelper.itemStringOredigMatches(entityItemxx.getItem(), OreDicHelper.SILVER)) {
                            SuperKnockback.applyMove(entityItemxx, -0.13F, this.pos.getX() + 0.5, this.pos.getY() + 1.65, this.pos.getZ() + 0.5);
    ...
    public void onInvasionEnd(boolean success) {
        if (success) {
            ...
            while (i > 0) {
                ...
                i -= j;
                this.world.spawnEntity(new EntityXPOrb(this.world, this.getPos().getX() + 0.5, this.getPos().getY() + 0.75, this.getPos().getZ() + 0.5, j));
            }
            ...
            this.sendNexusMessageToAllAround("Time: " + mins + " min " + secsadd + "sec" + ", Waves completed: " + this.WAVE + ", Mobs spawned: " + this.MOBS_SPAWNED);
        }
    ...
    public void initSpawnZones() {
        ...
        this.MOB_SPAWN_ZONES[0] = new AxisAlignedBB(posX + this.ZONEhollow, posY, posZ - this.ZONEhollow, posX + this.ZONEradius, posY + this.ZONEheight, posZ + this.ZONEradius);
        this.MOB_SPAWN_ZONES[1] = new AxisAlignedBB(posX - this.ZONEradius, posY, posZ + this.ZONEhollow, posX + this.ZONEhollow, posY + this.ZONEheight, posZ + this.ZONEradius);
        this.MOB_SPAWN_ZONES[2] = new AxisAlignedBB(posX - this.ZONEradius, posY, posZ - this.ZONEradius, posX - this.ZONEhollow, posY + this.ZONEheight, posZ + this.ZONEhollow);
        this.MOB_SPAWN_ZONES[3] = new AxisAlignedBB(posX - this.ZONEhollow, posY, posZ - this.ZONEradius, posX + this.ZONEradius, posY + this.ZONEheight, posZ - this.ZONEhollow);
    }
