...
public class TileAssemblyAugment extends TileEntity implements ITickable, ITileEntitySynchronized {
                ...
                if (this.augment == 3) {
                    ...
                    if (ft1 > 0.25F && ft1 < 0.8F) {
                        ...
                        if (facing != null) {
                            ...
                            GUNParticle part = new GUNParticle(sparkle3, 0.01F, 0.0F, 4, 240, this.world, partpos.x, partpos.y, partpos.z, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, rand.nextInt(360));
                            ...
                            if (rand.nextFloat() < 0.5) {
                                part = new GUNParticle(star3, 0.03F, 0.0F, 6, 240, this.world, partpos.x, partpos.y, partpos.z, 0.0F, 0.0F, 0.0F, 0.9F, 0.95F, 1.0F, true, rand.nextInt(360));
                                part.alphaGlowing = true;
                                part.scaleTickAdding = 0.12F + rand.nextFloat() * 0.03F;
                                this.world.spawnEntity(part);
                            }

                            this.world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, partpos.x, partpos.y, partpos.z, (rand.nextFloat() - 0.5) * 0.02, (rand.nextFloat() - 0.5) * 0.04, (rand.nextFloat() - 0.5) * 0.02, new int[0]);
                        }
                    }
                }
                ...
                if (this.augment == 4) {
                    ...
                    if (Minecraft.getMinecraft().gameSettings.particleSetting < 2 && this.plasmaToolOpening >= 1.0F && ft1 > one) {
                        ...
                        if (facing != null) {
                            ...
                            GUNParticle partx = new GUNParticle(plasma_cloud, 0.09F, 0.0F, 3, 240, this.world, partposx.x, partposx.y, partposx.z, 0.0F, -0.14F, 0.0F, 0.7F + randx.nextFloat() * 0.3F, 1.0F, 1.0F, true, randx.nextInt(360));
                            partx.alphaGlowing = true;
                            partx.scaleTickAdding = 0.02F;
                            partx.randomDeath = false;
                            this.world.spawnEntity(partx);
                            partx = new GUNParticle(plasma_ring, 0.1F, 0.0F, 6, 240, this.world, partposx.x, partposx.y - 0.3125, partposx.z, 0.0F, 0.0F, 0.0F, 0.7F + randx.nextFloat() * 0.3F, 0.7F + randx.nextFloat() * 0.3F, 1.0F, true, randx.nextInt(360));
                            partx.alphaGlowing = true;
    ...
    public void startWorkingOnClient(Random rand) {
        ...
        if (this.augment == 1) {
            ...
            this.world.playSound(this.pos.getX(), this.pos.getY(), this.pos.getZ(), Sounds.augment_turning, SoundCategory.BLOCKS, 1.0F, 1.0F, false);
        }
    ...
    public Vec3d calculateWeldParticlePos(EnumFacing blockfacing) {
        ...
        Vec3d v = GetMOP.yawToVec3D(blockfacing.getHorizontalAngle() + this.manBrotateAngleY * (float) (180.0 / Math.PI)).scale(1.2 - this.weldToolAngle * 0.4);
        Vec3d rotationPoint = new Vec3d(this.pos.getX() + 0.5 + facing.getXOffset() * 0.4375, this.pos.getY() + 0.6875, this.pos.getZ() + 0.5 + facing.getZOffset() * 0.4375);
    ...
    public Vec3d calculatePlasmaParticlePos(EnumFacing blockfacing) {
        ...
        Vec3d rotationPoint = new Vec3d(this.pos.getX() + 0.5 + facing.getXOffset() * 0.4375, this.pos.getY() + 1.0F, this.pos.getZ() + 0.5 + facing.getZOffset() * 0.4375);
    ...
    public void playsound(SoundEvent soundEvent, int averageLength, int lengthAmplitude, int length) {
        ...
        this.world.playSound(this.pos.getX(), this.pos.getY(), this.pos.getZ(), soundEvent, SoundCategory.BLOCKS, 1.0F, 1.0F + difference * amplitude, false);
    }
