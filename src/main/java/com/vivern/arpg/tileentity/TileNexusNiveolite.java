...
public class TileNexusNiveolite extends TileNexus {
    ...
    public void update() {
        ...
        if (this.world.isRemote) {
            ...
            EntityPlayer entityplayer = this.world.getClosestPlayer(this.pos.getX() + 0.5F, this.pos.getY() + 0.5F, this.pos.getZ() + 0.5F, 4.0, false);
    ...
    public void landMobGroup(double x, double y, double z) {
        ...
        while (y > minn) {
            ...
            if (state.isSideSolid(this.world, blockpos$pooledmutableblockpos, EnumFacing.UP) && state.getBlock() != BlocksRegister.NIVEOUS_HOLE && this.world.isAirBlock(blockpos$pooledmutableblockpos.setPos(x, y + 1.0, z))) {
    ...
    public void onInvasionEnd(boolean success) {
        if (success) {
            ...
            while (i > 0) {
                ...
                i -= j;
                this.world.spawnEntity(new EntityXPOrb(this.world, this.getPos().getX() + 0.5, this.getPos().getY() + 1.15, this.getPos().getZ() + 0.5, j));
            }

            int count = 15 + this.rand.nextInt(6);

            for (int j = 0; j < count; j++) {
                this.world.spawnEntity(new EntityItem(this.world, this.getPos().getX() + 0.5, this.getPos().getY() + 1.15, this.getPos().getZ() + 0.5, new ItemStack(ItemsRegister.ICE_CIRCLE)));
            }
