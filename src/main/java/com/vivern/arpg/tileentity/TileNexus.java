...
public abstract class TileNexus extends TileEntity implements ITickable {
    ...
    public void spawnNexusCap(String team) {
        ...
        cap.setPosition(this.pos.getX() + 0.5, this.pos.getY() + 0.5 - cap.height / 2.0, this.pos.getZ() + 0.5);
    ...
    public void initSpawnZones() {
        ...
        this.MOB_SPAWN_ZONES[0] = new AxisAlignedBB(posX + this.ZONEhollow, posY - this.ZONEheight, posZ - this.ZONEhollow, posX + this.ZONEradius, posY + this.ZONEheight, posZ + this.ZONEradius);
        this.MOB_SPAWN_ZONES[1] = new AxisAlignedBB(posX - this.ZONEradius, posY - this.ZONEheight, posZ + this.ZONEhollow, posX + this.ZONEhollow, posY + this.ZONEheight, posZ + this.ZONEradius);
        this.MOB_SPAWN_ZONES[2] = new AxisAlignedBB(posX - this.ZONEradius, posY - this.ZONEheight, posZ - this.ZONEradius, posX - this.ZONEhollow, posY + this.ZONEheight, posZ + this.ZONEhollow);
        this.MOB_SPAWN_ZONES[3] = new AxisAlignedBB(posX - this.ZONEhollow, posY - this.ZONEheight, posZ - this.ZONEradius, posX + this.ZONEradius, posY + this.ZONEheight, posZ - this.ZONEhollow);
    }
            ...
            if (!list.isEmpty()) {
                ...
                for (NBTBase nbt : list) {
                    if (nbt instanceof NBTTagCompound) {
                        ...
                        if (tagc.hasKey("maxX") && tagc.hasKey("maxY") && tagc.hasKey("maxZ") && tagc.hasKey("minX") && tagc.hasKey("minY") && tagc.hasKey("minZ")) {
                            AxisAlignedBB aabb = new AxisAlignedBB(tagc.getDouble("minX"), tagc.getDouble("minY"), tagc.getDouble("minZ"), tagc.getDouble("maxX"), tagc.getDouble("maxY"), tagc.getDouble("maxZ"));
    ...
    public void landMobGroup(double x, double y, double z) {
        ...
        while (y > minn) {
            if (this.world.getBlockState(blockpos$pooledmutableblockpos.setPos(x, y, z)).isSideSolid(this.world, blockpos$pooledmutableblockpos, EnumFacing.UP) && this.world.isAirBlock(blockpos$pooledmutableblockpos.setPos(x, y + 1.0, z))) {
    ...
    public void sendPacket() {
        if (!this.world.isRemote) {
            ...
            if (this.invasionStarted) {
                packet.writeArgs(MathHelper.clamp(this.nexusHealth / this.MAX_HEALTH, 0.0F, 1.0F), MathHelper.clamp(this.waveCompleteRate / 100.0F, 0.0F, 1.0F), (short) this.WAVE, this.nexusId, this.WAVE == this.FINAL_WAVE);
            } else {
                packet.writeArgs(0.0F, 0.0F, (short) 0, this.nexusId, this.WAVE == this.FINAL_WAVE);
            }

            PacketHandler.sendToAllAround(packet, this.world, this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(), this.ZONEradius);
        }
    }
    ...
    public void sendNexusMessageToAllAround(String text) {
        if (!this.world.isRemote) {
            for (EntityPlayerMP playerIn : this.world.getEntitiesWithinAABB(EntityPlayerMP.class, new AxisAlignedBB(this.pos.getX() + this.ZONEradius, this.pos.getY() + this.ZONEheight, this.pos.getZ() + this.ZONEradius, this.pos.getX() - this.ZONEradius, this.pos.getY() - this.ZONEheight, this.pos.getZ() - this.ZONEradius))) {
    ...
    public List<AbstractMob> getNexusMobsAround() {
        return this.world.getEntitiesWithinAABB(AbstractMob.class, new AxisAlignedBB(this.getPos()).grow(this.ZONEradius + 8.0), mob -> mob.nexus != null);
    }
