...
public class TileSoulCatcher extends TileEntity implements ITickable, ISidedInventory, ITileEntitySynchronized {
    ...
    public void onClient(double... args) {
        if (args.length == 4) {
            ...
            ParticleTentacle.PTPOINTsimple pointstart = new ParticleTentacle.PTPOINTsimple(start, new Vec3d(rand.nextFloat() - 0.5F, rand.nextFloat() - 0.5F, rand.nextFloat() - 0.5F).normalize());
            ParticleTentacle.PTPOINTsimple pointend = new ParticleTentacle.PTPOINTsimple(end, vec);
            ParticleTentacle particleTentacle = new ParticleTentacle(effectTex, (0.1675F + rand.nextFloat() * 0.05F) * size, 0.1F, 0.0F, 19 + rand.nextInt(9), 240, this.world, pointstart, pointend, 0.0F, 0.0F, 0.0F, 1.0F - rand.nextFloat() * 0.3F, 1.0F - rand.nextFloat() * 0.1F, 1.0F - rand.nextFloat() * 0.1F, true, 11, (1.0F + rand.nextFloat() * 0.25F) * 5.0F);
    ...
    public void update() {
        ...
        if (this.ticksExisted % 60 == 0 && this.getStackInSlot(0).getItem() == ItemsRegister.SOUL_STONE) {
            ...
            if (soulid == 0) {
                ...
                Vec3d thisvec = new Vec3d(this.pos.getX() + 0.5, this.pos.getY() + 0.5, this.pos.getZ() + 0.5);
                EntityLivingBase entityLivingBase = GetMOP.entityRayTrace(this.world, thisvec, thisvec.add(vec.scale(5.0)), 2.5, 1.5, CATCHERS);
                if (entityLivingBase != null && entityLivingBase.getHealth() > 0.0F) {
                    if (Weapons.dealDamage(new WeaponDamage(null, null, null, false, false, thisvec, WeaponDamage.soul), this.damage, null, entityLivingBase, true)) {
                        if (this.damage > 0.1) {
                            ITileEntitySynchronized.sendSynchronize(this, 48.0, entityLivingBase.posX, entityLivingBase.posY + entityLivingBase.height / 2.0F, entityLivingBase.posZ, this.damage);
                        }
                        ...
                        if (entityLivingBase.getHealth() <= 0.0F) {
                            ...
                            if (!(entityLivingBase instanceof EntityPlayer) && rand.nextFloat() < entityLivingBase.getMaxHealth() / 20.0F) {
                                ...
                                this.world.playSound(null, this.pos.getX() + 0.5, this.pos.getY() + 0.5, this.pos.getZ() + 0.5, Sounds.soul_steal, SoundCategory.BLOCKS, 1.3F, 0.9F + rand.nextFloat() / 5.0F);
                                IWeapon.fireEffect(ItemsRegister.SOUL_STONE, entityLivingBase, this.world, 32.0, entityLivingBase.posX, entityLivingBase.posY + entityLivingBase.height / 2.0F, entityLivingBase.posZ, thisvec.x, thisvec.y, thisvec.z, 0.0, 0.0, 0.0);
                            }
                        }
                    }
