...
public class TilePyrocrystallineConverter extends TileEntityLockable implements ISidedInventory, IFillDrain, ITickable {
    ...
    public void update() {
        ...
        if (!this.world.isRemote) {
                    ...
                    if (this.manaStored >= 1.0F && this.melt.equals(GeomanticFocus.getMeltFromFocus(this.getStackInSlot(3)))) {
                        ...
                        if (this.world.getBlockState(focuspos).getBlock() == this.source) {
                            if (this.progress < 1000) {
                                ...
                                IBlockState statein = blockfrom == null ? Block.getBlockFromItem(OreDictionary.getOres(this.melt).get(0).getItem()).getDefaultState() : blockfrom.getDefaultState();
            ...
            EntityCubicParticle spel = new EntityCubicParticle(texcub, 0.01F + rand.nextFloat() / 100.0F, 0.0F, 30, 240, this.world, this.getPos().getX() + 0.5 + rand.nextGaussian() / 10.0, this.getPos().getY() + 0.5 + rand.nextGaussian() / 10.0, this.getPos().getZ() + 0.5 + rand.nextGaussian() / 10.0, 0.0F, 0.0F, 0.0F, 1.0F - rand.nextFloat() / 3.0F, 1.0F - rand.nextFloat() / 3.0F, 1.0F - rand.nextFloat() / 3.0F, true, rand.nextFloat(), rand.nextFloat(), rand.nextFloat(), 0.08F, true, -0.0012F + rand.nextFloat() / 1000.0F);
            SuperKnockback.applyMove(spel, -(0.1F + rand.nextFloat() / 10.0F), this.lastParticlesource.x, this.lastParticlesource.y, this.lastParticlesource.z);
            this.world.spawnEntity(spel);
            if (this.ticksExisted % 20 == 0) {
                EntityCubicParticle spel2 = new EntityCubicParticle(resou, 0.065F, 0.0F, 20, 240, this.world, this.lastParticlesource.x, this.lastParticlesource.y, this.lastParticlesource.z, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, 0.0F, 0.0F, 0.0F, 0.0F, false, 0.0F);
            ...
            TrailParticle trailpart = new TrailParticle(crcenter, 0.1F, 0.0F, 50, 240, this.world, this.getPos().getX() + 0.5 + rand.nextGaussian() / 10.0, this.getPos().getY() + 0.5 + rand.nextGaussian() / 10.0, this.getPos().getZ() + 0.5 + rand.nextGaussian() / 10.0, (float) rand.nextGaussian() / 7.0F, (float) rand.nextGaussian() / 7.0F, (float) rand.nextGaussian() / 7.0F, R, G, B, true, 0, false, 0.0F, crtrail, 0.1F, 240, R, G, B, 0.15F, 5, 0.3F, 2.0F);
    ...
    public void sendParticles(World world, BlockPos focuspos) {
        if (!world.isRemote) {
            ...
            packet.writeArgs(focuspos.getX() + 0.5, focuspos.getY() + 0.5, focuspos.getZ() + 0.5, this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(), 3);
            PacketHandler.sendToAllAround(packet, world, this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(), 32.0);
        }
    }
