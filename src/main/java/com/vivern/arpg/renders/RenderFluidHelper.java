...
public class RenderFluidHelper {
    ...
    public static void drawFluidCube(float width, float height, float filledValue, Fluid fluid, FluidStack fluidstack) {
        ...
        bufferbuilder.pos(width, 0.0, 0.0).tex(still.getMaxU(), still.getMinV() + one - onelength).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(0.0, 0.0, 0.0).tex(still.getMinU(), still.getMinV() + one - onelength).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(0.0, height, 0.0).tex(still.getMinU(), still.getMaxV()).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(0.0, height, width).tex(still.getMaxU(), still.getMaxV()).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(0.0, 0.0, width).tex(still.getMaxU(), still.getMinV() + one - onelength).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(0.0, 0.0, 0.0).tex(still.getMinU(), still.getMinV() + one - onelength).color(r, g, b, 1.0F).endVertex();
        ...
        bufferbuilder.pos(width, 0.0, width).tex(still.getMaxU(), still.getMinV() + one - onelength).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(0.0, 0.0, width).tex(still.getMinU(), still.getMinV() + one - onelength).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(width, height, 0.0).tex(still.getMinU(), still.getMaxV()).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(width, height, width).tex(still.getMaxU(), still.getMaxV()).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(width, 0.0, width).tex(still.getMaxU(), still.getMinV() + one - onelength).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(width, 0.0, 0.0).tex(still.getMinU(), still.getMinV() + one - onelength).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(0.0, height, 0.0).tex(still.getMinU(), still.getMaxV()).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(width, height, 0.0).tex(still.getMaxU(), still.getMaxV()).color(r, g, b, 1.0F).endVertex();
    ...
    public static void drawTexturedFluidRect(int x, int y, int width, int height, float filledValue, Fluid fluid, FluidStack fluidstack, double zLevel) {
        ...
        BufferBuilder bufferbuilder = tessellator.getBuffer();
        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        bufferbuilder.pos(x + 0, y + height, zLevel).tex(still.getMinU(), still.getMaxV()).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(x + width, y + height, zLevel).tex(still.getMaxU(), still.getMaxV()).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(x + width, y + 0, zLevel).tex(still.getMaxU(), still.getMinV() + one - onelength).color(r, g, b, 1.0F).endVertex();
        bufferbuilder.pos(x + 0, y + 0, zLevel).tex(still.getMinU(), still.getMinV() + one - onelength).color(r, g, b, 1.0F).endVertex();
        tessellator.draw();
    }

    public static void drawTexturedFluidRect(int x, int y, int width, int height, float filledValue, Fluid fluid, FluidStack fluidstack, double zLevel, float rr, float gg, float bb) {
        int color = fluid.getColor(fluidstack);
