...
public class RunicMirrorTESR extends TileEntitySpecialRenderer<TileRunicMirror> {
    ...
    public void renderBeam(float length, float colorR, float colorG, float colorB, float rotationPitch, float rotationYaw, TileRunicMirror mirror, float partialTicks) {
        ...
        float angl = MyRenderHelper.calculateBeamFaceToCameraAngle(mirror.getPos().getX() + 0.5, mirror.getPos().getY() + 1, mirror.getPos().getZ() + 0.5, beamLook.x, beamLook.y, beamLook.z, partialTicks);
        ...
        if (mirror.elementType == ShardType.VOID) {
            ...
            bufferbuilder.pos(0.0, -beamWidth, -length).tex(1.0, frameYoffset + oneframeY).color(1.0F, 1.0F, 1.0F, 1.0F).endVertex();
            ...
            bufferbuilder.pos(0.0, -beamWidth, 0.0).tex(0.0, frameYoffset + oneframeY).color(colorR, colorG, colorB, 1.0F).endVertex();
            tessellator.draw();
        ...
        if (mirror.elementType != ShardType.VOID) {
            bufferbuilder.pos(-roundwidthLow, roundwidthLow, -length + 0.05).tex(0.0, 0.0).color(0.5F, 0.5F, 0.5F, 0.5F).endVertex();
            bufferbuilder.pos(roundwidthLow, roundwidthLow, -length + 0.05).tex(1.0, 0.0).color(0.5F, 0.5F, 0.5F, 0.5F).endVertex();
            bufferbuilder.pos(roundwidthLow, -roundwidthLow, -length + 0.05).tex(1.0, 1.0).color(0.5F, 0.5F, 0.5F, 0.5F).endVertex();
            bufferbuilder.pos(-roundwidthLow, -roundwidthLow, -length + 0.05).tex(0.0, 1.0).color(0.5F, 0.5F, 0.5F, 0.5F).endVertex();
