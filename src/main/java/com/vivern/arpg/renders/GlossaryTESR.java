...
public class GlossaryTESR extends TileEntitySpecialRenderer<TileGlossary> {
    ...
    public void render(TileGlossary te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (te != null) {
            ...
            double tick = AnimationTimer.tick / 70.0 + te.getPos().getX() + te.getPos().getY() + te.getPos().getZ();
            ...
            model.render(null, GetMOP.partial((float) te.animationPapers, (float) te.prevanimationPapers, partialTicks) / TileGlossary.animationPapersMax, 0.0F, 1.0F, levitation2, 0.0F, 0.0625F);
        } else {
