...
public class ManaPumpTESR extends TileEntitySpecialRenderer<TileManaPump> {
    ...
    public void renderColoredGlowingCuboid(float fillValue, float y, float[] color, float[] gradientColor, float sizeX, float sizeZ, float height, float glowOffset, float partialTicks, boolean bottom, boolean top, byte modifier) {
        if (fillValue > 0.0F) {
            TEISROther.renderColoredGlowingCuboid(0.0, y, 0.0, color, gradientColor, sizeX, sizeZ, height * fillValue, glowOffset, partialTicks, bottom, top, modifier);
        }
    }
        ...
        if (te != null && te.facing != null) {
            ...
            if (flow > 0.0F) {
                ...
                if (te.facing.getAxis() == Axis.X) {
                    this.renderColoredGlowingCuboid(1.0F, 0.3125F - sizeSmall, color, color2, sizeBig, sizeSmall, sizeSmall * 2.0F, 0.03125F, partialTicks, true, true, (byte) 4);
                } else {
                    this.renderColoredGlowingCuboid(1.0F, 0.3125F - sizeSmall, color, color2, sizeSmall, sizeBig, sizeSmall * 2.0F, 0.03125F, partialTicks, true, true, (byte) 4);
                }
            }
        }
