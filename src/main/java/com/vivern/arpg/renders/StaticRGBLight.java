...
public class StaticRGBLight extends Thread {
    ...
    public void run() {
        ...
        this.world.markBlockRangeForRenderUpdate(this.posup.getX() - 15, this.posup.getY() - 15, this.posup.getZ() - 15, this.posup.getX() + 15, this.posup.getY() + 15, this.posup.getZ() + 15);
    }
    ...
    public static long getColor(IBlockState state) {
        if (state.getLightValue() <= 0) {
            ...
            if (block == Blocks.TORCH) {
                ...
                return block == BlocksRegister.DEMONIC_FIRE ? 219899808980100L : ColorConverters.RGBtoDecimal2097140((int) Debugger.floats[0], (int) Debugger.floats[1], (int) Debugger.floats[2]);
            }
        }
    }
    ...
    public static void recursiveUnglow(World world, int lastlight, BlockPos pos, BlockPos firstpos, byte lastfacing, EnumFacing backfacing) {
        ...
        for (byte i = lastfacing; b < 6; b++) {
            ...
            if (facing != backfacing.getOpposite()) {
                ...
                boolean canglow = lastlight > getValue1d3dlist(newpos.getX() - firstpos.getX() + 16, newpos.getY() - firstpos.getY() + 16, newpos.getZ() - firstpos.getZ() + 16);
                if (world.getBlockLightOpacity(newpos) < 16 && lastlight > 0 && canglow && (!state.getMaterial().isLiquid() || state.getLightValue() <= 0)) {
                    setValue1d3dlist(newpos.getX() - firstpos.getX() + 16, newpos.getY() - firstpos.getY() + 16, newpos.getZ() - firstpos.getZ() + 16, (byte) lastlight);
    ...
    public static final Vec3d getAdditiveColorInPos(BlockPos pos) {
        ...
        if (loadedrgb != null) {
            float r = LoadedRGBChunk.finalColorAdditive(loadedrgb.getBakedLight(LoadedRGBChunk.getBakedCoordRed(pos.getX(), pos.getY(), pos.getZ())));
            float g = LoadedRGBChunk.finalColorAdditive(loadedrgb.getBakedLight(LoadedRGBChunk.getBakedCoordGreen(pos.getX(), pos.getY(), pos.getZ())));
            float b = LoadedRGBChunk.finalColorAdditive(loadedrgb.getBakedLight(LoadedRGBChunk.getBakedCoordBlue(pos.getX(), pos.getY(), pos.getZ())));
        ...
        for (int l = 0; l < 15; l++) {
            for (int xc = -14; xc < 15; xc++) {
                ...
                for (int zc = -14; zc < 15; zc++) {
                    ...
                    LoadedRGBChunk chunk = xshift == 0 ? (zshift == 0 ? this.chunkMid : (zshift == 1 ? this.chunkPlusZ : this.chunkMinusZ)) : (xshift == 1 ? (zshift == 0 ? this.chunkPlusX : (zshift == 1 ? this.chunkPlusXZ : this.chunkPluXMiuZ)) : (zshift == 0 ? this.chunkMinusX : (zshift == 1 ? this.chunkPluZMiuX : this.chunkMinusXZ)));
                    if (chunk != null) {
