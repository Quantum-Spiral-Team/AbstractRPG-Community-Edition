...
public class SplitterTESR extends TileEntitySpecialRenderer<TileSplitter> {
    ...
    public float[] getColor(float timestate, TileSplitter te) {
        if (timestate >= 0.0F && timestate < 0.25F) {
            ...
            return new float[]{te.color4[0] * uncloseness + te.color1[0] * closeness, te.color4[1] * uncloseness + te.color1[1] * closeness, te.color4[2] * uncloseness + te.color1[2] * closeness};
        } else if (timestate >= 0.25F && timestate < 0.5F) {
            float closeness = (timestate - 0.25F) * 4.0F;
            float uncloseness = 1.0F - closeness;
            return new float[]{te.color1[0] * uncloseness + te.color2[0] * closeness, te.color1[1] * uncloseness + te.color2[1] * closeness, te.color1[2] * uncloseness + te.color2[2] * closeness};
        } else if (timestate >= 0.5F && timestate < 0.75F) {
            float closeness = (timestate - 0.5F) * 4.0F;
            float uncloseness = 1.0F - closeness;
            return new float[]{te.color2[0] * uncloseness + te.color3[0] * closeness, te.color2[1] * uncloseness + te.color3[1] * closeness, te.color2[2] * uncloseness + te.color3[2] * closeness};
        } else if (timestate >= 0.75F && timestate < 1.0F) {
            float closeness = (timestate - 0.75F) * 4.0F;
            float uncloseness = 1.0F - closeness;
            return new float[]{te.color3[0] * uncloseness + te.color4[0] * closeness, te.color3[1] * uncloseness + te.color4[1] * closeness, te.color3[2] * uncloseness + te.color4[2] * closeness};
        } else {
