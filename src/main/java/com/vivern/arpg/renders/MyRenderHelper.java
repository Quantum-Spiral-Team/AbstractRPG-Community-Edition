...
public class MyRenderHelper {
    ...
    public static void renderLaserSpiral(float spirallength, int spiralsegments, float spiralRadius, float lineThickness, float edgeWidth, float animationSpeed, float[] color1, float[] color2, float[] color3) {
        float R3 = color3[0];
        float G3 = color3[1];
        float B3 = color3[2];
        float A3 = color3[3];
        ...
        for (int i = 0; i < spiralsegments; i++) {
            ...
            bufferbuilder.pos(edgeWidth2, -spiralRadius, -lineThickness - depthAdd * i).color(R3 * firstCol, G3 * firstCol, B3 * firstCol, A3 * firstCol).endVertex();
    ...
    public static void renderLaserLinear(float length, int beamsCount, float angle, float scale, float zoffset, float textureYOffset, float textureRepeats, int frame, int frameCount, float[] color1, float[] color2) {
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
        if (beamsCount == 1) {
    ...
    public static void renderLaserFacedToCamera(Vec3d laserWorldPosition, Vec3d laserLook, float length, float scale, float xoffset, float zoffset, float textureYOffset, float textureRepeats, int frame, int frameCount, float[] color1, float[] color2, float partialTicks) {
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
        GlStateManager.rotate(calculateBeamFaceToCameraAngle(laserWorldPosition.x, laserWorldPosition.y, laserWorldPosition.z, laserLook.x, laserLook.y, laserLook.z, partialTicks), 0.0F, 0.0F, 1.0F);
    ...
    public static void drawVerticalLaser(BufferBuilder bufferbuilder, float finalscale, float finallength, Tessellator tessellator, float xoffset, float zoffset, float textureYOffset, float textureRepeats, int frame, int frameCount) {
        ...
        bufferbuilder.pos(-xoffset, finalscale, zoffset - finallength).tex(oneFrameLen * nextframe, textureRepeats + textureYOffset).endVertex();
        bufferbuilder.pos(-xoffset, -finalscale, zoffset - finallength).tex(oneFrameLen * frame, textureRepeats + textureYOffset).endVertex();
        bufferbuilder.pos(xoffset, -finalscale, zoffset).tex(oneFrameLen * frame, 0.0F + textureYOffset).endVertex();
        tessellator.draw();
    }

    public static void drawVerticalColoredLaser(BufferBuilder bufferbuilder, float finalscale, float finallength, Tessellator tessellator, float xoffset, float zoffset, float textureYOffset, float textureRepeats, int frame, int frameCount, float[] color1, float[] color2) {
        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        float oneFrameLen = 1.0F / frameCount;
        float nextframe = frame + 1;
        bufferbuilder.pos(xoffset, finalscale, zoffset).tex(oneFrameLen * nextframe, 0.0F + textureYOffset).color(color1[0], color1[1], color1[2], color1[3]).endVertex();
        bufferbuilder.pos(-xoffset, finalscale, zoffset - finallength).tex(oneFrameLen * nextframe, textureRepeats + textureYOffset).color(color2[0], color2[1], color2[2], color2[3]).endVertex();
        bufferbuilder.pos(-xoffset, -finalscale, zoffset - finallength).tex(oneFrameLen * frame, textureRepeats + textureYOffset).color(color2[0], color2[1], color2[2], color2[3]).endVertex();
        bufferbuilder.pos(xoffset, -finalscale, zoffset).tex(oneFrameLen * frame, 0.0F + textureYOffset).color(color1[0], color1[1], color1[2], color1[3]).endVertex();
        tessellator.draw();
    }
    ...
    public static float calculateBeamFaceToCameraAngle(double posX, double posY, double posZ, double beamLookX, double beamLookY, double beamLookZ, float partialTicks) {
        ...
        Vec3d differenceVec = new Vec3d(posOnLine.x - cameraPos.x, posOnLine.y - cameraPos.y, posOnLine.z - cameraPos.z);
    ...
    public static void renderLaserCircles(double dist, int animspeed, int circlesCount, float circleRadiusStart, float circleRadiusEnd, float[] color1, float[] color2) {
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
