...
public class BulletParticle extends Entity {
    ...
    public BulletParticle(World world, Vec3d from, Vec3d to, float scale, int livetime, int hidetime, float length, float Red, float Green, float Blue, boolean doFullEffect) {
        super(world);
        this.setPosition(from.x + (to.x - from.x) / 2.0, from.y + (to.y - from.y) / 2.0, from.z + (to.z - from.z) / 2.0);
        this.Red = Red;
        this.Green = Green;
        this.Blue = Blue;
        ...
        this.to = to;
        this.length = length;
        this.smokeDisplace = new Vec3d((this.rand.nextFloat() - 0.5) / 2.0, (this.rand.nextFloat() - 0.5) / 2.0, (this.rand.nextFloat() - 0.5) / 2.0);
    ...
    public void onUpdate() {
        ...
        if (this.ticksExisted == this.livetime && this.doFullEffect) {
            ParticleFastSummon.bulletImpact(this.world, this.textureVariant, this.scale, this.Red, this.Green, this.Blue, this.from, this.to, this.smokeRed, this.smokeGreen, this.smokeBlue, this.smokesCount, this.hidetime, this.blockDustCount, this.blockDustId);
        }
    }
