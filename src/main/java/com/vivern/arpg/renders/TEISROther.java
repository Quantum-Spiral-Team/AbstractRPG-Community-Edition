...
public class TEISROther extends TileEntityItemStackRenderer {
    ...
    public void renderByItem(ItemStack itemstack, float partialTicks) {
            ...
            if (!beakerFluid.isEmpty) {
                ...
                for (int i = 0; i <= beakerFluid.maxLay; i++) {
                    if (beakerFluid.elements[i] > 0) {
                        ...
                        float height = 0.1F;
                        float[] color = shardType == ShardType.VOID && modifier != 0 ? RetortTESR.void_additional_color : new float[]{shardType.colorR, shardType.colorG, shardType.colorB};
        ...
        if (item == ItemsRegister.SPELL_ROLL) {
            ...
            if (spells != null) {
                ...
                GlStateManager.color(Math.min(1.0F * colorAdd, 1.0F), Math.min(0.96F * colorAdd, 1.0F), Math.min(0.8F * colorAdd, 1.0F), 0.75F + 0.2F * colorAdd);
                if (colorAdd > 0.0F) {
                    AbstractMobModel.light(240, false);
                }
    ...
    public static void renderSides(double x, double y, double z, float[] color, float[] prevColor, float size, float height, float partialTicks, float alphaa, byte modifier) {
        renderSidesScaled(x, y, z, color, prevColor, size, size, height, partialTicks, alphaa, modifier);
    }

    public static void renderCap(double x, double y, double z, float[] color, boolean up, float sizeX, float sizeZ, float height, float partialTicks, float alphaa, byte modifier) {
        AbstractMobModel.light(240, false);
        ...
        if (modifier != 0 && modifier != 4) {
            ...
            if (up) {
                ...
                bufferbuilder.pos(-sizeX, height, -sizeZ).tex(0.0, oneY + frame * oneY).color(color[0], color[1], color[2], alphaa).endVertex();
                bufferbuilder.pos(-sizeX, height, sizeZ).tex(0.0, 0.0F + frame * oneY).color(color[0], color[1], color[2], alphaa).endVertex();
                bufferbuilder.pos(sizeX, height, sizeZ).tex(1.0, 0.0F + frame * oneY).color(color[0], color[1], color[2], alphaa).endVertex();
                bufferbuilder.pos(sizeX, height, -sizeZ).tex(1.0, oneY + frame * oneY).color(color[0], color[1], color[2], alphaa).endVertex();
                tessellator.draw();
            } else {
                Tessellator tessellator = Tessellator.getInstance();
                BufferBuilder bufferbuilder = tessellator.getBuffer();
                bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
                bufferbuilder.pos(sizeX, 0.0, -sizeZ).tex(0.0, oneY + frame * oneY).color(color[0], color[1], color[2], alphaa).endVertex();
                bufferbuilder.pos(sizeX, 0.0, sizeZ).tex(0.0, 0.0F + frame * oneY).color(color[0], color[1], color[2], alphaa).endVertex();
                bufferbuilder.pos(-sizeX, 0.0, sizeZ).tex(1.0, 0.0F + frame * oneY).color(color[0], color[1], color[2], alphaa).endVertex();
                bufferbuilder.pos(-sizeX, 0.0, -sizeZ).tex(1.0, oneY + frame * oneY).color(color[0], color[1], color[2], alphaa).endVertex();
                tessellator.draw();
            }
        } else {
    ...
    public static void renderCap(double x, double y, double z, float[] color, boolean up, float size, float height, float partialTicks, float alphaa, byte modifier) {
        renderCap(x, y, z, color, up, size, size, height, partialTicks, alphaa, modifier);
    }

    public static void renderColoredGlowingCuboid(double x, double y, double z, float[] color, float[] gradientColor, float size, float height, float glowOffset, float partialTicks, boolean bottom, boolean top, byte modifier) {
        renderColoredGlowingCuboid(x, y, z, color, gradientColor, size, size, height, glowOffset, partialTicks, bottom, top, modifier);
    }

    public static void renderColoredGlowingCuboid(double x, double y, double z, float[] color, float[] gradientColor, float sizeX, float sizeZ, float height, float glowOffset, float partialTicks, boolean bottom, boolean top, byte modifier) {
        float alphaa1 = 1.0F;
        float alphaa2 = 0.4F;
        if (modifier == 4) {
    ...
    public static void renderColoredCuboid(double x, double y, double z, float[] color, float[] gradientColor, float sizeX, float sizeZ, float height, float glowOffset, float partialTicks, boolean bottom, boolean top, byte modifier) {
        float alphaa1 = 1.0F;
        float alphaa2 = 0.4F;
        GlStateManager.blendFunc(SourceFactor.SRC_ALPHA, DestFactor.ONE_MINUS_SRC_ALPHA);
        renderSidesScaled(x, y, z, color, gradientColor, sizeX, sizeZ, height, partialTicks, alphaa1, modifier);
    ...
    public static void renderSidesScaled(double x, double y, double z, float[] color, float[] prevColor, float sizeX, float sizeZ, float height, float partialTicks, float alphaa, byte modifier) {
        AbstractMobModel.light(240, false);
        RenderHelper.disableStandardItemLighting();
        GlStateManager.shadeModel(7425);
        GlStateManager.enableBlend();
        ...
        if (modifier != 0 && modifier != 4) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                BufferBuilder bufferbuilder = tessellator.getBuffer();
                bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
                bufferbuilder.pos(sizeX, 0.0, sizeX).tex(oneX + texX, 0.25F + texY).color(prevColor[0], prevColor[1], prevColor[2], alphaa).endVertex();
                bufferbuilder.pos(sizeX, height, sizeX).tex(oneX + texX, 0.0F + texY).color(color[0], color[1], color[2], alphaa).endVertex();
                bufferbuilder.pos(-sizeX, height, sizeX).tex(0.0F + texX, 0.0F + texY).color(color[0], color[1], color[2], alphaa).endVertex();
                bufferbuilder.pos(-sizeX, 0.0, sizeX).tex(0.0F + texX, 0.25F + texY).color(prevColor[0], prevColor[1], prevColor[2], alphaa).endVertex();
                tessellator.draw();
                GlStateManager.rotate(90.0F, 0.0F, 1.0F, 0.0F);
            }
        } else {
    ...
    public void renderEffect(ItemStack itemStack, float[] color, TransformType transform, boolean dark) {
        float alpha = 0.3F;
        int decimalcolor = dark ? ColorConverters.RGBAtoDecimal(1.0F, 1.0F, 1.0F, alpha) : ColorConverters.RGBAtoDecimal(color[0] * alpha, color[1] * alpha, color[2] * alpha, 1.0F);
    ...
    public static void drawTextured3dModalRect(float x, float y, float z, int textureX, int textureY, int ontexWidth, int ontexHeight, float rectWidth, float rectHeight) {
        float f = 0.0078125F;
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
