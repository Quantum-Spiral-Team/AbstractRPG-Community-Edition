...
public class ManaBar {
    ...
    public void renderPlayerManaBar() {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            if (!this.mc.gameSettings.showDebugInfo) {
                ...
                for (int vess = 1; vess <= 3; vess++) {
                    ...
                    if (energy > 0.0F) {
                        ...
                        drawCustomSizedTexturedRect(i - 58 - screenOffset + vess * 12, f - progrEner - screenOffset, 64, 256, 18 + u, v + 64 - progrEner, 10, progrEner);
                    }
                }
                ...
                if (AnimationTimer.normaltick < timeSpellForge) {
                    ...
                    if (spellForgeDisplayed != null && spellForgeDisplayed.currentForging != null) {
                        ...
                        float progress = MathHelper.clamp(spellForgeDisplayed.currentForging.hammerCompleteness / spellForgeDisplayed.currentForging.hitsNeed, 0.0F, 1.0F);
                        ...
                        if (spellForgeDisplayed.currentForging.canAcceptSpells) {
                            float progress2 = spellForgeDisplayed.currentForging.fullComplete ? 1.0F : MathHelper.clamp(1.0F - spellForgeDisplayed.currentForging.ticks / 160.0F, 0.0F, 1.0F);
                            drawCustomSizedTexturedRect((double) i / 2 - 104, 30 + Yadd, 256, 256, 24, spellForgeDisplayed.currentForging.fullComplete ? 36 : 34, (int) (208.0F * progress2), 2);
            ...
            if (this.mc.currentScreen != null && this.mc.currentScreen instanceof InventoryEffectRenderer) {
                ...
                this.mc.fontRenderer.drawStringWithShadow(asString(player.getHealth()) + "/" + asString((float) player.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).getAttributeValue()), xOffsetText, f - 95, 16777215);
    ...
    public static void drawCustomSizedTexturedRect(double x, double y, double z, int allTextureSizeX, int allTextureSizeY, int texu, int texv, int width, int height) {
    ...
    public void renderItemOverlayIntoGUI(IWeapon weapon, ItemStack stack, int xPosition, int yPosition) {
        if (!stack.isEmpty()) {
            ...
            if (f3 > 0.0F) {
                ...
                this.draw(bufferbuilder1, xPosition, yPosition + MathHelper.floor(16.0F * (1.0F - f3)), 16, MathHelper.ceil(16.0F * f3), 255, 219, 148, 127);
                GlStateManager.enableTexture2D();
                GlStateManager.enableLighting();
                GlStateManager.enableDepth();
            }
