...
public class RenderSpecial<T extends Entity> extends Render<Entity> {
    ...
    public void doRender(Entity entityTo, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        if (this.type == 3) {
            ...
            if (entity.to != null) {
                GlStateManager.pushMatrix();
                GlStateManager.translate(x + entity.to.x - entity.posX, y + entity.to.y - entity.posY, z + entity.to.z - entity.posZ);
                GlStateManager.enableRescaleNormal();
        ...
        if (this.type == 12) {
            ...
            float swing = MathHelper.sin(swingtick * (5.0F / entityxxxx.bubbleSize)) * swingtick * (0.06F + (3.0F - MathHelper.clamp(entityxxxx.bubbleSize, 0.0F, 3.0F)) * 0.1F);
            GlStateManager.translate((float) x, (float) y + entityTo.height / 2.0F, (float) z);
            GlStateManager.scale(1.0F + entityxxxx.renderSizeSwingX * swing, 1.0F + entityxxxx.renderSizeSwingY * swing, 1.0F + entityxxxx.renderSizeSwingZ * swing);
        ...
        if (this.type == 13) {
            ...
            GlStateManager.rotate(entityxxxxx.prevRotationYaw + (entityxxxxx.rotationYaw - entityxxxxx.prevRotationYaw) * partialTicks - 90.0F, 0.0F, 1.0F, 0.0F);
            ...
            float beamwidth = entityxxxxx.cutterSize / 2.0F * (fromto - (entityxxxxx.destroying ? GetMOP.getFromTo(entityxxxxx.destroyTick + partialTicks, 0.0F, 4.0F) : 0.0F));
        ...
        if (this.type == 14) {
            ...
            if (entityxxxxx.impacted && entityxxxxx.impactPos != null && player != null) {
                if (entityxxxxx.impactEntity != null) {
                    x = entityxxxxx.impactPos.x + GetMOP.partial(entityxxxxx.impactEntity.posX, entityxxxxx.impactEntity.prevPosX, partialTicks) - GetMOP.partial(player.posX, player.prevPosX, partialTicks);
                    y = entityxxxxx.impactPos.y + GetMOP.partial(entityxxxxx.impactEntity.posY, entityxxxxx.impactEntity.prevPosY, partialTicks) - GetMOP.partial(player.posY, player.prevPosY, partialTicks);
                    z = entityxxxxx.impactPos.z + GetMOP.partial(entityxxxxx.impactEntity.posZ, entityxxxxx.impactEntity.prevPosZ, partialTicks) - GetMOP.partial(player.posZ, player.prevPosZ, partialTicks);
                } else {
            ...
            double motion = Math.sqrt(entityxxxxx.motionX * entityxxxxx.motionX + entityxxxxx.motionY * entityxxxxx.motionY + entityxxxxx.motionZ * entityxxxxx.motionZ);
        ...
        if (this.type == 16) {
            ...
            OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, 240.0F, 240.0F);
            GlStateManager.translate((float) x, (float) y, (float) z);
            GlStateManager.rotate(entityxxxxxx.prevRotationYaw + (entityxxxxxx.rotationYaw - entityxxxxxx.prevRotationYaw) * partialTicks - 90.0F, 0.0F, 1.0F, 0.0F);
        ...
        if (this.type == 17) {
            ...
            bufferbuilder.pos(-beamwidth + onePartWidth, -scaleY, -0.04F * zScale + offsetZ).tex(0.33333334F, posBeamSmallHigher).endVertex();
            bufferbuilder.pos(-beamwidth + onePartWidth, -scaleY, -0.04F * zScale + offsetZ).tex(0.33333334F, posBeamSmallHigher).endVertex();
            bufferbuilder.pos(-beamwidth + onePartWidth, scaleY, -0.04F * zScale + offsetZ).tex(0.33333334F, posBeamBigHigher).endVertex();
            bufferbuilder.pos(-beamwidth + onePartWidth * 2.0F, scaleY, -0.04F * zScale + offsetZ).tex(0.6666667F, posBeamBigHigher).endVertex();
            bufferbuilder.pos(-beamwidth + onePartWidth * 2.0F, -scaleY, -0.04F * zScale + offsetZ).tex(0.6666667F, posBeamSmallHigher).endVertex();
            bufferbuilder.pos(-beamwidth + onePartWidth * 2.0F, -scaleY, -0.04F * zScale + offsetZ).tex(0.6666667F, posBeamSmallHigher).endVertex();
            bufferbuilder.pos(-beamwidth + onePartWidth * 2.0F, scaleY, -0.04F * zScale + offsetZ).tex(0.6666667F, posBeamBigHigher).endVertex();
        ...
        if (this.type == 20) {
            ...
            if (entityxxxxxx.segmentPoses != null) {
                ...
                for (int i = 0; i < entityxxxxxx.segmentPoses.length - 1; i++) {
                    ...
                    RenderTentacles.renderSegment(distTo, distTo / 14.0, i > halfLength ? (scalePrev += scaleAdding) : scalePrev, scalePrev, pX1 - cameraX, pY1 - cameraY, pZ1 - cameraZ, rotatPitch, rotatYaw);
                }
            }
