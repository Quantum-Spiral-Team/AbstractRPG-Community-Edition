        ...
        if (beakerFluid != null && !beakerFluid.isEmpty) {
            ...
            for (int i = 0; i <= beakerFluid.maxLay; i++) {
                if (beakerFluid.elements[i] > 0) {
                    ...
                    float[] color = shardType == ShardType.VOID && modifier != 0 ? void_additional_color : new float[]{shardType.colorR, shardType.colorG, shardType.colorB};
                    boolean firstLay = i == 0;
                    boolean lastLay = i == beakerFluid.maxLay;
                    if (firstLay && lastLay) {
                        TEISROther.renderSides(0.0, 0.1875F + height * i - boundOffset, 0.0, color, color, 0.25F, height + boundOffset * 2.0F, partialTicks, 0.45F, modifier);
