...
public class TRRenderer {
    ...
    public static class TRRendererScatter extends TRRenderer {
        ...
        public TRRendererScatter(ResourceLocation texture, int texturesAmount, int density, int x, int y, int texx, int texy, int texWidth, int texHeight, int allTextureSizeX, int allTextureSizeY, int boundCheckSize) {
        ...
        public TRRendererScatter(ResourceLocation texture, int texturesAmount, int density, int texx, int texy, int texWidth, int texHeight, int allTextureSizeX, int allTextureSizeY, int boundCheckSize) {
            this.texture = texture;
            this.x = 0;
            this.y = 0;
            this.texx = texx;
        ...
        public void render(RenderTerraformingResearch renderTFR, float posx, float posy, int arrayX, int arrayY, long seed, float fullness) {
            ...
            for (int i = 0; i < this.density; i++) {
                ...
                double animAdd = this.animationDelay == 0 ? 0.0 : onepixelY * this.texHeight * ((double) (this.time() + rand.nextInt(999999)) / this.animationDelay % this.animationCycle);
                ...
                if (renderTFR.checkForBounds(xadd, yadd, arrayX, arrayY, TerraformingResearchSurface.TRSurfaceType.TERRAIN, this.boundCheckSize)) {
                    bufferbuilder.pos(posx - halfW + xadd, posy + halfHminus + yadd, this.layer).tex(u + uadd, v + animAdd).color(r, g, b, alpha).endVertex();
                    bufferbuilder.pos(posx - halfW + xadd, posy + halfHpluss + yadd, this.layer).tex(u + uadd, v + onepixelY * this.texHeight + animAdd).color(r, g, b, alpha).endVertex();
                    bufferbuilder.pos(posx + halfW + xadd, posy + halfHpluss + yadd, this.layer).tex(u + uadd + onepixelX * this.texWidth, v + onepixelY * this.texHeight + animAdd).color(r, g, b, alpha).endVertex();
                    bufferbuilder.pos(posx + halfW + xadd, posy + halfHminus + yadd, this.layer).tex(u + uadd + onepixelX * this.texWidth, v + animAdd).color(r, g, b, alpha).endVertex();
                }
            }
    ...
    public static class TRRendererSprite extends TRRenderer {
        ...
        public void render(RenderTerraformingResearch renderTFR, float posx, float posy, int arrayX, int arrayY, long seed, float fullness) {
            ...
            BufferBuilder bufferbuilder = tessellator.getBuffer();
            bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
            bufferbuilder.pos(posx - halfW, posy - halfH, this.layer).tex(u, v + animAdd).color(r2 * alpha1, g2 * alpha1, b2 * alpha1, alpha1).endVertex();
            bufferbuilder.pos(posx - halfW, posy + halfH, this.layer).tex(u, v + onepixelY * this.texHeight + animAdd).color(r1 * alpha2, g1 * alpha2, b1 * alpha2, alpha2).endVertex();
            bufferbuilder.pos(posx + halfW, posy + halfH, this.layer).tex(u + onepixelX * this.texWidth, v + onepixelY * this.texHeight + animAdd).color(r1 * alpha3, g1 * alpha3, b1 * alpha3, alpha3).endVertex();
            bufferbuilder.pos(posx + halfW, posy - halfH, this.layer).tex(u + onepixelX * this.texWidth, v + animAdd).color(r2 * alpha4, g2 * alpha4, b2 * alpha4, alpha4).endVertex();
            tessellator.draw();
    ...
    public static class TRRendererSpriteGlimmer extends TRRenderer {
        ...
        public TRRendererSpriteGlimmer(ResourceLocation texture, int x, int y, int texx, int texy, int texWidth, int texHeight, int allTextureSizeX, int allTextureSizeY, float glimmerPeriod) {
            this.texture = texture;
            this.x = x;
            this.y = y;
            this.texx = texx;
        ...
        public TRRendererSpriteGlimmer(ResourceLocation texture, int texx, int texy, int texWidth, int texHeight, int allTextureSizeX, int allTextureSizeY, float glimmerPeriod) {
            this.texture = texture;
            this.x = 0;
            this.y = 0;
            this.texx = texx;
        ...
        public void render(RenderTerraformingResearch renderTFR, float posx, float posy, int arrayX, int arrayY, long seed, float fullness) {
            ...
            bufferbuilder.pos(posx + halfW, posy + halfH, this.layer).tex(u + onepixelX * this.texWidth, v + onepixelY * this.texHeight + animAdd).endVertex();
