...
public class InitMobRenders {
    ...
    public static void init() {
        ...
        for (AbstractMob.RenderAbstractMobEntry entry : torender) {
            if (entry.rocket) {
                RenderingRegistry.registerEntityRenderingHandler(entry.mobClass, RenderRocketMob.getFACTORY(entry.model, entry.mobTexture, entry.shadowSize, entry.verticalRocket, entry).setLayerHeldItem(entry.layerHeldItem).setlayerArmor(entry.layerArmor));
            } else if (entry.getMultitexture) {
                RenderingRegistry.registerEntityRenderingHandler(entry.mobClass, RenderMobMultitexture.getFACTORY(entry.model, entry.shadowSize).setLayerHeldItem(entry.layerHeldItem).setlayerArmor(entry.layerArmor));
            } else {
                RenderingRegistry.registerEntityRenderingHandler(entry.mobClass, RenderUniversal.getFACTORY(entry.model, entry.mobTexture, entry.shadowSize, entry.useIRender, entry).setLayerHeldItem(entry.layerHeldItem).setlayerArmor(entry.layerArmor).setLayerRandomItem(entry.layerrandItem));
            }
        }
    }
