...
public class RenderTentacles {
    ...
    public static void testRender(Entity entity, float partialTicks) {
        Vec3d[] segmentsPoses = getTentacleSegmenstPositions(entity.getPositionVector().add(0.0, -0.5, 0.0), new Vec3d(Debugger.floats[0] - 154.5, Debugger.floats[1] + 71.0F, Debugger.floats[2] + 246.5), new Vec3d(0.0, -1.0, 0.0), new Vec3d(0.0, 1.0, 0.0), 10.0F + Debugger.floats[3], 20 + (int) Debugger.floats[4]);
    ...
    public static void renderTentacleAnimFulltexture(Vec3d[] segmentsPoses, float overlapShare, float progress, float partialTicks, float scaleStart, float scaleEnd, int lightStart, int lightEnd) {
        double cameraX = 0.0;
        double cameraY = 0.0;
        double cameraZ = 0.0;
        ...
        for (int i = 0; i < segmentsPoses.length - 1; i++) {
            ...
            renderSegment(distTo, distTo * overlapShare, scalePrev, scalePrev + scaleAdding, point1.x - cameraX, point1.y - cameraY, point1.z - cameraZ, rotatPitch, rotatYaw, textureYmin, textureYmax);
    ...
    public static void renderTentacleAnimGlow(Vec3d[] segmentsPoses, float partialTicks, float scaleStart, float scaleEnd, int lightStart, int lightEnd, int segmentGlowed, float R, float G, float B, int segmentGlowedLight) {
        double cameraX = 0.0;
        double cameraY = 0.0;
        double cameraZ = 0.0;
        Entity rvEntity = Minecraft.getMinecraft().getRenderViewEntity();
        ...
        for (int i = 0; i < segmentsPoses.length - 1; i++) {
            ...
            renderSegment(distTo, distTo / 14.0, scalePrev, scalePrev + scaleAdding, point1.x - cameraX, point1.y - cameraY, point1.z - cameraZ, rotatPitch, rotatYaw);
            AbstractMobModel.returnlight();
            if (doUndo) {
    ...
    public static void renderSegment(double dist, double overlap, double scaleMin, double scaleMax, double x, double y, double z, float pitch, float yaw, float textureYmin, float textureYmax) {
        GlStateManager.pushMatrix();
        GlStateManager.translate((float) x, (float) y, (float) z);
        GlStateManager.rotate(-yaw, 0.0F, 1.0F, 0.0F);
        GlStateManager.rotate(pitch, 1.0F, 0.0F, 0.0F);
    ...
    public static Vec3d[] getTentacleSegmenstPositions(Vec3d pos1, Vec3d pos2, Vec3d normalizedRotation1, Vec3d normalizedRotation2, double rotationVecLength, int posesAmount) {
        ...
        for (int i = 0; i < posesAmount; i++) {
            ...
            Vec3d line1 = pos1.add(normalizedRotation1.x * lengthSplitted, normalizedRotation1.y * lengthSplitted, normalizedRotation1.z * lengthSplitted);
            Vec3d line2 = pos2.add(normalizedRotation2.x * unlengthSplitted, normalizedRotation2.y * unlengthSplitted, normalizedRotation2.z * unlengthSplitted);
            poses[i] = new Vec3d(line1.x * unratio + line2.x * ratio, line1.y * unratio + line2.y * ratio, line1.z * unratio + line2.z * ratio);
        }
