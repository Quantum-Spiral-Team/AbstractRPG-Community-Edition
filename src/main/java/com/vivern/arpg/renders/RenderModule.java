...
public abstract class RenderModule {
    ...
    public interface IRenderModuleMulticolored {
        ...
        @Nullable Vec3d getColor(int var1);
    ...
    public static class RMod2dSprite extends RenderModule {
        ...
        public static int[] randomOffsets = new int[]{rand.nextInt(256), rand.nextInt(256), rand.nextInt(256), rand.nextInt(256), rand.nextInt(256), rand.nextInt(256), rand.nextInt(256), rand.nextInt(256), rand.nextInt(256), rand.nextInt(256)};
        ...
        public void render(Entity entity, double x, double y, double z, float entityYaw, float partialTicks) {
            ...
            if (!this.disableOnEnd || time < this.framecount) {
                ...
                if (this.turnToCamera) {
                    xx += (Minecraft.getMinecraft().getRenderManager().options.thirdPersonView == 2 ? -1 : 1) * Minecraft.getMinecraft().getRenderManager().playerViewX;
    ...
    public static class RModCeratargetTail extends RenderModule {
        ...
        public RModCeratargetTail(ResourceLocation tex, float red1, float green1, float blue1, float red2, float green2, float blue2, float motionLengthMultiplier, float lengthMin, float lengthMax, float displacement, float constriction, int light) {
            this.tex = tex;
            this.red1 = red1;
            this.green1 = green1;
            this.blue1 = blue1;
        ...
        public RModCeratargetTail(ResourceLocation tex, float motionLengthMultiplier, float lengthMin, float lengthMax, float displacement, float constriction, int light) {
            this.tex = tex;
            this.motionLengthMultiplier = motionLengthMultiplier;
            this.lengthMin = lengthMin;
            this.lengthMax = lengthMax;
        ...
        public void render(Entity entity, double x, double y, double z, float entityYaw, float partialTicks) {
            ...
            double motion = Math.sqrt(entity.motionX * entity.motionX + entity.motionY * entity.motionY + entity.motionZ * entity.motionZ);
            ...
            bufferbuilder.pos(-xDisp - xDispAdd, yUp - yDispAdd, zLength).tex(0.0, 1.0F + texYAdd).color(R2, G2, B2, 0.0F).endVertex();
            bufferbuilder.pos(xDisp + xDispAdd, yUp - yDispAdd, zLength).tex(1.0, 1.0F + texYAdd).color(R2, G2, B2, 0.0F).endVertex();
            ...
            bufferbuilder.pos(-xDisp - xDispAdd, yDown + yDispAdd, zLength).tex(0.0, 1.0F + texYAdd).color(R2, G2, B2, 0.0F).endVertex();
            bufferbuilder.pos(xDisp + xDispAdd, yDown + yDispAdd, zLength).tex(1.0, 1.0F + texYAdd).color(R2, G2, B2, 0.0F).endVertex();
    ...
    public static class RModCubic extends RenderModule {
        ...
        public RModCubic(ResourceLocation tex, float scale, int light, float Red, float Green, float Blue, float Alpha, boolean blend, int rotateMode, float rotateSpeed) {
            this.scale = scale;
            this.light = light;
            this.tex = tex;
            this.Red = Red;
    ...
    public static class RModCutter extends RenderModule {
        ...
        public RModCutter(ResourceLocation tex, float cutterSize, float zScale, float yScale, int light, float red, float green, float blue, float alpha, boolean blend, float rotationRoll, int ticksForMaxWidth, int pixelsTopView, int pixelsFrontView) {
            this.light = light;
            this.tex = tex;
            this.Red = red;
            this.Green = green;
        ...
        public void render(Entity entity, double x, double y, double z, float entityYaw, float partialTicks) {
            ...
            bufferbuilder.pos(-beamwidth + onePartWidth, scaleY, -0.04F * this.zScale + offsetZ).tex(0.33333334F, posBeamBigHigher).endVertex();
            bufferbuilder.pos(-beamwidth + onePartWidth, -scaleY, -0.04F * this.zScale + offsetZ).tex(0.33333334F, posBeamSmallHigher).endVertex();
            bufferbuilder.pos(-beamwidth + onePartWidth, -scaleY, -0.04F * this.zScale + offsetZ).tex(0.33333334F, posBeamSmallHigher).endVertex();
            bufferbuilder.pos(-beamwidth + onePartWidth, scaleY, -0.04F * this.zScale + offsetZ).tex(0.33333334F, posBeamBigHigher).endVertex();
            bufferbuilder.pos(-beamwidth + onePartWidth * 2.0F, scaleY, -0.04F * this.zScale + offsetZ).tex(0.6666667F, posBeamBigHigher).endVertex();
            bufferbuilder.pos(-beamwidth + onePartWidth * 2.0F, -scaleY, -0.04F * this.zScale + offsetZ).tex(0.6666667F, posBeamSmallHigher).endVertex();
            bufferbuilder.pos(-beamwidth + onePartWidth * 2.0F, -scaleY, -0.04F * this.zScale + offsetZ).tex(0.6666667F, posBeamSmallHigher).endVertex();
            bufferbuilder.pos(-beamwidth + onePartWidth * 2.0F, scaleY, -0.04F * this.zScale + offsetZ).tex(0.6666667F, posBeamBigHigher).endVertex();
    ...
    public static class RModModel extends RenderModule {
        ...
        public RModModel(ResourceLocation tex, ModelBase model, float scale, int light, float Red, float Green, float Blue, float Alpha, boolean blend, int rotateMode, float rotateSpeed) {
            this.scale = scale;
            this.light = light;
            this.tex = tex;
            this.Red = Red;
        ...
        public RModModel(ResourceLocation tex, ModelBase model, float scale, int light, float Red, float Green, float Blue, float Alpha, boolean blend, int rotateMode, float rotateSpeed, int rotateSeed) {
            this.scale = scale;
            this.light = light;
            this.tex = tex;
            this.Red = Red;
    ...
    public static class RModRenderByBoundingBox extends RenderModule {
        ...
        public void render(Entity entity, double x, double y, double z, float entityYaw, float partialTicks) {
            ...
            if (this.stretchTexture) {
                ...
                bufferbuilder.pos(aabb.minX, aabb.minY, aabb.minZ).tex(aabb.minX, aabb.minZ).endVertex();
                bufferbuilder.pos(aabb.maxX, aabb.minY, aabb.minZ).tex(aabb.maxX, aabb.minZ).endVertex();
                bufferbuilder.pos(aabb.maxX, aabb.minY, aabb.maxZ).tex(aabb.maxX, aabb.maxZ).endVertex();
                bufferbuilder.pos(aabb.minX, aabb.minY, aabb.maxZ).tex(aabb.minX, aabb.maxZ).endVertex();
                bufferbuilder.pos(aabb.minX, aabb.maxY, aabb.minZ).tex(aabb.minX, aabb.minZ).endVertex();
                bufferbuilder.pos(aabb.maxX, aabb.maxY, aabb.minZ).tex(aabb.maxX, aabb.minZ).endVertex();
                bufferbuilder.pos(aabb.maxX, aabb.maxY, aabb.maxZ).tex(aabb.maxX, aabb.maxZ).endVertex();
                bufferbuilder.pos(aabb.minX, aabb.maxY, aabb.maxZ).tex(aabb.minX, aabb.maxZ).endVertex();
                bufferbuilder.pos(aabb.minX, aabb.minY, aabb.minZ).tex(aabb.maxX, -aabb.minY).endVertex();
                bufferbuilder.pos(aabb.maxX, aabb.minY, aabb.minZ).tex(aabb.minX, -aabb.minY).endVertex();
                bufferbuilder.pos(aabb.maxX, aabb.maxY, aabb.minZ).tex(aabb.minX, -aabb.maxY).endVertex();
                bufferbuilder.pos(aabb.minX, aabb.maxY, aabb.minZ).tex(aabb.maxX, -aabb.maxY).endVertex();
                bufferbuilder.pos(aabb.minX, aabb.minY, aabb.maxZ).tex(aabb.minX, -aabb.minY).endVertex();
                bufferbuilder.pos(aabb.maxX, aabb.minY, aabb.maxZ).tex(aabb.maxX, -aabb.minY).endVertex();
                bufferbuilder.pos(aabb.maxX, aabb.maxY, aabb.maxZ).tex(aabb.maxX, -aabb.maxY).endVertex();
                bufferbuilder.pos(aabb.minX, aabb.maxY, aabb.maxZ).tex(aabb.minX, -aabb.maxY).endVertex();
                bufferbuilder.pos(aabb.minX, aabb.minY, aabb.minZ).tex(aabb.minZ, -aabb.minY).endVertex();
                bufferbuilder.pos(aabb.minX, aabb.minY, aabb.maxZ).tex(aabb.maxZ, -aabb.minY).endVertex();
                bufferbuilder.pos(aabb.minX, aabb.maxY, aabb.maxZ).tex(aabb.maxZ, -aabb.maxY).endVertex();
                bufferbuilder.pos(aabb.minX, aabb.maxY, aabb.minZ).tex(aabb.minZ, -aabb.maxY).endVertex();
                bufferbuilder.pos(aabb.maxX, aabb.minY, aabb.minZ).tex(aabb.maxZ, -aabb.minY).endVertex();
                bufferbuilder.pos(aabb.maxX, aabb.minY, aabb.maxZ).tex(aabb.minZ, -aabb.minY).endVertex();
                bufferbuilder.pos(aabb.maxX, aabb.maxY, aabb.maxZ).tex(aabb.minZ, -aabb.maxY).endVertex();
                bufferbuilder.pos(aabb.maxX, aabb.maxY, aabb.minZ).tex(aabb.maxZ, -aabb.maxY).endVertex();
            }
    ...
    public static class RModTail extends RenderModule {
        ...
        public RModTail(ResourceLocation tex, float scale, float length, boolean lengthMultipliedBySpeed, float centerRatio, int light, float Red, float Green, float Blue, float Alpha, boolean blend, int style) {
            this.scale = scale;
            this.length = length;
        ...
        public void render(Entity entity, double x, double y, double z, float entityYaw, float partialTicks) {
            ...
            if (this.style == 0) {
                ...
                GlStateManager.rotate(MyRenderHelper.calculateBeamFaceToCameraAngle(entity.posX, entityY, entity.posZ, entity.motionX, entity.motionY, entity.motionZ, partialTicks), 0.0F, 0.0F, 1.0F);
