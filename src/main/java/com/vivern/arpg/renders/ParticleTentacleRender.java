...
public class ParticleTentacleRender<T extends ParticleTentacle> extends Render<ParticleTentacle> {
    ...
    public void doRender(ParticleTentacle entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        Vec3d[] segmentsPoses = RenderTentacles.getTentacleSegmenstPositions(pos1, pos2, entity.start.getNormalizedRotationVector(partialTicks), entity.end.getNormalizedRotationVector(partialTicks), entity.rotationVecLength, entity.segments + 1);
        RenderTentacles.renderTentacleAnimFulltexture(segmentsPoses, entity.overlap, (entity.ticksExisted + partialTicks) / entity.livetime, partialTicks, entity.scale, entity.scaleEnd, lightStart, lightEnd);
        if (entity.disableLightning) {
            GL11.glEnable(2896);
        }
