...
public class ParticleTracker<T extends Entity> {
    ...
    public static class TrackerChangeTexOnDropAnim extends ParticleTracker<AnimatedGParticle> {
        ...
        public TrackerChangeTexOnDropAnim(ResourceLocation tex, boolean enableAcidRend, boolean enableAlphaGlow, int newFramecount, int newDelay, boolean newDisableOnEnd) {
            this.tex = tex;
            this.enableAcidRend = enableAcidRend;
            this.enableAlphaGlow = enableAlphaGlow;
    ...
    public static class TrackerDrunkFollowPoint extends ParticleTracker {
        ...
        public TrackerDrunkFollowPoint(Vec3d point, boolean deadOnArrive, float arriveradius, float tickPowerIncrease, float power, float sinusMultipl, float drunkPower, Vec3d direction) {
            this.point = point;
    ...
    public static class TrackerFollowDynamicPoint extends ParticleTracker {
        ...
        public void update(Entity entity) {
            ...
            this.power = this.power + this.tickPowerIncrease;
            this.frictionMult = this.frictionMult + this.tickfrictionAdd;
            if (this.deadOnArrive && entity.getDistanceSq(this.point.posX, this.point.posY + this.addY, this.point.posZ) < this.arriveradius) {
                entity.setDead();
            }
        }
    ...
    public static class TrackerFractalRender extends ParticleTracker<GUNParticle> {
        ...
        public TrackerFractalRender(int count, float stepMultiplier, float rotateAngle, float rotateExponent, float redirectionChance, int showTime, int showedTime, int hideTime) {
            this.count = count;
        ...
        public void render(GUNParticle entity, double x, double y, double z, float entityYaw, float partialTicks) {
            ...
            for (int i = 0; i < this.count; i++) {
                float alpha = GetMOP.getFromTo(time, (float) i, (float) (i + this.showTime)) - GetMOP.getFromTo(time, (float) (i + this.showedTime), (float) (i + this.showedTime + this.hideTime));
            ...
            float alpha1 = GetMOP.getFromTo(time, 0.0F, (float) this.showTime) - GetMOP.getFromTo(time, (float) this.showedTime, (float) (this.showedTime + this.hideTime));
    ...
    public static class TrackerLinkBeams extends ParticleTracker<GUNParticle> {
        ...
        public TrackerLinkBeams(int identifier, ResourceLocation beamTex, float colorR, float colorG, float colorB, float size, int maxNodes, int livetimeBeam, float chance, float findRadius) {
                ...
                if (other != null) {
                    ...
                    for (int i = 1; i <= nodes; i++) {
                        ...
                        if (i == nodes) {
                            ...
                            pos2 = pos1.add(subVector.x * i + rand.nextGaussian() * this.spread, subVector.y * i + rand.nextGaussian() * this.spread + this.upMotion * rand.nextFloat(), subVector.z * i + rand.nextGaussian() * this.spread);
                        }

                        BetweenParticle laser = new BetweenParticle(entity.world, this.beamTex, this.useMultipliedSize ? entity.scale * this.size : this.size, 240, this.colorR, this.colorG, this.colorB, 1.0F / this.livetimeBeam, prevpos1.distanceTo(pos2), this.livetimeBeam, 0.0F, 9999.0F, prevpos1, pos2);
    ...
    public static class TrackerSinusScaling extends ParticleTracker<GUNParticle> {
        ...
        @Override
        public void render(GUNParticle entity, double x, double y, double z, float entityYaw, float partialTicks) {
            float finalscale = 1.0F + MathHelper.sin((entity.ticksExisted + entity.getEntityId() * 2.3481033F + partialTicks) * this.sinSpeed) * this.amplitude;
            ...
            for (int i = 1; i < this.times.length; i++) {
                ...
                if (entity.ticksExisted < time && entity.ticksExisted >= prevtime) {
                    ...
                    if (Debugger.floats[0] == 0.0F) {
                        ...
                        entity.setPosition(GetMOP.partial(point.x, prevpoint.x, ft1), GetMOP.partial(point.y, prevpoint.y, ft1), GetMOP.partial(point.z, prevpoint.z, ft1));
                    }
                }
            }
        }
