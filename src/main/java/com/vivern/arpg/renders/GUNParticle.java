...
public class GUNParticle extends EntityThrowable {
    ...
    public GUNParticle(ResourceLocation texture, float scale, float gravity, int livetime, int light, World world, double x, double y, double z, float speedx, float speedy, float speedz, float Red, float Green, float Blue, boolean opa, int rotation) {
        super(world, x, y, z);
        this.scale = scale;
    ...
    public GUNParticle(ResourceLocation texture, float scale, float gravity, int livetime, int light, World world, double x, double y, double z, float speedx, float speedy, float speedz, float Red, float Green, float Blue, boolean opa, int rotation, boolean collide, float frictionMultipl) {
        super(world, x, y, z);
        this.scale = scale;
        this.gravity = gravity;
        this.livetime = livetime;
    ...
    public GUNParticle(ResourceLocation texture, float scale, float gravity, int livetime, int light, World world, double x, double y, double z, float speedx, float speedy, float speedz, float Red, float Green, float Blue, boolean opa, int rotation, boolean collide, float frictionMultipl, float scaleTickAdding, float scaleMax, float scaleTickDropAdding) {
        super(world, x, y, z);
        this.scale = scale;
        this.gravity = gravity;
        this.livetime = livetime;
    ...
    protected void onImpact(RayTraceResult result) {
        if (this.collide && this.ticksExisted > 1 && result.typeOfHit == Type.BLOCK && this.world.getBlockState(result.getBlockPos()).getBlock().getCollisionBoundingBox(this.world.getBlockState(result.getBlockPos()), this.world, result.getBlockPos()) != null) {
            if (this.sticky) {
                this.motionY = 0.0;
                this.motionX = 0.0;
                this.motionZ = 0.0;
