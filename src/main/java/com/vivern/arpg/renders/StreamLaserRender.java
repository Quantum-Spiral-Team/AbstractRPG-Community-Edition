...
public class StreamLaserRender<T extends EntityStreamLaserP> extends Render<EntityStreamLaserP> {
    ...
    public void doRender(EntityStreamLaserP entity, double x, double y, double z, float entityYaw, float partialTicks) {
        if (entity.useModel) {
            ...
            float rp = entity.player != null ? (this.renderManager.options.thirdPersonView == 2 ? 1 : 1) * entity.player.rotationPitch : GetMOP.partial(entity.rotatPitch, entity.prevrotatPitch, partialTicks);
            float rw = entity.player != null ? GetMOP.partial(entity.player.rotationYaw, entity.player.prevRotationYaw, partialTicks) : GetMOP.partial(entity.rotatYaw, entity.prevrotatYaw, partialTicks);
            rp = -rp;
            rw = -rw + 180.0F;
            ...
            if (entity.player != null && (entity.hand != EnumHand.MAIN_HAND || entity.player.getPrimaryHand() != EnumHandSide.RIGHT) && (entity.hand != EnumHand.OFF_HAND || entity.player.getPrimaryHand() != EnumHandSide.LEFT)) {
                anglefix1 = -anglefix1;
            }

            Vec3d rvePos = MyRenderHelper.getRenderViewEntityPosition(partialTicks);
            Vec3d entityPos = entity.getPositionEyes(partialTicks);
            entity.model.renderLaserModel(entity, entityPos, rp - anglefix2, rw + anglefix1, entity.distance, entity.horizOffset, entityPos.x - rvePos.x, entityPos.y - rvePos.y, entityPos.z - rvePos.z, partialTicks);
        } else {
            ...
            bufferbuilder.pos(-entity.scale - entity.horizOffset, -0.1, entity.distanceStart).tex(0.0, 1.0 + dist / entity.lengh + entity.Tickpl * entity.animation).endVertex();
            ...
            bufferbuilder.pos(entity.scale - entity.horizOffset, -0.1, entity.distanceStart).tex(1.0, 1.0 + dist / entity.lengh + entity.Tickpl * entity.animation).endVertex();
            tessellator.draw();
            ...
            if (entity.horizontal) {
                ...
                bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
                bufferbuilder2.pos(-entity.horizOffset, -entity.scale - 0.1, entity.distanceStart).tex(0.0, 1.0 + dist / entity.lengh + entity.Tickpl * entity.animation).endVertex();
                ...
                bufferbuilder2.pos(-entity.horizOffset, entity.scale - 0.1, entity.distanceStart).tex(1.0, 1.0 + dist / entity.lengh + entity.Tickpl * entity.animation).endVertex();
                tessellator2.draw();
