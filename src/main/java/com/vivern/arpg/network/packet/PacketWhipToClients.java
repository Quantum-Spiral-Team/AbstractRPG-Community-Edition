...
public class PacketWhipToClients extends Packet {
    ...
    public void client(EntityPlayer player, Packet sp, MessageContext ctx) {
        FMLCommonHandler.instance().getWorldThread(ctx.netHandler).addScheduledTask(() -> this.processMessage(Item.getItemById(this.id)));
    }
    ...
    void processMessage(Item item) {
        try {
            ((FireWhip) item).effect(this.clientPlayer().world, this.x, this.y, this.z, (float) this.a, (float) this.b, (float) this.c, this.d1, this.d2, this.d3, (float) this.v1, (float) this.v2, (float) this.v3);
