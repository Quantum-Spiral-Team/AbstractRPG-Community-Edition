...
public class PacketNexusInfoToClients extends Packet {
    ...
    public void client(EntityPlayer player, Packet sp, MessageContext ctx) {
        ...
        if (this.waveCompleteRate == -200.0F) {
            ...
            player.world.playSound(player, player.posX, player.posY, player.posZ, Sounds.invasion_lose, SoundCategory.AMBIENT, 2.0F, 1.0F);
            InvasionInfo.nexusHealth = 0.0F;
        } else if (this.waveCompleteRate == -100.0F) {
            InvasionInfo.wave = nw;
            InvasionInfo.winAnim = 200;
            player.world.playSound(player, player.posX, player.posY, player.posZ, Sounds.final_wave, SoundCategory.AMBIENT, 2.0F, 1.0F);
            player.world.playSound(player, player.posX, player.posY, player.posZ, SoundEvents.UI_TOAST_CHALLENGE_COMPLETE, SoundCategory.AMBIENT, 2.0F, 1.0F);
            ...
            if (nw > InvasionInfo.wave && !fw) {
                ...
                player.world.playSound(player, player.posX, player.posY, player.posZ, Sounds.next_wave, SoundCategory.AMBIENT, 2.0F, 1.0F);
            } else {
                InvasionInfo.wave = nw;
            }
