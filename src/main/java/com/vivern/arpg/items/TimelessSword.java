...
public class TimelessSword extends ItemWeapon {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack) {
                    ...
                    if (hand == EnumHand.MAIN_HAND) {
                        ...
                        if (sweep(this, itemstack, player, hand, 10 + sharpness, 0.4F + knockback * 0.45F, 3.2F + sweeping * 0.3F, 0.9F + sweeping * 0.2F, 1.0F + sweeping * 0.2F, itemRand.nextFloat() < 0.2)) {
    ...
    public static boolean sweep(IWeapon iweapon, ItemStack stack, EntityPlayer player, EnumHand hand, float damage, float knockback, float length, float size, float endSize, boolean isCritical) {
        ...
        IWeapon.fireEffect(ItemsRegister.TIMELESS_SWORD, player, world, 64.0, vec.x, vec.y, vec.z, effectrotat, player.rotationPitch, player.rotationYaw, 0.0, 0.0, 0.0);
        ...
        if (shouldTimeless && !world.isRemote) {
            float damag = (float) (10.0 + player.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue() + EnchantmentHelper.getEnchantmentLevel(Enchantments.SHARPNESS, stack) + crit);
            float kn = (float) (player.getEntityAttribute(PropertiesRegistry.MELEE_KNOCKBACK).getAttributeValue() + knockback / 2.0);
            EntityTimelessSword esword = new EntityTimelessSword(world, stack, player, vec, damag, crit > 0.0F, kn, player.rotationYaw, player.rotationPitch, effectrotat);
            Vec3d pose = player.getPositionEyes(1.0F);
            esword.setPosition(pose.x, pose.y, pose.z);
            world.spawnEntity(esword);
            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.time_sword, SoundCategory.PLAYERS, 0.6F, 0.9F + itemRand.nextFloat() / 5.0F);
        }
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        SweepParticle particle = new SweepParticle(sweeptex, 1.5F + itemRand.nextFloat() / 4.0F, 6, 1, 4, 2, (int) (player.getBrightness() * 150.0F), world, x, y, z, (float) b, (float) c, 1.0F, 1.0F, 1.0F, true, (int) a);
