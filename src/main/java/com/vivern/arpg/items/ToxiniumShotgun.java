                ...
                if (click && player.getHeldItemMainhand() == itemstack) {
                    if (ammo > 0 && this.isReloaded(itemstack)) {
                        if (!hascooldown) {
                            ...
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.explode, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                                        ...
                                        if (Team.checkIsOpponent(player, entity)) {
                                            ...
                                            if (entity instanceof EntityLivingBase) {
                                                ...
                                                if (entitylivingbase.getHealth() <= 0.0F) {
                                                    ...
                                                    PotionEffect baff = new PotionEffect(PotionEffects.TOXIN, parameters.getEnchantedI("potion_time", witchery), parameters.getEnchantedI("potion_power", witchery));
                                                    entitylivingbase.addPotionEffect(baff);
                                                } else {
                                                    PotionEffect baff = new PotionEffect(PotionEffects.TOXIN, parameters.getEnchantedI("potion_time", witchery) + ItemBullet.BulletToxic.duration, parameters.getEnchantedI("potion_power", witchery));
                                                    entitylivingbase.addPotionEffect(baff);
                                                }
                                            }
                                        }
                            ...
                            IWeapon.fireEffect(this, player, world, 64.0, player.getEntityId(), seed, edist, amount, c, impacts, inaccuracy, 0.0, 0.0);
                        }
                    } else if (this.initiateBulletReload(itemstack, player, ItemsRegister.TOXINIUM_SHOTGUN_CLIP, maxammo, true)) {
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.shotgun_rel, SoundCategory.AMBIENT, 0.7F, 1.0F);
    ...
    public void effect(EntityPlayer clientplayer, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        if (playe instanceof EntityPlayer) {
            ...
            for (int i = 0; i < Math.min(a, 31.0); i++) {
                ...
                BulletParticle part = new BulletParticle(world, from, to, 0.1F, (int) MathHelper.clamp(dist / 3.0F, 2.0F, 8.0F), 7, dist, (float) col.x, (float) col.y, (float) col.z, full);
                part.Red2 = 0.3F;
                part.Green2 = 1.0F;
                part.Blue2 = 0.3F;
                if (full) {
