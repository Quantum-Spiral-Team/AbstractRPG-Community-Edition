...
public class ToolWand extends Item {
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        if (this.place == null) {
            this.place = this.placeMeta < 0 ? Block.getBlockFromName(this.placeName).getDefaultState() : Block.getBlockFromName(this.placeName).getStateFromMeta(this.placeMeta);
        }

        if (this.canReplaceOnSneak) {
            if (this.replace == null) {
                this.replace = this.replaceMeta < 0 ? Block.getBlockFromName(this.replaceName).getDefaultState() : Block.getBlockFromName(this.replaceName).getStateFromMeta(this.replaceMeta);
            }

            if (this.toReplace == null) {
                this.toReplace = this.toReplaceMeta < 0 ? Block.getBlockFromName(this.toReplaceName).getDefaultState() : Block.getBlockFromName(this.toReplaceName).getStateFromMeta(this.toReplaceMeta);
            }
        }
        ...
        if (player.isSneaking() && this.canReplaceOnSneak) {
            ...
            if (blockFrom == this.replace.getBlock()) {
                ...
                world.playSound(null, pos, blockTo.getSoundType(this.toReplace, world, pos, player).getPlaceSound(), SoundCategory.BLOCKS, 0.8F, 0.9F + itemRand.nextFloat() / 5.0F);
            ...
            if (result.sideHit != null) {
                ...
                if (block.canPlaceBlockOnSide(world, offsetPos, result.sideHit) && block.canPlaceBlockAt(world, offsetPos) && (replaceable || world.isAirBlock(offsetPos)) && player.inventory.hasItemStack(this.consume)) {
                    player.inventory.clearMatchingItems(this.consume.getItem(), this.ignoreMetaConsume ? -1 : this.consume.getMetadata(), this.consume.getCount(), null);
                    if (replaceable) {
                        world.destroyBlock(offsetPos, true);
                    }

                    world.setBlockState(offsetPos, this.place);
                    world.playSound(null, offsetPos, block.getSoundType(this.place, world, offsetPos, player).getPlaceSound(), SoundCategory.BLOCKS, 0.8F, 0.9F + itemRand.nextFloat() / 5.0F);
                    return new ActionResult<>(EnumActionResult.SUCCESS, itemstack);
                }
            }
        }
