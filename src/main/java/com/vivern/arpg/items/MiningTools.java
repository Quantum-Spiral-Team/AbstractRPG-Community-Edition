...
import com.vivern.arpg.items.animation.AnimationAndFluidCapProvider;
import com.vivern.arpg.items.models.ChainsawModel;
import com.vivern.arpg.items.models.DrillModel;
import com.vivern.arpg.items.models.LaserModel;
import com.vivern.arpg.items.models.MiningLaserModel;
...
public class MiningTools {
    ...
    public MiningTools(ResourceLocation mainTexture, String name, int toolLEVEL, float averageDigSpeed, String nameDrill, String nameChainsaw, int maxRF, int throughput, int needRF, int averageDurability, int fuelCapacity, MiningLaser miningLaser, Vec3d mainColor) {
                ...
                if (mode == 0) {
                    ...
                    for (BlockPos blockPos : listPoses) {
                        ...
                        if (BlockBreaking.isToolEffective("axe", state)) {
                            if (blockBreaking.damageBlock(player, blockPos, breakingSpeed, "axe", toolLevel, EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, itemstack), EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, itemstack) > 0)) {
                                blocksBreak++;
                            }
                        } else if (BlockBreaking.isToolEffective("shears", state)) {
                            if (blockBreaking.damageBlock(player, blockPos, breakingSpeed, "shears", toolLevel, EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, itemstack), EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, itemstack) > 0)) {
                                blocksBreak++;
                            }
                        } else if (blockBreaking.damageBlock(player, blockPos, breakingSpeed, "axe", toolLevel, EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, itemstack), EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, itemstack) > 0)) {
                            blocksBreak++;
                        }
                    }
                    ...
                    if (player.ticksExisted % (4 - (int) (Math.min(speed, 20) * 0.15F)) == 0) {
                        ...
                        for (Entity entity : world.getEntitiesWithinAABBExcludingEntity(player, miningBox)) {
                            if (Team.checkIsOpponent(player, entity)) {
                                ...
                                Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, false, player, WeaponDamage.blade), damage, player, entity, true, this.getKnockbackToMobsChainsaw(itemstack));
                ...
                if (mode == 1 || mode == 2) {
                    ...
                    if (result.typeOfHit == Type.BLOCK && result.getBlockPos() != null) {
                        ...
                        if (player.ticksExisted % 5 == 0) {
                            ...
                            if (BlockBreaking.isToolEffective("axe", state)) {
                                ...
                                blocksBreak = blockBreaking.damageMultiblock(this, player, blockPosx, breakingSpeed * 5.0F * digSpeedMult, "axe", toolLevel, EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, itemstack), EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, itemstack) > 0);
                            }
                        }
                    }
                    ...
                    if (player.ticksExisted % (5 - (int) (Math.min(speed, 20) * 0.15F)) == 0) {
                        ...
                        if (result.typeOfHit == Type.ENTITY && result.entityHit != null && Team.checkIsOpponent(player, result.entityHit)) {
                            ...
                            Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, false, player, WeaponDamage.blade), damage, player, result.entityHit, true, 0.1F);
                    ...
                    for (int y = startY; y <= 1; y++) {
                        for (int z = -1; z <= 1; z++) {
                            if (x != 0 || y != 0 || z != 0) {
                                ...
                                if ((offpos.getX() != prev.getX() || offpos.getY() != prev.getY() || offpos.getZ() != prev.getZ()) && (offpos.getX() != prevprev.getX() || offpos.getY() != prevprev.getY() || offpos.getZ() != prevprev.getZ()) && this.isLog(world, offpos)) {
                                    boolean is = true;

                                    for (BlockPos has : list) {
                                        if (offpos.getX() == has.getX() && offpos.getY() == has.getY() && offpos.getZ() == has.getZ()) {
                                            is = false;
                                            break;
                                        }
                                    }
    ...
    public static class CorrosiveWaterblaster extends MiningLaser {
        ...
        @Override
        public void makeLaserParticles(World world, @Nullable EntityPlayer player, double xStart, double yStart, double zStart, double xEnd, double yEnd, double zEnd, boolean hit, int mode, double distance, float rotatPitch, float rotatYaw) {
            ...
            if (hit) {
                ...
                GUNParticle part = new GUNParticle(slimesplash, 0.05F, 0.0F, lt, 225, world, xEnd, yEnd, zEnd, 0.0F, 0.0F, 0.0F, 0.5F + 0.3F * randcol, 0.75F + 0.15F * randcol, 1.0F, true, itemRand.nextInt(360));
                part.alphaTickAdding = -1.0F / lt;
                part.scaleTickAdding = scl / lt;
                world.spawnEntity(part);
                if (itemRand.nextFloat() < 0.3F) {
                    randcol = 0.3F;
                    Vec3d spedVec = new Vec3d(xEnd - xStart, yEnd - yStart, zEnd - zStart).normalize().scale(-0.1).add((itemRand.nextFloat() - 0.5F) * randcol, (itemRand.nextFloat() - 0.5F) * randcol, (itemRand.nextFloat() - 0.5F) * randcol);
                    ShardType.WATER.spawnNativeParticle(world, 1.0F, xEnd, yEnd, zEnd, (float) spedVec.x, (float) spedVec.y, (float) spedVec.z, true);
                } else {
                    randcol = 0.4F;
                    lt = 16 + itemRand.nextInt(12);
                    Vec3d spedVec = new Vec3d(xEnd - xStart, yEnd - yStart, zEnd - zStart).normalize().scale(-0.3).add((itemRand.nextFloat() - 0.5F) * randcol, (itemRand.nextFloat() - 0.5F) * randcol, (itemRand.nextFloat() - 0.5F) * randcol);
                    part = new GUNParticle(pixel, 0.03125F, 0.04F, lt, 240, world, xEnd, yEnd, zEnd, (float) spedVec.x, (float) spedVec.y, (float) spedVec.z, 0.45F - itemRand.nextFloat() * 0.2F, 1.0F - itemRand.nextFloat() * 0.3F, 1.0F - itemRand.nextFloat() * 0.1F, false, 0, true, 1.2F);
                ...
                AnimatedGParticle particle = new AnimatedGParticle(typee ? plasma_a : plasma_b, 0.3F + itemRand.nextFloat() * 0.2F, 0.0F, ltx, 240, world, xEnd, yEnd, zEnd, (itemRand.nextFloat() - 0.5F) * scl, (itemRand.nextFloat() - 0.5F) * scl, (itemRand.nextFloat() - 0.5F) * scl, 0.2F - itemRand.nextFloat() * 0.1F, 0.4F - itemRand.nextFloat() * 0.1F, 0.9F, true, itemRand.nextInt(360));
        ...
        public void onHitEntity(Entity entity, ItemStack itemstack, World world, EntityPlayer player, int mode, int speed, int itemSlot) {
            if (!world.isRemote && Team.checkIsOpponent(player, entity)) {
                float damage = mode == 1 ? this.getDamageToMobsLaser(itemstack) * oneshotDamageMult : this.getDamageToMobsLaser(itemstack) * (speed / this.gatMaxSpeed(itemstack, player));
                Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, false, player, WeaponDamage.water), damage, player, entity, true, this.getKnockbackToMobsLaser(itemstack));
                entity.hurtResistantTime = 0;
                entity.extinguish();
            }
        }
            ...
            if (!world.isRemote) {
                ...
                for (BlockPos blockPos : listposes) {
                    ...
                    if (BlockBreaking.isToolEffective("pickaxe", state)) {
                        if (blockBreaking.damageBlock(player, blockPos, breakingSpeed, "pickaxe", toolLevel, EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, itemstack), EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, itemstack) > 0)) {
                            blocksBreak++;
                        }
                    } else if (BlockBreaking.isToolEffective("shovel", state)) {
                        if (blockBreaking.damageBlock(player, blockPos, breakingSpeed, "shovel", toolLevel, EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, itemstack), EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, itemstack) > 0)) {
                            blocksBreak++;
                        }
                    } else if (blockBreaking.damageBlock(player, blockPos, breakingSpeed, "pickaxe", toolLevel, EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, itemstack), EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, itemstack) > 0)) {
                        blocksBreak++;
                    }
                }
                ...
                if (player.ticksExisted % (5 - (int) (Math.min(speed, 20) * 0.15F)) == 0) {
                    ...
                    for (Entity entity : world.getEntitiesWithinAABBExcludingEntity(player, miningBox)) {
                        if (Team.checkIsOpponent(player, entity)) {
                            ...
                            Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, false, player, WeaponDamage.drill), damage, player, entity, true, this.getKnockbackToMobsDrill(itemstack));
                            entity.hurtResistantTime = 0;
                            Weapons.mixPotion(entity, PotionEffects.BROKEN_ARMOR, 40.0F, 1.0F, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumMathOperation.PLUS, Weapons.EnumMathOperation.PLUS, 2000, 10);
                        }
                    }
                }
    ...
    public static class EyelightProspector extends ElectricMiningLaser {
        ...
        public void makeLaserParticles(World world, @Nullable EntityPlayer player, double xStart, double yStart, double zStart, double xEnd, double yEnd, double zEnd, boolean hit, int mode, double distance, float rotatPitch, float rotatYaw) {
            ...
            if (hit) {
                ...
                GUNParticle part = new GUNParticle(magic_effect_2, 0.05F, 0.0F, lt, 240, world, xEnd, yEnd, zEnd, 0.0F, 0.0F, 0.0F, 0.95F - itemRand.nextFloat() * 0.1F, 1.0F - itemRand.nextFloat() * 0.1F, 0.5F, true, itemRand.nextInt(360));
                ...
                Vec3d spedVec = new Vec3d(xEnd - xStart, yEnd - yStart, zEnd - zStart).normalize().scale(-0.3).add((itemRand.nextFloat() - 0.5F) * spd0, (itemRand.nextFloat() - 0.5F) * spd0, (itemRand.nextFloat() - 0.5F) * spd0);
                GUNParticle pix = new GUNParticle(pixel, 0.03125F, 0.04F, lt0, 240, world, xEnd, yEnd, zEnd, (float) spedVec.x, (float) spedVec.y, (float) spedVec.z, 1.0F - itemRand.nextFloat() * 0.2F, 1.0F - itemRand.nextFloat() * 0.2F, 0.6F - itemRand.nextFloat() * 0.2F, false, 0, true, 1.2F);
                ...
                float spd1 = 0.1F;
                float randcol = itemRand.nextFloat();
                AnimatedGParticle particle = new AnimatedGParticle(typee ? plasma_a : plasma_b, 0.3F + itemRand.nextFloat() * 0.2F, 0.0F, ltx, 240, world, xEnd, yEnd, zEnd, (itemRand.nextFloat() - 0.5F) * spd1, (itemRand.nextFloat() - 0.5F) * spd1, (itemRand.nextFloat() - 0.5F) * spd1, 0.95F - itemRand.nextFloat() * 0.2F, 1.0F - itemRand.nextFloat() * 0.2F, 0.5F, true, itemRand.nextInt(360));
                particle.alphaGlowing = true;
                particle.alphaTickAdding = -1.0F / ltx;
                particle.framecount = typee ? 14 : 24;
                particle.animCounter = itemRand.nextInt(10);
        ...
        public void onHitEntity(Entity entity, ItemStack itemstack, World world, EntityPlayer player, int mode, int speed, int itemSlot) {
            if (!world.isRemote && Team.checkIsOpponent(player, entity)) {
                float damage = mode == 1 ? this.getDamageToMobsLaser(itemstack) * oneshotDamageMult : this.getDamageToMobsLaser(itemstack) * (speed / this.gatMaxSpeed(itemstack, player));
                Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, false, player, WeaponDamage.laser), damage, player, entity, true, this.getKnockbackToMobsLaser(itemstack));
                entity.hurtResistantTime = 0;
            }
        }
    ...
    public static class LaserDigger extends ElectricMiningLaser {
        ...
        public void makeLaserParticles(World world, @Nullable EntityPlayer player, double xStart, double yStart, double zStart, double xEnd, double yEnd, double zEnd, boolean hit, int mode, double distance, float rotatPitch, float rotatYaw) {
            ...
            GUNParticle bigsmoke = new GUNParticle(forge_hit_a, 0.01F, 0.0F, 5, 240, world, xEnd, yEnd, zEnd, 0.0F, 0.0F, 0.0F, 1.0F, colrand * 0.3F, colrand * 0.3F, true, itemRand.nextInt(360));
            ...
            int lt0 = 16 + itemRand.nextInt(12);
            Vec3d spedVec = new Vec3d(xEnd - xStart, yEnd - yStart, zEnd - zStart).normalize().scale(-0.3).add((itemRand.nextFloat() - 0.5F) * spd0, (itemRand.nextFloat() - 0.5F) * spd0, (itemRand.nextFloat() - 0.5F) * spd0);
            GUNParticle pix = new GUNParticle(pixel, 0.03125F, 0.04F, lt0, 240, world, xEnd, yEnd, zEnd, (float) spedVec.x, (float) spedVec.y, (float) spedVec.z, 1.0F, colrand * 0.25F, colrand * 0.25F, false, 0, true, 1.2F);
            pix.scaleTickAdding = -0.03125F / lt0;
            world.spawnEntity(pix);
            int lt = 10 + itemRand.nextInt(10);
            boolean typee = itemRand.nextFloat() < 0.5F;
            float spd1 = 0.1F;
            AnimatedGParticle particle = new AnimatedGParticle(typee ? plasma_a : plasma_b, 0.3F + itemRand.nextFloat() * 0.2F, 0.0F, lt, 240, world, xEnd, yEnd, zEnd, (itemRand.nextFloat() - 0.5F) * spd1, (itemRand.nextFloat() - 0.5F) * spd1, (itemRand.nextFloat() - 0.5F) * spd1, 1.0F - colrand * 0.1F, 0.0F, colrand * 0.4F, true, itemRand.nextInt(360));
            particle.alphaGlowing = true;
    ...
    public static class MiningLaser extends AbstractMiningTool {
        ...
        public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
            ...
            if (entity != null && entity instanceof EntityPlayer) {
                ...
                Vec3d shootPoint = Weapons.getPlayerShootPoint(shootingPlayer, EnumHand.MAIN_HAND, 1.0F, this.getLaserParam(16), this.getLaserParam(17), this.getLaserParam(18), false);
                ...
                this.makeLaserParticles(world, shootingPlayer, shootPoint.x, shootPoint.y, shootPoint.z, x, y, z, c >= 20.0, (int) (c >= 20.0 ? c - 20.0 : c), dist, (float) a, (float) b);
            }
        }
        ...
        public void onUpdateMining(ItemStack itemstack, World world, EntityPlayer player, int mode, int speed, int itemSlot) {
            ...
            if (mode == 0 || mode == 1) {
                ...
                RayTraceResult result = GetMOP.fixedRayTraceBlocks(world, player, 0.2, true, vec3d, vec3d2, false, true, false, this.canImpactGlass ? null : this.filterGlass);
                Vec3d vec = vec3d2;
                int blocksBreak = 0;
                boolean hit = false;
                if (result != null && result.typeOfHit == Type.BLOCK && result.getBlockPos() != null) {
                    ...
                    if (!world.isRemote) {
                        ...
                        if (blockBreaking.damageBlock(player, blockPos, breakingSpeed, mostEffective, toolLevel, EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, itemstack), EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, itemstack) > 0)) {
                            blocksBreak++;
                        }
                    }
                ...
                if (!world.isRemote) {
                    if (mode == 1) {
                        IWeapon.fireEffect(this, player, world, 64.0, vec.x, vec.y, vec.z, player.rotationPitch, player.rotationYaw, hit ? mode + 20 : mode, player.getEntityId(), 0.0, 0.0);
                    } else {
                        IWeapon.fireEffectExcl(this, player, player, world, 64.0, vec.x, vec.y, vec.z, player.rotationPitch, player.rotationYaw, hit ? mode + 20 : mode, player.getEntityId(), 0.0, 0.0);
                    }
                } else {
                    Vec3d shootPoint = Weapons.getPlayerShootPoint(player, EnumHand.MAIN_HAND, 1.0F, this.getLaserParam(16), this.getLaserParam(17), this.getLaserParam(18), false);
                    this.makeLaserParticles(world, player, shootPoint.x, shootPoint.y, shootPoint.z, vec.x, vec.y, vec.z, hit, mode, vec.distanceTo(shootPoint), 0.0F, 0.0F);
                }
                ...
                if (speed == 0) {
                    world.playSound(null, player.posX, player.posY, player.posZ, mode == 1 ? this.oneshotSound : this.startSound, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                }
            }
        }
    ...
    public static class NuclearMiningRay extends ElectricMiningLaser {
        ...
        public void makeLaserParticles(World world, @Nullable EntityPlayer player, double xStart, double yStart, double zStart, double xEnd, double yEnd, double zEnd, boolean hit, int mode, double distance, float rotatPitch, float rotatYaw) {
            ...
            world.spawnEntity(laser);
            if (hit) {
                AnimatedGParticle rad = new AnimatedGParticle(radiation, 0.2F, 0.0F, 16, 240, world, xEnd, yEnd, zEnd, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 0.92F + itemRand.nextFloat() * 0.08F, true, itemRand.nextInt(360));
                rad.framecount = 16;
                ...
                int lt0 = 16 + itemRand.nextInt(12);
                Vec3d spedVec = new Vec3d(xEnd - xStart, yEnd - yStart, zEnd - zStart).normalize().scale(-0.3).add((itemRand.nextFloat() - 0.5F) * spd0, (itemRand.nextFloat() - 0.5F) * spd0, (itemRand.nextFloat() - 0.5F) * spd0);
                GUNParticle pix = new GUNParticle(pixel, 0.03125F, 0.04F, lt0, 240, world, xEnd, yEnd, zEnd, (float) spedVec.x, (float) spedVec.y, (float) spedVec.z, 0.98F, 1.0F, 0.07F, false, 0, true, 1.2F);
                pix.scaleTickAdding = -0.03125F / lt0;
                world.spawnEntity(pix);
                int lt = 10 + itemRand.nextInt(10);
                boolean typee = itemRand.nextFloat() < 0.5F;
                float spd1 = 0.1F;
                float randcol = itemRand.nextFloat();
                AnimatedGParticle particle = new AnimatedGParticle(typee ? plasma_a : plasma_b, 0.3F + itemRand.nextFloat() * 0.2F, 0.0F, lt, 240, world, xEnd, yEnd, zEnd, (itemRand.nextFloat() - 0.5F) * spd1, (itemRand.nextFloat() - 0.5F) * spd1, (itemRand.nextFloat() - 0.5F) * spd1, 0.98F - 0.7F * randcol, 1.0F - 0.23F * randcol, 0.07F + 0.2F * randcol, true, itemRand.nextInt(360));
                particle.alphaGlowing = true;
                particle.alphaTickAdding = -1.0F / lt;
                particle.framecount = typee ? 14 : 24;
                particle.animCounter = itemRand.nextInt(10);
    ...
    public static class PlasmaCutter extends ElectricMiningLaser {
        ...
        @Override
        public void makeLaserParticles(World world, @Nullable EntityPlayer player, double xStart, double yStart, double zStart, double xEnd, double yEnd, double zEnd, boolean hit, int mode, double distance, float rotatPitch, float rotatYaw) {
            EntityStreamLaserP laser = new EntityStreamLaserP(world, player, shard_trace, 0.1F, 240, 1.0F, 0.9F, 0.9F, 0.0F, distance, mode == 1 ? 4 : 1, 0.2F, 2.0F);
            laser.setPosition(xStart, yStart, zStart);
            if (player == null) {
                laser.rotatPitch = rotatPitch;
                laser.rotatYaw = rotatYaw;
            ...
            if (hit) {
                ...
                GUNParticle part = new GUNParticle(blueexplode, 0.05F, 0.0F, lt, 240, world, xEnd, yEnd, zEnd, 0.0F, 0.0F, 0.0F, r, g, b, true, itemRand.nextInt(360));
                part.alphaGlowing = true;
                part.alphaTickAdding = -1.0F / lt;
                part.scaleTickAdding = scl / lt;
                world.spawnEntity(part);
                ...
                int lt0 = 16 + itemRand.nextInt(12);
                Vec3d spedVec = new Vec3d(xEnd - xStart, yEnd - yStart, zEnd - zStart).normalize().scale(-0.3).add((itemRand.nextFloat() - 0.5F) * spd0, (itemRand.nextFloat() - 0.5F) * spd0, (itemRand.nextFloat() - 0.5F) * spd0);
                GUNParticle pix = new GUNParticle(pixel, 0.03125F, 0.04F, lt0, 240, world, xEnd, yEnd, zEnd, (float) spedVec.x, (float) spedVec.y, (float) spedVec.z, r, g, b, false, 0, true, 1.2F);
                pix.scaleTickAdding = -0.03125F / lt0;
                world.spawnEntity(pix);
                int ltx = 10 + itemRand.nextInt(10);
                boolean typee = itemRand.nextFloat() < 0.5F;
                float spd1 = 0.1F;
                float randcol = itemRand.nextFloat();
                AnimatedGParticle particle = new AnimatedGParticle(typee ? plasma_a : plasma_b, 0.3F + itemRand.nextFloat() * 0.2F, 0.0F, ltx, 240, world, xEnd, yEnd, zEnd, (itemRand.nextFloat() - 0.5F) * spd1, (itemRand.nextFloat() - 0.5F) * spd1, (itemRand.nextFloat() - 0.5F) * spd1, 0.75F - itemRand.nextFloat() * 0.2F, 0.7F - itemRand.nextFloat() * 0.2F, 1.0F, true, itemRand.nextInt(360));
                particle.alphaGlowing = true;
                particle.alphaTickAdding = -1.0F / ltx;
                particle.framecount = typee ? 14 : 24;
                particle.animCounter = itemRand.nextInt(10);
        ...
        public void onHitEntity(Entity entity, ItemStack itemstack, World world, EntityPlayer player, int mode, int speed, int itemSlot) {
            if (!world.isRemote && Team.checkIsOpponent(player, entity)) {
                float damage = mode == 1 ? this.getDamageToMobsLaser(itemstack) * oneshotDamageMult : this.getDamageToMobsLaser(itemstack) * (speed / this.gatMaxSpeed(itemstack, player));
                Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, false, player, WeaponDamage.plasma), damage, player, entity, true, this.getKnockbackToMobsLaser(itemstack));
                entity.hurtResistantTime = 0;
