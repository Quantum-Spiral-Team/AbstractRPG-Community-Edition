...
public class PhoenixGhostCape extends Item implements IBauble, IRenderBauble {
    ...
    public void Effects(World world, EntityPlayer player, boolean bom1, boolean bom2) {
        ...
        Entity bigboom = new GUNParticle(this.sparkle, 0.1F + (float) itemRand.nextGaussian() / 25.0F, -0.001F, 30, 240, world, player.posX + param1, player.posY + 1.0 + param2, player.posZ + param3, 0.0F, 0.0F, 0.0F, 1.0F, 0.8F + (float) itemRand.nextGaussian() / 5.0F, 1.0F, true, 0);
        world.spawnEntity(bigboom);
        Entity bigboom2 = new GUNParticle(this.sparkle, 0.1F + (float) itemRand.nextGaussian() / 25.0F, -0.001F, 30, 240, world, player.posX + param2, player.posY + 1.0 + param1, player.posZ + param3, 0.0F, 0.0F, 0.0F, 1.0F, 0.8F + (float) itemRand.nextGaussian() / 5.0F, 1.0F, true, 0);
        world.spawnEntity(bigboom2);
        Entity bigboom3 = new GUNParticle(this.sparkle, 0.1F + (float) itemRand.nextGaussian() / 25.0F, -0.001F, 30, 240, world, player.posX + param2, player.posY + 1.0 + param3, player.posZ + param1, 0.0F, 0.0F, 0.0F, 1.0F, 0.8F + (float) itemRand.nextGaussian() / 5.0F, 1.0F, true, 0);
        world.spawnEntity(bigboom3);
        if (bom1) {
            for (int ss = 0; ss < 25; ss++) {
                Entity bigsmoke = new GUNParticle(this.largesmoke, 0.6F, 0.0F, 14, 60, world, player.posX, player.posY + 1.0, player.posZ, (float) itemRand.nextGaussian() / 10.0F, (float) itemRand.nextGaussian() / 10.0F, (float) itemRand.nextGaussian() / 10.0F, 1.0F, 1.0F, 1.0F, true, 0);
                world.spawnEntity(bigsmoke);
            }

            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.phoenix_ghost, SoundCategory.AMBIENT, 0.6F, 0.7F + itemRand.nextFloat() / 4.0F);
        }

        if (bom2) {
            for (int ss = 0; ss < 15; ss++) {
                Entity bigsmoke = new GUNParticle(this.largesmoke, 0.4F, 0.0F, 10, 60, world, player.posX, player.posY + 1.0, player.posZ, (float) itemRand.nextGaussian() / 15.0F, (float) itemRand.nextGaussian() / 15.0F, (float) itemRand.nextGaussian() / 15.0F, 1.0F, 1.0F, 1.0F, true, 0);
                world.spawnParticle(EnumParticleTypes.FLAME, player.posX, player.posY + 1.0, player.posZ, (float) itemRand.nextGaussian() / 25.0F, (float) itemRand.nextGaussian() / 25.0F, (float) itemRand.nextGaussian() / 25.0F, new int[0]);
                world.spawnEntity(bigsmoke);
            }

            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.fire, SoundCategory.AMBIENT, 0.9F, 0.8F + itemRand.nextFloat() / 5.0F);
        }
    }
    ...
    public void onWornTick(ItemStack itemstack, EntityLivingBase entityIn) {
        if (entityIn instanceof EntityPlayer) {
            ...
            if (damage > 0.0F && damage < 10.0F) {
                ...
                player.motionY = y;
                player.motionZ = z;
                AxisAlignedBB axisalignedbb = new AxisAlignedBB(player.posX - 0.5, player.posY, player.posZ - 0.5, player.posX + 0.5, player.posY + 2.0, player.posZ + 0.5);
