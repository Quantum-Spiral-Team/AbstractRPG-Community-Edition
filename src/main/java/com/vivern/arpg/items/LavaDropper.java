        ...
        if (!world.isRemote) {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (click && player.getHeldItemMainhand() == itemstack && mana > manacost && !hascooldown) {
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.lava_dropper, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                ...
                if (click2 && player.getHeldItemOffhand() == itemstack && mana > manacost && !hascooldown2) {
                    ...
                    LavaDropperShoot projectile = new LavaDropperShoot(world, player, itemstack, power);
                    Weapons.shoot(projectile, EnumHand.OFF_HAND, player, player.rotationPitch, player.rotationYaw, 0.0F, parameters.getFloat("velocity"), parameters.getEnchantedF("inaccuracy", acc), -0.1F, 0.5F, 0.2F);
                    world.spawnEntity(projectile);
                }
            }
        }
