    ...
    public void onWornTick(ItemStack itemstack, EntityLivingBase player) {
        if (!((EntityPlayer) player).getCooldownTracker().hasCooldown(this)) {
            ...
            if (!list.isEmpty()) {
                for (EntityLivingBase entitylivingbase : list) {
                    if (Team.checkIsOpponent(player, entitylivingbase) && entitylivingbase != player && EntitySelectors.NOT_SPECTATING.apply(entitylivingbase)) {
                        RayTraceResult raytraceresult = player.world.rayTraceBlocks(startPos, entitylivingbase.getPositionVector().add(0.0, entitylivingbase.height / 2.0F, 0.0), false, true, false);
            ...
            if (targ != null) {
                ...
                Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, false, startPos, WeaponDamage.electric), 4.0F * WeaponParameters.EXLevelPLASMA * Mana.getMagicPowerMax((EntityPlayer) player), player, targ, false);
                targ.hurtResistantTime = 0;
                if (targ.getHealth() <= 0.0F && itemRand.nextFloat() < 0.75) {
                    DeathEffects.applyDeathEffect(targ, DeathEffects.DE_ELECTRIC);
                }

                player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.lightning_gh_grap, SoundCategory.AMBIENT, 0.5F, 1.2F + itemRand.nextFloat() / 5.0F);
                if (player.world.isRemote) {
                    ...
                    if (vect.lengthSquared() > 1.0E-6 && newvec.lengthSquared() > 1.0E-6) {
                        BetweenParticle laser = new BetweenParticle(player.world, this.start, 0.1F, 240, 1.0F, 1.0F, 1.0F, 0.1F, vect.distanceTo(newvec), 5, 0.3F, 1.0F, vect, newvec);
