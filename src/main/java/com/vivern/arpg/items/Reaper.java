...
public class Reaper extends ItemWeapon {
    ...
    public boolean attackEntityMelee(Entity entity, ItemStack stack, EntityPlayer player, EnumHand hand, boolean isCritical) {
        ...
        if (ch > 0.0F || dd > 0.0F) {
            player.world.playSound(null, entity.posX, entity.posY, entity.posZ, Sounds.magic_e, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
        }

        boolean ret = entity.attackEntityFrom(DamageSource.causePlayerDamage(player), (float) player.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue() * artropods * holy + dd + Math.min(ch, 10.0F));
        ...
        if (!world.isRemote) {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack && !player.getCooldownTracker().hasCooldown(this)) {
                    if (click2) {
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.reaper_throw, SoundCategory.AMBIENT, 0.8F, 0.9F + itemRand.nextFloat() / 5.0F);
                        ...
                        treaper.shoot(player, player.rotationPitch, player.rotationYaw, 0.2F, 1.4F, 0.5F - (float) EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.ACCURACY, itemstack) / 7);
                        ...
                        if (IWeapon.doMeleeSwordAttack(this, itemstack, player, EnumHand.MAIN_HAND, 5 + sharpness, knockback * 0.4F, 2.8F + sweeping * 0.3F, 0.3F + sweeping * 0.1F, 0.3F + sweeping * 0.1F, itemRand.nextFloat() < 0.2).success) {
                            world.playSound(null, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_STRONG, SoundCategory.PLAYERS, 0.7F, 0.9F + itemRand.nextFloat() / 5.0F);
                        } else {
                            world.playSound(null, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_WEAK, SoundCategory.PLAYERS, 0.6F, 0.9F + itemRand.nextFloat() / 5.0F);
                        }
