...
public class Hellmark extends ItemWeapon implements IItemAttacked {
    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot equipmentSlot, ItemStack stack) {
        ...
        if (NBTHelper.GetNBTint(stack, "blocking") > 0) {
            if (equipmentSlot == EntityEquipmentSlot.MAINHAND) {
                multimap.put(SharedMonsterAttributes.MOVEMENT_SPEED.getName(), new AttributeModifier(UUID.fromString("916DD27B-A123-455F-8C7F-6112A1B50A4C"), "Shield speed mh", -0.35, 1));
            }

            if (equipmentSlot == EntityEquipmentSlot.OFFHAND) {
                multimap.put(SharedMonsterAttributes.MOVEMENT_SPEED.getName(), new AttributeModifier(UUID.fromString("134CA27A-B123-501F-4D8F-3782C6B52C0A"), "Shield speed oh", -0.35, 1));
            }
        }
                ...
                if (player.getHeldItemMainhand() == itemstack && click || player.getHeldItemOffhand() == itemstack && click2) {
                    if (!player.getCooldownTracker().hasCooldown(this)) {
                        if (blocks <= 0) {
                            ...
                            if (EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, itemstack) > 0 && IWeapon.doMeleeSwordAttack(this, itemstack, player, player.getHeldItemMainhand() == itemstack ? EnumHand.MAIN_HAND : EnumHand.OFF_HAND, false).success) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.shield_hit_metall, SoundCategory.AMBIENT, 0.8F, 0.95F + itemRand.nextFloat() / 10.0F);
                            }
                ...
                if (attacker != null || source != DamageSource.CRAMMING && source != DamageSource.DROWN && source != DamageSource.FALL && source != DamageSource.HOT_FLOOR && source != DamageSource.IN_FIRE && source != DamageSource.IN_WALL && source != DamageSource.LAVA && source != DamageSource.MAGIC && source != DamageSource.ON_FIRE && source != DamageSource.OUT_OF_WORLD && source != DamageSource.STARVE && source != DamageSource.WITHER) {
                    if (player.hurtResistantTime > 0) {
                        ...
                        if (attacker != null) {
                            ...
                            float damage = parameters.getEnchantedF("damage", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, stack)) + (attacker.isBurning() ? parameters.getFloat("fire_damage_bonus") : 0.0F);
                            Weapons.dealDamage(wDamageSource, damage, player, attacker, true);
                            if (attacker.isBurning()) {
                                player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.hellmark, SoundCategory.AMBIENT, 0.8F, 0.9F + itemRand.nextFloat() / 5.0F);
                            }
                        }
    ...
    public boolean attackEntityMelee(Entity entity, ItemStack stack, EntityPlayer player, EnumHand hand, boolean isCritical) {
        ...
        boolean ret = entity.attackEntityFrom(new WeaponDamage(stack, player, null, false, false, player, WeaponDamage.blade), (float) player.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue() * artropods * holy + (entity.isBurning() ? WeaponParameters.getWeaponParameters(stack.getItem()).getFloat("fire_damage_bonus") : 0.0F));
        entity.hurtResistantTime = 0;
        if (ret && entity.isBurning()) {
            EntityLiveHeart.spawnHearts(player.world, entity.posX, entity.posY, entity.posZ, 2, WeaponParameters.getWeaponParameters(stack.getItem()).getFloat("heart_health"), true, 5.0F, player);
        }
