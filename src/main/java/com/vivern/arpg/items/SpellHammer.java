            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack) {
                    if (delay <= 0 && click && !hascooldown) {
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.swosh_d, SoundCategory.PLAYERS, 0.8F, 0.95F + itemRand.nextFloat() / 10.0F);
                    }
                    ...
                    if (delay == 1) {
                        ...
                        if (result != null && result.sideHit == EnumFacing.UP && result.getBlockPos() != null) {
                            ...
                            if (tileEntity instanceof TileSpellForge) {
                                ...
                                if (spellForge.onHammerHit(itemstack, player)) {
                                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.spell_forge, SoundCategory.PLAYERS, 3.0F, 0.95F + itemRand.nextFloat() / 10.0F);
                                } else {
                                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.spell_hammer, SoundCategory.PLAYERS, 1.4F, 0.95F + itemRand.nextFloat() / 10.0F);
                                }
                        ...
                        if (doMobAttack) {
                            if (IWeapon.doMeleeHammerAttack(this, itemstack, player, EnumHand.MAIN_HAND, false, 45, 7).success) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.spell_hammer, SoundCategory.PLAYERS, 1.4F, 0.98F + itemRand.nextFloat() / 10.0F);
                                IWeapon.fireBomEffect(this, player, world, 2);
                            } else {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.melee_miss_axe, SoundCategory.PLAYERS, 1.0F, 0.9F + itemRand.nextFloat() / 5.0F);
