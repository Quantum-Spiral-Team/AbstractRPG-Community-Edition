...
public class AnnihilationGun extends ItemWeapon {
    ...
    public void sound(World world, EntityPlayer player, int intcharge, int rapidity) {
        if (intcharge == 2) {
            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.biglazer1, SoundCategory.AMBIENT, 1.0F, 1.0F);
        }

        if (intcharge == 30 - rapidity * 5) {
            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.biglazer2, SoundCategory.AMBIENT, 1.0F, 1.0F);
        }

        if (intcharge == 60 - rapidity * 10) {
            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.biglazer3, SoundCategory.AMBIENT, 1.0F, 1.0F);
        }
    }
        ...
        if (IWeapon.canShoot(itemstack)) {
            ...
            if (player.getHeldItemMainhand() == itemstack && click && !player.getCooldownTracker().hasCooldown(this)) {
                ...
                if (!world.isRemote) {
                    if (ammo > 0 && this.isReloaded(itemstack)) {
                        if (intcharge > chargeneed) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.biglazer_a, SoundCategory.AMBIENT, 0.7F, 1.0F);
                            ...
                            if (itemRand.nextFloat() < parameters.getEnchantedF("explode_chance", rapidity)) {
                                Explosion explosion = new Explosion(world, null, vec.x, vec.y, vec.z, parameters.getEnchantedF("explode_power", might), true, true);
                            ...
                            if (!list.isEmpty()) {
                                for (Entity entity : list) {
                                    if (Team.checkIsOpponent(player, entity)) {
                                        ...
                                        Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, false, vec, WeaponDamage.explode), parameters.getEnchantedF("damage", might), player, entity, true, kmult * parameters.getEnchantedF("knockback", impulse), vec.x, vec.y, vec.z);
                                        entity.hurtResistantTime = 0;
                                    }
                                }
                            }

                            if (!player.capabilities.isCreativeMode && !(itemRand.nextFloat() < EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.REUSE, itemstack) * 0.15F)) {
                ...
                if (intcharge > chargeneed && world.isRemote && ammo > 0 && this.isReloaded(itemstack)) {
                    this.effect(player, world, vec.x, vec.y, vec.z, 0.0, 0.0, horizoffset, player.posX, player.posY, player.posZ);
                }
            }
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        if (itemRand.nextFloat() < 0.5) {
            GunPEmitter emi = new GunPEmitter(this.fire, 0.3F, 0.06F, 30, 240, world, x, y, z, (float) this.rand.nextGaussian() / 2.0F, (float) this.rand.nextGaussian() / 2.0F + 0.5F, (float) this.rand.nextGaussian() / 2.0F, 1.0F, 1.0F, 1.0F, true, 1, false, this.smoke, 0.35F, 0.0F, 10, 100, 100.0F, 100.0F, 100.0F, 1.0F, 1.0F, 1.0F, true);
            world.spawnEntity(emi);
        }

        for (int ss = 0; ss < 5; ss++) {
            Entity bigsmoke = new GUNParticle(this.largesmoke, 1.5F + (float) this.rand.nextGaussian() / 5.0F, 0.0F, 15, 70, world, x, y, z, (float) this.rand.nextGaussian() / 4.0F, (float) this.rand.nextGaussian() / 10.0F, (float) this.rand.nextGaussian() / 4.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
            world.spawnEntity(bigsmoke);
        }

        Entity bigboom = new GUNParticle(this.electr, 3.5F + (float) this.rand.nextGaussian(), 0.0F, 1, 240, world, x, y, z, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, (int) Math.round(this.rand.nextGaussian() * 20.0));
        ...
        if (a == 0.0 && b == 0.0) {
            ...
            EntityStreamLaserP laser = new EntityStreamLaserP(world, this.start, 0.2F, 240, 1.0F, 1.0F, 1.0F, 0.5F, dist, 1, 0.3F, 8.0F, (float) a, (float) b, player.ticksExisted);
            laser.setPosition(d1, d2 + 1.55, d3);
            laser.horizOffset = (float) c;
            world.spawnEntity(laser);
        }
