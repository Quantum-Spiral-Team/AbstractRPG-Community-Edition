...
public abstract class LaserModel {
    ...
    public void renderLaserModel(Entity entity, Vec3d laserPos, float laserPitch, float laserYaw, double distance, float horizoffset, double x, double y, double z, float partialTicks) {
    ...
    public void renderLaserModelInWorld(Entity entity, Vec3d start, Vec3d end, float partialTicks) {
        ...
        this.renderLaserModel(entity, start, rp, rw, start.distanceTo(end), 0.0F, start.x - rvePos.x, start.y - rvePos.y, start.z - rvePos.z, partialTicks);
    }
    ...
    public static class LaserModelCap extends LaserModel {
        ...
        @Override
        public void renderLaser(Entity entity, Vec3d laserPos, float laserPitch, float laserYaw, double distance, double x, double y, double z, float partialTicks) {
    ...
    public static class LaserModelCircles extends LaserModel {
        ...
        public LaserModelCircles(ResourceLocation texture, int lightAdd, double maximumDistance, int animspeed, int circlesCount, float circleRadiusStart, float circleRadiusEnd, float[] color1, @Nullable float[] color2) {
            super(texture, lightAdd);
        ...
        @Override
        public void renderLaser(Entity entity, Vec3d laserPos, float laserPitch, float laserYaw, double distance, double x, double y, double z, float partialTicks) {
            MyRenderHelper.renderLaserCircles(Math.min(distance, this.maximumDistance), this.animspeed, this.circlesCount, this.circleRadiusStart, this.circleRadiusEnd, this.color1, this.color2);
        }
    ...
    public static class LaserModelLightnings extends LaserModel {
        ...
        public LaserModelLightnings(ResourceLocation texture, int lightAdd, int segmentsCount, int changeRandomDelay, float width, float lightningScale, float bias, boolean randomizeAngle, float[] color1, float[] color2) {
            super(texture, lightAdd);
            this.width = width;
        ...
        public void renderLaser(Entity entity, Vec3d laserPos, float laserPitch, float laserYaw, double distance, double x, double y, double z, float partialTicks) {
            ...
            for (int i = 1; i <= this.segmentsCount; i++) {
                ...
                bufferbuilder.pos(0.0, this.lightningScale + prevdisplacement, prevdepth).tex(0.0, 0.0).color(prevR, prevG, prevB, prevA).endVertex();
                ...
                bufferbuilder.pos(0.0, -this.lightningScale + prevdisplacement, prevdepth).tex(1.0, 0.0).color(prevR, prevG, prevB, prevA).endVertex();
    ...
    public static class LaserModelLinear extends LaserModel {
        ...
        public LaserModelLinear(ResourceLocation texture, int lightAdd, int beamsCount, float scale, float zoffset, float[] color1, @Nullable float[] color2, float angle, float angleAnimation, float textureLength, float textureAnimation) {
            super(texture, lightAdd);
        ...
        public void renderLaser(Entity entity, Vec3d laserPos, float laserPitch, float laserYaw, double distance, double x, double y, double z, float partialTicks) {
            ...
            if (this.beamsCount > 0) {
                MyRenderHelper.renderLaserLinear((float) distance, this.beamsCount, this.angle + this.angleAnimation * AnimationTimer.tick, this.scale, this.zoffset, AnimationTimer.tick * this.textureAnimation, repeats, frame, this.framesCount, this.color1, this.color2);
            } else {
                MyRenderHelper.renderLaserFacedToCamera(laserPos, GetMOP.pitchYawToVec3D(laserPitch, laserYaw), (float) distance, this.scale, 0.0F, this.zoffset, AnimationTimer.tick * this.textureAnimation, repeats, frame, this.framesCount, this.color1, this.color2, partialTicks);
            }
        }
    ...
    public static class LaserModelSpiral extends LaserModel {
        ...
        public LaserModelSpiral(ResourceLocation texture, int lightAdd, double maximumDistance, int animspeed, int spiralsegments, float spiralRadius, float lineThickness, float edgeWidth, float[] color1, @Nullable float[] color2, @Nullable float[] color3) {
            super(texture, lightAdd);
            this.animspeed = animspeed;
        ...
        @Override
        public void renderLaser(Entity entity, Vec3d laserPos, float laserPitch, float laserYaw, double distance, double x, double y, double z, float partialTicks) {
            MyRenderHelper.renderLaserSpiral((float) Math.min(distance, this.maximumDistance), this.spiralsegments, this.spiralRadius, this.lineThickness, this.edgeWidth, this.animspeed, this.color1, this.color2, this.color3);
        }
