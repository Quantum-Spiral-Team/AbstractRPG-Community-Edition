    ...
    public void onUpdate(ItemStack itemstack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        if (entityIn instanceof EntityPlayer) {
            ...
            if (player.getActiveItemStack() == itemstack && mana > 0.2F - (float) sor / 30 && click && !player.getCooldownTracker().hasCooldown(itemIn)) {
                ...
                if (charge == -27) {
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.staff_of_corpse, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                }
                ...
                if (charge >= 0) {
                            ...
                            if (!list.isEmpty()) {
                                for (Entity entity : list) {
                                    if (Team.checkIsOpponent(player, entity)) {
                                        Weapons.dealDamage(null, (0.8F + (1.5F + might * 0.9F) / (charge + 1)) * power, player, entity, true, EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, itemstack) / 5.0F, vec.x, vec.y, vec.z);
                    ...
                    if (worldIn.isRemote && damage <= itemIn.getMaxDamage() - 1) {
                        for (int ss = 0; ss < 2; ss++) {
                            GUNParticle bigsmoke = new GUNParticle(this.largesmoke, 1.5F + (float) itemRand.nextGaussian() / 3.0F, 0.0F, 30, 180, world, vec.x, vec.y, vec.z, (float) itemRand.nextGaussian() / 10.0F, (float) itemRand.nextGaussian() / 19.0F, (float) itemRand.nextGaussian() / 10.0F, 0.3F, 1.0F, 0.7F + (float) itemRand.nextGaussian() / 5.0F, true, itemRand.nextInt(360), true, 1.0F);
                            bigsmoke.alphaTickAdding = -0.025F;
                            bigsmoke.alphaGlowing = true;
                            world.spawnEntity(bigsmoke);
                        }

                        EntityStreamLaserP laser = new EntityStreamLaserP(world, player, this.start, 0.07F + (float) charge / 280, 156 + charge * 3, 1.0F, 1.0F, 1.0F, 0.5F, player.getDistance(vec.x, vec.y, vec.z), 1, 0.3F, 8.0F);
