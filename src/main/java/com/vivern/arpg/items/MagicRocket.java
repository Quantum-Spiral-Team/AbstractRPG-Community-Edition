        ...
        if (IWeapon.canShoot(itemstack)) {
            ...
            if (player.getHeldItemMainhand() == itemstack) {
                    ...
                    if (captured instanceof EntityMagicRocket) {
                        if (EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, itemstack) == 0) {
                            ...
                            if (!list.isEmpty() && list.get(0) != null) {
                                ...
                                if (world.isRemote) {
                                    GUNParticle bigboom = new GUNParticle(textur, list.get(0).width, -0.001F, 12, 240, world, list.get(0).posX, list.get(0).posY + 0.1, list.get(0).posZ, 0.0F, 0.1F, 0.0F, 0.5F, 0.75F, 1.0F, true, 1);
                ...
                if (Mana.getMana(player) > manacost && !world.isRemote && !player.getCooldownTracker().hasCooldown(this) && click) {
                    ...
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.magic_n, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
