...
public class FireworkLauncher extends ItemWeapon {
                        ...
                        if (ammo > 0 && this.isReloaded(itemstack)) {
                            if (itemstack.getTagCompound().hasKey("dragon")) {
                                if (!itemstack.getTagCompound().getBoolean("dragon")) {
                                    ...
                                    Weapons.shoot(projectile, EnumHand.MAIN_HAND, player, player.rotationPitch, player.rotationYaw, -2.0F, parameters.getEnchantedF("velocity", range), parameters.getEnchantedF("inaccuracy", acc), -0.1F, 0.5F, 0.5F);
                                    ...
                                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.firework, SoundCategory.AMBIENT, 1.0F, 0.9F + itemRand.nextFloat() / 5.0F);
                                ...
                                if (itemstack.getTagCompound().getBoolean("dragon")) {
                                    ...
                                    Weapons.shoot(dragon, EnumHand.MAIN_HAND, player, player.rotationPitch - 7.5F, player.rotationYaw, 0.0F, parameters.getFloat("velocity_dragon"), parameters.getEnchantedF("inaccuracy_dragon", acc), -0.1F, 0.5F, 0.5F);
                                    world.spawnEntity(dragon);
                                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.firework_dragon, SoundCategory.AMBIENT, 1.0F, 0.9F + itemRand.nextFloat() / 5.0F);
                            ...
                            if (ammotype != null || isrelneed) {
                                ...
                                if (this.initiateReload(itemstack, player, ammotype, maxammo)) {
                                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.firework_reload, SoundCategory.NEUTRAL, 0.7F, 0.95F + itemRand.nextFloat() / 10.0F);
    ...
    public int getReloadTime(ItemStack itemstack) {
        ...
        return NBTHelper.GetNBTboolean(itemstack, "dragon") ? parameters.getEnchantedI("reload_dragon", reloading) : parameters.getEnchantedI("reload", reloading);
    }
    ...
    public int getCooldownTime(ItemStack itemstack) {
        ...
        return NBTHelper.GetNBTboolean(itemstack, "dragon") ? parameters.getEnchantedI("cooldown_dragon", reloading) : parameters.getEnchantedI("cooldown", reloading);
    }
