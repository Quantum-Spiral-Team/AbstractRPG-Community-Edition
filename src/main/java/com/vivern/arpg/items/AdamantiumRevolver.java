...
public class AdamantiumRevolver extends ItemWeapon {
    ...
    public void onUpdate(ItemStack itemstack, World world, Entity entityIn, int itemSlot, boolean isSelected) {
        if (!world.isRemote) {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                boolean almostReloaded = NBTHelper.GetNBTint(itemstack, "reload_time") == 1 && EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, itemstack) > 0;
                if (hand != null) {
                    if (ammo > 0 && (this.isReloaded(itemstack) || almostReloaded)) {
                        if ((!hascooldown || hand != EnumHand.MAIN_HAND) && (!hascooldown2 || hand != EnumHand.OFF_HAND)) {
                            ...
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.adamantium_revolver, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                            ...
                            if (!list.isEmpty()) {
                                for (Entity entity : list) {
                                    if (Team.checkIsOpponent(player, entity)) {
                                        if (Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, true, player, WeaponDamage.bullet), parameters.getEnchantedF("damage", might) + damageadd * parameters.getFloat("bullet_damage"), player, entity, true, parameters.getEnchantedF("knockback", impulse) + knockbackadd * parameters.getFloat("bullet_knockback"), player.posX, player.posY, player.posZ)) {
                                            if (almostReloaded) {
                                                Weapons.setPotionIfEntityLB(entity, parameters.getPotion(entity instanceof EntityPlayer ? "stun_players" : "stun", MobEffects.SLOWNESS, EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.WITCHERY, itemstack)));
                                                EntityLiveHeart.spawnHearts(world, vec.x, vec.y, vec.z, parameters.getInt("hearts"), parameters.getFloat("hearts_health"), true, 4.0F, player);
                                            }
                            ...
                            if (almostReloaded && !damageDealed) {
                                Weapons.setPotionIfEntityLB(player, parameters.getPotion("stun_players", MobEffects.SLOWNESS, EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.WITCHERY, itemstack)));
                            }
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.pistol_rel2, SoundCategory.AMBIENT, 0.7F, 0.85F + itemRand.nextFloat() / 10.0F);
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        BulletParticle part = new BulletParticle(world, from, to, 0.2F, (int) MathHelper.clamp(dist / 4.0F, 1.0F, 6.0F), 9, dist, (float) col.x, (float) col.y, (float) col.z, d3 > 0.0);
        part.smokeRed = 1.0F;
