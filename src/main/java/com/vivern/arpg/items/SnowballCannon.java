                    ...
                    if (ammo > 0 && this.isReloaded(itemstack)) {
                        if (!player.getCooldownTracker().hasCooldown(cooldownItem)) {
                            ...
                            if (ammotype == Items.SNOWBALL) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.snowball_cannon, SoundCategory.AMBIENT, 0.8F, 0.9F + itemRand.nextFloat() / 5.0F);
                                ...
                                Weapons.shoot(bullet, hand, player, player.rotationPitch - 3.5F, player.rotationYaw, 0.2F, parameters.getFloat("velocity"), parameters.getEnchantedF("inaccuracy", acc), -0.25F, 0.5F, 0.3F);
                                ...
                                for (int ss = 0; ss < shots; ss++) {
                                    CannonSnowball bullet = new CannonSnowball(world, player, itemstack);
                                    Weapons.shoot(bullet, hand, player, player.rotationPitch - 3.5F, player.rotationYaw, 0.2F, parameters.getFloat("velocity"), parameters.getEnchantedF("inaccuracy_special", acc), -0.25F, 0.5F, 0.3F);
                                    bullet.damage = parameters.getEnchantedF("damage", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, itemstack));
                                    world.spawnEntity(bullet);
                                }
                        ...
                        if (NBTHelper.GetNBTint(itemstack, "ammo") == 0) {
                            ...
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.snowball_cannon_rel, SoundCategory.AMBIENT, 0.7F, 0.95F + itemRand.nextFloat() / 20.0F);
                            this.startReload(itemstack);
