...
public class FireWhip extends ItemWeapon {
                    ...
                    for (int sx = 0; sx < this.segmentCount - 1; sx++) {
                        ...
                        if (motionX + motionY + motionZ > 0.6) {
                            ...
                            if (entitylivingbase != null) {
                                SuperKnockback.applyKnockback(entitylivingbase, 0.4F + (float) EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, itemstack) / 5, player.posX, player.posY, player.posZ);
                                ...
                                entitylivingbase.attackEntityFrom(DamageSource.causePlayerDamage(player), 3.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, itemstack) / 2.0F);
    ...
    public void effect(World world, double fromX, double fromY, double z, float startMotionX, float startMotionY, float startMotionZ, double d1, double d2, double d3, float targetMotionX, float targetMotionY, float targetMotionZ) {
