        ...
        if (!world.isRemote) {
            ...
            if (entityIn instanceof EntityPlayer) {
                ...
                if ((player.getHeldItemMainhand() == stack || player.getHeldItemOffhand() == stack) && click && !player.getCooldownTracker().hasCooldown(this)) {
                    RayTraceResult result = GetMOP.fixedRayTraceBlocks(world, player, player.getEntityAttribute(EntityPlayer.REACH_DISTANCE).getAttributeValue(), 0.2F, false, false, true, true);
                    if (NBTHelper.GetNBTboolean(stack, "picked")) {
                        if (result.hitVec != null) {
                            if (stack.getTagCompound().hasKey("picked_item")) {
                                ...
                                EntitySpellForgeCatalyst catalyst = new EntitySpellForgeCatalyst(world, result.hitVec.x, result.hitVec.y, result.hitVec.z, releasedstack, NBTHelper.GetNBTint(stack, "heat"));
