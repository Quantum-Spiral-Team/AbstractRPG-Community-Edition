...
public class MoltenGreatAxe extends ItemWeapon {
                ...
                if (player.getHeldItemMainhand() == itemstack) {
                    ...
                    if (delay == 1) {
                        if (IWeapon.doMeleeHammerAttack(this, itemstack, player, EnumHand.MAIN_HAND, !player.onGround && player.motionY < -0.02, 90, 8).success) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.melee_axe, SoundCategory.PLAYERS, 0.7F, 0.9F + itemRand.nextFloat() / 5.0F);
                        } else {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.melee_miss_axe, SoundCategory.PLAYERS, 0.6F, 0.9F + itemRand.nextFloat() / 5.0F);
                        }
    ...
    public boolean attackEntityMelee(Entity entity, ItemStack stack, EntityPlayer player, EnumHand hand, boolean isCritical) {
        ...
        if (entity instanceof EntityLivingBase) {
            ...
            if (duration > 0) {
                ...
                if (potione != null) {
                    ...
                    PotionEffect baff = new PotionEffect(potione, parameters.getEnchantedI("potion_time", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.WITCHERY, stack)), amplifier);
        ...
        boolean ret = entity.attackEntityFrom(new WeaponDamage(stack, player, null, false, false, player, WeaponDamage.blade), (float) player.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue() * artropods * holy + (isCritical ? 3 : 0));
        ...
        if (isCritical) {
            entity.world.playSound(null, entity.posX, entity.posY, entity.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_CRIT, SoundCategory.PLAYERS, 0.7F, 0.9F + itemRand.nextFloat() / 5.0F);
        }
