...
public class ThistleThorn extends ItemWeapon {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack && Mana.getMana(player) > 1.5F - sor / 3.3F && (click || click2)) {
                    ...
                    if (!player.getCooldownTracker().hasCooldown(itemIn)) {
                        if (click) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.thistle, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                            ...
                            if (result != null) {
                                IWeapon.fireEffect(this, player, world, 32.0, result.hitVec.x, result.hitVec.y, result.hitVec.z, player.rotationPitch, player.rotationYaw, 0.0, 0.0, 0.0, 0.0);
                            }
                        } else if (click2 && NBTHelper.GetNBTboolean(itemstack, "powered")) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.thistle, SoundCategory.AMBIENT, 0.9F, 0.7F + itemRand.nextFloat() / 5.0F);
                            ...
                            IWeapon.fireEffect(this, player, world, 32.0, player.posX, player.posY, player.posZ, player.rotationPitch, player.rotationYaw, 1.0, 0.0, 0.0, 0.0);
                        }
                    }
                } else {
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        for (int i = 0; i < 2; i++) {
            GunPEmitter emit = new GunPEmitter(this.sparkle, 0.02F, 0.001F, 15, 240, world, x, y, z, 0.0F, 0.0F, 0.0F, rr, gg, bb, true, 1, false, this.sparkle, 0.025F, 0.003F, 13, 240, 74.0F, 74.0F, 74.0F, rr, gg, bb, true);
