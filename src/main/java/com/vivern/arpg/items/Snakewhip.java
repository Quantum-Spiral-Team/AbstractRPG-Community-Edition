...
public class Snakewhip extends Whip {
    ...
    public void onSpecAttack(Vec3d pos, IWeapon iweapon, ItemStack itemstack, EntityPlayer player, EnumHand hand, float damage, boolean hitSomeone, boolean killSomeone) {
        ...
        if (hitSomeone) {
            ...
            if (EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, itemstack) > 0 && !player.world.isRemote) {
                EntityAreaEffectCloud entityareaeffectcloud = new EntityAreaEffectCloud(player.world, pos.x, pos.y, pos.z);
                entityareaeffectcloud.setOwner(player);
                entityareaeffectcloud.setParticle(EnumParticleTypes.SLIME);
                float splashAttackRadius = WeaponParameters.getWeaponParameters(this).getEnchantedF("splash_radius", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.RANGE, itemstack));
            ...
            IWeapon.fireEffect(this, player, player.world, 64.0, scaledVec.x, scaledVec.y, scaledVec.z, hand == EnumHand.MAIN_HAND ? 4.0 : 5.0, player.rotationPitch, player.rotationYaw, 0.0, 0.0, 0.0);
        }
    }
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        if (a >= 2.0) {
            ...
            AnimatedGParticle part = new AnimatedGParticle(sweep_cloud, scl, 0.0F, lt, 220, world, x, y, z, 0.0F, 0.0F, 0.0F, 0.5F, 0.82F, 0.47F, true, lh ? 45 : 225);
            part.framecount = 11;
            part.animDelay = 3;
            ...
            if (a < 4.0) {
                for (int ss = 0; ss < 16; ss++) {
                    ...
                    GUNParticle part1 = new GUNParticle(slimesplash, scl2, 0.025F, lt2, -1, world, x, y, z, (float) itemRand.nextGaussian() / 12.0F, (float) itemRand.nextGaussian() / 12.0F + 0.08F, (float) itemRand.nextGaussian() / 12.0F, 0.5F, 0.78F, 0.45F, true, itemRand.nextInt(360), true, 10.0F);
            ...
            GUNParticle part1 = new GUNParticle(forge_hit_a, 0.12F + itemRand.nextFloat() * 0.1F, 0.0F, 4, 240, world, x, y, z, 0.0F, 0.0F, 0.0F, 0.45F, 0.85F, 0.5F, true, itemRand.nextInt(360));
            ...
            if (a == 1.0) {
                for (int ss = 0; ss < 3; ss++) {
                    ...
                    GUNParticle bigsmoke = new GUNParticle(largecloud, 0.16F + itemRand.nextFloat() * 0.16F, -0.004F, ltx, 240, world, x, y, z, (float) itemRand.nextGaussian() / 26.0F, (float) itemRand.nextGaussian() / 26.0F, (float) itemRand.nextGaussian() / 26.0F, 0.5F, 1.0F, 0.5F, true, itemRand.nextInt(360));
