...
public class AquaticBow extends AbstractBow {
    ...
    public void boom(int param) {
        ...
        if (param >= 0) {
            ...
            this.clientPullSound = new MovingSoundEntity(player, this.getPullSound(), SoundCategory.PLAYERS, 1.0F, this.pullSoundPitch, false);
            soundHandler.playSound(this.clientPullSound);
        } else if (param == -3 && player != null) {
            this.clientPullSound = new MovingSoundEntity(player, Sounds.aquatic_bow_charge, SoundCategory.PLAYERS, 1.0F, this.pullSoundPitch, false);
            soundHandler.playSound(this.clientPullSound);
        }
    }
            ...
            if (!specialAttack) {
                ...
                if (!ammo.isEmpty() || creative) {
                    ...
                    if (pulling >= this.getCooldownTime(itemstack)) {
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.ocean_shoot, SoundCategory.AMBIENT, 1.1F, 0.9F + itemRand.nextFloat() / 5.0F);
                        HostileProjectiles.OceanSpiritShoot projectile = new HostileProjectiles.OceanSpiritShoot(world, player);
                        projectile.shoot(player, player.rotationPitch, player.rotationYaw, 0.0F, parameters.getFloat("waterblast_velocity"), parameters.getEnchantedF("waterblast_inaccuracy", acc));
                        ...
                        for (int i = 0; i < amountSpin; i++) {
                            ...
                            int k = GetMOP.floatToIntWithChance(parameters.getEnchantedF("knockback", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, itemstack)), itemRand);
                            if (k > 0) {
                            ...
                            EntityInfluence.addEntityInfluence(entityarrow, new EntityInfluenceFollowRotate(false, projectile, i, parameters.getFloat("angle_between_arrows")), 64.0);
                        }
    ...
    private static class EntityInfluenceFollowRotate extends EntityInfluence {
        ...
        public void onUpdate(Entity entity) {
            if (this.toFollow.isEntityAlive() && this.toFollow.isAddedToWorld()) {
                ...
                Vec3d rotatedVec = GetMOP.rotateVecAroundAxis(angledVec, motionVec, (this.angleBetween * this.num + this.toFollow.ticksExisted * 9.6F) * 0.017453F);
