    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot equipmentSlot, ItemStack stack) {
        ...
        if (NBTHelper.GetNBTint(stack, "blocking") > 0) {
            if (equipmentSlot == EntityEquipmentSlot.MAINHAND) {
                multimap.put(SharedMonsterAttributes.MOVEMENT_SPEED.getName(), new AttributeModifier(UUID.fromString("916DD27B-A123-455F-8C7F-6112A1B50A4C"), "Shield speed mh", -0.25, 1));
            }

            if (equipmentSlot == EntityEquipmentSlot.OFFHAND) {
                multimap.put(SharedMonsterAttributes.MOVEMENT_SPEED.getName(), new AttributeModifier(UUID.fromString("134CA27A-B123-501F-4D8F-3782C6B52C0A"), "Shield speed oh", -0.25, 1));
            }
        }
                ...
                if (attacker != null || source != DamageSource.CRAMMING && source != DamageSource.DROWN && source != DamageSource.FALL && source != DamageSource.HOT_FLOOR && source != DamageSource.IN_FIRE && source != DamageSource.IN_WALL && source != DamageSource.LAVA && source != DamageSource.MAGIC && source != DamageSource.ON_FIRE && source != DamageSource.OUT_OF_WORLD && source != DamageSource.STARVE && source != DamageSource.WITHER) {
                    if (player.hurtResistantTime > 0) {
                        ...
                        if (attacker != null) {
                            ...
                            PotionEffect lastdebaff = Weapons.mixPotion(attacker, PotionEffects.FREEZING, (float) potionValue1, (float) (potionValue2 - blocking), Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumMathOperation.PLUS, Weapons.EnumMathOperation.PLUS, potionValue3, potionValue4);
                        ...
                        if (blocking <= 0) {
                            ...
                            player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.shield_hit_metall, SoundCategory.AMBIENT, 0.6F, 0.9F + itemRand.nextFloat() / 5.0F);
                        }
        ...
        if (NBTHelper.GetNBTint(stack, "snow") >= snowNeed) {
            ...
            for (Entity en : list) {
                if (Team.checkIsOpponent(player, en)) {
                    ...
                    SuperKnockback.applyKnockback(en, parameters.getEnchantedF("winterbreak_knockback", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, stack)), player.posX, player.posY - 0.3, player.posZ);
                    int witchery = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.WITCHERY, stack);
                    ...
                    Weapons.mixPotion(en, MobEffects.SLOWNESS, (float) potionValue1, (float) potionValue2, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumPotionMix.GREATEST, Weapons.EnumMathOperation.PLUS, Weapons.EnumMathOperation.PLUS, potionValue3, 0);
                    if (spec > 0) {
                        ...
                        Weapons.mixPotion(en, PotionEffects.FROSTBURN, (float) potionValue1, (float) potionValue2, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumPotionMix.GREATEST, Weapons.EnumMathOperation.PLUS, Weapons.EnumMathOperation.PLUS, potionValue3, 0);
                    }
                }
            }
            ...
            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.winter_break, SoundCategory.NEUTRAL, 1.4F, 0.9F + itemRand.nextFloat() / 5.0F);
        }
    }
                    ...
                    for (int z = -3; z <= 3; z++) {
                        if (itemRand.nextFloat() < 0.7) {
                            ...
                            if (bl == Blocks.SNOW || bl == Blocks.SNOW_LAYER || bl == BlocksRegister.LOOSE_SNOW) {
                                ...
                                if (NBTHelper.GetNBTint(stack, "snow") >= need) {
                                    IWeapon.fireEffect(this, player, world, 64.0, player.posX, player.posY, player.posZ, 1.0, player.getEntityId(), 0.0, 0.0, 0.0, 0.0);
                                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.item_misc_d, SoundCategory.NEUTRAL, 0.7F, 0.95F + itemRand.nextFloat() / 5.0F);
                                    return;
                                }
                            }
                        }
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        if (a == 0.0) {
            ...
            for (int i = 0; i < countOfParticles; i++) {
                ...
                GUNParticle particle = new GUNParticle(snow, 0.15F + (float) itemRand.nextGaussian() / 30.0F, 0.01F, 22 + itemRand.nextInt(10), light, world, x, y, z, X, Y, Z, 1.0F - (float) itemRand.nextGaussian() / 14.0F, 1.0F, 1.0F, false, itemRand.nextInt(360), true, 1.0F);
                world.spawnEntity(particle);
            }
        } else {
            Entity en = world.getEntityByID((int) b);
            ParticleTracker.TrackerFollowDynamicPoint tfdp = en == null ? null : new ParticleTracker.TrackerFollowDynamicPoint(en, false, 0.0F, 1.0E-4F, 0.005F).setYadd(en.height / 2.0F);
            int limit = 30;
                ...
                for (int yy = -2; yy <= 2; yy++) {
                    for (int zz = -3; zz <= 3; zz++) {
                        if (limit > 0 && itemRand.nextFloat() < 0.7) {
                            ...
                            if (bl == Blocks.SNOW || bl == Blocks.SNOW_LAYER || bl == BlocksRegister.LOOSE_SNOW) {
                                limit--;
                                GUNParticle particle = new GUNParticle(snow2, 0.08F + itemRand.nextFloat() * 0.1F, 0.0F, 20 + itemRand.nextInt(15), 180, world, pos.getX() + 0.5, pos.getY() + 0.1, pos.getZ() + 0.5, 0.0F, 0.0F, 0.0F, 1.0F - (float) itemRand.nextGaussian() / 14.0F, 1.0F, 1.0F, false, itemRand.nextInt(360));
