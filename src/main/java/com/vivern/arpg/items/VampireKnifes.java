        ...
        if (!worldIn.isRemote && entityIn instanceof EntityPlayer) {
            ...
            if (player.getActiveItemStack() == itemstack && click && !player.getCooldownTracker().hasCooldown(itemIn)) {
                if (damage <= itemIn.getMaxDamage() - 1) {
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.knife, SoundCategory.AMBIENT, 0.8F, 0.4F / (itemRand.nextFloat() * 0.4F + 0.8F));
                ...
                if (itemstack.getItemDamage() > itemIn.getMaxDamage() - 1 && FindAmmo.find(player.inventory, ItemsRegister.VAMPIRE_KNIFE) >= 20) {
                    ...
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.vampireknifes, SoundCategory.NEUTRAL, 0.7F, 1.0F);
                }
                ...
                if (!world.isRemote && damage <= itemIn.getMaxDamage() - 1) {
                    ...
                    apple.shoot(player, player.rotationPitch, player.rotationYaw, 0.0F, 1.1F, 0.9F - (float) EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.ACCURACY, itemstack) / 5);
