...
public class Ceratarget extends ItemWeapon {
                    ...
                    if (click) {
                        if (ammo > 0 && this.isReloaded(itemstack)) {
                            if (!player.getCooldownTracker().hasCooldown(this)) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.ceratarget, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                                ...
                                projectile.followPoint = resul != null && resul.hitVec != null && resul.typeOfHit != Type.MISS ? resul.hitVec : vec3d2;
                                Weapons.shoot(projectile, EnumHand.MAIN_HAND, player, player.rotationPitch, player.rotationYaw + yawAdd, 0.0F, parameters.getFloat("velocity"), parameters.getEnchantedF("inaccuracy", acc), -0.2F, 0.0F, 0.6F, 0.1F);
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        if (en != null) {
            MovingSoundEntity movingSound = new MovingSoundEntity(en, Sounds.ceratarget_rel, SoundCategory.PLAYERS, 0.7F, 0.975F + itemRand.nextFloat() / 20.0F, false);
