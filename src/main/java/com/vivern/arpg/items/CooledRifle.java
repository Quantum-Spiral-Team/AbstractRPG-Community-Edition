...
public class CooledRifle extends ItemWeapon {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack) {
                    if (click2 && NBTHelper.GetNBTboolean(itemstack, "shotgun") && this.isReloaded(itemstack)) {
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.pump_shotgun, SoundCategory.AMBIENT, 1.3F, 0.9F + itemRand.nextFloat() / 5.0F);
                        ...
                        for (int j = 0; j < shots; j++) {
                            ...
                            Weapons.shoot(projectile, EnumHand.MAIN_HAND, player, player.rotationPitch - 3.0F, player.rotationYaw, 0.0F, parameters.getFloat("velocity"), parameters.getEnchantedF("inaccuracy_shotgun", acc), -0.2F, 0.5F, 0.5F);
                    ...
                    if (click) {
                        if (ammo > 0 && this.isReloaded(itemstack)) {
                            if (!player.getCooldownTracker().hasCooldown(this)) {
                                ...
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.cooled_rifle, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                                ...
                                if (world.collidesWithAnyBlock(aabb)) {
                                    ...
                                    world.playSound(null, vec.x, vec.y, vec.z, Sounds.bullet, SoundCategory.AMBIENT, 0.9F, 0.97F + itemRand.nextFloat() / 5.0F);
                                }
                            ...
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.cooled_rifle_rel, SoundCategory.AMBIENT, 0.7F, 1.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.RELOADING, itemstack) * 0.2F);
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        BulletParticle part = new BulletParticle(world, from, to, 0.06F, (int) MathHelper.clamp(dist / 4.0F, 2.0F, 7.0F), 4, dist, (float) col.x, (float) col.y, (float) col.z, d3 > 0.0);
        part.Red2 = 0.2F;
        part.Green2 = 0.7F;
        part.Blue2 = 1.0F;
        part.smokeRed = 0.42F;
