            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack) {
                    ...
                    if (firemode > 0 && firemode < needfiremodeTick) {
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.rocket_launcher_mode, SoundCategory.AMBIENT, 0.9F, 0.97F + itemRand.nextFloat() / 15.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.RAPIDITY, itemstack) * 0.2F);
                    }

                    if (click && firemode <= 0) {
                        if (ammo > 0 && this.isReloaded(itemstack)) {
                            if (!hascooldown) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.rocket_launcher, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                                ...
                                Weapons.shoot(projectile, EnumHand.MAIN_HAND, player, player.rotationPitch, player.rotationYaw, 0.0F, parameters.getFloat("velocity"), parameters.getEnchantedF("inaccuracy", acc), -0.05F, 0.5F, 0.7F);
                            ...
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.rocket_launcher_rel, SoundCategory.AMBIENT, 0.8F, 1.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.RELOADING, itemstack) * 0.2F);
                    ...
                    if (click && firemode >= needfiremodeTick) {
                        if (ammo < 3 || !this.isReloaded(itemstack)) {
                            ...
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.rocket_launcher, SoundCategory.AMBIENT, 1.1F, 0.8F + itemRand.nextFloat() / 5.0F);
                            player.getCooldownTracker().setCooldown(this, cooldownTime);
                            player.addStat(StatList.getObjectUseStats(this));
                            IWeapon.fireBomEffect(this, player, world, 0);
                            if (!player.capabilities.isCreativeMode) {
                            ...
                            for (int i = 0; i < 3; i++) {
                                ...
                                projectile.damage = parameters.getEnchantedF("damage", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, itemstack));
                                projectile.knockback = parameters.getEnchantedF("knockback", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, itemstack));
                                Weapons.shoot(projectile, EnumHand.MAIN_HAND, player, player.rotationPitch, player.rotationYaw + (i - 1) * 5, 0.0F, parameters.getFloat("velocity"), parameters.getEnchantedF("inaccuracy", acc), -0.05F, 0.5F, 0.7F);
