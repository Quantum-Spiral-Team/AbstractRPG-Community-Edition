    ...
    public boolean inUpdate(ItemStack itemstack, World world, Entity entityIn, int itemSlot, boolean isSelected, boolean[] removePull) {
        ...
        if (player.getHeldItemMainhand() == itemstack) {
            ...
            if (ice < 10 || !click2) {
                ...
                if (click2) {
                    if (!player.getCooldownTracker().hasCooldown(this)) {
                        ...
                        for (int i = 0; i < nn; i++) {
                            ...
                            Weapons.shoot(projectile, EnumHand.MAIN_HAND, player, player.rotationPitch, player.rotationYaw, 0.0F, parameters.getFloat("shard_velocity"), parameters.getEnchantedF("shard_inaccuracy", acc) + i * 2, -0.1F, 0.4F, 0.1F);
                        ...
                        if (EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, itemstack) > 0) {
                            for (int i = -20; i <= 20; i += 40) {
                                ...
                                Vec3d randlook = GetMOP.pitchYawToVec3D(player.rotationPitch + (itemRand.nextFloat() - 0.5F) * 22.0F, player.rotationYaw + i + (itemRand.nextFloat() - 0.5F) * 12.0F).scale(2.0);
                ...
                } else if (player.ticksExisted % parameters.getEnchantedI("charges_decay_rate", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.REUSE, itemstack)) == 0) {
                    NBTHelper.AddNBTint(itemstack, 1, "ice");
                }
