...
public class VampiricHeart extends Item implements IBauble, IAttributedBauble, IItemKilling {
    ...
    public void onWornTick(ItemStack itemstack, EntityLivingBase player) {
        if (player.world.getLightFor(EnumSkyBlock.SKY, player.getPosition()) > 14 && player.world.isDaytime() && player.ticksExisted % 20 == 0 && !player.world.isRemote) {
            player.attackEntityFrom(DamageSource.ON_FIRE, 4.0F);
            player.setFire(10);
        }

        if (player instanceof EntityPlayer && player.ticksExisted % 150 == 0 && !((EntityPlayer) player).getCooldownTracker().hasCooldown(this) && !AdvancedMobEffects.hasEffect(player, AdvancedMobEffects.VAMPIRIC_HEART_EFFECT)) {
