            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack && !player.getCooldownTracker().hasCooldown(itemIn)) {
                    if (Mana.getMana(player) > manacost && click) {
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.acid_fire, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                        player.getCooldownTracker().setCooldown(this, this.getCooldownTime(itemstack));
                        player.addStat(StatList.getObjectUseStats(this));
                        this.bom();
                        EntityAcidFire projectile = new EntityAcidFire(world, player, itemstack, power);
                        Weapons.shoot(projectile, EnumHand.MAIN_HAND, player, player.rotationPitch, player.rotationYaw, 0.0F, parameters.getFloat("velocity"), parameters.getEnchantedF("inaccuracy", acc), -0.1F, 0.5F, 0.5F);
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.acid_fire_mode, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                    }
                }
            }
        }
