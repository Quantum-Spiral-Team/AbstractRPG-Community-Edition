...
public interface IWeapon {
    ...
    //	public int timeAimingMainHand = 0;
    //	public int timeAimingOffHand = 0;
    //	public int cooldown = 2;
    //	public Item ammo = null;
    //	public SoundEvent reloadSound = null;
    //	public SoundEvent useSound = null;
    ...
    //	default float getAnimRotationMultiplier(ItemStack itemstack)
    //	{
    //		return 1.5F;
    //	}
     ...
     *
     * @return 1.0F (full ammo / full bar),   0.0F (fully damaged / empty bar)
     */
    @SideOnly(Side.CLIENT)
    default float getAdditionalDurabilityBar(ItemStack stack) {
        return 0.0F;
    }
    ...
    /**
     * returns should play reload sound
     */
    ...
    //	default boolean initiateReload(ItemStack itemstack, EntityPlayer player, int amountSpend, Item itemAmmo, int maxAmmo)
    //	{
    //		return ini
    //	}
        ...
        if (!hascooldown) {
            ...
            if (NBTHelper.GetNBTint(itemstack, "ammo") == 0) {
                if (player.inventory.hasItemStack(new ItemStack(itemClip))) {
                    ...
                    //	player.inventory.clearMatchingItems(itemClip, -1, 1, null);
                    //	clipstack.shrink(1);
    ...
    default boolean initiateBulletReload(ItemStack itemstack, EntityPlayer player, ItemStack itemClip, int maxAmmo, boolean returnEmptyClip) {
        ...
        if (!hascooldown) {
            NBTHelper.GiveNBTint(itemstack, 0, "ammo");
            if (NBTHelper.GetNBTint(itemstack, "ammo") == 0) {
                //  if (player.inventory.hasItemStack(itemClip))
    ...
    static boolean canShoot(ItemStack itemstack) {
        ...
        if (nbt == null || !nbt.hasKey("iw_usable")) {
            return true;
        }
        return nbt.getBoolean("iw_usable");
    }
            ...
            if (offH != itemstack && mainH != itemstack) {
                ...
                if (offH == itemstack && !mainH.isEmpty()) {
                    ...
                    for (Entry<String, AttributeModifier> entry : map.entries()) {
                        if (SharedMonsterAttributes.ATTACK_DAMAGE.getName().equals(entry.getKey()) || SharedMonsterAttributes.ATTACK_SPEED.getName().equals(entry.getKey())) {
                            return false;
                        }
                    }
                }
                ...
                switch (this.getWeaponHandleType(itemstack)) {
                    ...
                    case SEMI_ONE_HANDED:
                        ...
                        //      if (offH.getItem() instanceof IWeapon) {
                        //          return ((IWeapon) offH.getItem()).getWeaponHandleType() != WeaponHandleType.TWO_HANDED;
                        //      } else {
                        //          return true;
                        //      }
                        //  }
    ...
    //  only !world.isRemote
    static void fireEffect(Item thisItem, EntityPlayer player, World world, double distance, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        if (!world.isRemote) {
    ...
    static void fireEffectExcl(Item thisItem, EntityPlayer player, EntityPlayer excludingplayer, World world, double distance, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        if (!world.isRemote) {
            PacketWeaponEffectToClients packet = new PacketWeaponEffectToClients();
            packet.writeArgs(x, y, z, a, b, c, d1, d2, d3, Item.getIdFromItem(thisItem));
    ...
    static void fireEffect(Item thisItem, Entity senderForPosition, World world, double distance, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        if (!world.isRemote) {
            PacketWeaponEffectToClients packet = new PacketWeaponEffectToClients();
            packet.writeArgs(x, y, z, a, b, c, d1, d2, d3, Item.getIdFromItem(thisItem));
            PacketHandler.sendToAllAround(packet, world, senderForPosition.posX, senderForPosition.posY, senderForPosition.posZ, distance);
        }
    }
    ...
    default boolean attackEntityMelee(Entity entity, ItemStack stack, EntityPlayer player, EnumHand hand, boolean isCritical) {
        ...
        boolean ret = entity.attackEntityFrom(new WeaponDamage(stack, player, null, false, false, player, WeaponDamage.blade), (float) player.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue() * artropods * holy);
        entity.hurtResistantTime = 0;
        int fireLvl = WeaponParameters.getWeaponParameters(stack.getItem()).getEnchantedI("fire", EnchantmentHelper.getEnchantmentLevel(Enchantments.FIRE_ASPECT, stack));
        if (fireLvl > 0) {
    ...
    static MeleeAttackResult doMeleeSwordAttack(IWeapon iweapon, ItemStack stack, EntityPlayer player, EnumHand hand, boolean isCritical) {
        ...
        return doMeleeSwordAttack(iweapon, stack, player, hand, parameters.getEnchantedF("damage", sharpness), parameters.getEnchantedF("knockback", knockback), parameters.getEnchantedF("length", sweeping), parameters.getEnchantedF("size", sweeping), parameters.getEnchantedF("end_size", sweeping), isCritical);
    }

    static MeleeAttackResult doMeleeSwordAttack(IWeapon iweapon, ItemStack stack, EntityPlayer player, EnumHand hand, float damage, float knockback, float length, float size, float endSize, boolean isCritical) {
        World world = player.world;
        Vec3d vec = GetMOP.posRayTrace(length, 1.0F, player, size, size / 2.0F);
        AxisAlignedBB aabb = new AxisAlignedBB(vec.x - endSize, vec.y - endSize, vec.z - endSize, vec.x + endSize, vec.y + endSize, vec.z + endSize);
    ...
    static MeleeAttackResult doMeleeSpearAttack(IWeapon iweapon, ItemStack stack, EntityPlayer player, EnumHand hand, boolean isCritical) {
        ...
        int knockback = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, stack);
        WeaponParameters parameters = WeaponParameters.getWeaponParameters(stack.getItem());
        return doMeleeSpearAttack(iweapon, stack, player, hand, parameters.getEnchantedF("damage", sharpness), parameters.getEnchantedF("knockback", knockback), parameters.getEnchantedF("length", sweeping), parameters.getEnchantedF("size", sweeping), isCritical);
    }

    static MeleeAttackResult doMeleeSpearAttack(IWeapon iweapon, ItemStack stack, EntityPlayer player, EnumHand hand, float damage, float knockback, float length, float size, boolean isCritical) {
        World world = player.world;
    ...
    static MeleeAttackResult doMeleeHammerAttack(IWeapon iweapon, ItemStack stack, EntityPlayer player, EnumHand hand, boolean isCritical, int sweepAngle, int sweepStepAngle) {
        int sharpness = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, stack);
        int sweeping = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.RANGE, stack);
        int knockback = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, stack);
        WeaponParameters parameters = WeaponParameters.getWeaponParameters(stack.getItem());
        return doMeleeHammerAttack(iweapon, stack, player, hand, parameters.getEnchantedF("damage", sharpness), parameters.getEnchantedF("knockback", knockback), parameters.getEnchantedF("length", sweeping), parameters.getEnchantedF("size", sweeping), parameters.getEnchantedF("end_size", sweeping), isCritical, sweepAngle, sweepStepAngle);
    }

    static MeleeAttackResult doMeleeHammerAttack(IWeapon iweapon, ItemStack stack, EntityPlayer player, EnumHand hand, float damage, float knockback, float length, float size, float endSize, boolean isCritical, int sweepAngle, int sweepStepAngle) {
        World world = player.world;
        sweepAngle /= 2;
        Vec3d vec = null;
        ...
        while (pit > player.rotationPitch - sweepAngle) {
            ...
            AxisAlignedBB aabb2 = new AxisAlignedBB(vec.x - size, vec.y - size, vec.z - size, vec.x + size, vec.y + size, vec.z + size);
            lastaabb = aabb2;
        ...
        return new MeleeAttackResult(vec, lastaabb, Collections.emptyList(), null, false);
    }
    ...
    static MeleeAttackResult doMeleeDaggerAttack(IWeapon iweapon, ItemStack stack, EntityPlayer player, EnumHand hand, float damage, float knockback, float length, boolean isCritical) {
        ...
        if (result != null) {
            if (result.entityHit != null) {
                ...
                List<Entity> list = Collections.singletonList(result.entityHit);
                return new MeleeAttackResult(result.hitVec, null, list, result, ret);
            } else {
                return new MeleeAttackResult(result.hitVec, null, Collections.emptyList(), result, false);
            }
        }

        return new MeleeAttackResult(vec3d2, null, Collections.emptyList(), null, false);
    }
    ...
    static MeleeAttackResult doMeleeWhipAttack(IWeapon iweapon, ItemStack stack, EntityPlayer player, EnumHand hand, float damage, float knockback, float length, float size, float endSize) {
        ...
        AxisAlignedBB aabbEnd = new AxisAlignedBB(yawVec.x - endSize, yawVec.y - endSize, yawVec.z - endSize, yawVec.x + endSize, yawVec.y + endSize, yawVec.z + endSize);
    ...
    static boolean doMeleeSplashAroundAttack(IWeapon iweapon, ItemStack stack, EntityPlayer player, EnumHand hand, float damage, float knockback, float sizeXZ, float sizeY, boolean isCritical) {
        World world = player.world;
        Vec3d vec = GetMOP.entityCenterPos(player);
        AxisAlignedBB aabb = new AxisAlignedBB(vec.x - sizeXZ, vec.y - sizeY, vec.z - sizeXZ, vec.x + sizeXZ, vec.y + sizeY, vec.z + sizeXZ);
        List<Entity> list = world.getEntitiesWithinAABBExcludingEntity(player, aabb);
        if (!list.isEmpty()) {
            IAttributeInstance entityAttribute = player.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE);
            AttributeModifier modifier = new AttributeModifier("iw_attackmelee", damage, 0);
    ...
    static boolean checkShieldAngle(ItemStack stack, EntityPlayer playerWithShield, DamageSource source) {
        ...
        double angle = GetMOP.getAngleBetweenVectors(vec1.x, vec1.y, vec1.z, vec3.x - vec2.x, vec3.y - vec2.y, vec3.z - vec2.z);
    ...
    default WeaponParameters setDefaultDescription(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        ...
        tooltip.add("\u00A77Knockback: " + parameters.getFloat("knockback") + parameters.getFloat("knockback_ench") * EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, stack));
    ...
    class MeleeAttackResult {
        ...
        public final List<Entity> attackedEntities;
        public List<Entity> attackedEntities2;

        public MeleeAttackResult(@Nullable Vec3d position, @Nullable AxisAlignedBB axisAlignedBB, List<Entity> attackedEntities, @Nullable RayTraceResult raytrace, boolean success) {
