...
public class Submachine extends ItemWeapon {
    ...
    public void onUpdate(ItemStack itemstack, World world, Entity entityIn, int itemSlot, boolean isSelected) {
        if (!world.isRemote) {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                EnumHand hand = click && player.getHeldItemMainhand() == itemstack ? EnumHand.MAIN_HAND : (click2 && player.getHeldItemOffhand() == itemstack ? EnumHand.OFF_HAND : null);
                if (hand != null && heat < 2000) {
                    if (ammo > 0 && this.isReloaded(itemstack)) {
                        if ((!hascooldown || hand != EnumHand.MAIN_HAND) && (!hascooldown2 || hand != EnumHand.OFF_HAND)) {
                            ...
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.submachine_shoot, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                            ...
                            if (addheat + heat >= 2000) {
                                world.playSound(null, player.posX, player.posY, player.posZ, SoundEvents.BLOCK_FIRE_EXTINGUISH, SoundCategory.AMBIENT, 0.6F, 0.9F + itemRand.nextFloat() / 5.0F);
                            }
                            ...
                            if (world.collidesWithAnyBlock(aabb)) {
                                ...
                                world.playSound(null, vec.x, vec.y, vec.z, Sounds.bullet, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                            }

                            float wdamage = parameters.getEnchantedF("damage", might) + damageadd * parameters.getFloat("bullet_damage") + heat * parameters.getFloat("per_heat_damage");
                            float wknockback = parameters.getEnchantedF("knockback", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, itemstack)) + knockbackadd * parameters.getFloat("bullet_knockback");
                            if (!list.isEmpty()) {
                                for (Entity entity : list) {
                                    if (Team.checkIsOpponent(player, entity)) {
                                        Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, true, player.getPositionEyes(1.0F), WeaponDamage.bullet), wdamage, player, entity, true, wknockback, player.posX, player.posY, player.posZ);
                            ...
                            IWeapon.fireEffect(this, player, world, 64.0, shootPoint.x, shootPoint.y, shootPoint.z, vec.x, vec.y, vec.z, c, dustState == null ? -1.0 : Block.getStateId(dustState), collidesWithAny ? (isFired ? 3 : 2) : (isFired ? 1 : 0));
                        }
                    } else if (this.initiateBulletReload(itemstack, player, ItemsRegister.SUBMACHINE_CLIP, maxammo, true)) {
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.pistol_rel, SoundCategory.AMBIENT, 0.7F, 0.95F + itemRand.nextFloat() / 10.0F);
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        BulletParticle part = new BulletParticle(world, from, to, 0.1F, (int) MathHelper.clamp(dist / 3.0F, 2.0F, 8.0F), 7, dist, (float) col.x, (float) col.y, (float) col.z, collidesWithAny);
        part.Red2 = 0.75F;
        part.Green2 = 0.75F;
        part.Blue2 = 0.5F;
        ...
        if (fired) {
            ...
            for (int i = 0; i < amount; i++) {
                ...
                SparkleSubparticle sparklepart = new SparkleSubparticle(from.x + difference.x * randF, from.y + difference.y * randF, from.z + difference.z * randF, 0.02F, 40 + itemRand.nextInt(20), (float) differenceNorm.x + (itemRand.nextFloat() - 0.5F) * 0.04F, (float) differenceNorm.y + (itemRand.nextFloat() - 0.5F) * 0.04F, (float) differenceNorm.z + (itemRand.nextFloat() - 0.5F) * 0.04F, 0.0F);
            ...
            if (collidesWithAny) {
                ...
                for (int i = 0; i < 4; i++) {
                    float randF = itemRand.nextFloat();
                    SparkleSubparticle sparklepart = new SparkleSubparticle(a, b, c, 0.02F, 40 + itemRand.nextInt(40), (itemRand.nextFloat() - 0.5F) * 0.1F, (itemRand.nextFloat() - 0.5F) * 0.1F, (itemRand.nextFloat() - 0.5F) * 0.1F, 0.0F);
                    SparkleSubparticle.particles.add(sparklepart);
                }
            }
        }
    ...
    public int getCooldownTime(ItemStack itemstack) {
        ...
        return parameters.getEnchantedI("cooldown", rapidity) - (EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, itemstack) > 0 ? Math.round(Math.max(NBTHelper.GetNBTint(itemstack, "heat") * parameters.getFloat("special_heat_overclock") - 1.0F, 0.0F)) : 0);
    }
