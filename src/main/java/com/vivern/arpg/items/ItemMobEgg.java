...
public class ItemMobEgg extends ItemItem {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (worldIn.isRemote) {
            ...
            Entity entity = ItemMonsterPlacer.spawnCreature(worldIn, this.mobname, blockpos.getX() + 0.5, blockpos.getY() + d0, blockpos.getZ() + 0.5);
            if (entity != null) {
        ...
        if (worldIn.isRemote) {
            ...
            if (raytraceresult != null && raytraceresult.typeOfHit == Type.BLOCK) {
                ...
                if (!(worldIn.getBlockState(blockpos).getBlock() instanceof BlockLiquid)) {
                    ...
                    Entity entity = ItemMonsterPlacer.spawnCreature(worldIn, this.mobname, blockpos.getX() + 0.5, blockpos.getY() + 0.5, blockpos.getZ() + 0.5);
                    if (entity == null) {
                        return new ActionResult(EnumActionResult.PASS, itemstack);
                    } else {
                        if (entity instanceof EntityLivingBase && itemstack.hasDisplayName()) {
