        ...
        if (!world.isRemote) {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack && !player.getCooldownTracker().hasCooldown(this)) {
                    if (click2) {
                        player.openGui(AbstractRPG.instance, 11, world, MathHelper.floor(player.posX), MathHelper.floor(player.posY), MathHelper.floor(player.posZ));
                    } else if (click) {
                        ...
                        if (mode == 0 && mana > 5.0F - sor) {
                            ...
                            if (res != null && res.hitVec != null) {
                                ...
                                AxisAlignedBB aabb = new AxisAlignedBB(res.hitVec.x - damageRadius, res.hitVec.y - damageRadius, res.hitVec.z - damageRadius, res.hitVec.x + damageRadius, res.hitVec.y + damageRadius, res.hitVec.z + damageRadius);
