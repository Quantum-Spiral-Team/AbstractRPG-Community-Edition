...
import com.vivern.arpg.items.models.AntiRadPackModel;
import com.vivern.arpg.items.models.EnigmateTwinsModel;
import com.vivern.arpg.items.models.GasmaskModel;
...
public class BaublesPack {
    ...
    public static class BleedingRoot extends Item implements IBauble, IRenderBauble, IItemHurted, IAttributedBauble {
        ...
        public float onHurtWithItem(float hurtdamage, ItemStack stack, EntityPlayer player, DamageSource source) {
            if (!player.world.isRemote && !player.getCooldownTracker().hasCooldown(stack.getItem()) && source.getTrueSource() != null && source.getTrueSource() instanceof EntityLivingBase) {
                ToxicomaniaMobsPack.Mosquito mosq = new ToxicomaniaMobsPack.Mosquito(player.world, player, (EntityLivingBase) source.getTrueSource(), true, false);
                mosq.setLocationAndAngles(player.posX, player.posY + itemRand.nextFloat(), player.posZ, itemRand.nextFloat() * 360.0F, 0.0F);
                ...
                player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.larva_water_attack, SoundCategory.PLAYERS, 0.6F, 0.95F + itemRand.nextFloat() / 10.0F);
            }
    ...
    public static class CharmOfUndying extends Item implements IBauble, IRenderBauble, IItemDamaged {
        ...
        public void onWornTick(ItemStack itemstack, EntityLivingBase entityIn) {
            if (!entityIn.world.isRemote && (entityIn.getHeldItemMainhand().getItem() == Items.TOTEM_OF_UNDYING || entityIn.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING)) {
    ...
    public static class CyberAmulet extends Item implements IBauble, IRenderBauble, IItemHurted, IAttributedBauble {
        ...
        public Multimap<String, AttributeModifier> getAttributeModifiers(EntityPlayer player, int equipmentSlot, ItemStack itemstack) {
            ...
            multimap.put(SharedMonsterAttributes.MOVEMENT_SPEED.getName(), new AttributeModifier(uuid, "movement speed modifier", player.getCooldownTracker().hasCooldown(itemstack.getItem()) ? 0.01 : 0.08, 0));
            return multimap;
        }
    ...
    public static class EnderCrown extends Item implements IBauble, IRenderBauble, IAttributedBauble {
        ...
        public static ParticleTracker.TrackerSmoothShowHide ssh = new ParticleTracker.TrackerSmoothShowHide(null, new Vec3d[]{new Vec3d(0.0, 8.0, 0.85), new Vec3d(7.0, 10.0, 0.2), new Vec3d(9.0, 12.0, -0.2), new Vec3d(11.0, 20.0, -0.85)});
            ...
            if (!entity.world.isRemote && entity instanceof EntityPlayer) {
                ...
                if (!player.getCooldownTracker().hasCooldown(this) && ServerKeyTracker.isKeyPressed(player, ServerKeyTracker.Keys.ABILITY)) {
                    ...
                    if (res.entityHit instanceof EntityLiving) {
                        ...
                        if (eff != null && entitylive.getHealth() <= (eff.getAmplifier() + 1) * 21 - entitylive.getEntityAttribute(PropertiesRegistry.ARMOR_PROTECTION).getAttributeValue()) {
                            ...
                            player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.fast_teleport, SoundCategory.AMBIENT, 0.8F, pit);
                            PacketDoSomethingToClients packet = new PacketDoSomethingToClients();
                            packet.writeArgs(entitylive.posX, entitylive.posY + entitylive.height / 2.0F, entitylive.posZ, player.posX, player.posY + player.height / 2.0F, player.posZ, 7);
                            ...
                            EntityLiveHeart.spawnHearts(entity.world, entitylive.posX, entitylive.posY + 0.1, entitylive.posZ, 3, 2.0F, true, 2.5F, Team.getTeamFor(player));
                            player.world.playSound(null, entitylive.posX, entitylive.posY, entitylive.posZ, Sounds.fast_teleport, SoundCategory.AMBIENT, 0.8F, pit);
                            ...
                            if (!list.isEmpty()) {
                                for (EntityLivingBase entitylivingbase : list) {
                                    if (Team.checkIsOpponent(player, entitylivingbase) && entitylivingbase != entitylive) {
                                        SuperKnockback.applyKnockback(entitylivingbase, 2.0F, entitylive.posX, entitylive.posY - 1.0, entitylive.posZ);
                                        Weapons.mixPotion(entitylivingbase, MobEffects.SLOWNESS, 60.0F, 3.0F, Weapons.EnumPotionMix.ABSOLUTE, Weapons.EnumPotionMix.GREATEST, Weapons.EnumMathOperation.NONE, Weapons.EnumMathOperation.NONE, 0, 0);
                                        if (entitylivingbase.isPotionActive(PotionEffects.FROSTBURN)) {
                                            EntityLiveHeart.spawnHearts(entity.world, entitylive.posX, entitylive.posY + 0.1, entitylive.posZ, entitylivingbase.getActivePotionEffect(PotionEffects.FROSTBURN).getAmplifier() + 1, 2.0F, true, 2.5F, Team.getTeamFor(player));
        ...
        public static RayTraceResult fixedRayTraceBlocks(World world, Entity shooter, double size, double raystep, boolean checkTeam, Vec3d start, Vec3d end, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock) {
            ...
            for (double k = 0.0; k <= 1.0; k += step) {
                ...
                if (!list.isEmpty()) {
                    for (EntityLivingBase entityliving : list) {
                        ...
                        if (entityliving != shooter && eff != null && entityliving.getHealth() <= (eff.getAmplifier() + 1) * 21 - entityliving.getEntityAttribute(PropertiesRegistry.ARMOR_PROTECTION).getAttributeValue()) {
                            if (!checkTeam) {
        ...
        public static void effect(World world, double x, double y, double z, double prevx, double prevy, double prevz) {
            for (int ss = 0; ss < 55; ss++) {
                world.spawnParticle(EnumParticleTypes.DRAGON_BREATH, x, y, z, itemRand.nextGaussian() / 10.0, itemRand.nextGaussian() / 10.0, itemRand.nextGaussian() / 10.0, new int[0]);
            }

            for (int ss = 0; ss < 200; ss++) {
                world.spawnParticle(EnumParticleTypes.SUSPENDED_DEPTH, x + itemRand.nextGaussian() * 2.0, y + itemRand.nextGaussian() * 2.0, z + itemRand.nextGaussian() * 2.0, 0.0, 0.0, 0.0, new int[0]);
            }

            float fsize = (float) (2.0 + itemRand.nextGaussian() / 6.0);
            int lt = 20;
            GUNParticle part = new GUNParticle(BaublesPack.voidexplode, 0.1F, 0.0F, lt, 240, world, x, y, z, 0.0F, 0.0F, 0.0F, 0.8F + itemRand.nextFloat() * 0.1F, 1.0F, 1.0F, true, itemRand.nextInt(360));
            ...
            for (float f = 0.0F; f < 1.0F; f += add) {
                ...
                for (int i = 0; i < 2; i++) {
                    world.spawnParticle(EnumParticleTypes.PORTAL, pX + (itemRand.nextDouble() - 0.5), pY + itemRand.nextDouble() - 0.25, pZ + (itemRand.nextDouble() - 0.5), (itemRand.nextDouble() - 0.5) * 2.0, -itemRand.nextDouble(), (itemRand.nextDouble() - 0.5) * 2.0, new int[0]);
                }
            }
        }
            ...
            if (!playerattacker.world.isRemote && playerattacker.ticksExisted % 3 == 0) {
                ...
                if (shots > 0) {
                    ...
                    if (target != null && target instanceof EntityLivingBase) {
                        Vec3d vec3dEyes = new Vec3d(playerattacker.posX, playerattacker.posY + playerattacker.getEyeHeight() + 0.2F, playerattacker.posZ);
                        ...
                        projectile.shoot(playerattacker, playerattacker.rotationPitch - 7.0F, playerattacker.rotationYaw + (nexttwin ? 14 : -14), 0.0F, 0.5F + itemRand.nextFloat() * 0.2F, 4.0F);
                        ...
                        playerattacker.world.playSound(null, pos.x, pos.y, pos.z, Sounds.enigmate_twins, SoundCategory.AMBIENT, 0.7F, 0.9F + itemRand.nextFloat() / 5.0F);
                ...
                try {
                    ...
                    if (objectt instanceof Integer) {
                        i = (Integer) objectt;
                         ...
                         InvocationTargetException | NoSuchMethodException var8) {
        ...
        public void onWornTick(ItemStack itemstack, EntityLivingBase entityIn) {
            ...
            if (entityIn instanceof EntityPlayer) {
                ...
                if (click && (dm == 0 || dm == 2 || dm == 4)) {
                    ...
                    player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.cyber_jump, SoundCategory.PLAYERS, 0.5F, 0.95F + itemRand.nextFloat() / 10.0F);
            ...
            if (!entity.world.isRemote && entity instanceof EntityPlayer) {
                ...
                if (!player.getCooldownTracker().hasCooldown(itemstack.getItem())) {
                    ...
                    if (target != null) {
                        ...
                        SuperKnockback.applyMove(shoot, target.isInWater() ? -3.2F : -1.4F, target.posX, target.posY + (target.isInWater() ? target.height / 2.0F : 2.0F), target.posZ);
                        shoot.isMana = true;
                        player.world.spawnEntity(shoot);
                        player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.larva_water_attack, SoundCategory.AMBIENT, 0.7F, 0.9F + itemRand.nextFloat() / 5.0F);
    ...
    public static class PainfulRoot extends Item implements IBauble, IRenderBauble, IItemHurted {
        ...
        public float onHurtWithItem(float hurtdamage, ItemStack stack, EntityPlayer player, DamageSource source) {
            if (!player.world.isRemote && !player.getCooldownTracker().hasCooldown(stack.getItem()) && source.getTrueSource() != null && source.getTrueSource() instanceof EntityLivingBase) {
                ToxicomaniaMobsPack.Mosquito mosq = new ToxicomaniaMobsPack.Mosquito(player.world, player, (EntityLivingBase) source.getTrueSource(), false, false);
                mosq.setLocationAndAngles(player.posX, player.posY + itemRand.nextFloat(), player.posZ, itemRand.nextFloat() * 360.0F, 0.0F);
                mosq.motionX = (itemRand.nextFloat() - 0.5F) / 2.0F;
                mosq.motionZ = (itemRand.nextFloat() - 0.5F) / 2.0F;
                mosq.velocityChanged = true;
            ...
            if (!entityIn.world.isRemote) {
                ...
                for (Entity entityx : list) {
                    if (entityx instanceof EntityItem || entityx instanceof EntityCoin) {
                        entityIn.world.spawnParticle(EnumParticleTypes.END_ROD, entityx.posX, entityx.posY, entityx.posZ, 0.0, 0.0, 0.0, new int[0]);
                        if (--max <= 0) {
                            break;
                        }
                    }
    ...
    public static class SpiritThorn extends Item implements IBauble, IRenderBauble, IItemHurted, IAttributedBauble {
        ...
        public float onHurtWithItem(float hurtdamage, ItemStack stack, EntityPlayer player, DamageSource source) {
            if (!player.world.isRemote && !player.getCooldownTracker().hasCooldown(stack.getItem()) && source.getTrueSource() != null && source.getTrueSource() instanceof EntityLivingBase) {
                ToxicomaniaMobsPack.Mosquito mosq = new ToxicomaniaMobsPack.Mosquito(player.world, player, (EntityLivingBase) source.getTrueSource(), false, true);
                mosq.setLocationAndAngles(player.posX, player.posY + itemRand.nextFloat(), player.posZ, itemRand.nextFloat() * 360.0F, 0.0F);
                mosq.motionX = (itemRand.nextFloat() - 0.5F) / 2.0F;
                mosq.motionZ = (itemRand.nextFloat() - 0.5F) / 2.0F;
                mosq.velocityChanged = true;
            ...
            if (!entity.world.isRemote && entity instanceof EntityPlayer) {
                ...
                if (!player.getCooldownTracker().hasCooldown(itemstack.getItem())) {
                    ...
                    if (target != null) {
                        ThornkeeperShoot shoot = new ThornkeeperShoot(player.world, player, itemstack, Mana.getMagicPowerMax(player));
                        shoot.setLocationAndAngles(player.posX, player.posY + 1.0, player.posZ, 0.0F, 0.0F);
                        SuperKnockback.applyMove(shoot, target.isInWater() ? -3.0F : -1.3F, target.posX, target.posY + (target.isInWater() ? target.height / 2.0F : 1.5), target.posZ);
                        player.world.spawnEntity(shoot);
                        player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.larva_water_attack, SoundCategory.AMBIENT, 0.7F, 0.9F + itemRand.nextFloat() / 5.0F);
