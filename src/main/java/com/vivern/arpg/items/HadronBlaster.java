...
public class HadronBlaster extends ItemWeapon implements IEnergyItem {
        ...
        if (IWeapon.canShoot(itemstack)) {
            ...
            if (inmainhand) {
                if (player.ticksExisted % 5 == 0) {
                    ...
                    if (points > hadrons_to_laser) {
                        if (!NBTHelper.GetNBTboolean(itemstack, "sensor")) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.hadron_blaster_sensor, SoundCategory.AMBIENT, 0.4F, 0.9F + itemRand.nextFloat() / 5.0F);
                ...
                if (capturetime <= 0 && !beam && !hascooldown && click2) {
                    ...
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.plasma_accelerator_charge, SoundCategory.AMBIENT, 0.5F, 0.9F + itemRand.nextFloat() / 5.0F);
                }
                ...
                if (capturetime > 0) {
                    ...
                    if (!beam) {
                        if (!world.isRemote && click && RF >= RFtoShoot) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.hadron_blaster, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                            ...
                            Weapons.shoot(projectile, EnumHand.MAIN_HAND, player, player.rotationPitch, player.rotationYaw, 0.0F, parameters.getFloat("velocity"), parameters.getEnchantedF("inaccuracy", acc), -0.05F, 0.5F, 0.3F);
                            world.spawnEntity(projectile);
                        }
                    } else if (click && RF >= RFtoLazer) {
                        if (lazerdel < 16) {
                            if (lazerdel == 1) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.hadron_blaster_charge, SoundCategory.AMBIENT, 0.8F, 0.9F + itemRand.nextFloat() / 5.0F);
                                Weapons.setPlayerAnimationOnServer(player, 11, EnumHand.MAIN_HAND);
                            }

                            if (lazerdel == 15) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.hadron_blaster_beam, SoundCategory.AMBIENT, 0.7F, 0.95F + itemRand.nextFloat() / 10.0F);
                                Weapons.setPlayerAnimationOnServer(player, 11, EnumHand.MAIN_HAND);
                            }
                            ...
                            if (!world.isRemote) {
                                ...
                                if (hadronss <= 0) {
                                    NBTHelper.SetNBTboolean(itemstack, false, "beam");
                                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.hadron_blaster_end, SoundCategory.AMBIENT, 0.7F, 0.9F + itemRand.nextFloat() / 5.0F);
                                ...
                                AxisAlignedBB aabbxx = new AxisAlignedBB(vec.x - damageRadius, vec.y - damageRadius, vec.z - damageRadius, vec.x + damageRadius, vec.y + damageRadius, vec.z + damageRadius);
                                ...
                                if (!listxx.isEmpty()) {
                                    for (Entity entity : listxx) {
                                        if (Team.checkIsOpponent(player, entity)) {
                                            Weapons.dealDamage(new WeaponDamage(itemstack, player, null, true, false, vec, WeaponDamage.plasma), wdamage_laser, player, entity, true, wknockback_laser, vec.x, vec.y, vec.z);
                            ...
                            if (soundAndPlasma) {
                                world.playSound(null, vec.x, vec.y, vec.z, Sounds.hadron_blaster_impact, SoundCategory.AMBIENT, 0.5F, 0.9F + itemRand.nextFloat() / 4.0F);
                            }

                            if (!world.isRemote) {
                                IWeapon.fireEffectExcl(this, player, player, world, 64.0, vec.x, vec.y, vec.z, player.rotationPitch, player.rotationYaw, 1.0, player.getEntityId(), 0.0, 0.0);
                            } else {
                                Vec3d shootPoint = Weapons.getPlayerShootPoint(player, EnumHand.MAIN_HAND, 1.0F, 0.9F, 0.2F, -0.15F, false);
                                this.makeLaserParticles(world, player, shootPoint.x, shootPoint.y, shootPoint.z, vec.x, vec.y, vec.z, true, vec.distanceTo(shootPoint), 0.0F, 0.0F);
                                this.particlesEffect(player, world, vec.x, vec.y, vec.z, player.rotationPitch, player.rotationYaw, horizoffset, player.posX, player.posY + player.getEyeHeight(), player.posZ, soundAndPlasma);
                            }
                        }
                    } else if (lazerdel > 0) {
                        if (lazerdel > 14) {
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        if (entity != null && entity instanceof EntityPlayer) {
            ...
            this.makeLaserParticles(world, shootingPlayer, shootPoint.x, shootPoint.y, shootPoint.z, x, y, z, c != 0.0, dist, (float) a, (float) b);
        }
    }

    public void particlesEffect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3, boolean b1) {
        if (player.ticksExisted % 2 == 0) {
            GUNParticle bigsmoke1 = new GUNParticle(sparkle4, 0.5F, 0.0F, 2, 240, world, x, y, z, 0.0F, 0.0F, 0.0F, 1.0F, 0.8F + itemRand.nextFloat() / 5.0F, 0.3F + itemRand.nextFloat() / 10.0F, true, itemRand.nextInt(360));
        ...
        GUNParticle bigsmoke2 = new GUNParticle(sparkles, scl, 0.01F, lt, 240, world, x, y, z, (float) itemRand.nextGaussian() / 8.0F, (float) itemRand.nextGaussian() / 8.0F, (float) itemRand.nextGaussian() / 8.0F, 1.0F, bg, bg, false, itemRand.nextInt(360), true, 1.0F);
        ...
        if (b1) {
            ...
            AnimatedGParticle anim = new AnimatedGParticle(hadronblast, 2.2F, 0.0F, 13, 240, world, x, y, z, 0.0F, 0.0F, 0.0F, 1.0F, 0.9F + itemRand.nextFloat() / 10.0F, 0.9F + itemRand.nextFloat() / 10.0F, true, itemRand.nextInt(360));
            anim.framecount = 13;
    ...
    public void makeLaserParticles(World world, @Nullable EntityPlayer player, double xStart, double yStart, double zStart, double xEnd, double yEnd, double zEnd, boolean hit, double distance, float rotatPitch, float rotatYaw) {
