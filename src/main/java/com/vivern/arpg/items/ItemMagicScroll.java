...
public class ItemMagicScroll extends ItemWeapon {
    ...
    public static ParticleTracker tracker1 = new ParticleTracker.TrackerSmoothShowHideBetweenP(new Vec3d[]{new Vec3d(-1.0, 5.0, 0.25), new Vec3d(4.0, 13.0, -0.125)}, null);
    ...
    public static HashMap<Integer, ItemMagicScroll> createScrolls() {
        ...
        ItemMagicScroll scroll = new ItemMagicScroll("freeze", 0, CreativeTabs.COMBAT, 200, 0.5F, 0.5F, 1.0F) {
            ...
            public void onAffectEntity(ItemStack itemstack, World worldIn, Entity entity, boolean isopponent, EntityPlayer player) {
                if (isopponent) {
                    PotionEffect lastdebaff = Weapons.mixPotion(entity, PotionEffects.FREEZING, (float) (100 + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, itemstack) * 20), 8.0F, Weapons.EnumPotionMix.GREATEST, Weapons.EnumPotionMix.GREATEST, Weapons.EnumMathOperation.NONE, Weapons.EnumMathOperation.NONE, 0, 0);
        ...
        scroll = new ItemMagicScroll("demonic_curse", 1, CreativeTabs.COMBAT, 300, 0.65F, 0.4F, 0.9F) {
            ...
            public void onAffectEntity(ItemStack itemstack, World worldIn, Entity entity, boolean isopponent, EntityPlayer player) {
                if (isopponent && entity.isBurning()) {
                    ...
                    Weapons.setPotionIfEntityLB(entity, PotionEffects.DEMONIC_BURN, firetick + 100 + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, itemstack) * 10, 1);
                    entity.extinguish();
                }
            }
            ...
            public void onSpellImpact(ItemStack itemstack, World world, Vec3d position, EntityPlayer player) {
                world.playSound(null, position.x, position.y, position.z, Sounds.soul_steal, SoundCategory.PLAYERS, 1.0F, 1.05F + itemRand.nextFloat() / 5.0F);
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        if (d1 == 0.0) {
            for (int h = 0; h < 4; h++) {
                ...
                for (int i = 1; i <= max; i++) {
                    Vec3d newvec = i == max ? new Vec3d(a, b, c) : new Vec3d(x + itx * i + itemRand.nextGaussian() / 2.0, y + ity * i + itemRand.nextGaussian() / 2.0, z + itz * i + itemRand.nextGaussian() / 2.0);
                    BetweenParticle laser = new BetweenParticle(world, beam, 0.1F, 240, this.Red, this.Green, this.Blue, 0.0F, lastpos.distanceTo(newvec), 12, 0.3F, 8.0F, lastpos, newvec);
            ...
            for (int ss = 0; ss < 15; ss++) {
                GUNParticle part = new GUNParticle(spellblue, 0.02F + itemRand.nextFloat() / 50.0F, 0.022F, 20 + itemRand.nextInt(25), 240, world, a, b, c, (float) itemRand.nextGaussian() / 9.0F, (float) itemRand.nextGaussian() / 9.0F + 0.1F, (float) itemRand.nextGaussian() / 9.0F, this.Red, this.Green, this.Blue, false, itemRand.nextInt(360), true, 2.5F);
                world.spawnEntity(part);
            }

            float fsize = 1.5F + itemRand.nextFloat() / 3.0F;
            int lt = 8 + itemRand.nextInt(5);
            GUNParticle part = new GUNParticle(effect, 0.2F, 0.0F, lt, 240, world, a, b, c, 0.0F, 0.0F, 0.0F, this.Red, this.Green, this.Blue, true, itemRand.nextInt(360));
                    ...
                    for (float zz = -this.spellRadius; zz <= this.spellRadius; zz++) {
                        if (itemRand.nextFloat() < 0.5F) {
                            ...
                            if (itemRand.nextFloat() > distinterp) {
                                IBlockState iblockstate1 = world.getBlockState(blockpos$pooledmutableblockpos.setPos(MathHelper.floor(a + xx), MathHelper.floor(b + yy), MathHelper.floor(c + zz)));
                                if (!iblockstate1.isOpaqueCube()) {
                                    ...
                                    GUNParticle part1 = new GUNParticle(manaf, fsize1, 0.004F * (float) itemRand.nextGaussian(), lt1, 240, world, a + xx, b + yy, c + zz, (float) itemRand.nextGaussian() / 9.0F, (float) itemRand.nextGaussian() / 9.0F + 0.1F, (float) itemRand.nextGaussian() / 9.0F, this.Red, this.Green, this.Blue, true, 0);
        ...
        if (d1 == 1.0) {
            ...
            for (int h = 0; h < 4; h++) {
                TrailParticle trailpart = new TrailParticle(impact, 0.1F, 0.0F, 50, 240, world, x, y, z, (float) itemRand.nextGaussian() / 7.0F, (float) itemRand.nextGaussian() / 7.0F, (float) itemRand.nextGaussian() / 7.0F, this.Red, this.Green, this.Blue, true, 0, false, 0.0F, beam, 0.1F, 240, this.Red, this.Green, this.Blue, 0.15F, 5, 0.3F, 1.0F);
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack) {
                    ...
                    if (Mana.getMana(player) > fmanacost && !cooldown && click) {
                        world.playSound(null, player.posX, player.posY, player.posZ, this.castSound, SoundCategory.PLAYERS, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                        ...
                        if (this.effectType == 2) {
                            ...
                            IWeapon.fireEffect(this, player, world, 64.0, vec0.x + vec2.x, vec0.y + vec2.y, vec0.z + vec2.z, vec0.x, vec0.y, vec0.z, this.effectType, 0.0, 0.0);
                        }
                        ...
                        if (this.projectile) {
                            ...
                            if (this.effectType == 0 || this.effectType == 1) {
                                Vec3d vec0 = player.getPositionEyes(1.0F);
                                IWeapon.fireEffect(this, player, world, 64.0, vec0.x, vec0.y - 0.3, vec0.z, vec.x, vec.y, vec.z, this.effectType, 0.0, 0.0);
                            }

                            AxisAlignedBB aabb = new AxisAlignedBB(vec.x - this.spellRadius, vec.y - this.spellHeight, vec.z - this.spellRadius, vec.x + this.spellRadius, vec.y + this.spellHeight, vec.z + this.spellRadius);
