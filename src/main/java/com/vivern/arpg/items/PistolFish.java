                ...
                if (hand != null) {
                    if (ammo > 0 && this.isReloaded(itemstack)) {
                        if ((!hascooldown || hand != EnumHand.MAIN_HAND) && (!hascooldown2 || hand != EnumHand.OFF_HAND)) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.pistol_fish, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                            ...
                            if (!player.capabilities.isCreativeMode) {
                                if (itemRand.nextFloat() < parameters.getEnchantedF("ammo_consume_chance", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.REUSE, itemstack))) {
                            ...
                            if (itemRand.nextFloat() < parameters.getEnchantedF("special_shoot_chance", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, itemstack))) {
                                ...
                                if (htarget != null) {
                                    ...
                                    List<EntityLivingBase> list = GetMOP.getHostilesInAABBto(world, GetMOP.entityCenterPos(htarget), radius, radius / 2.0, player);
                                    if (list.size() >= 1) {
                                        for (int i = 0; i < 6; i++) {
                                            EntityLivingBase newtarget = list.get(itemRand.nextInt(list.size()));
                                            if (GetMOP.thereIsNoBlockCollidesBetween(world, player.getPositionEyes(1.0F), GetMOP.entityCenterPos(newtarget), null, false)) {
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.pistol_fish_rel, SoundCategory.AMBIENT, 0.7F, 0.95F + itemRand.nextFloat() / 10.0F);
