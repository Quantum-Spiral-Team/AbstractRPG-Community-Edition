                ...
                if ((click || click2) && player.getHeldItemMainhand() == itemstack) {
                    if (ammo <= 0 || !this.isReloaded(itemstack)) {
                        if (this.initiateBulletReload(itemstack, player, new ItemStack(ItemsRegister.BUCKSHOT, 2), maxammo, false)) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.holy_shotgun_rel, SoundCategory.AMBIENT, 0.8F, 1.0F);
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.holy_shotgun, SoundCategory.AMBIENT, 1.3F, 0.9F + itemRand.nextFloat() / 5.0F + (doubleMult > 1 ? 0.0F : 0.2F));
                                ...
                                for (Entity entity : list) {
                                    if (Team.checkIsOpponent(player, entity)) {
                                        ...
                                        if (itemRand.nextFloat() < 0.2F) {
                                            parameters.mixPotion(entity, "broken_armor", PotionEffects.BROKEN_ARMOR, witchery, Weapons.EnumPotionMix.GREATEST, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumMathOperation.NONE, Weapons.EnumMathOperation.PLUS);
                                        }
                                    }
                                }
                            }
    ...
    public void effect(EntityPlayer clientplayer, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        if (playe instanceof EntityPlayer) {
            ...
            for (int i = 0; i < Math.min(a, 31.0); i++) {
                ...
                BulletParticle part = new BulletParticle(world, from, to, 0.08F, (int) MathHelper.clamp(dist / 3.0F, 2.0F, 6.0F), 5, dist, (float) col.x, (float) col.y, (float) col.z, full);
                part.Red2 = 0.75F;
                part.Green2 = 0.73F;
                part.Blue2 = 0.6F;
                if (full) {
