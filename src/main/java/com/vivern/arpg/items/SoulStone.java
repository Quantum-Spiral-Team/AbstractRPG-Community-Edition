...
public class SoulStone extends ItemWeapon {
    ...
    public static void catchSoul(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker, int catchPower) {
        if (getSoul(stack) == 0 && target.getHealth() <= 0.0F && target.deathTime < 2 && target instanceof EntityCreature && !(target instanceof EntityPlayer) && itemRand.nextFloat() < target.getMaxHealth() / 28.0F) {
            ...
            attacker.world.playSound(null, attacker.posX, attacker.posY, attacker.posZ, Sounds.soul_steal, SoundCategory.PLAYERS, 1.3F, 0.9F + itemRand.nextFloat() / 5.0F);
            IWeapon.fireEffect(ItemsRegister.SOUL_STONE, attacker, attacker.world, 32.0, target.posX, target.posY + target.height / 2.0F, target.posZ, attacker.posX, attacker.posY + attacker.height / 2.0F, attacker.posZ, 0.0, 0.0, 0.0);
            if (attacker instanceof EntityPlayer) {
                ...
                if (empty != -1) {
                    ...
                    player.world.spawnEntity(new EntityItem(player.world, player.posX, player.posY, player.posZ, filledStone));
                }
            } else {
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        for (int i = 0; i < 10; i++) {
            GUNParticle bigsmoke = new GUNParticle(largesmoke, 0.2F + (float) itemRand.nextGaussian() / 15.0F, 0.0F, 30, 240, world, x, y, z, (float) itemRand.nextGaussian() / 20.0F, (float) itemRand.nextGaussian() / 25.0F, (float) itemRand.nextGaussian() / 20.0F, 0.8F + itemRand.nextFloat() / 5.0F, 0.8F + itemRand.nextFloat() / 5.0F, 1.0F, true, itemRand.nextInt(360));
