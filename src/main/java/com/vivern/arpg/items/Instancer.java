...
public class Instancer extends ItemWeapon {
                ...
                if (player.getHeldItemMainhand() != itemstack) {
                    ...
                    if (click || click2) {
                        ...
                        if (ready < 15) {
                            if (ready == 0) {
                                sendAnim = true;
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.instancer_ready, SoundCategory.AMBIENT, 0.9F, 1.0F);
                            }

                            NBTHelper.AddNBTint(itemstack, 1, "ready");
                            if (ready == 14) {
                                world.playSound(null, player.posX, player.posY, player.posZ, click ? Sounds.instancer_deploy : Sounds.instancer_carry, SoundCategory.AMBIENT, 0.9F, 1.0F);
                        ...
                        NBTHelper.AddNBTint(itemstack, -1, "ready");
                        if (ready == 15) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.instancer_unready, SoundCategory.AMBIENT, 0.9F, 1.0F);
                        }
                    }
                            ...
                            for (EntityLivingBase base : this.getTracedCreatures(itemstack, player)) {
                                if (base instanceof AbstractMob) {
                                    ...
                                    if (mob.owner == player) {
                                        ...
                                        if (NBTHelper.GetNBTint(itemstack, "leadershipHold") + leadershipNeed <= this.getMaxLeadership(itemstack) && this.canCarry(itemstack, player, mob, leadershipNeed)) {
                                    ...
                                    int leadershipNeed = this.calculateLeadershipForNonAbstractMob(mob);
                                    if (NBTHelper.GetNBTint(itemstack, "leadershipHold") + leadershipNeed <= this.getMaxLeadership(itemstack) && this.canCarry(itemstack, player, mob, leadershipNeed) && mob.getAttackTarget() != player) {
                                        this.doCarry(mob, null, leadershipNeed, player, itemstack);
                                        leadershipCaptured += leadershipNeed;
                                    }
                                }
    ...
    public void doCarry(EntityLiving mob, @Nullable AbstractMob absMob, int leadershipNeed, EntityPlayer player, ItemStack itemstack) {
        ...
        NBTHelper.GiveNBTint(itemstack, 0, "leadershipHold");
        NBTHelper.AddNBTint(itemstack, leadershipNeed, "leadershipHold");
        mob.world.playSound(null, mob.posX, mob.posY, mob.posZ, Sounds.instancer_capture, SoundCategory.AMBIENT, 0.9F, 1.0F);
        if (!player.isCreative()) {
            itemstack.damageItem(1, player);
        }
    }
