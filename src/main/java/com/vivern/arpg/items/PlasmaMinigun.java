...
public class PlasmaMinigun extends ItemWeapon implements IEnergyItem {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack) {
                    ...
                    if (click2) {
                        ...
                        if (deploy <= 0.0F) {
                            if (!hascooldown) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.plasma_minigun_deploy, SoundCategory.AMBIENT, 0.9F, 0.975F + itemRand.nextFloat() / 20.0F);
                        ...
                        if (deploy >= 1.0F) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.plasma_minigun_undeploy, SoundCategory.AMBIENT, 0.9F, 0.975F + itemRand.nextFloat() / 20.0F);
                            player.getCooldownTracker().setCooldown(this, 10);
                            NBTHelper.SetNBTfloat(itemstack, 0.0F, "speed");
                        }
                    ...
                    if (!click) {
                        if (speed >= maxSpeed) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.plasma_minigun_stop, SoundCategory.AMBIENT, 0.65F, 0.975F + itemRand.nextFloat() / 20.0F);
                        }
                        ...
                        if (ammo > 0 && this.isReloaded(itemstack)) {
                            if (speed <= 0.0F) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.plasma_minigun_start, SoundCategory.AMBIENT, 0.65F, 0.975F + itemRand.nextFloat() / 20.0F);
                            } else if (speed >= maxSpeed / 2.0F && player.ticksExisted % 20 == 0) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.plasma_minigun_loop, SoundCategory.AMBIENT, 0.65F, 1.0F - itemRand.nextFloat() / 20.0F);
                            }
                            ...
                            if (!hascooldown && this.getEnergyStored(itemstack) >= RFtoShoot && speed > 10.0F && heat >= 0) {
                                ...
                                world.playSound(null, player.posX, player.posY, player.posZ, fullTurret ? Sounds.plasma_minigun_turret : Sounds.plasma_minigun, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                                ...
                                for (int i = 0; i < turretMult; i++) {
                                    ...
                                    IWeapon.fireEffect(this, player, world, 64.0, shootPoint.x, shootPoint.y, shootPoint.z, vec.x, vec.y, vec.z, c, dustState == null ? -1.0 : Block.getStateId(dustState), collidesWithAny ? turretMult : -turretMult);
                                }
                            }
                        } else if (deploy <= 0.0F && this.initiateBulletReload(itemstack, player, ItemsRegister.PLASMA_MINIGUN_CLIP, maxammo, true)) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.plasma_railgun_rel, SoundCategory.AMBIENT, 0.7F, 0.95F + itemRand.nextFloat() / 10.0F);
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        BulletParticle part = new BulletParticle(world, from, to, 0.1F, (int) MathHelper.clamp(dist / 4.0F, 1.0F, 6.0F), 6, dist, 0.2F, 0.7F, 1.0F, d3 > 0.0);
        part.smokeRed = 0.0F;
