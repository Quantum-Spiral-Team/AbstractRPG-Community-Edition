...
public class SnowstormStaff extends Item {
    ...
    public void onUpdate(ItemStack itemstack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        if (!worldIn.isRemote && entityIn instanceof EntityPlayer) {
            ...
            if (player.getActiveItemStack() == itemstack && mana > 5.0F && click && !player.getCooldownTracker().hasCooldown(itemIn) && itemstack.getItemDamage() < itemIn.getMaxDamage()) {
                if (itemstack.getItemDamage() == 1) {
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.snowstorm, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                }
