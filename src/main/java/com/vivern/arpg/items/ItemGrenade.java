...
import com.vivern.arpg.items.models.GrenadeModel;
...
public class ItemGrenade extends ItemWeapon {
    ...
    public void onUpdate(ItemStack stack, World world, Entity entityIn, int itemSlot, boolean isSelected) {
        if (!world.isRemote) {
            ...
            if (IWeapon.canShoot(stack)) {
                ...
                if (!hasCooldown && (ServerKeyTracker.isKeyPressed(player, ServerKeyTracker.Keys.PRIMARY) && player.getHeldItemMainhand() == stack || ServerKeyTracker.isKeyPressed(player, ServerKeyTracker.Keys.SECONDARY) && player.getHeldItemOffhand() == stack || ServerKeyTracker.isKeyPressed(player, ServerKeyTracker.Keys.GRENADE) && this.isFirst(player, stack))) {
    ...
    public void playThrowSound(World world, EntityPlayer player) {
        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.grenade_throw, SoundCategory.NEUTRAL, 0.8F, 0.85F + itemRand.nextFloat() * 0.3F);
    }
    ...
    public void onProjectileUpdate(EntityGrenade projectile) {
        if (!projectile.world.isRemote && projectile.flyingTime > projectile.grenade.getFirstExplodeDelay() - projectile.impacts * projectile.grenade.getFirstExplodeDelay() / 5 && !projectile.isDead) {
            projectile.grenade.explode(projectile.world, projectile.getThrower(), projectile.posX, projectile.posY, projectile.posZ, null, projectile);
            projectile.world.setEntityState(projectile, (byte) -1);
        }
    }

    public void explode(World world, @Nullable EntityLivingBase player, double x, double y, double z, @Nullable RayTraceResult result, @Nullable EntityGrenade projectile) {
        if (!world.isRemote) {
            ...
            if (!list.isEmpty()) {
                ...
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(player, entity)) {
                        Weapons.dealDamage(new WeaponDamage(stack, player, projectile, true, false, new Vec3d(x, y, z), WeaponDamage.explode), this.getDamage(), player, entity, true, this.getKnockback(), x, y, z);
                        entity.hurtResistantTime = 0;
                    } else {
                        Weapons.dealDamage(new WeaponDamage(stack, player, projectile, true, false, new Vec3d(x, y, z), WeaponDamage.explode), this.getDamage() * 0.7F, player, entity, true, this.getKnockback(), x, y, z);
                    }
                }
            }
            ...
            for (int ss = 0; ss < 7; ss++) {
                GUNParticle bigSmoke = new GUNParticle(LARGE_SMOKE, 0.8F + itemRand.nextFloat() / 2.0F, -0.001F, 40, 60, world, x, y, z, (float) itemRand.nextGaussian() / 9.0F, (float) itemRand.nextGaussian() / 10.0F, (float) itemRand.nextGaussian() / 9.0F, 1.0F, 1.0F, 1.0F, true, itemRand.nextInt(360));
            ...
            for (int ss = 0; ss < 13; ss++) {
                ...
                GUNParticle fire = new GUNParticle(FLAME, 0.4F + itemRand.nextFloat() * 1.6F, -0.003F, lt, 240, world, x, y, z, (float) itemRand.nextGaussian() / 7.0F, (float) itemRand.nextGaussian() / 7.0F, (float) itemRand.nextGaussian() / 7.0F, 1.0F - itemRand.nextFloat() * 0.2F, 1.0F - itemRand.nextFloat() * 0.2F, 0.7F, true, itemRand.nextInt(100) - 50);
            ...
            for (int ss = 0; ss < 25; ss++) {
                world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, x + itemRand.nextGaussian(), y + itemRand.nextGaussian(), z + itemRand.nextGaussian(), itemRand.nextGaussian() / 13.0, itemRand.nextGaussian() / 13.0, itemRand.nextGaussian() / 13.0, new int[0]);
            }
        }
    }
    ...
    public void onProjectileImpact(EntityGrenade projectile, @Nullable RayTraceResult result) {
        ...
        if (result != null && result.entityHit instanceof EntityLivingBase && result.entityHit != projectile.getThrower()) {
            projectile.slowdown(0.5);
        }
    }

    @SideOnly(Side.CLIENT)
    public void renderGrenade(@Nullable EntityGrenade entity, double x, double y, double z, float entityYaw, float partialTicks, @Nullable Render render, boolean forEntity) {
        GlStateManager.pushMatrix();
        if (forEntity) {
            GlStateManager.translate((float) x, (float) y + 0.1F, (float) z);
            GlStateManager.rotate(180.0F, 1.0F, 0.0F, 0.0F);
            GlStateManager.rotate((entity.prevrotationProgr + (entity.rotationProgr - entity.prevrotationProgr) * partialTicks) * 8.2F, (float) entity.randomRotate.x, (float) entity.randomRotate.y, (float) entity.randomRotate.z);
        }
        ...
        public void explode(World world, @Nullable EntityLivingBase player, double x, double y, double z, @Nullable RayTraceResult result, @Nullable EntityGrenade projectile) {
            if (!world.isRemote) {
                ...
                if (!list.isEmpty()) {
                    ...
                    for (Entity entity : list) {
                        if (Team.checkIsOpponent(player, entity)) {
                            Weapons.dealDamage(new WeaponDamage(stack, player, projectile, true, false, new Vec3d(x, y, z), WeaponDamage.frost), this.getDamage(), player, entity, true, this.getKnockback(), x, y, z);
                            entity.hurtResistantTime = 0;
                            PotionEffect lastDebaff = Weapons.mixPotion(entity, PotionEffects.FREEZING, (float) parameters.getInt("potion_time_add"), (float) parameters.getInt("potion_power_add"), Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumMathOperation.PLUS, Weapons.EnumMathOperation.PLUS, parameters.getInt("potion_time_max"), parameters.getInt("potion_power_max"));
                            if (itemRand.nextFloat() < 0.5 && sounds > 0) {
                                Freezing.tryPlaySound(entity, lastDebaff);
                                sounds--;
                            }
                        } else {
                            PotionEffect lastDebaff = Weapons.mixPotion(entity, PotionEffects.FREEZING, (float) parameters.getInt("friendlyfire_potion_time_add"), (float) parameters.getInt("friendlyfire_potion_power_add"), Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumMathOperation.PLUS, Weapons.EnumMathOperation.PLUS, parameters.getInt("friendlyfire_potion_time_max"), parameters.getInt("friendlyfire_potion_power_max"));
                            if (itemRand.nextFloat() < 0.5 && sounds > 0) {
                                Freezing.tryPlaySound(entity, lastDebaff);
                                sounds--;
                            }
                ...
                for (BlockPos pos : GetMOP.getPosesInsideSphere(MathHelper.floor(x), MathHelper.floor(y), MathHelper.floor(z), Math.round((float) damageRadius * 0.652F))) {
                ...
                for (int ss = 0; ss < 20; ss++) {
                    ...
                    GUNParticle bigSmoke = new GUNParticle(LARGE_CLOUD, 0.6F + Math.abs((float) itemRand.nextGaussian()), -0.005F, lt, 240, world, x, y, z, (float) (itemRand.nextGaussian() / 9.0), (float) (itemRand.nextGaussian() / 9.0), (float) (itemRand.nextGaussian() / 9.0), 0.5F + itemRand.nextFloat() / 10.0F, 1.0F, 1.0F, true, itemRand.nextInt(360));
                ...
                for (int ss = 0; ss < 19; ss++) {
                    ...
                    if (!world.getBlockState(pos).isOpaqueCube()) {
                        ...
                        GUNParticle bigSmoke = new GUNParticle(STAR, scl, 0.0F, lt, 240, world, xx, yy, zz, 0.0F, 0.0F, 0.0F, 0.8F + itemRand.nextFloat() * 0.2F, 1.0F, 1.0F, false, itemRand.nextInt(360));
                ...
                for (int i = 0; i < countOfParticles; i++) {
                    ...
                    GUNParticle particle = new GUNParticle(SNOW, 0.35F + (float) itemRand.nextGaussian() / 30.0F, 0.01F, 22 + itemRand.nextInt(10), 180, world, x, y, z, newX, newY, newZ, 0.75F + itemRand.nextFloat() / 10.0F, 0.9F, 1.0F, false, itemRand.nextInt(360), true, 1.0F);
                    world.spawnEntity(particle);
                }

                for (int ssx = 0; ssx < 25; ssx++) {
                    world.spawnParticle(EnumParticleTypes.CLOUD, x + itemRand.nextGaussian(), y + itemRand.nextGaussian(), z + itemRand.nextGaussian(), itemRand.nextGaussian() / 15.0, itemRand.nextGaussian() / 15.0, itemRand.nextGaussian() / 15.0);
                }
            }
        }
    ...
    public static class GasGrenade extends ItemGrenade {
        ...
        @Override
        public void onProjectileUpdate(EntityGrenade projectile) {
            if (projectile.ticksExisted % 12 == 0 && projectile.flyingTime > projectile.grenade.getFirstExplodeDelay() - projectile.impacts * 20 && !projectile.isDead) {
                this.gas(projectile.world, projectile.getThrower(), projectile.posX, projectile.posY, projectile.posZ, projectile);
            }

            if (projectile.world.isRemote && projectile.explodes > 0 && projectile.ticksExisted % 4 == 0) {
                GUNParticle bigSmoke = new GUNParticle(LARGE_CLOUD, 1.5F + (float) itemRand.nextGaussian() / 3.0F, 3.0E-4F * (itemRand.nextFloat() - 0.5F), 65 + (int) Debugger.floats[0], 111, projectile.world, projectile.posX, projectile.posY, projectile.posZ, (float) itemRand.nextGaussian() / 16.0F, (float) itemRand.nextGaussian() / 16.0F, (float) itemRand.nextGaussian() / 16.0F, 0.8F + itemRand.nextFloat() * 0.2F, 0.8F + itemRand.nextFloat() * 0.2F, 0.4F, true, itemRand.nextInt(360));
        ...
        public void gas(World world, @Nullable EntityLivingBase player, double x, double y, double z, EntityGrenade projectile) {
            if (!world.isRemote) {
                ...
                if (!list.isEmpty()) {
                    for (Entity entity : list) {
                        Weapons.mixPotion(entity, PotionEffects.CHLORITE, (float) parameters.getInt("potion_time_add"), (float) parameters.getInt("potion_power_add"), Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumMathOperation.PLUS, Weapons.EnumMathOperation.PLUS, parameters.getInt("potion_time_max"), parameters.getInt("potion_power_max"));
                        if (entity.isBurning()) {
                ...
                for (BlockPos pos : GetMOP.getPosesInsideSphere(MathHelper.floor(x), MathHelper.floor(y), MathHelper.floor(z), MathHelper.floor(damageRadius))) {
                ...
                if (projectile.explodes == 1) {
                    ...
                    for (int ss = 0; ss < 15; ss++) {
                        world.spawnParticle(EnumParticleTypes.CLOUD, x, y, z, itemRand.nextGaussian() / 15.0, itemRand.nextGaussian() / 25.0, itemRand.nextGaussian() / 15.0);
                    }
                }
            }
        }
        ...
        public void explode(World world, @Nullable EntityLivingBase player, double x, double y, double z, @Nullable RayTraceResult result, @Nullable EntityGrenade projectile) {
            ...
            if (!world.isRemote) {
                ...
                if (!list.isEmpty()) {
                    ...
                    for (Entity entity : list) {
                        if (Team.checkIsOpponent(player, entity)) {
                            Weapons.dealDamage(new WeaponDamage(stack, player, projectile, true, false, null, WeaponDamage.explode), this.getDamage(), player, entity, true, this.getKnockback(), x, y, z);
                            entity.hurtResistantTime = 0;
                            entity.setFire(4);
                        } else {
                            Weapons.dealDamage(new WeaponDamage(stack, player, projectile, true, false, null, WeaponDamage.explode), this.getDamage() * 0.7F, player, entity, true, this.getKnockback(), x, y, z);
                            entity.setFire(3);
                        }
                    }
                }

                for (BlockPos pos : GetMOP.getPosesInsideSphere(MathHelper.floor(x), MathHelper.floor(y), MathHelper.floor(z), Math.round((float) damageRadius))) {
                    Weapons.triggerExplodeBlock(world, pos, player, projectile);
                }
                ...
                AxisAlignedBB aabb = new AxisAlignedBB(x - damageRadius, y - damageRadius, z - damageRadius, x + damageRadius, y + damageRadius, z + damageRadius);
                ...
                for (int ss = 0; ss < 7; ss++) {
                    GUNParticle bigSmoke = new GUNParticle(LARGE_SMOKE, 0.8F + itemRand.nextFloat() / 2.0F, -0.001F, 40, 60, world, x, y, z, (float) itemRand.nextGaussian() * gasMult, (float) itemRand.nextGaussian() * gasMult, (float) itemRand.nextGaussian() * gasMult, 1.0F, 1.0F, 1.0F, true, itemRand.nextInt(360));
                    bigSmoke.alphaTickAdding = -0.01F;
                    world.spawnEntity(bigSmoke);
                }
                ...
                for (int ss = 0; ss < 13.0 + damageRadius; ss++) {
                    int lt = 14 + itemRand.nextInt(8);
                    GUNParticle fire = new GUNParticle(FLAME, 0.4F + itemRand.nextFloat() * 1.6F, -0.003F, lt, 240, world, x, y, z, (float) itemRand.nextGaussian() * gasMult, (float) itemRand.nextGaussian() * gasMult, (float) itemRand.nextGaussian() * gasMult, 1.0F - itemRand.nextFloat() * 0.2F, 1.0F - itemRand.nextFloat() * 0.2F, 0.7F, true, itemRand.nextInt(100) - 50);
                    fire.alphaTickAdding = -1.0F / lt;
                    fire.alphaGlowing = true;
                    fire.tracker = ItemRocket.trackerExplode1;
                    world.spawnEntity(fire);
                ...
                for (int ss = 0; ss < 35; ss++) {
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, x + itemRand.nextGaussian(), y + itemRand.nextGaussian(), z + itemRand.nextGaussian(), itemRand.nextGaussian() * gasMult, itemRand.nextGaussian() * gasMult, itemRand.nextGaussian() * gasMult);
                }
            }
        }
    ...
    public static class GravityGrenade extends ItemGrenade {
        ...
        public void onProjectileUpdate(EntityGrenade projectile) {
            if (!projectile.world.isRemote) {
                ...
                projectile.world.spawnParticle(EnumParticleTypes.PORTAL, projectile.posX, projectile.posY, projectile.posZ, 0.0, 0.0, 0.0, new int[0]);
            }
        }
        ...
        public void explode(World world, @Nullable EntityLivingBase player, double x, double y, double z, @Nullable RayTraceResult result, @Nullable EntityGrenade projectile) {
            ...
            if (!world.isRemote) {
                ...
                projectile.explodes++;
                ParticleTracker.TrackerFollowDynamicPoint tracker = new ParticleTracker.TrackerFollowDynamicPoint(projectile, false, 1.0F, 0.0F + Debugger.floats[0], 0.08F + Debugger.floats[1]);
                ...
                GUNParticle part = new GUNParticle(STAR, scl, 0.0F, lt, 240, world, poss.x, poss.y, poss.z, (float) direction.x, (float) direction.y, (float) direction.z, 0.5F + itemRand.nextFloat() * 0.5F, 0.5F + itemRand.nextFloat() * 0.3F, 1.0F, false, itemRand.nextInt(360));
                ...
                if (projectile.ticksExisted % 20 == 0) {
                    ...
                    GUNParticle partx = new GUNParticle(WHIRL, scale, 0.0F, 50, 210, world, x, y, z, 0.0F, 0.0F, 0.0F, 1.0F, 0.8F, 1.0F, true, itemRand.nextInt(360));
                    partx.tracker = tms;
                ...
                if (projectile.ticksExisted % 2 == 0) {
                    AnimatedGParticle anim = new AnimatedGParticle(TileSpellForge.forge_absorption, 1.4F + itemRand.nextFloat() * 0.8F, 0.0F, 28, 240, world, x, y, z, 0.0F, 0.0F, 0.0F, 0.67F, 0.36F, 0.6F + itemRand.nextFloat() * 0.3F, true, itemRand.nextInt(360));
                    anim.framecount = 25;
    ...
    public static class HellGrenade extends ItemGrenade {
        ...
        public void explode(World world, @Nullable EntityLivingBase player, double x, double y, double z, @Nullable RayTraceResult result, @Nullable EntityGrenade projectile) {
            if (!world.isRemote) {
                ...
                for (BlockPos pos : GetMOP.getPosesInsideSphere(MathHelper.floor(x), MathHelper.floor(y), MathHelper.floor(z), (int) damageRadius)) {
                    Weapons.triggerExplodeBlock(world, pos, player, projectile);
                }
                ...
                for (int ss = 0; ss < 10; ss++) {
                    ...
                    GUNParticle fire = new GUNParticle(FLAME, 0.1F + (float) itemRand.nextGaussian() / 6.0F, -0.003F, lt, 240, world, x, y, z, (float) itemRand.nextGaussian() / 7.0F, (float) itemRand.nextGaussian() / 7.0F, (float) itemRand.nextGaussian() / 7.0F, 1.0F - itemRand.nextFloat() * 0.2F, 0.5F - itemRand.nextFloat() * 0.4F, 1.0F, true, itemRand.nextInt(100) - 50);
                    fire.alphaTickAdding = -1.0F / lt;
                    fire.alphaGlowing = true;
                    fire.tracker = ItemRocket.trackerExplode1;
                    world.spawnEntity(fire);
                ...
                for (int ss = 0; ss < 3; ss++) {
                    ...
                    GUNParticle part = new GUNParticle(FIRE_CIRCLE, 0.4F, 0.0F, lt, 240, world, x, y, z, 0.0F, 0.0F, 0.0F, 1.0F, 0.4F + itemRand.nextFloat() * 0.4F, 1.0F, true, itemRand.nextInt(360));
        ...
        public void onProjectileImpact(EntityGrenade projectile, @Nullable RayTraceResult result) {
            if (projectile.bounce(result, 0.666)) {
                ...
                if (!projectile.world.isRemote && projectile.explodes == 0) {
                    this.explode(projectile.world, projectile.getThrower(), projectile.posX, projectile.posY, projectile.posZ, result, projectile);
                    projectile.world.setEntityState(projectile, (byte) -1);
                }
            }
    ...
    public static class MolotovCocktail extends ItemGrenade {
        ...
        public void onProjectileUpdate(EntityGrenade projectile) {
            if (!projectile.world.isRemote) {
                ...
                projectile.world.spawnParticle(EnumParticleTypes.FLAME, projectile.posX, projectile.posY, projectile.posZ, 0.0, 0.0, 0.0, new int[0]);
            }
        }
            ...
            if (!world.isRemote) {
                ...
                if (projectile.explodes == 1) {
                    ...
                    for (int ss = 0; ss < 45; ss++) {
                        world.spawnParticle(EnumParticleTypes.FLAME, x, y, z, itemRand.nextGaussian() / 15.0, itemRand.nextGaussian() / 25.0, itemRand.nextGaussian() / 15.0, new int[0]);
                    }

                    for (int ss = 0; ss < 15; ss++) {
                        world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, x, y, z, itemRand.nextGaussian() / 15.0, itemRand.nextGaussian() / 25.0, itemRand.nextGaussian() / 15.0);
                    }

                    for (int ss = 0; ss < 4; ss++) {
                        GUNParticle part = new GUNParticle(CrystalFanShoot.shards[itemRand.nextInt(3)], 0.08F + itemRand.nextFloat() / 30.0F, 0.08F, 30 + itemRand.nextInt(10), -1, world, x, y, z, (float) itemRand.nextGaussian() / 13.0F, (float) itemRand.nextGaussian() / 13.0F + 0.1F, (float) itemRand.nextGaussian() / 13.0F, 1.0F, 1.0F, 1.0F, false, itemRand.nextInt(360), true, 1.3F);
                        part.dropMode = true;
                        part.rotationPitchYaw = new Vec2f(itemRand.nextInt(360), itemRand.nextInt(360));
                        part.tracker = new ParticleTracker.TrackerGlassShard((float) itemRand.nextGaussian() * 2.0F, (float) itemRand.nextGaussian() * 2.0F, (int) itemRand.nextGaussian() * 2, false);
                        world.spawnEntity(part);
                    }
                }
            }
        ...
        public void explode(World world, @Nullable EntityLivingBase player, double x, double y, double z, @Nullable RayTraceResult result, @Nullable EntityGrenade projectile) {
            if (!world.isRemote) {
                ...
                if (!list.isEmpty()) {
                    ...
                    for (Entity entity : list) {
                        Weapons.dealDamage(new WeaponDamage(stack, player, projectile, true, false, new Vec3d(x, y, z), WeaponDamage.shards), this.getDamage(), player, entity, true, this.getKnockback(), x, y, z);
                        entity.hurtResistantTime = 0;
                        Weapons.mixPotion(entity, PotionEffects.FIERYOIL, (float) parameters.getInt("potion_time"), (float) parameters.getInt("potion_power_add"), Weapons.EnumPotionMix.GREATEST, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumMathOperation.NONE, Weapons.EnumMathOperation.PLUS, 0, parameters.getInt("potion_power_max"));
                    }
                }
                ...
                for (int ss = 0; ss < 20; ss++) {
                    int lt = 30 + itemRand.nextInt(20);
                    GUNParticle bigSmoke = new GUNParticle(OIL_DROP, 0.1F + itemRand.nextFloat() * 0.1F, 0.025F, lt, -1, world, x, y, z, (float) (itemRand.nextGaussian() / 9.0), (float) (itemRand.nextGaussian() / 9.0) + 0.1F, (float) (itemRand.nextGaussian() / 9.0), 0.8F + itemRand.nextFloat() * 0.2F, 1.0F, 1.0F, false, itemRand.nextInt(360), true, 2.0F);
                ...
                for (int ss = 0; ss < 7; ss++) {
                    ...
                    GUNParticle part = new GUNParticle(SLIME_SPLASH, 0.4F, 0.0F, lt, -1, world, x + itemRand.nextGaussian() / 2.0, y, z + itemRand.nextGaussian() / 2.0, (float) itemRand.nextGaussian() / 16.0F, (float) itemRand.nextGaussian() / 16.0F, (float) itemRand.nextGaussian() / 16.0F, 0.7F, 0.5F + itemRand.nextFloat() * 0.15F, 0.1F + itemRand.nextFloat() * 0.2F, true, itemRand.nextInt(360));
                ...
                for (int ss = 0; ss < 4; ss++) {
                    GUNParticle part = new GUNParticle(CrystalFanShoot.shards[itemRand.nextInt(3)], 0.08F + itemRand.nextFloat() / 30.0F, 0.05F, 30 + itemRand.nextInt(10), -1, world, x, y, z, (float) itemRand.nextGaussian() / 13.0F, (float) itemRand.nextGaussian() / 13.0F + 0.1F, (float) itemRand.nextGaussian() / 13.0F, 1.0F, 1.0F, 1.0F, false, itemRand.nextInt(360), true, 1.3F);
                    part.dropMode = true;
                    part.rotationPitchYaw = new Vec2f(itemRand.nextInt(360), itemRand.nextInt(360));
                    part.tracker = new ParticleTracker.TrackerGlassShard((float) itemRand.nextGaussian() * 2.0F, (float) itemRand.nextGaussian() * 2.0F, (int) itemRand.nextGaussian() * 2, false);
                    world.spawnEntity(part);
            ...
            if (!world.isRemote) {
                ...
                for (int i = 0; i < bombs; i++) {
                    ...
                    for (int j = 0; j < 16; j++) {
                        ...
                        Vec3d start = new Vec3d(x + (itemRand.nextDouble() - 0.5) * 0.5, y + (itemRand.nextDouble() - 0.5) * 0.5, z + (itemRand.nextDouble() - 0.5) * 0.5);
                        Vec3d end = start.add((itemRand.nextDouble() - 0.5) * veclength, (itemRand.nextDouble() - 0.5) * veclength, (itemRand.nextDouble() - 0.5) * veclength);
                        RayTraceResult rayTraceResult = GetMOP.fixedRayTraceBlocks(world, player, 0.1, true, start, end, false, true, false);
                        boolean hitEntity = rayTraceResult != null && Team.checkIsOpponent(player, rayTraceResult.entityHit);
    ...
    public static class SnowGrenade extends ItemGrenade {
        ...
        public void explode(World world, @Nullable EntityLivingBase player, double x, double y, double z, @Nullable RayTraceResult result, @Nullable EntityGrenade projectile) {
            if (!world.isRemote) {
                ...
                if (world.getBlockState(new BlockPos(posv.x, posv.y, posv.z)).getCollisionBoundingBox(world, new BlockPos(posv.x, posv.y, posv.z)) != null) {
                ...
                for (int i = 0; i < imax; i++) {
                    Vec3d speedVec = new Vec3d((itemRand.nextFloat() - 0.5) * snowSpeed, (itemRand.nextFloat() - 0.5) * snowSpeed, (itemRand.nextFloat() - 0.5) * snowSpeed);
                    BlockPos poss = new BlockPos(posv.x + speedVec.x, posv.y + speedVec.y, posv.z + speedVec.z);
                    if (world.getBlockState(poss).getCollisionBoundingBox(world, poss) == null) {
                        IBlockState statie = Blocks.SNOW_LAYER.getDefaultState();
                        AdvancedFallingBlock afb = new AdvancedFallingBlock(world, posv.x + speedVec.x, posv.y + speedVec.y, posv.z + speedVec.z, statie);
                ...
                for (BlockPos pos : GetMOP.getPosesInsideSphere(MathHelper.floor(x), MathHelper.floor(y), MathHelper.floor(z), (int) this.getDamageRadius())) {
                    boolean notLiquid = !world.getBlockState(pos).getMaterial().isLiquid();
                    if (notLiquid && world.getBlockState(pos).getBlock().isReplaceable(world, pos)) {
                        IBlockState dn = world.getBlockState(pos.down());
                        if (Blocks.SNOW_LAYER.canPlaceBlockAt(world, pos) && dn.getBlock() != Blocks.SNOW_LAYER && dn.getBlock() != BlocksRegister.LOOSE_SNOW) {
                ...
                for (int ss = 0; ss < 20; ss++) {
                    ...
                    GUNParticle bigSmoke = new GUNParticle(SNOW_FLAKE, 0.1F + itemRand.nextFloat() * 0.05F, 0.01F, lt, -1, world, x, y, z, (float) (itemRand.nextGaussian() / 9.0), (float) (itemRand.nextGaussian() / 9.0), (float) (itemRand.nextGaussian() / 9.0), 0.8F + itemRand.nextFloat() / 10.0F, 1.0F, 1.0F, true, itemRand.nextInt(360));
                    bigSmoke.alphaTickAdding = -1.0F / lt;
                    world.spawnEntity(bigSmoke);
                }
                ...
                for (int ix = 0; ix < countOfParticles; ix++) {
                    ...
                    GUNParticle particle = new GUNParticle(SNOW, 0.35F + (float) itemRand.nextGaussian() / 30.0F, 0.015F, 35 + itemRand.nextInt(20), 180, world, x, y, z, newX, newY, newZ, 0.8F + itemRand.nextFloat() / 10.0F, 0.9F, 1.0F, false, itemRand.nextInt(360), true, 1.0F);
                    world.spawnEntity(particle);
                }

                for (int ss = 0; ss < 25; ss++) {
                    world.spawnParticle(EnumParticleTypes.CLOUD, x, y, z, itemRand.nextGaussian() / 13.0, itemRand.nextGaussian() / 13.0, itemRand.nextGaussian() / 13.0);
                    world.spawnParticle(EnumParticleTypes.SNOWBALL, x + itemRand.nextFloat() - 0.5, y + itemRand.nextFloat() - 0.5, z + itemRand.nextFloat() - 0.5, itemRand.nextGaussian() / 7.0, itemRand.nextGaussian() / 7.0, itemRand.nextGaussian() / 7.0);
                }
            }
        }
    ...
    public static class WatchingGrenade extends ItemGrenade {
        ...
        public static final ParticleTracker.TrackerSmoothShowHide blackPortalTssh = new ParticleTracker.TrackerSmoothShowHide(null, new Vec3d[]{new Vec3d(0.0, 6.0, 0.1), new Vec3d(6.0, 8.0, -0.03), new Vec3d(8.0, 10.0, 0.03), new Vec3d(13.0, 15.0, 0.03), new Vec3d(15.0, 22.0, -0.101)});
        ...
        public void explode(World world, @Nullable EntityLivingBase player, double x, double y, double z, @Nullable RayTraceResult result, @Nullable EntityGrenade projectile) {
            if (!world.isRemote) {
                ...
                for (int i = 0; i < 15; i++) {
                    world.spawnParticle(EnumParticleTypes.PORTAL, x + (itemRand.nextDouble() - 0.5) * 2.0, y + itemRand.nextDouble() - 0.25, z + (itemRand.nextDouble() - 0.5) * 2.0, (itemRand.nextDouble() - 0.5) * 2.0, -itemRand.nextDouble(), (itemRand.nextDouble() - 0.5) * 2.0, new int[0]);
                }

                GUNParticle part = new GUNParticle(SHADOW_ROUND, 0.01F, 0.0F, 22, 0, world, x, y, z, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, false, itemRand.nextInt(360));
                ...
                int lt = 4 + itemRand.nextInt(3);
                GUNParticle partx = new GUNParticle(VOID_EXPLODE, 0.4F, 0.0F, lt, 240, world, x, y, z, 0.0F, 0.0F, 0.0F, 0.5F, 0.8F, 1.0F, true, itemRand.nextInt(360));
                ...
                for (int i = 0; i < 10; i++) {
                    ...
                    GUNParticle particle = new GUNParticle(LIGHTNING, scl, 0.0F, ltx + 4, 240, world, x + posAdd.x, y + posAdd.y, z + posAdd.z, 0.0F, 0.0F, 0.0F, 1.0F, 0.4F + itemRand.nextFloat() * 0.3F, 1.0F, true, (int) (-pitchYaw.x + 270.0));
