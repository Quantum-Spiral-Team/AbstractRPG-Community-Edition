            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (hand != null) {
                    ...
                    if ((!click || hand != EnumHand.MAIN_HAND) && (!click2 || hand != EnumHand.OFF_HAND)) {
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, level_stop_at == -1 ? Sounds.crystal_star : Sounds.crystal_star_powered, SoundCategory.AMBIENT, 0.9F, 0.95F + itemRand.nextFloat() / 5.0F);
                ...
                if (firedelay == 1) {
                    if (level_stop_at == -1) {
                        ...
                        for (int i = 0; i < nn; i++) {
                            ...
                            projectile.shoot(player, player.rotationPitch, player.rotationYaw, 0.0F, parameters.getFloat("velocity"), parameters.getFloat("inaccuracy") + i / 2.0F - parameters.getFloat("inaccuracy_ench") * acc);
                            world.spawnEntity(projectile);
                        }
                    } else {
                        CrystalStarPoweredShoot projectile = new CrystalStarPoweredShoot(world, player, itemstack, power);
                        projectile.shoot(player, player.rotationPitch, player.rotationYaw, 0.0F, parameters.getFloat("velocity"), parameters.getEnchantedF("inaccuracy", acc));
                        world.spawnEntity(projectile);
                    }
                }
            } else {
