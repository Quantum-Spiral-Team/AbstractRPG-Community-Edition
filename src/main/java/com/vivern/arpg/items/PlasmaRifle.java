...
public class PlasmaRifle extends ItemWeapon implements IEnergyItem {
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        float offset = 0.5F;
        int lt = 9;
        GUNParticle part = new GUNParticle(circle3, 0.5F, 0.0F, lt, 240, world, x + a * offset, y + b * offset, z + c * offset, speedX, speedY, speedZ, 0.6F, 0.45F + itemRand.nextFloat() / 10.0F, 1.0F, true, itemRand.nextInt(360));
        ...
        offset = 0.5F;
        lt = 9;
        part = new GUNParticle(circle3, 0.01F, 0.0F, lt, 240, world, x + a * offset, y + b * offset, z + c * offset, speedX, speedY, speedZ, 0.7F, 0.7F + itemRand.nextFloat() / 10.0F, 1.0F, true, itemRand.nextInt(360));
        part.rotationPitchYaw = new Vec2f((float) pitchYaw.x, (float) pitchYaw.y);
        part.alpha = 3.0F;
        part.alphaTickAdding = -part.alpha / lt;
        part.alphaGlowing = true;
        ...
        offset = 2.0F;
        lt = 10;
        part = new GUNParticle(circle3, 0.5F, 0.0F, lt, 240, world, x + a * offset, y + b * offset, z + c * offset, speedX, speedY, speedZ, 0.4F, 0.25F + itemRand.nextFloat() / 10.0F, 1.0F, true, itemRand.nextInt(360));
        part.rotationPitchYaw = new Vec2f((float) pitchYaw.x, (float) pitchYaw.y);
        part.alpha = 1.0F;
        part.alphaTickAdding = -part.alpha / lt;
        part.alphaGlowing = true;
        ...
        for (int i = 0; i < 360; i += 4 + itemRand.nextInt(3)) {
            ...
            if (result != null && result.hitVec != null && result.typeOfHit == Type.BLOCK) {
                ...
                rotated = new Vec3d(rotated.x * 0.5 + look.x * 0.5, rotated.y * 0.5 + look.y * 0.5, rotated.z * 0.5 + look.z * 0.5);
            }

            int ltx = 30 + itemRand.nextInt(30);
            float scl = 0.1F + itemRand.nextFloat() * 0.05F;
            AnimatedGParticle partx = new AnimatedGParticle(sparkle7, scl, 0.04F, ltx, 240, world, pos.x, pos.y, pos.z, (float) rotated.x * offset, (float) rotated.y * offset, (float) rotated.z * offset, 0.6F + itemRand.nextFloat() * 0.1F, 0.5F, 1.0F, true, itemRand.nextInt(360), true, 1.0F);
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack) {
                    ...
                    if (ServerKeyTracker.isKeyPressed(player, ServerKeyTracker.Keys.SECONDARY) && heat >= parameters.getInt("heat_max") && !hascooldown) {
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.plasma_rifle_wave, SoundCategory.AMBIENT, 1.5F, 0.9F + itemRand.nextFloat() / 5.0F);
                        ...
                        for (Entity entity : GetMOP.findEntitiesOnPath(vec3d, vec3d2, world, player, size, size * 0.8F)) {
                            if (Team.checkIsOpponent(player, entity)) {
                                Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, false, vec3d, WeaponDamage.plasma), parameters.getEnchantedF("wave_damage", might), player, entity, true, parameters.getEnchantedF("wave_knockback", impulse));
                        ...
                        IWeapon.fireEffect(this, player, world, 64.0, shootpoint.x, shootpoint.y, shootpoint.z, vec3d1.x, vec3d1.y, vec3d1.z, 0.0, 0.0, 0.0);
                    ...
                    if (click) {
                        ...
                        if (this.getEnergyStored(itemstack) >= RFtoShoot && !hascooldown) {
                            ...
                            world.playSound(null, player.posX, player.posY, player.posZ, powered ? Sounds.plasma_rifle_powered : Sounds.plasma_rifle, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                            ...
                            if (powered) {
                                ...
                                if (heat < max && heat + heatadd >= max) {
                                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.hadron_blaster_sensor, SoundCategory.AMBIENT, 0.8F, 0.9F + itemRand.nextFloat() / 5.0F);
                                }
                            }

                            PlasmaRifleBall projectile = new PlasmaRifleBall(world, player, itemstack);
                            Weapons.shoot(projectile, EnumHand.MAIN_HAND, player, player.rotationPitch, player.rotationYaw, 0.0F, powered ? parameters.getFloat("velocity_powered") : parameters.getFloat("velocity"), parameters.getEnchantedF("inaccuracy", acc), -0.23F, 0.5F, 0.5F);
                            if (powered) {
