...
public class ItemSpellRoll extends ItemWeapon {
                    ...
                    if (casting <= 0) {
                        if (click) {
                            ...
                            if (using >= useMaxTime) {
                                ...
                                world.playSound(null, player.posX, player.posY, player.posZ, sound, SoundCategory.AMBIENT, 1.0F, 1.0F);
                            } else {
                                if (using == 0) {
                                    Weapons.setPlayerAnimationOnServer(player, 32, EnumHand.MAIN_HAND);
                                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.roll_open, SoundCategory.AMBIENT, 0.8F, 0.95F + itemRand.nextFloat() * 0.1F);
                                }
                        ...
                        if (spells != null) {
                            ...
                            for (BlockPos pos : GetMOP.getPosesInsideSphere((int) player.posX, (int) player.posY, (int) player.posZ, spellcastRadius)) {
                                ...
                                if (tileEntity != null && tileEntity instanceof ISpellcastListener && ((ISpellcastListener) tileEntity).onSpellcast(player.posX, player.posY, player.posZ, itemstack, spells, player)) {
                                    burn = true;
                                }
                            }
    ...
    public void spawnParticles(World world, Entity player, float chanceOrange) {
        ...
        Vec3d spawnPoint = GetMOP.pitchYawToVec3D(player.rotationPitch, player.rotationYaw + (itemRand.nextFloat() - 0.5F) * 30.0F).add(player.posX, player.posY + player.getEyeHeight() - 0.2F, player.posZ);

        for (BlockPos pos : GetMOP.getPosesInsideSphere((int) player.posX, (int) player.posY, (int) player.posZ, spellcastRadius)) {
            TileEntity tileEntity = world.getTileEntity(pos);
            if (tileEntity != null && tileEntity instanceof ISpellcastListener && ((ISpellcastListener) tileEntity).canAttractParticles(player)) {
                ParticleTracker.TrackerFollowStaticPoint tfsp = new ParticleTracker.TrackerFollowStaticPoint(new Vec3d(pos.getX() + 0.5, pos.getY() + 0.75, pos.getZ() + 0.5), true, 0.4F, 0.001F, 0.02F);
        ...
        if (chanceOrange > 0.0F) {
            SparkleSubparticle sparkleSubparticle = new SparkleSubparticle(spawnPoint.x + (itemRand.nextFloat() - 0.5) * 0.35, spawnPoint.y + (itemRand.nextFloat() - 0.5) * 0.35, spawnPoint.z + (itemRand.nextFloat() - 0.5) * 0.35, scale, 60, 0.0F, 0.0F, 0.0F, 0.0F);
