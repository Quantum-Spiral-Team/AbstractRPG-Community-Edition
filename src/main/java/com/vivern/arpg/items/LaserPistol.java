...
public class LaserPistol extends ItemWeapon {
            ...
            if (click && hand == EnumHand.MAIN_HAND || click2 && hand == EnumHand.OFF_HAND) {
                if (ammo > 0 && this.isReloaded(itemstack)) {
                    if (!player.getCooldownTracker().hasCooldown(this)) {
                        ...
                        b1 = false;
                        RayTraceResult result = GetMOP.fixedRayTraceBlocks(world, player, edist, size, true, false, true, false, player.rotationPitch, player.rotationYaw + (hand == EnumHand.MAIN_HAND ? 1 : -1));
                        ...
                        if (world.isRemote) {
                            if (result.sideHit != null) {
                                ...
                                GUNParticle bigsmoke = new GUNParticle(this.res, 0.2F + itemRand.nextFloat() * 0.1F, 0.0F, 45, 240, world, vec.x + displaceX, vec.y + displaceY, vec.z + displaceZ, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, itemRand.nextInt(360));
                            ...
                            EntityStreamLaserP laser = new EntityStreamLaserP(world, player, this.texture, 0.07F, 240, 0.5F, 1.0F, 0.5F, 0.5F, player.getDistance(vec.x, vec.y, vec.z), 1, 0.0F, 1.0F);
                            laser.setPosition(player.posX, player.posY + 1.55, player.posZ);
                            laser.horizOffset = horizoffset;
                            world.spawnEntity(laser);
                        } else {
                            if (player.ticksExisted % 2 == 0) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.laserpistol, SoundCategory.AMBIENT, 0.7F, 0.95F + itemRand.nextFloat() / 10.0F);
                            }
                            ...
                            AxisAlignedBB aabb = new AxisAlignedBB(vec.x - siz, vec.y - siz, vec.z - siz, vec.x + siz, vec.y + siz, vec.z + siz);
                            ...
                            if (!list.isEmpty()) {
                                for (Entity entitylivingbase : list) {
                                    if (Team.checkIsOpponent(player, entitylivingbase)) {
                                        Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, false, player.getPositionEyes(1.0F), WeaponDamage.laser), wdamage * rapidMult, player, entitylivingbase, true, wknockback * rapidMult);
                ...
                } else if (this.initiateMetadataReload(itemstack, player, new ItemStack(ItemsRegister.ION_BATTERY, 1, 1), this.getMaxAmmo(itemstack), new ItemStack(ItemsRegister.ION_BATTERY, 1, 0))) {
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.lasergun_rel, SoundCategory.NEUTRAL, 0.7F, 0.95F + itemRand.nextFloat() / 10.0F);
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        GUNParticle bigsmoke = new GUNParticle(this.res, 0.8F + (float) itemRand.nextGaussian() / 13.0F, 0.0F, 15, 240, world, x, y, z, (float) itemRand.nextGaussian() / 20.0F, (float) itemRand.nextGaussian() / 25.0F, (float) itemRand.nextGaussian() / 20.0F, 0.6F + (float) itemRand.nextGaussian() / 10.0F, 0.7F, 1.0F, true, itemRand.nextInt(360));
        ...
        EntityStreamLaserP laser = new EntityStreamLaserP(world, this.texture, 0.07F, 240, 0.5F, 1.0F, 0.5F, 0.5F, dist, 1, 0.0F, 1.0F, (float) a, (float) b, player.ticksExisted);
            ...
            if (player.getActiveItemStack() == itemstack && click && !player.getCooldownTracker().hasCooldown(itemIn)) {
                ...
                if (!worldIn.isRemote) {
                    if (damage <= itemIn.getMaxDamage() - 1) {
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.laserpistol, SoundCategory.AMBIENT, 0.7F, 1.0F);
                        player.addStat(StatList.getObjectUseStats(this));
                        AxisAlignedBB aabb = new AxisAlignedBB(vec.x - 0.1, vec.y - 0.1, vec.z - 0.1, vec.x + 0.1, vec.y + 0.1, vec.z + 0.1);
                    ...
                    if (itemstack.getItemDamage() > itemIn.getMaxDamage() - 1 && player.inventory.hasItemStack(new ItemStack(ItemsRegister.ION_BATTERY, 1))) {
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.laserpistol_rel, SoundCategory.NEUTRAL, 0.7F, 0.6F / (itemRand.nextFloat() * 0.4F + 0.8F));
                    }
                }

                if (worldIn.isRemote && damage <= itemIn.getMaxDamage() - 1) {
                    Entity spelll = new GUNParticle(this.res, 0.3F, -0.05F, 7, 240, world, vec.x, vec.y, vec.z, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, 0);
    ...
    public float getAdditionalDurabilityBar(ItemStack stack) {
        return MathHelper.clamp((float) NBTHelper.GetNBTint(stack, "ammo") / this.getMaxAmmo(stack), 0.0F, 1.0F);
    }
