...
public class AdamantiumMinigun extends ItemWeapon {
    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot equipmentSlot, ItemStack itemstack) {
        ...
        if (equipmentSlot == EntityEquipmentSlot.MAINHAND) {
            ...
            multimap.put(SharedMonsterAttributes.MOVEMENT_SPEED.getName(), new AttributeModifier(GUN_MODIFIER, "Speed modifier", -0.25 * (speed / 60.0) - 0.02, 2));
        }
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (heat >= 0) {
                    if (isInHand && reuse > 0 && ServerKeyTracker.isKeyPressed(player, ServerKeyTracker.Keys.SECONDARY) && player.isSneaking() && !hascooldown) {
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.item_misc_d, SoundCategory.AMBIENT, 0.9F, coolingMode ? 0.8F : 1.0F + itemRand.nextFloat() / 5.0F);
                    }
                    ...
                    if (click && isInHand) {
                        if (ammo > 0 && this.isReloaded(itemstack)) {
                            ...
                            if (!hascooldown) {
                                ...
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.adamantium_minigun, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                                ...
                                if (heat < parameters.getInt("heat_to_melee")) {
                                    ...
                                    world.playSound(null, player.posX, player.posY, player.posZ, SoundEvents.BLOCK_FIRE_EXTINGUISH, SoundCategory.AMBIENT, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                                }
                                ...
                                if (world.collidesWithAnyBlock(aabb)) {
                                    ...
                                    world.playSound(null, vec.x, vec.y, vec.z, Sounds.bullet_a, SoundCategory.AMBIENT, 0.9F, 0.85F + itemRand.nextFloat() / 5.0F);
                                    playedSound = true;
                                }
                                ...
                                if (!list.isEmpty()) {
                                    for (Entity entity : list) {
                                        if (Team.checkIsOpponent(player, entity)) {
                                            if (!playedSound) {
                                                world.playSound(null, vec.x, vec.y, vec.z, Sounds.bullet_a, SoundCategory.AMBIENT, 0.9F, 0.95F + itemRand.nextFloat() / 5.0F);
                                                playedSound = true;
                                            }

                                            Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, true, player, WeaponDamage.bullet), parameters.getEnchantedF("damage_ranged", might) + damageadd * parameters.getFloat("bullet_damage"), player, entity, true, parameters.getEnchantedF("knockback_ranged", impulse) + knockbackadd * parameters.getFloat("bullet_knockback"), player.posX, player.posY, player.posZ);
                                ...
                                IWeapon.fireEffect(this, player, world, 64.0, shootPoint.x, shootPoint.y, shootPoint.z, vec.x, vec.y, vec.z, c, dustState == null ? -1.0 : Block.getStateId(dustState), collidesWithAny ? 1.0 : 0.0);
                            }
                        } else if (this.initiateBulletReload(itemstack, player, ItemsRegister.ADAMANTIUM_MINIGUN_CLIP, maxammo, true)) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.adamantium_minigun_rel, SoundCategory.AMBIENT, 0.8F, 1.0F);
                    ...
                    if (heat < 0 && isInHand) {
                        ...
                        if (delay <= 0 && ServerKeyTracker.isKeyPressed(player, ServerKeyTracker.Keys.SECONDARY) && !hascooldown) {
                            ...
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.swosh_d, SoundCategory.PLAYERS, 0.6F, 0.8F + itemRand.nextFloat() / 5.0F);
                        }
                        ...
                        if (delay == 1) {
                            ...
                            if (IWeapon.doMeleeSwordAttack(this, itemstack, player, EnumHand.MAIN_HAND, heat < -100).success) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.melee_axe, SoundCategory.PLAYERS, 0.9F, 0.9F + itemRand.nextFloat() / 5.0F);
                                NBTHelper.SetNBTint(itemstack, Math.min(heat + (20 - reuse * 6), 0), "heat");
                            } else {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.swosh_b, SoundCategory.PLAYERS, 0.6F, 0.5F + itemRand.nextFloat() / 5.0F);
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        BulletParticle part = new BulletParticle(world, from, to, 0.15F, (int) MathHelper.clamp(dist / 3.5, 1.0, 8.0), 5, dist, (float) col.x, (float) col.y, (float) col.z, d3 > 0.0);
        part.smokeRed = 0.9F;
    ...
    public int getCooldownTime(ItemStack itemstack) {
        ...
        if (NBTHelper.GetNBTint(itemstack, "heat") < 0) {
            ...
            int speed = NBTHelper.GetNBTint(itemstack, "speed");
            return Math.max(parameters.getEnchantedI("max_shoot_delay", rapidity) + (int) (speed * parameters.getEnchantedF("speed_to_cooldown", rapidity)), parameters.getEnchantedI("min_shoot_delay", rapidity));
        }
    }
