            ...
            if (hand != null && mana > manacost && click) {
                if (!world.isRemote) {
                    ...
                    if (EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, itemstack) > 0 && crystal < parameters.getInt("max_crystal_charge")) {
                        ...
                        if (crystal < 64 && crystal + rapidOne >= 64) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.rrrrar, SoundCategory.AMBIENT, 0.7F, 0.9F + itemRand.nextFloat() / 5.0F);
                        }
                    }

                    Weapons.setPlayerAnimationOnServer(player, 22, hand);
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.sand_scepter, SoundCategory.AMBIENT, 0.7F, 1.3F);
                ...
                if (world.isRemote) {
                    for (int i = 0; i < 3; i++) {
                        ...
                        GUNParticle sandp = new GUNParticle(sandptex, scale, 0.01F, 30 + itemRand.nextInt(15), -1, world, player.posX, player.posY + player.getEyeHeight() - 0.1F, player.posZ, (float) itemRand.nextGaussian() / 30.0F, (float) itemRand.nextGaussian() / 30.0F, (float) itemRand.nextGaussian() / 30.0F, 1.0F, 1.0F, 0.9F + (float) itemRand.nextGaussian() / 10.0F, false, itemRand.nextInt(360), true, 1.1F);
                ...
                for (int i = 0; i < traceCount; i++) {
                    ...
                    Vec3d end = GetMOP.rotatedPosRayTrace(edist, 1.0F, player, 0.1, 0.1, player.rotationPitch + (itemRand.nextFloat() - 0.5F) * spread, player.rotationYaw + (itemRand.nextFloat() - 0.5F) * spread);
                    AxisAlignedBB aabbox = new AxisAlignedBB(end.x - 0.7, end.y - 0.7, end.z - 0.7, end.x + 0.7, end.y + 0.7, end.z + 0.7);
                    List<EntityLivingBase> list = world.getEntitiesWithinAABB(EntityLivingBase.class, aabbox);
                    if (!list.isEmpty()) {
                        for (EntityLivingBase entitylivingbase : list) {
                            if (entitylivingbase != player) {
                                SuperKnockback.applyKnockback(entitylivingbase, parameters.getEnchantedF("knockback_trace", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, itemstack)), player.posX, player.posY, player.posZ);
                                if (world.isRemote) {
                                    ...
                                    GUNParticle sandp = new GUNParticle(sandptex, scale, 0.01F, 30 + itemRand.nextInt(15), -1, world, rX, rY, rZ, (float) itemRand.nextGaussian() / 30.0F, (float) itemRand.nextGaussian() / 30.0F, (float) itemRand.nextGaussian() / 30.0F, 1.0F, 1.0F, 0.9F + (float) itemRand.nextGaussian() / 10.0F, false, itemRand.nextInt(360), true, 1.1F);
                                    world.spawnEntity(sandp);
