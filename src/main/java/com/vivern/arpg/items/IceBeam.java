...
public class IceBeam extends ItemWeapon {
    ...
    public void onUpdate(ItemStack itemstack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        ...
        if (IWeapon.canShoot(itemstack)) {
            ...
            if (player.getHeldItemMainhand() == itemstack && mana > manacost && click && !player.getCooldownTracker().hasCooldown(itemIn)) {
                ...
                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.frost_shoot, SoundCategory.AMBIENT, 0.5F, 0.9F + itemRand.nextFloat() / 5.0F);
                b1 = false;
                    ...
                    if (!list.isEmpty()) {
                        for (Entity entity : list) {
                            if (Team.checkIsOpponent(player, entity)) {
                                Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, false, player.getPositionEyes(1.0F), WeaponDamage.frost), parameters.getEnchantedF("damage", might) * rapidMult * power, player, entity, true, parameters.getEnchantedF("knockback", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, itemstack)) * rapidMult, player.posX, player.posY, player.posZ);
                                ...
                                if (entity instanceof EntityLivingBase) {
                                    ...
                                    PotionEffect lastdebaff = Weapons.mixPotion(entitylivingbase, PotionEffects.FREEZING, parameters.getEnchantedI("potion_time_add", witchery) * rapidMult, (float) ((int) (dur * parameters.getFloat("potion_power_depends_on_time") + 1.0F)), Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumMathOperation.PLUS, Weapons.EnumMathOperation.PLUS, parameters.getEnchantedI("potion_time_max", witchery), parameters.getEnchantedI("potion_power_max", witchery));
                        ...
                        if (result != null && result.typeOfHit == Type.BLOCK) {
                            ...
                            } else if (IceSpikes.canPlaceAt(worldIn, posoff, result.sideHit) && (blockoff.isReplaceable(world, posoff) || blockoff == Blocks.AIR)) {
                ...
                if (world.isRemote) {
                    GUNParticle bigsmoke = new GUNParticle(this.largesmoke, 0.8F + (float) itemRand.nextGaussian() / 13.0F, 0.0F, 15, 240, world, vec.x, vec.y, vec.z, (float) itemRand.nextGaussian() / 20.0F, (float) itemRand.nextGaussian() / 25.0F, (float) itemRand.nextGaussian() / 20.0F, 0.6F + (float) itemRand.nextGaussian() / 10.0F, 0.7F, 1.0F, true, itemRand.nextInt(360));
                    bigsmoke.alphaTickAdding = -0.05F;
                    bigsmoke.alphaGlowing = true;
                    world.spawnEntity(bigsmoke);
                    GUNParticle bigsmoke2 = new GUNParticle(this.snow, 0.15F + (float) itemRand.nextGaussian() / 20.0F, 0.01F, 10 + itemRand.nextInt(20), 180, world, vec.x, vec.y, vec.z, (float) itemRand.nextGaussian() / 20.0F, (float) itemRand.nextGaussian() / 20.0F, (float) itemRand.nextGaussian() / 20.0F, 1.0F, 1.0F, 1.0F, false, itemRand.nextInt(360), true, 1.3F);
                    world.spawnEntity(bigsmoke2);
                    EntityStreamLaserP laser = new EntityStreamLaserP(world, player, this.start, 0.21F, 240, 1.0F, 0.9F, 0.9F, 0.5F, player.getDistance(vec.x, vec.y, vec.z), 1, 0.3F, 8.0F);
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        GUNParticle bigsmoke = new GUNParticle(this.largesmoke, 0.8F + (float) itemRand.nextGaussian() / 13.0F, 0.0F, 15, 240, world, x, y, z, (float) itemRand.nextGaussian() / 20.0F, (float) itemRand.nextGaussian() / 25.0F, (float) itemRand.nextGaussian() / 20.0F, 0.6F + (float) itemRand.nextGaussian() / 10.0F, 0.7F, 1.0F, true, itemRand.nextInt(360));
        bigsmoke.alphaTickAdding = -0.05F;
        bigsmoke.alphaGlowing = true;
        world.spawnEntity(bigsmoke);
        GUNParticle bigsmoke2 = new GUNParticle(this.snow, 0.15F + (float) itemRand.nextGaussian() / 20.0F, 0.01F, 10 + itemRand.nextInt(20), 180, world, x, y, z, (float) itemRand.nextGaussian() / 20.0F, (float) itemRand.nextGaussian() / 20.0F, (float) itemRand.nextGaussian() / 20.0F, 1.0F, 1.0F, 1.0F, false, itemRand.nextInt(360), true, 1.3F);
        world.spawnEntity(bigsmoke2);
        double dd0 = d1 - x;
        double dd1 = d2 - y;
        double dd2 = d3 - z;
        double dist = MathHelper.sqrt(dd0 * dd0 + dd1 * dd1 + dd2 * dd2);
        EntityStreamLaserP laser = new EntityStreamLaserP(world, this.start, 0.21F, 240, 1.0F, 0.9F, 0.9F, 0.5F, dist, 1, 0.3F, 8.0F, (float) a, (float) b, player.ticksExisted);
