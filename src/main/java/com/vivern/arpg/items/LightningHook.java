...
public class LightningHook extends GraplingHook {
    ...
    public void onThrow(EntityPlayer player, ItemStack itemstack) {
        player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.lightning_gh_throw, SoundCategory.PLAYERS, 0.7F, 0.9F + itemRand.nextFloat() / 5.0F);
    }
    ...
    public void onGrap(EntityPlayer player, ItemStack itemstack) {
        ...
        GUNParticle bigsmoke = new GUNParticle(this.largesmoke, 0.4F + (float) itemRand.nextGaussian() / 20.0F, 0.0F, 10, 240, player.world, xpos, ypos, zpos, 0.0F, 0.0F, 0.0F, 0.95F + (float) itemRand.nextGaussian() / 10.0F, 1.0F, 1.0F, true, itemRand.nextInt(360));
        bigsmoke.alphaTickAdding = -0.1F;
        bigsmoke.alphaGlowing = true;
        player.world.spawnEntity(bigsmoke);
        player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.lightning_gh_grap, SoundCategory.PLAYERS, 0.5F, 0.9F + itemRand.nextFloat() / 5.0F);
    }
    ...
    public void spawnParticle(Vec3d pos1, Vec3d pos2, World world, EntityPlayer player) {
        if (world.isRemote && pos1.lengthSquared() > 1.0E-6 && pos2.lengthSquared() > 1.0E-6) {
            GraplingHookParticle laser = new GraplingHookParticle(world, this.texture, this.texture2, 0.04F, 240, 1.0F, 1.0F, 1.0F, 0.0F, pos1.distanceTo(pos2), 1, 0.1F, 0.5F, player.ticksExisted, 0.16F, 0.16F, pos1, pos2);
