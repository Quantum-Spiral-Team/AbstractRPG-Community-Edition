...
import com.vivern.arpg.items.models.CalibrationCrystalModel;
import com.vivern.arpg.items.models.MagicCandleModel;
...
public class ItemCalibrationThing extends Item {
    ...
    public ItemCalibrationThing(String name, float attraction, float range, float speed, boolean canAcceptPassword, float hitboxPixelRadius, float hitboxPixelHeight) {
        ...
        this.aabb = new AxisAlignedBB(-hitboxPixelRadius * 0.0625, 0.0, -hitboxPixelRadius * 0.0625, hitboxPixelRadius * 0.0625, hitboxPixelHeight * 0.0625, hitboxPixelRadius * 0.0625);
    }
    ...
    public void randomDisplayTick(TileCalibrationBundle tile, TileCalibrationBundle.CalibrationThing thing, IBlockState stateIn, World world, BlockPos blockpos, Random rand) {
        if (this.randomDisplayType == 1) {
            ...
            if (thing.password != null && !thing.password.isEmpty()) {
                ...
                if (col != null) {
                    Vec3d pos = new Vec3d(blockpos.getX() + thing.posX / 16.0F, blockpos.getY() + candleHeight, blockpos.getZ() + thing.posZ / 16.0F);
    ...
    public static void spawnCandleLightParticle(World world, Vec3d pos, @Nullable Vec3d col, boolean voidd) {
        if (col == null) {
            GUNParticle particle = new GUNParticle(candle_light, 0.125F, 0.0F, 30, 240, world, pos.x, pos.y, pos.z, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, 0);
            particle.alphaGlowing = true;
            world.spawnEntity(particle);
        } else {
            GUNParticle particle = new GUNParticle(voidd ? candle_light_void : candle_light_uncolored, 0.125F, 0.0F, 30, 240, world, pos.x, pos.y, pos.z, 0.0F, 0.0F, 0.0F, (float) col.x, (float) col.y, (float) col.z, true, 0);
            ...
            if (raytraceresult.typeOfHit == Type.BLOCK) {
                ...
                if (!worldIn.isRemote) {
                    ...
                    if (BlocksRegister.BLOCK_CALIBRATION_BUNDLE.canPlaceBlockAt(worldIn, posoff)) {
                        ...
                        worldIn.playSound(playerIn, posoff, soundtype.getPlaceSound(), SoundCategory.BLOCKS, (soundtype.getVolume() + 1.0F) / 2.0F, soundtype.getPitch() * 0.8F);
                        tentity = worldIn.getTileEntity(posoff);
                        if (tentity != null && tentity instanceof TileCalibrationBundle) {
                            TileCalibrationBundle tileVial = (TileCalibrationBundle) tentity;
                            int posx = (int) ((raytraceresult.hitVec.x + 0.03125 - blockpos.getX()) * 16.0);
