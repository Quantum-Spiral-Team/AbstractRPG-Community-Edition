            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack) {
                    if (charge >= (spec ? 16 : 8) && !powerOn) {
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.voltrident_boost, SoundCategory.AMBIENT, 0.9F, 0.95F + itemRand.nextFloat() / 5.0F);
                    ...
                    if (Mana.getMana(player) > 0.7F - sor / 7.0F && click && !player.getCooldownTracker().hasCooldown(itemIn)) {
                        if (powerOn) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.swosh_a, SoundCategory.AMBIENT, 0.7F, 0.9F + itemRand.nextFloat() / 5.0F);
                            ...
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.crystal_fan, SoundCategory.AMBIENT, 0.7F, 0.95F + itemRand.nextFloat() / 5.0F);
                            player.getCooldownTracker().setCooldown(this, this.getCooldownTime(itemstack));
                            player.addStat(StatList.getObjectUseStats(this));
                            this.bom(false);
