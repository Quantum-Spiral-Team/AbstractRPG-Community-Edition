...
public class SkullCrasher extends ItemWeapon {
                ...
                if (player.getHeldItemMainhand() == itemstack) {
                    ...
                    if (delay == 1) {
                        if (IWeapon.doMeleeHammerAttack(this, itemstack, player, EnumHand.MAIN_HAND, !player.onGround && player.motionY < -0.02, 90, 8).success) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.melee_axe, SoundCategory.PLAYERS, 0.7F, 0.6F + itemRand.nextFloat() / 5.0F);
                            ...
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.melee_miss_axe, SoundCategory.PLAYERS, 0.6F, 0.6F + itemRand.nextFloat() / 5.0F);
                        }
    ...
    public boolean attackEntityMelee(Entity entity, ItemStack stack, EntityPlayer player, EnumHand hand, boolean isCritical) {
        ...
        if (entity instanceof EntityLivingBase && isCritical) {
            ...
            if (entitylivingbase.width < 3.0F && entitylivingbase.getEntityAttribute(SharedMonsterAttributes.KNOCKBACK_RESISTANCE).getAttributeValue() < Math.min(impulse / 5.0F + itemRand.nextFloat(), 1.0F) * itemRand.nextFloat()) {
