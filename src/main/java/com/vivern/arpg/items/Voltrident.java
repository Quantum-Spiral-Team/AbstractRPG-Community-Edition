        ...
        if (IWeapon.canShoot(itemstack)) {
            ...
            if (player.getHeldItemMainhand() == itemstack) {
                ...
                if (mana > manacost && click && !player.getCooldownTracker().hasCooldown(itemIn)) {
                    ...
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.voltrident, SoundCategory.AMBIENT, 0.7F, 0.9F + itemRand.nextFloat() / 5.0F);
                    if (!powerOn) {
                    ...
                    if (!world.isRemote && !list.isEmpty()) {
                        for (Entity entity : list) {
                            if (Team.checkIsOpponent(player, entity)) {
                                Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, false, player.getPositionEyes(1.0F), WeaponDamage.electric), parameters.getEnchantedF("damage", might) * power, player, entity, true, entity.isEntityAlive() ? parameters.getEnchantedF("knockback", impulse) : 0.0F, player.posX, player.posY, player.posZ);
                                if (entity instanceof EntityLivingBase) {
                                    EntityLivingBase entitylivingbase = (EntityLivingBase) entity;
                                    if (entitylivingbase.deathTime > 0) {
                                        NBTHelper.AddNBTint(itemstack, 6, "voltage");
                                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.plasma_railgun_impact, SoundCategory.AMBIENT, 0.3F, 1.2F + itemRand.nextFloat() / 5.0F);
                                    }

                                    if (entitylivingbase.getHealth() <= 0.0F) {
                                        DeathEffects.applyDeathEffect(entitylivingbase, DeathEffects.DE_ELECTRIC);
                                        if (voltage > voltage_to_overkill && entitylivingbase.deathTime > 14 && (entitylivingbase.hurtTime > 9150 || entitylivingbase.hurtTime < 9130)) {
                    ...
                    if (!world.isRemote && overkill != null) {
                        ...
                        for (int i = 0; i < parameters.getEnchantedI("targets", spec); i++) {
                            AxisAlignedBB aabb2 = new AxisAlignedBB(overkill.posX - damradius, overkill.posY - damradius, overkill.posZ - damradius, overkill.posX + damradius, overkill.posY + damradius, overkill.posZ + damradius);
                            ...
                            if (!listo.isEmpty()) {
                                for (EntityLivingBase entitylivingbasex : listo) {
                                    ...
                                    Vec3d newPos = new Vec3d(entitylivingbasex.posX, entitylivingbasex.posY + entitylivingbasex.height / 2.0F, entitylivingbasex.posZ);
                                    if (GetMOP.thereIsNoBlockCollidesBetween(world, lastPos, newPos, null, false) && !damaget.contains(entitylivingbasex) && Team.checkIsOpponent(entitylivingbasex, player)) {
                                        Weapons.dealDamage(new WeaponDamage(itemstack, player, null, false, false, lastPos, WeaponDamage.electric), parameters.getEnchantedF("damage_overkill", might) * power, player, entitylivingbasex, true);
                                        DeathEffects.applyDeathEffect(entitylivingbasex, DeathEffects.DE_ELECTRIC, 0.3F);
                                        damaget.add(entitylivingbasex);
                                        IWeapon.fireEffect(this, player, world, 64.0, lastPos.x, lastPos.y, lastPos.z, 0.0, 0.0, 10.0, newPos.x, newPos.y, newPos.z);
                                        overkill = entitylivingbasex;
                                        world.playSound(null, entitylivingbasex.posX, entitylivingbasex.posY, entitylivingbasex.posZ, Sounds.lightning_strike_a, SoundCategory.AMBIENT, 2.3F, 0.9F + itemRand.nextFloat() / 5.0F);
                                        break;
                                    }
                                }
                            }
                    ...
                    if (world.isRemote) {
                        for (int p = 0; p < 4; p++) {
                            ...
                            GUNParticle bigsmoke2 = new GUNParticle(this.sparkle, scl, 0.1F, livetm, 240, world, vec.x, vec.y, vec.z, (float) itemRand.nextGaussian() / 10.0F, (float) itemRand.nextGaussian() / 10.0F + 0.06F, (float) itemRand.nextGaussian() / 10.0F, 1.0F - itemRand.nextFloat() / 5.0F, 1.0F - itemRand.nextFloat() / 8.0F, 1.0F, true, itemRand.nextInt(360), true, 1.9F);
                            bigsmoke2.scaleTickAdding = -scl / livetm;
                            bigsmoke2.alphaGlowing = true;
                            world.spawnEntity(bigsmoke2);
                        }

                        EntityStreamLaserP laser = new EntityStreamLaserP(world, player, this.start, 0.08F, 240, 0.8F, 0.9F, 1.0F, 0.5F, player.getDistance(vec.x, vec.y, vec.z), 3, 0.3F, 8.0F);
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        if (c == 10.0) {
            ...
            for (int p = 0; p < 4; p++) {
                int livetm = 15 + itemRand.nextInt(10);
                float scl = 0.08F + (float) itemRand.nextGaussian() / 20.0F;
                GUNParticle bigsmoke2 = new GUNParticle(this.sparkle, scl, 0.1F, livetm, 240, world, x, y, z, (float) itemRand.nextGaussian() / 10.0F, (float) itemRand.nextGaussian() / 10.0F + 0.06F, (float) itemRand.nextGaussian() / 10.0F, 1.0F - itemRand.nextFloat() / 5.0F, 1.0F - itemRand.nextFloat() / 8.0F, 1.0F, true, itemRand.nextInt(360), true, 1.9F);
                bigsmoke2.scaleTickAdding = -scl / livetm;
                bigsmoke2.alphaGlowing = true;
                world.spawnEntity(bigsmoke2);
            }
            ...
            EntityStreamLaserP laser = new EntityStreamLaserP(world, this.start, 0.08F, 240, 0.8F, 0.9F, 1.0F, 0.5F, dist, 3, 0.3F, 8.0F, (float) a, (float) b, player.ticksExisted);
