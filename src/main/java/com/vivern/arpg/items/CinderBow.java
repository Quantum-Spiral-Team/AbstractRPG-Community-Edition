...
public class CinderBow extends AbstractBow {
    ...
    public static EntityInfluence cinderArrowInfluence = new EntityInfluence(true) {
        ...
        public void onImpact(Entity entity, RayTraceResult result) {
            if (entity.world.isRemote && result != null && entity.ticksExisted < 20 && GetMOP.getSpeed(entity) > 2.0) {
                for (int i = 0; i < 8; i++) {
                    entity.world.spawnParticle(EnumParticleTypes.LAVA, entity.posX, entity.posY, entity.posZ, 0.0, 0.0, 0.0, new int[0]);
                }
            }
        }
    ...
    public void customizeArrow(EntityArrow arrow, ItemStack ammo, World world, EntityPlayer player, ItemStack bow, int pulling, float arrowvelocity, boolean isArrowUnlimit) {
        if (pulling >= 55) {
            ...
            int k = GetMOP.floatToIntWithChance(WeaponParameters.getWeaponParameters(this).getEnchantedF("fullcharged_knockback", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, bow)), itemRand);
