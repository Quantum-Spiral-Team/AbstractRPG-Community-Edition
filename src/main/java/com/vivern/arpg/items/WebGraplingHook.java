...
public class WebGraplingHook extends GraplingHook {
    ...
    public void onGrap(EntityPlayer player, ItemStack itemstack) {
        ...
        player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.grapling_hook_soft, SoundCategory.PLAYERS, 0.5F, 0.9F + itemRand.nextFloat() / 5.0F);
    }
    ...
    public void spawnParticle(Vec3d pos1, Vec3d pos2, World world, EntityPlayer player) {
        if (world.isRemote && pos1.lengthSquared() > 1.0E-6 && pos2.lengthSquared() > 1.0E-6) {
            float pike = 0.13F;
            GraplingHookParticle laser = new GraplingHookParticle(world, this.texture, this.texture2, 0.03F, world.getLight(player.getPosition()) * 11, 1.0F, 1.0F, 1.0F, 0.0F, pos1.distanceTo(pos2), 1, 0.0F, 0.5F, player.ticksExisted, 0.17F + pike, 0.1F + pike, pos1, pos2);
