    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot equipmentSlot, ItemStack stack) {
        ...
        if (NBTHelper.GetNBTint(stack, "blocking") > 0) {
            if (equipmentSlot == EntityEquipmentSlot.MAINHAND) {
                multimap.put(SharedMonsterAttributes.MOVEMENT_SPEED.getName(), new AttributeModifier(UUID.fromString("916DD27B-A123-455F-8C7F-6112A1B50A4C"), "Shield speed mh", -0.05, 0));
            }

            if (equipmentSlot == EntityEquipmentSlot.OFFHAND) {
                multimap.put(SharedMonsterAttributes.MOVEMENT_SPEED.getName(), new AttributeModifier(UUID.fromString("134CA27A-B123-501F-4D8F-3782C6B52C0A"), "Shield speed oh", -0.05, 0));
            }
        }
        ...
        if (!world.isRemote) {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack && click || player.getHeldItemOffhand() == itemstack && click2) {
                    ...
                    player.getCooldownTracker().setCooldown(this, (int) ((player.getFoodStats().getFoodLevel() <= foodToStarve ? starvedCooldownMultiplier : 1.0F) * this.getCooldownTime(itemstack)));
                }
            }
        }
    }
                ...
                if (attacker != null || source != DamageSource.CRAMMING && source != DamageSource.DROWN && source != DamageSource.FALL && source != DamageSource.HOT_FLOOR && source != DamageSource.IN_FIRE && source != DamageSource.IN_WALL && source != DamageSource.LAVA && source != DamageSource.MAGIC && source != DamageSource.ON_FIRE && source != DamageSource.OUT_OF_WORLD && source != DamageSource.STARVE && source != DamageSource.WITHER) {
                    if (player.hurtResistantTime > 0) {
                        ...
                        if (attacker != null) {
                            ...
                            if (itemRand.nextFloat() < eatChance && player.isPotionActive(MobEffects.HUNGER)) {
                                ...
                                if (Weapons.dealDamage(wDamageSource, damage, player, attacker, true)) {
                                    player.world.playSound(null, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_GENERIC_EAT, SoundCategory.PLAYERS, 0.7F, 0.9F + itemRand.nextFloat() / 5.0F);
                                    EntityLiveHeart.spawnHearts(player.world, attacker.posX, attacker.posY, attacker.posZ, 2, parameters.getFloat("heart_health"), true, 4.0F, player);
                                }
                            }
                        }
                        ...
                        if (blocking <= 0) {
                            ...
                            player.getCooldownTracker().setCooldown(this, (int) ((player.getFoodStats().getFoodLevel() <= foodToStarve ? starvedCooldownMultiplier : 1.0F) * this.getCooldownTime(stack)));
                        } else {
