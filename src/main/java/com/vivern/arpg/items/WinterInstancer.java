...
public class WinterInstancer extends Instancer {
    ...
    public void spawnPartickles(World world, EntityPlayer player, boolean deploy) {
        if (deploy) {
            ...
            GUNParticle part = new GUNParticle(snow2, scl, 0.0F, lt, 180, world, partpos.x + (itemRand.nextFloat() - 0.5F) / 5.0F, partpos.y + (itemRand.nextFloat() - 0.5F) / 5.0F, partpos.z + (itemRand.nextFloat() - 0.5F) / 5.0F, 0.0F, 0.0F, 0.0F, 0.85F, 0.9F, 1.0F, true, itemRand.nextInt(360), true, 1.0F);
