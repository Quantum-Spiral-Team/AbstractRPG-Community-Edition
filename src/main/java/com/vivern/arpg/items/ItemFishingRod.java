...
public class ItemFishingRod extends Item {
    ...
    public float getFishingLuck(EntityLivingBase player, ItemStack itemstack, World worldIn) {
        ...
        float rluck = bpower + this.luck + (float) player.getEntityAttribute(SharedMonsterAttributes.LUCK).getAttributeValue() + EnchantmentHelper.getEnchantmentLevel(Enchantments.LUCK_OF_THE_SEA, itemstack);
        ...
        if (entityLiving instanceof EntityPlayer) {
            ...
            if (player.getHeldItemMainhand() == itemstack && !player.getCooldownTracker().hasCooldown(itemIn)) {
                ...
                if (NBTHelper.GetNBTboolean(itemstack, "hasHook")) {
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.fishing_throw, SoundCategory.AMBIENT, 0.8F, 1.0F);
                    ...
                    if (!world.isRemote) {
                        ...
                        FishingHook bolt = new FishingHook(world, player, (ItemFishingRod) ItemsRegister.FISHING_ROD, itemstack, allowedLiquids, Fishing.getBait(NBTHelper.GetNBTitemFromString(itemstack, "bait")));
                    ...
                    world.playSound(null, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_BOBBER_RETRIEVE, SoundCategory.AMBIENT, 0.8F, 1.0F);
                    NBTHelper.SetNBTboolean(itemstack, true, "destroy");
                } else {
                    NBTHelper.SetNBTint(itemstack, 3, "use");
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.fishing_capture, SoundCategory.AMBIENT, 0.8F, 1.0F);
                }
            }
        }
    }
