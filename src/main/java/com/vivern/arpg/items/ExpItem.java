                    ...
                    if (!"craftwritetool".equals(itemstack.getDisplayName()) && !nooredigname) {
                        if ("classfinder".equals(itemstack.getDisplayName())) {
                                        ...
                                        for (int j = 0; j < 3; j++) {
                                            for (int ix = 0; ix < 4; ix++) {
                                                ...
                                                if (!chest.getStackInSlot(indx).isEmpty()) {
                                                    array[MathHelper.clamp(chest.getStackInSlot(indx).getMetadata() - 1, 0, 11)] += chest.getStackInSlot(indx).getCount();
                                                }
                                            }
                                        }
                            ...
                            if (player.isSneaking()) {
                                ...
                                if (tile instanceof TileIndustrialMixer) {
                                    try {
                                        ...
                                        text = text + (mixer.getStackInSlot(7).isEmpty() ? "null, " : Debugger.itemGetter(mixer.getStackInSlot(7).getItem()) + ", ");
                                        text = text + (mixer.getStackInSlot(6).isEmpty() ? "null, " : Debugger.itemGetter(mixer.getStackInSlot(6).getItem()) + ", ");
                                        text = text + ", ";
                                        ...
                                        if (mixer.tank1.getFluidAmount() > 0) {
                                            text = text + ".setFluidCost1(\"" + mixer.tank1.getFluid().getUnlocalizedName().replaceFirst("fluid.", "") + "\", " + mixer.tank1.getFluidAmount() + ")";
                                        }

                                        if (mixer.tank2.getFluidAmount() > 0) {
                                            text = text + ".setFluidCost2(\"" + mixer.tank2.getFluid().getUnlocalizedName().replaceFirst("fluid.", "") + "\", " + mixer.tank2.getFluidAmount() + ")";
                                        }

                                        if (mixer.tank3.getFluidAmount() > 0) {
                                            text = text + ".setFluidOutput1(\"" + mixer.tank3.getFluid().getUnlocalizedName().replaceFirst("fluid.", "") + "\", " + mixer.tank3.getFluidAmount() + ")";
                                        }

                                        if (mixer.tank4.getFluidAmount() > 0) {
                                            text = text + ".setFluidOutput2(\"" + mixer.tank4.getFluid().getUnlocalizedName().replaceFirst("fluid.", "") + "\", " + mixer.tank4.getFluidAmount() + ")";
                                        }
                                ...
                                if (tile instanceof TileAlchemicLab) {
                                    try {
                                        ...
                                        if (lab.tank1.getFluidAmount() > 0) {
                                            textx = textx + ".setFluidCost(\"" + lab.tank1.getFluid().getUnlocalizedName().replaceFirst("fluid.", "") + "\", " + lab.tank1.getFluidAmount() + ")";
                                        }

                                        if (lab.tank2.getFluidAmount() > 0) {
                                            textx = textx + ".setFluidOutput(\"" + lab.tank2.getFluid().getUnlocalizedName().replaceFirst("fluid.", "") + "\", " + lab.tank2.getFluidAmount() + ")";
                                        }
                            ...
                            if (tileentity instanceof TileEntityLockableLoot) {
                                ...
                                try {
                                    ...
                                    field.setAccessible(true);
                                    LoadingCache<ResourceLocation, LootTable> registeredLootTables = (LoadingCache<ResourceLocation, LootTable>) field.get(world.getLootTableManager());
                        ...
                        if (tile instanceof TileEntityChest) {
                            ...
                            if (!chest.getStackInSlot(12).isEmpty()) {
                                ItemStack[] stacks = new ItemStack[]{chest.getStackInSlot(0), chest.getStackInSlot(1), chest.getStackInSlot(2), chest.getStackInSlot(9), chest.getStackInSlot(10), chest.getStackInSlot(11), chest.getStackInSlot(18), chest.getStackInSlot(19), chest.getStackInSlot(20)};
                            ...
                            if (!chest.getStackInSlot(17).isEmpty()) {
                                ItemStack[] stacks = new ItemStack[]{chest.getStackInSlot(5), chest.getStackInSlot(6), chest.getStackInSlot(7), chest.getStackInSlot(14), chest.getStackInSlot(15), chest.getStackInSlot(16), chest.getStackInSlot(23), chest.getStackInSlot(24), chest.getStackInSlot(25)};
                    ...
                    if (chunk != null) {
                        for (int yc = -14; yc < -13; yc++) {
                            ...
                            for (byte i = 0; i < 1; i++) {
                                ...
                                if (chunk2 != null) {
                                    Debugger.startPROFILING();
                                    byte redOnOffsetPos = LoadedRGBChunk.getLightForChannel(chunk2.getBakedLight(LoadedRGBChunk.getBakedCoordRed(offsX, offsY, offsZ)), i);
                                    byte greenOnOffsetPos = LoadedRGBChunk.getLightForChannel(chunk2.getBakedLight(LoadedRGBChunk.getBakedCoordGreen(offsX, offsY, offsZ)), i);
                                    byte blueOnOffsetPos = LoadedRGBChunk.getLightForChannel(chunk2.getBakedLight(LoadedRGBChunk.getBakedCoordBlue(offsX, offsY, offsZ)), i);
        ...
        for (int l = 0; l < 15; l++) {
            for (int xc = -14; xc < 15; xc++) {
                ...
                for (int zc = -14; zc < 15; zc++) {
                    ...
                    LoadedRGBChunk chunk = xshift == 0 ? (zshift == 0 ? chunkMid : (zshift == 1 ? chunkPlusZ : chunkMinusZ)) : (xshift == 1 ? (zshift == 0 ? chunkPlusX : (zshift == 1 ? chunkPlusXZ : chunkPluXMiuZ)) : (zshift == 0 ? chunkMinusX : (zshift == 1 ? chunkPluZMiuX : chunkMinusXZ)));
                    if (chunk != null) {
            ...
            for (LightOnPos lightOn : lights) {
                lightOn.chunk.arrXYZ[lightOn.bakedCoord] = lightOn.chunk.arrXYZ[lightOn.bakedCoord] & ~(15L << (int) lightOn.channel) | Math.max(lightOn.value, 0L) << (int) lightOn.channel;
            }
