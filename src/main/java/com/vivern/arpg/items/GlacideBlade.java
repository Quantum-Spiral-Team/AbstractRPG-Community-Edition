...
public class GlacideBlade extends ItemWeapon {
        ...
        if (!world.isRemote) {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (hand != null) {
                    ...
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.glacide_blade, SoundCategory.PLAYERS, 1.0F, 0.9F + itemRand.nextFloat() / 5.0F);
                    ...
                    if (meleeAttackResult.success && EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, itemstack) > 0) {
                        for (int i = 0; i < 10; i++) {
                            BlockPos pos = new BlockPos(vec.x + itemRand.nextGaussian(), vec.y + itemRand.nextGaussian(), vec.z + itemRand.nextGaussian());
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        for (int i = 0; i < countOfParticles; i++) {
            ...
            GUNParticle particle = new GUNParticle(sparkle, 0.15F + (float) itemRand.nextGaussian() / 30.0F, 0.01F, 22 + itemRand.nextInt(10), light, world, x, y, z, X, Y, Z, 1.0F - (float) itemRand.nextGaussian() / 14.0F, 1.0F, 1.0F, false, itemRand.nextInt(360), true, 1.0F);
