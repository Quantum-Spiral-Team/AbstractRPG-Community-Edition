...
public class InfernalBlade extends ItemWeapon {
    ...
    public void explodeEntity(ItemStack itemstack, World world, Entity entityExploded, EntityPlayer player) {
        ...
        AxisAlignedBB axisalignedbb = entityExploded.getEntityBoundingBox().expand(damageRadius * 2.0, damageRadius * 2.0, damageRadius * 2.0).offset(-damageRadius, -damageRadius, -damageRadius);
        ...
        if (!list.isEmpty()) {
            for (Entity entity : list) {
                if (Team.checkIsOpponent(player, entity)) {
                    ...
                    Weapons.dealDamage(new WeaponDamage(itemstack, player, entityExploded, true, false, entityExploded, WeaponDamage.explode), Math.min(parameters.getEnchantedF("damage_explode_per_health", might) * health, parameters.getFloat("damage_explode_max")), player, entity, true, parameters.getEnchantedF("knockback_explode", impulse), entityExploded.posX, entityExploded.posY, entityExploded.posZ);
        ...
        world.playSound(null, entityExploded.posX, entityExploded.posY, entityExploded.posZ, Sounds.explode7, SoundCategory.HOSTILE, 1.0F, 0.9F + itemRand.nextFloat() / 5.0F);
        IWeapon.fireEffect(this, entityExploded, world, 48.0, entityExploded.posX, entityExploded.posY, entityExploded.posZ, entityExploded.width, entityExploded.height, 0.0, 0.0, 0.0, 0.0);
    }

    @SideOnly(Side.CLIENT)
    @Override
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        GUNParticle particle = new GUNParticle(fire_circle2, 0.1F, 0.0F, 5, 240, world, x, y + 0.1, z, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, itemRand.nextInt(360));
        ...
        for (int i = 0; i < 10; i++) {
            ...
            AnimatedGParticle part = new AnimatedGParticle(sparkle6, scl, 0.028F, lt, 240, world, x + (itemRand.nextFloat() - 0.5F) * a, y + itemRand.nextFloat() * b, z + (itemRand.nextFloat() - 0.5F) * a, (float) itemRand.nextGaussian() / 8.0F, (float) itemRand.nextGaussian() / 8.0F + 0.1F, (float) itemRand.nextGaussian() / 8.0F, 1.0F, 0.5F + itemRand.nextFloat() * 0.5F, itemRand.nextFloat() * itemRand.nextFloat(), true, 0, true, 2.5F);
        ...
        for (int i = 0; i < 20; i++) {
            world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, x + (itemRand.nextFloat() - 0.5F) * a, y + itemRand.nextFloat() * b, z + (itemRand.nextFloat() - 0.5F) * a, (itemRand.nextFloat() - 0.5F) * 0.5F, (itemRand.nextFloat() - 0.5F) * 0.5F, (itemRand.nextFloat() - 0.5F) * 0.5F, new int[0]);
        }
        ...
        for (int i = 0; i < 10; i++) {
            ...
            GUNParticle part = new GUNParticle(fireball, scl, 0.05F, lt, 240, world, x, y, z, (float) itemRand.nextGaussian() / 7.0F, (float) itemRand.nextGaussian() / 7.0F + 0.2F, (float) itemRand.nextGaussian() / 7.0F, 1.0F, colF, colF, false, itemRand.nextInt(360), true, 10.0F);
            part.scaleTickAdding = -scl / lt;
            part.alphaGlowing = true;
            part.dropMode = true;
            world.spawnEntity(part);
