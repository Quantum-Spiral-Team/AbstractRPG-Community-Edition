...
public class StaticLance extends ItemWeapon {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack) {
                    ...
                    if (damage < this.getMaxDamage() && blowdelay <= 0 && delay <= 0 && !click && click2 && boomcooldown <= 0 && !hascooldown) {
                        ...
                        double attackspeed = player.getEntityAttribute(SharedMonsterAttributes.ATTACK_SPEED).getAttributeValue();
                        player.getCooldownTracker().setCooldown(this, this.getModifiedMeleeCooldown(attackspeed, this.getCooldownTime(itemstack)));
                        NBTHelper.SetNBTint(itemstack, parameters.getEnchantedI("blow_cooldown", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.RAPIDITY, itemstack)), "boomcooldown");
                    }
                    ...
                    if (blowdelay > 0 && blowdelay <= 4) {
                        ...
                        Vec3d vec3d2 = vec3d.add(vec3d1.x * boomDistance, vec3d1.y * boomDistance, vec3d1.z * boomDistance);
                        ...
                        if (result != null && result.typeOfHit != null && result.typeOfHit == Type.BLOCK && result.hitVec != null) {
                            ...
                            world.playSound(null, vec.x, vec.y, vec.z, Sounds.static_lance, SoundCategory.PLAYERS, 1.2F, 0.9F + itemRand.nextFloat() / 5.0F);
                            ...
                            double power = Math.min(parameters.getFloat("special_max_power"), EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, itemstack) > 0 ? Math.abs(player.motionY) : 0.0);
                            if (!list.isEmpty()) {
                                for (Entity entity : list) {
                                    if (Team.checkIsOpponent(player, entity)) {
                                        Weapons.dealDamage(new WeaponDamage(itemstack, player, null, true, false, vec, WeaponDamage.electric), (float) (player.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue() + parameters.getEnchantedF("blow_damage", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, itemstack)) + power * parameters.getFloat("special_damage")), player, entity, true, parameters.getEnchantedF("blow_knockback", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, itemstack)), vec.x, vec.y - 0.5, vec.z);
                                        entity.hurtResistantTime = 0;
                                        Weapons.setPotionIfEntityLB(entity, MobEffects.LEVITATION, (int) (parameters.getEnchantedF("potion_time", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.WITCHERY, itemstack)) + (float) Math.round(power * parameters.getEnchantedF("special_potion_time", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.WITCHERY, itemstack)))), 0);
                                    }
                                }
                            }
                            ...
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.melee_miss_axe, SoundCategory.PLAYERS, 0.6F, 0.8F + itemRand.nextFloat() / 5.0F);
                        }
                    }
                }
            }
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        GUNParticle bigsmoke1 = new GUNParticle(this.explode, 0.1F, 0.0F, 11, 240, world, x, y + 0.05, z, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, itemRand.nextInt(360));
        ...
        for (int ss = 0; ss < 7; ss++) {
            GUNParticle bigsmoke = new GUNParticle(this.cloud, 1.1F + (float) itemRand.nextGaussian() / 6.0F, 0.0F, 30, 180, world, x, y, z, (float) itemRand.nextGaussian() / 14.0F, (float) itemRand.nextGaussian() / 14.0F, (float) itemRand.nextGaussian() / 14.0F, 0.75F + (float) itemRand.nextGaussian() / 10.0F, 0.45F, 1.0F, true, itemRand.nextInt(360));
        ...
        for (int ss = 0; ss < 10; ss++) {
            GUNParticle bigsmoke = new GUNParticle(this.spell, 0.08F + itemRand.nextFloat() / 15.0F, 0.1F, 70, 240, world, x, y, z, (float) itemRand.nextGaussian() / 6.0F, (float) itemRand.nextGaussian() / 5.0F + 0.1F, (float) itemRand.nextGaussian() / 6.0F, 0.8F + itemRand.nextFloat() / 10.0F, 0.9F, 1.0F, false, itemRand.nextInt(360), true, 1.5F);
    ...
    public float getAdditionalDurabilityBar(ItemStack stack) {
        WeaponParameters parameters = WeaponParameters.getWeaponParameters(this);
        return NBTHelper.GetNBTint(stack, "boomcooldown") / parameters.getEnchantedF("blow_cooldown", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.RAPIDITY, stack));
    }
