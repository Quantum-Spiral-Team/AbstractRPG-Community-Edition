        ...
        if (leadershipNeed < 10) {
            ...
            if (maxH != 0.0F) {
                ...
                if (healthRatio < 1.0F && itemRand.nextFloat() > 0.9 * healthRatio) {
                    player.world.playSound(null, entity.posX, entity.posY, entity.posZ, Sounds.walking_bomb_explode, SoundCategory.AMBIENT, 1.8F, 1.15F + itemRand.nextFloat() / 5.0F);
                    double damageRadius = 2.6;
                    AxisAlignedBB axisalignedbb = entity.getEntityBoundingBox().expand(damageRadius * 2.0, damageRadius * 2.0, damageRadius * 2.0).offset(-damageRadius, -damageRadius, -damageRadius);
                    ...
                    if (!list.isEmpty()) {
                        for (Entity entit : list) {
                            if (Team.checkIsOpponent(player, entit)) {
                                Weapons.dealDamage(new WeaponDamage(stack, player, entity, true, false, entity, WeaponDamage.radiation), leadershipNeed * 2, player, entit, true, 0.6F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, stack) / 2.0F, entity.posX, entity.posY - 1.0, entity.posZ);
                                entit.hurtResistantTime = 0;
                                if (entit instanceof EntityPlayer) {
                                    Mana.addRad((EntityPlayer) entit, 20, true);
                                }
                            } else {
                                SuperKnockback.applyKnockback(entit, 0.4F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, stack) / 4.0F, entity.posX, entity.posY, entity.posZ);
                                if (entit instanceof EntityPlayer) {
    ...
    public void effect(EntityPlayer player, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        if (y == 3.0) {
            ...
            for (int i = 0; i < amount; i++) {
                AnimatedGParticle rad = new AnimatedGParticle(radiation, 0.5F, 0.0F, 16, 240, world, entity.posX + itemRand.nextGaussian() * entity.width, entity.posY + itemRand.nextFloat() * entity.height, entity.posZ + itemRand.nextGaussian() * entity.width, (float) itemRand.nextGaussian() / 8.0F, (float) itemRand.nextGaussian() / 8.0F + 0.1F, (float) itemRand.nextGaussian() / 8.0F, 1.0F, 1.0F, 0.92F + itemRand.nextFloat() * 0.08F, true, itemRand.nextInt(360));
                rad.framecount = 16;
    ...
    public void spawnPartickles(World world, EntityPlayer player, boolean deploy) {
        if (deploy) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                GUNParticle part = new GUNParticle(toxic_spell, scl, 0.0F, lt, 200, world, partpos.x + (itemRand.nextFloat() - 0.5F) / 2.0F, partpos.y + (itemRand.nextFloat() - 0.5F) / 2.0F, partpos.z + (itemRand.nextFloat() - 0.5F) / 2.0F, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, itemRand.nextInt(360), true, 1.0F);
                ...
                if (itemRand.nextFloat() < 0.17F) {
                    ...
                    AnimatedGParticle rad = new AnimatedGParticle(radiation, 0.2F, 0.0F, 16, 240, world, partpos.x + (itemRand.nextFloat() - 0.5F) / 2.0F + part.motionX * len, partpos.y + (itemRand.nextFloat() - 0.5F) / 2.0F + part.motionY * len, partpos.z + (itemRand.nextFloat() - 0.5F) / 2.0F + part.motionZ * len, (float) (part.motionX / 5.0), (float) (part.motionY / 5.0), (float) (part.motionZ / 5.0), 1.0F, 1.0F, 0.92F + itemRand.nextFloat() * 0.08F, true, itemRand.nextInt(360));
                    rad.framecount = 16;
            ...
            for (int ix = 0; ix < 4; ix++) {
                ...
                if (result != null && result.hitVec != null) {
                    ...
                    GUNParticle part = new GUNParticle(toxic_spell, scl, 0.0F, lt, 200, world, result.hitVec.x, result.hitVec.y, result.hitVec.z, (float) itemRand.nextGaussian() / 20.0F, (float) itemRand.nextGaussian() / 20.0F, (float) itemRand.nextGaussian() / 20.0F, 1.0F, 1.0F, 1.0F, true, itemRand.nextInt(360));
                    part.alphaGlowing = true;
                    part.scaleTickAdding = -scl / lt;
                    ...
                    if (itemRand.nextFloat() < 0.2F) {
                        AnimatedGParticle rad = new AnimatedGParticle(radiation, 0.2F, 0.0F, 16, 240, world, result.hitVec.x, result.hitVec.y, result.hitVec.z, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 0.92F + itemRand.nextFloat() * 0.08F, true, itemRand.nextInt(360));
                        rad.framecount = 16;
                        rad.scaleTickAdding = 0.015F + itemRand.nextFloat() * 0.03F;
                        rad.randomDeath = false;
                        rad.alphaGlowing = true;
