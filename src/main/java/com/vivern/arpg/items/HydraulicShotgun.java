                    ...
                    if ((ammo > 0 || currentShot > 0) && this.isReloaded(itemstack)) {
                        if (!hascooldown) {
                            if (currentShot <= 0) {
                                ...
                                NBTHelper.SetNBTint(itemstack, parameters.getInt("burst"), "currentshot");
                                currentShot = 3;
                            }
                            ...
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.hydraulic_shotgun, SoundCategory.AMBIENT, 1.3F, 0.95F + itemRand.nextFloat() / 10.0F);
                            player.getCooldownTracker().setCooldown(this, currentShot == 1 ? this.getCooldownTime(itemstack) : parameters.getInt("cooldown_small"));
                            player.addStat(StatList.getObjectUseStats(this));
                            IWeapon.fireBomEffect(this, player, world, 0);
                            Weapons.setPlayerAnimationOnServer(player, currentShot == 1 ? 3 : 13, EnumHand.MAIN_HAND);
                            if (currentShot == 1) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.hydraulic_shotgun_cool, SoundCategory.AMBIENT, 1.0F, 0.9F + itemRand.nextFloat() / 5.0F);
                            ...
                            for (int i = 0; i < amount; i++) {
                                ...
                                if (world.collidesWithAnyBlock(aabb)) {
                                    if (itemRand.nextFloat() < 0.5F) {
                                        world.playSound(null, vec.x, vec.y, vec.z, Sounds.bullet, SoundCategory.AMBIENT, 0.5F, 0.9F + itemRand.nextFloat() / 5.0F);
                                    }
                            ...
                            IWeapon.fireEffect(this, player, world, 64.0, player.getEntityId(), seed, acc, amount, c, impacts, edist, 0.0, 0.0);
                        }
                    } else if (this.initiateBulletReload(itemstack, player, new ItemStack(ItemsRegister.HYDRAULIC_SHOTGUN_CLIP, 1), maxammo, true)) {
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.hydraulic_shotgun_rel, SoundCategory.AMBIENT, 0.9F, 1.0F);
    ...
    public void effect(EntityPlayer clientplayer, World world, double x, double y, double z, double a, double b, double c, double d1, double d2, double d3) {
        ...
        if (playe instanceof EntityPlayer) {
            ...
            for (int i = 0; i < Math.min(a, 31.0); i++) {
                ...
                BulletParticle part = new BulletParticle(world, from, to, 0.11F, (int) MathHelper.clamp(dist / 2.2, 2.0, 8.0), 4, dist, (float) col.x, (float) col.y, (float) col.z, full);
                part.Red2 = 0.5F;
                part.Green2 = 0.7F;
                part.Blue2 = 0.8F;
