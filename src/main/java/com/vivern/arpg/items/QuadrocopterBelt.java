...
public class QuadrocopterBelt extends ItemArmor implements IBauble, IRenderBauble {
    ...
    public static ArmorMaterial armorMaterial = EnumHelper.addArmorMaterial("arpg:armor", "arpg:armor", 9, new int[]{2, 4, 6, 3}, 7, SoundEvents.ITEM_ARMOR_EQUIP_IRON, 2.0F);
    ...
    public void onWornTick(ItemStack itemstack, EntityLivingBase entityIn) {
        ...
        if (entityIn instanceof EntityPlayer) {
            ...
            if (itemstack.getItemDamage() > 19999 && FindAmmo.find(player.inventory, ItemsRegister.ION_BATTERY) >= 6) {
                ...
                player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.lasergun_rel, SoundCategory.NEUTRAL, 0.7F, 1.0F);
            }
        }
    }
    ...
    public void Effects(World world, EntityPlayer player) {
        Entity bigBoom = new GUNParticle(TEXTURE_PARTICLE, 0.1F + (float) itemRand.nextGaussian() / 20.0F, -0.07F, 15, 240, world, player.posX + (float) itemRand.nextGaussian() / 2.0F, player.posY, player.posZ + (float) itemRand.nextGaussian() / 2.0F, (float) itemRand.nextGaussian() / 22.0F, (float) itemRand.nextGaussian() / 22.0F - 0.38F, (float) itemRand.nextGaussian() / 22.0F, 1.0F, 1.0F, 1.0F, false, (int) Math.round(itemRand.nextGaussian() * 20.0));
        ...
        if (player.ticksExisted % 4 == 0) {
            ...
            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.quadromotor, SoundCategory.NEUTRAL, 0.7F, 0.8F + sss);
        }
    }
        ...
        if (entityIn instanceof EntityPlayer) {
            ...
            if (player.inventory.armorInventory.get(1) == itemstack) {
                ...
                if (itemstack.getItemDamage() > 19999 && FindAmmo.find(player.inventory, ItemsRegister.ION_BATTERY) >= 6) {
                    ...
                    itemstack.setItemDamage(0);
                    player.getCooldownTracker().setCooldown(this, 45);
                    worldIn.playSound(null, player.posX, player.posY, player.posZ, Sounds.lasergun_rel, SoundCategory.NEUTRAL, 0.7F, 1.0F);
                }
            }
        }
    }
