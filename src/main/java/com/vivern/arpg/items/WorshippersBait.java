            ...
            for (int[] offset : krakenOffsets) {
                ...
                if (result != null) {
                    for (int y = result.pos.getY() + 8; y < entityItem.posY - 16.0; y++) {
                        AxisAlignedBB aabb = new AxisAlignedBB(xzxpos.getX() - 4, y, xzxpos.getZ() - 4, xzxpos.getX() + 4, y + 8, xzxpos.getZ() + 4);
                        ...
                        if (list.isEmpty()) {
                            ...
                            entityItem.world.playSound(null, xzxpos.getX(), y, xzxpos.getZ(), Sounds.kraken_roar, SoundCategory.HOSTILE, 10.0F, 0.85F + itemRand.nextFloat() / 10.0F);
                            entityItem.setDead();
