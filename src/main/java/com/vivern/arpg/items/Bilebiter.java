    ...
    public void onUpdate(ItemStack itemstack, World world, Entity entityIn, int itemSlot, boolean isSelected) {
        if (!world.isRemote) {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack && click && !player.getCooldownTracker().hasCooldown(this) && NBTHelper.GetNBTint(itemstack, "charge_primary") >= for1shootP) {
                    if (ammo > 0 && this.isReloaded(itemstack)) {
                        if (!player.getCooldownTracker().hasCooldown(this)) {
                            world.playSound(null, player.posX, player.posY, player.posZ, Sounds.bb_shoot, SoundCategory.AMBIENT, 0.8F, 0.95F + itemRand.nextFloat() / 10.0F);
                        ...
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.bb_reloading, SoundCategory.NEUTRAL, 0.7F, 0.95F + itemRand.nextFloat() / 10.0F);
                        Weapons.setPlayerAnimationOnServer(player, 4, EnumHand.MAIN_HAND);
                    }
                }

                if (player.getHeldItemMainhand() == itemstack && clickcec && !player.getCooldownTracker().hasCooldown(this) && NBTHelper.GetNBTint(itemstack, "charge_secondary") >= maxchargeS && ammo >= 10) {
                ...
                if (player.getHeldItemMainhand() == itemstack && !player.getCooldownTracker().hasCooldown(this) && NBTHelper.GetNBTint(itemstack, "charge_secondary") < 0) {
                    world.playSound(null, player.posX, player.posY, player.posZ, Sounds.bb_homing, SoundCategory.AMBIENT, 0.8F, 0.9F + itemRand.nextFloat() / 5.0F);
                    player.getCooldownTracker().setCooldown(this, this.getCooldownTime(itemstack));
                    player.addStat(StatList.getObjectUseStats(this));
