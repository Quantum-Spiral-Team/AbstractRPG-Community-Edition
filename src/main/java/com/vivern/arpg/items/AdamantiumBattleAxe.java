...
public class AdamantiumBattleAxe extends ItemWeapon {
        ...
        if (!world.isRemote) {
            ...
            if (IWeapon.canShoot(itemstack)) {
                ...
                if (player.getHeldItemMainhand() == itemstack) {
                    if (delay <= 0 && click && !hascooldown) {
                    ...
                    } else if (delay <= 0 && EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, itemstack) > 0 && ServerKeyTracker.isKeyPressed(player, ServerKeyTracker.Keys.SECONDARY) && !hascooldown) {
                        ...
                        double attackspeed = player.getEntityAttribute(SharedMonsterAttributes.ATTACK_SPEED).getAttributeValue();
                        player.getCooldownTracker().setCooldown(this, this.getModifiedMeleeCooldown(attackspeed, this.getCooldownTime(itemstack)));
                        world.playSound(null, player.posX, player.posY, player.posZ, Sounds.swosh_d, SoundCategory.PLAYERS, 0.7F, 0.7F + itemRand.nextFloat() / 5.0F);
                    ...
                    if (delay == 1) {
                        if (NBTHelper.GetNBTboolean(itemstack, "specattack")) {
                            if (doAlternativeHammerAttack(this, itemstack, player, EnumHand.MAIN_HAND, false, 70, 7).success) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.melee_block, SoundCategory.PLAYERS, 0.9F, 0.6F + itemRand.nextFloat() / 5.0F);
                                IWeapon.fireBomEffect(this, player, world, 2);
                                world.playSound(null, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_CRIT, SoundCategory.PLAYERS, 0.9F, 0.8F + itemRand.nextFloat() / 5.0F);
                            } else {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.melee_miss_axe, SoundCategory.PLAYERS, 0.6F, 0.7F + itemRand.nextFloat() / 5.0F);
                            ...
                            if (IWeapon.doMeleeHammerAttack(this, itemstack, player, EnumHand.MAIN_HAND, false, 70, 7).success) {
                                world.playSound(null, player.posX, player.posY, player.posZ, Sounds.melee_axe, SoundCategory.PLAYERS, 0.7F, 0.8F + itemRand.nextFloat() / 5.0F);
                            } else {
    ...
    public static MeleeAttackResult doAlternativeHammerAttack(IWeapon iweapon, ItemStack stack, EntityPlayer player, EnumHand hand, boolean isCritical, int sweepAngle, int sweepStepAngle) {
        ...
        return IWeapon.doMeleeHammerAttack(iweapon, stack, player, hand, parameters.getEnchantedF("damage_special", sharpness), parameters.getEnchantedF("knockback", knockback), parameters.getEnchantedF("length", sweeping), parameters.getEnchantedF("size", sweeping), parameters.getEnchantedF("end_size", sweeping), isCritical, sweepAngle, sweepStepAngle);
    }
