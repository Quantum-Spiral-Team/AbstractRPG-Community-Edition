...
public class GraplingHook extends Item {
        ...
        if (!world.isRemote && entityIn.isEntityAlive() && entityIn instanceof EntityPlayer) {
            ...
            if (NBTHelper.GetNBTboolean(itemstack, "graped")) {
                ...
                if (distToGrapPoint <= this.maxTension) {
                    ...
                    AxisAlignedBB aabb = new AxisAlignedBB(xpos - this.grapRadius, ypos - this.grapRadius, zpos - this.grapRadius, xpos + this.grapRadius, ypos + this.grapRadius, zpos + this.grapRadius);
    ...
    public void onThrow(EntityPlayer player, ItemStack itemstack) {
        player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.swosh_a, SoundCategory.PLAYERS, 0.7F, 0.95F + itemRand.nextFloat() / 10.0F);
    }
    ...
    public void onGrap(EntityPlayer player, ItemStack itemstack) {
        ...
        player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.grapling_hook, SoundCategory.PLAYERS, 0.5F, 0.9F + itemRand.nextFloat() / 5.0F);
    }
    ...
    public void spawnParticle(Vec3d pos1, Vec3d pos2, World world, EntityPlayer player) {
        if (world.isRemote && pos1.lengthSquared() > 1.0E-6 && pos2.lengthSquared() > 1.0E-6) {
            GraplingHookParticle laser = new GraplingHookParticle(world, this.texture, this.texture2, 0.05F, -1, 1.0F, 1.0F, 1.0F, 0.0F, pos1.distanceTo(pos2), 1, 0.0F, 0.5F, player.ticksExisted, 0.2F, 0.2F, pos1, pos2);
