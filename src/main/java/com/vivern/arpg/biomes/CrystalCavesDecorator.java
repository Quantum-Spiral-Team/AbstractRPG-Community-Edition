...
class CrystalCavesDecorator extends BiomeDecorator {
    ...
    public void setStalact(World world, BlockPos pos, int prop, Block blockstalact, Random rand) {
        world.setBlockState(pos, (rand.nextFloat() < 0.15 ? BlocksRegister.SELENITE_SPELEOTHEM : blockstalact).getDefaultState().withProperty(BlockSpeleothem.TYPE, prop), 2);
    }
        ...
        if (this.decorating) {
            ...
            for (int ss = 0; ss < 160; ss++) {
                ...
                if (world.getBlockState(postogenerate).getMaterial() == Material.ROCK) {
                    CrystalCaves.blockPosCrystalTrace2(world, rand, postogenerate, pos.getX() + 3, pos.getX() + 31, pos.getZ() + 1, pos.getZ() + 29, BlocksRegister.SELENITE);
                }
            }
                    ...
                    if (rand.nextFloat() < 0.43) {
                        ...
                        for (int yy = 254; yy > 5; yy--) {
                            ...
                            if (state.getBlock() == Blocks.AIR) {
                                ...
                                if (state.isSideSolid(world, fpos, EnumFacing.UP) && stateup != null && (stateup.getBlock() instanceof BlockSpeleothem || stateup.getBlock() == Blocks.AIR)) {
                                        ...
                                        if (rand.nextFloat() < 0.2) {
                                            ...
                                            if (rand.nextFloat() < 0.9) {
                                                if (rand.nextFloat() < 0.6) {
                                                    ...
                                                    if (rand.nextFloat() < 0.6 && (state3.getBlock() instanceof BlockSpeleothem || state3.getBlock() == Blocks.AIR)) {
                                                        this.setStalact(world, fpos.up(3), 2, blockstalact, rand);
                                                        IBlockState state4 = world.getBlockState(fpos.up(4));
                                                        if (rand.nextFloat() < 0.5 && (state4.getBlock() instanceof BlockSpeleothem || state4.getBlock() == Blocks.AIR)) {
                                                            this.setStalact(world, fpos.up(4), 2, blockstalact, rand);
                                                            IBlockState state5 = world.getBlockState(fpos.up(5));
                                                            if (rand.nextFloat() < 0.5 && (state5.getBlock() instanceof BlockSpeleothem || state5.getBlock() == Blocks.AIR)) {
