    ...
    public void decorate(World worldIn, Random random, Biome biome, BlockPos pos) {
        if (this.decorating) {
            ...
            if (random.nextFloat() < 0.1F) {
                BlockPos topposAh = GetMOP.getTopBlock(worldIn, pos.add(random.nextInt(16) + 8, 254, random.nextInt(16) + 8), BlocksRegister.FULMINIFLORA).up();
            ...
            if (random.nextFloat() < 0.92F) {
                ...
                if (topposAh.getY() > 5 && GenerationHelper.isReplaceable(worldIn, topposAh)) {
                    ...
                    gen.generateOldFernTree(worldIn, random, topposAh, 13 + random.nextInt(10) + noisePerlin1, 5 + random.nextInt(6) + noisePerlin2, BlocksRegister.FULMINIFLORA, BlocksRegister.ARTHROSTELECHA_LOG_BRASS);
                }
            }
