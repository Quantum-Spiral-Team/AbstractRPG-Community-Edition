        ...
        public void decorate(World worldIn, Random random, Biome biome, BlockPos pos) {
            if (this.decorating) {
                ...
                for (int ss = 0; ss < 90; ss++) {
                    BlockPos postogenerate = new BlockPos(pos.getX() + random.nextInt(16) + 8, random.nextInt(254) + 1, pos.getZ() + random.nextInt(16) + 8);
                    if (worldIn.getBlockState(postogenerate).getMaterial() == Material.ROCK) {
                        CrystalCaves.blockPosCrystalTrace2(worldIn, random, postogenerate, pos.getX() + 3, pos.getX() + 31, pos.getZ() + 1, pos.getZ() + 29, BlocksRegister.MAGIC_STONE);
                    }
                }
