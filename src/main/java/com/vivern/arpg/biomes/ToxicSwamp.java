        ...
        public void decorate(World worldIn, Random random, Biome biome, BlockPos pos) {
            if (this.decorating) {
                ...
                if (random.nextFloat() < 0.8F) {
                    BlockPos position = worldIn.getTopSolidOrLiquidBlock(new BlockPos(pos.getX() + 8 + random.nextInt(16), 0, pos.getZ() + 8 + random.nextInt(16)));
                    Block blockd = worldIn.getBlockState(position.down()).getBlock();
                    if (blockd == BlocksRegister.JUNK || blockd == BlocksRegister.TOXIC_DIRT || blockd == BlocksRegister.SLUDGE || blockd == BlocksRegister.TOXIC_GRASS || blockd == BlocksRegister.NUCLEAR_WASTE) {
                        ...
                        Template templatexxx = templateManagerxxx.get(minecraftServerxxx, new ResourceLocation("arpg:toxic_swamp_tree_" + (random.nextInt(3) + 1)));
