...
public class AquaticaBeach extends Biome {
    ...
    public AquaticaBeach() {
        super(new BiomeProperties("Aquatica beach").setBaseHeight(5.5F).setHeightVariation(0.03F).setTemperature(0.95F).setWaterColor(11921407).setRainfall(0.0F).setRainDisabled());
        ...
        public void decorate(World world, Random random, Biome biome, BlockPos decpos) {
            if (this.decorating) {
                ...
                if (random.nextFloat() < 0.2) {
                    BlockPos position = GetMOP.getTrueHeight(world, new BlockPos(decpos.getX() + 8 + random.nextInt(16), 215, decpos.getZ() + 8 + random.nextInt(16))).up();
