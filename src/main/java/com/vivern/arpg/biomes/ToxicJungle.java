        ...
        public void decorate(World worldIn, Random random, Biome biome, BlockPos pos) {
            if (this.decorating) {
                ...
                for (int ji = 0; ji < 3; ji++) {
                    if (random.nextFloat() < 0.5F) {
                        ...
                        if (blockd == BlocksRegister.TOXIC_GRASS || blockd == BlocksRegister.TOXIC_DIRT || blockd == BlocksRegister.TOXIBERRY_LOG) {
                            ...
                            Template template = templateManager.get(minecraftServer, new ResourceLocation("arpg:toxic_tree_jungle_" + (random.nextInt(2) + 1)));
                ...
                if (random.nextFloat() < 0.25F) {
                    BlockPos position = worldIn.getTopSolidOrLiquidBlock(new BlockPos(pos.getX() + 8 + random.nextInt(16), 0, pos.getZ() + 8 + random.nextInt(16)));
                    Block blockd = worldIn.getBlockState(position.down()).getBlock();
                    if (blockd == BlocksRegister.TOXIC_GRASS || blockd == BlocksRegister.TOXIC_DIRT || blockd == BlocksRegister.TOXIBERRY_LOG || blockd == BlocksRegister.SLUDGE || blockd == BlocksRegister.RADIOACTIVE_STONE) {
                ...
                for (int ixxx = 0; ixxx < 18; ixxx++) {
                    int j = random.nextInt(16) + 8;
                    int k = random.nextInt(16) + 8;
                    GetMOP.BlockTraceResult res = random.nextFloat() < 0.5 ? GetMOP.blockTrace(worldIn, this.chunkPos.add(j, 62 + random.nextInt(40), k), random.nextFloat() < 0.5 ? EnumFacing.UP : EnumFacing.DOWN, 30, null) : GetMOP.blockTrace(worldIn, this.chunkPos.add(j, 62 + random.nextInt(40), k), EnumFacing.byHorizontalIndex(random.nextInt(4)), 6, null);
                    if (res != null) {
                        if (res.prevState.getMaterial() == Material.AIR) {
                            if (GetMOP.SOLID_BLOCKS.apply(res.impactState)) {
                                worldIn.setBlockState(res.prevPos, BlocksRegister.CHLORINE_BELCHER.getDefaultState().withProperty(ChlorineBelcher.FACING, res.facing.getOpposite()), 2);
                            }
