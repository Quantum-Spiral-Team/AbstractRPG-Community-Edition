...
public class EntityBoomerangMagic extends StandardBullet {
    ...
    public void onUpdate() {
        ...
        if (target != null && (this.ticksExisted > parameters.getEnchantedI("ticks_flying", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.RANGE, this.weaponstack)) / flyspeed || this.shouldReturn)) {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.hitmagic, SoundCategory.AMBIENT, 0.7F, 1.5F, false);

            for (int ss = 0; ss < 7; ss++) {
                GUNParticle spelll = new GUNParticle(resoue, 0.1F, 0.0F, 16, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, 0.7F + (float) this.rand.nextGaussian() / 4.0F, 0.7F + (float) this.rand.nextGaussian() / 4.0F, 1.0F, false, 0);
