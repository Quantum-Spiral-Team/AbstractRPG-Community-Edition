...
public class GunPEmitter extends GUNParticle {
    ...
    public GunPEmitter(ResourceLocation texture, float scale, float gravity, int livetime, int light, World world, double x, double y, double z, float speedx, float speedy, float speedz, float Red, float Green, float Blue, boolean opa, int delay, boolean collide, ResourceLocation EEtexture, float EEscale, float EEgravity, int EElivetime, int EElight, float EEspeedx, float EEspeedy, float EEspeedz, float EERed, float EEGreen, float EEBlue, boolean EEopa) {
    ...
    public GunPEmitter(ResourceLocation texture, float scale, float gravity, int livetime, int light, World world, double x, double y, double z, float speedx, float speedy, float speedz, float Red, float Green, float Blue, boolean opa, int delay, boolean collide, EnumParticleTypes spawnVanillaParticles, float EEspeedx, float EEspeedy, float EEspeedz) {
        super(texture, scale, gravity, livetime, light, world, x, y, z, speedx, speedy, speedz, Red, Green, Blue, opa, 0);
    ...
    public void onUpdate() {
        ...
        if (this.ticksExisted % this.delay == 0) {
            if (this.spawnVanillaParticles != null) {
                this.world.spawnParticle(this.spawnVanillaParticles, this.posX, this.posY, this.posZ, this.EEspeedx, this.EEspeedy, this.EEspeedz, new int[0]);
            } else {
                GUNParticle part = new GUNParticle(this.EEtexture, this.EEscale, this.EEgravity, this.EElivetime, this.EElight, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / this.EEspeedx, (float) this.rand.nextGaussian() / this.EEspeedy, (float) this.rand.nextGaussian() / this.EEspeedz, this.EERed, this.EEGreen, this.EEBlue, this.EEopa, 0);
