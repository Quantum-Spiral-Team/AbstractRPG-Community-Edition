...
public class EntityArrowFrozen extends AbstractArrow {
    ...
    public void arrowHit(EntityLivingBase living) {
        ...
        if (this.pickupStatus == PickupStatus.ALLOWED) {
            PotionEffect lastdebaff = Weapons.mixPotion(living, PotionEffects.FREEZING, 48.0F, 2.0F, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumMathOperation.PLUS, Weapons.EnumMathOperation.PLUS, 100, 16);
    ...
    public void spawnArrowParticles(int particleCount) {
        for (int i = 0; i < particleCount; i++) {
            GUNParticle snow2 = new GUNParticle(snow, 0.07F + this.rand.nextFloat() * 0.04F, 0.008F, 15 + this.rand.nextInt(10), 170, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 50.0F, (float) this.rand.nextGaussian() / 50.0F, (float) this.rand.nextGaussian() / 50.0F, 1.0F, 1.0F, 1.0F, false, this.rand.nextInt(360));
