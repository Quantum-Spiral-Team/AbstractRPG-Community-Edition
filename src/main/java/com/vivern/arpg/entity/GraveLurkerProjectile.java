...
public class GraveLurkerProjectile extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        ...
        if (id == 5) {
            GUNParticle fire2 = new GUNParticle(this.largesmoke, 0.16F + (float) this.rand.nextGaussian() / 20.0F, -0.009F, 10 + this.rand.nextInt(5), 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 1.0F, (0.7F + (float) this.rand.nextGaussian() / 5.0F) * mult, mult, true, this.rand.nextInt(100) - 50);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote) {
                ...
                Weapons.dealDamage(null, (damagee + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack)) * this.magicPower, this.thrower, result.entityHit, true, 0.5F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) / 2.0F, this.thrower.posX, this.thrower.posY, this.thrower.posZ);
                ...
                this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.magic_d, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
