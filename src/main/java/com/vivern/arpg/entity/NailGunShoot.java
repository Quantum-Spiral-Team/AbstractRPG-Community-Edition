...
public class NailGunShoot extends EntityArrow implements IRepulsable, INailer {
    ...
    public static boolean isEntityPricked(World world, Entity entity) {
        int range = 15;
        List<Entity> list = world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(entity.posX + range, entity.posY + range, entity.posZ + range, entity.posX - range, entity.posY - range, entity.posZ - range), entity2 -> entity2 instanceof INailer);
        if (!list.isEmpty()) {
        ...
        if (entity != null) {
            if (!this.repulse(entity) && Team.checkIsOpponent(this.shootingEntity, entity) && !this.world.isRemote) {
                ...
                if (entity instanceof EntityLivingBase) {
                    ...
                    boolean healthmin = entitylivingbase.getHealth() <= parameters.getEnchantedF("health_to_prick", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, this.weaponstack));
                ...
                if (!this.damageDealed) {
                    this.damageDealed = Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.shootingEntity, this, false, true, this, WeaponDamage.pierce), parameters.getEnchantedF("damage", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack)), this.shootingEntity, entity, true, parameters.getEnchantedF("knockback", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack)));
    ...
    public void onUpdate() {
        ...
        if (this.prickedToWall && this.world.getBlockState(this.wallPos).getBlock().getCollisionBoundingBox(this.world.getBlockState(this.wallPos), this.world, this.wallPos) == null) {
            this.prickedToWall = false;
            this.wallPos = null;
        }
