...
public class StingingCellEntity extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            for (int ss = 0; ss < 4; ss++) {
                GUNParticle bubble = new GUNParticle(this.slime, 0.4F + this.rand.nextFloat() / 3.0F, -0.001F, 10 + this.rand.nextInt(10), -1, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 19.0F, (float) this.rand.nextGaussian() / 22.0F + 0.05F, (float) this.rand.nextGaussian() / 19.0F, 0.6F + (float) this.rand.nextGaussian() / 10.0F, 0.85F + (float) this.rand.nextGaussian() / 10.0F, 0.95F + (float) this.rand.nextGaussian() / 10.0F, true, this.rand.nextInt(180));
            ...
            for (int s = 0; s < 7; s++) {
                GUNParticle bubble2 = new GUNParticle(this.texturbubble, 0.08F + this.rand.nextFloat() / 20.0F, -0.004F, 50 + this.rand.nextInt(20), 200, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 16.0F, (float) this.rand.nextGaussian() / 16.0F - 0.07F, (float) this.rand.nextGaussian() / 16.0F, 0.6F, 0.8F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, this.rand.nextInt(180));
    ...
    public void hitEntity(Entity entity) {
        ...
        Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.water), damage * this.magicPower, this.getThrower(), entity, true, parameters.getEnchantedF("knockback", impulse), this.posX, this.posY, this.posZ);
        entity.hurtResistantTime = 0;
        this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.stinging_cell_impact, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote && Weapons.canDealDamageTo(result.entityHit) && !(result.entityHit instanceof StingingCellEntity)) {
