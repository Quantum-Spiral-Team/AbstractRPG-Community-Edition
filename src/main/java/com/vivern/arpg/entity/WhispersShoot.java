...
public class WhispersShoot extends EntityThrowable implements ISynchronizedEntity {
        ...
        if (!this.world.isRemote) {
            ...
            if (!this.isDead) {
                ...
                for (Entity entity : list) {
                    if (GetMOP.isBoxInPlane(center, this.bladeNormal, entity.getEntityBoundingBox()) && Team.checkIsOpponent(this.thrower, entity) && !this.impacted.contains(entity)) {
                        Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, false, this, this.powered ? WeaponDamage.dismember : WeaponDamage.blade), (this.powered ? parameters.getEnchantedF("damage_powered", might) : parameters.getEnchantedF("damage_edge", might)) * this.magicPower, this.getThrower(), entity, true, this.powered ? parameters.getEnchantedF("knockback_powered", impulse) : parameters.getEnchantedF("knockback_edge", impulse));
    ...
    public void onClient(double... args) {
        if (args.length == 2) {
            ...
            for (int ss = 0; ss < 21; ss++) {
                ...
                GUNParticle bigsmoke = new GUNParticle(cloud, 0.17F + this.rand.nextFloat() * 0.07F, 0.0F, lt, 240, this.world, partpos.x, partpos.y, partpos.z, (float) this.rand.nextGaussian() / 25.0F, (float) this.rand.nextGaussian() / 25.0F, (float) this.rand.nextGaussian() / 25.0F, 1.0F, 0.9F + this.rand.nextFloat() / 10.0F, 1.0F, true, this.rand.nextInt(360));
    ...
    public void handleStatusUpdate(byte id) {
        ...
        if (id == 9) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.whispers_blade_ricochet, SoundCategory.AMBIENT, 1.0F, 0.95F + this.rand.nextFloat() * 0.1F, false);
        }

        if (id == 10) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.whispers_blade_hit, SoundCategory.AMBIENT, 1.1F, 0.9F + this.rand.nextFloat() * 0.2F, false);
        }

        if (id == 11) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.whispers_blade_bighit, SoundCategory.AMBIENT, 1.0F, 0.9F + this.rand.nextFloat() * 0.2F, false);
        }
        ...
        if (!this.world.isRemote) {
            if (this.powered) {
                ...
                if (Team.checkIsOpponent(this.thrower, result.entityHit)) {
                    ...
                    int might = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack);
                    int impulse = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack);
                    Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.blade), parameters.getEnchantedF("damage", might) * this.magicPower, this.getThrower(), result.entityHit, true, parameters.getEnchantedF("knockback", impulse));
