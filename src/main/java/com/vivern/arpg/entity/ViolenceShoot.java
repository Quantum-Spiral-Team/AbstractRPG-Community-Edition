...
public class ViolenceShoot extends EntityThrowable {
    ...
    public void generateDirection() {
        this.direction = new Vec3d(this.directionRand.nextGaussian() / 7.0, -0.04F - this.directionRand.nextFloat() * 0.02F, this.directionRand.nextGaussian() / 7.0);
    }
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.violence_impact, SoundCategory.AMBIENT, 1.2F, 0.875F + this.rand.nextFloat() / 4.0F, false);
            AnimatedGParticle anim = new AnimatedGParticle(explode, 0.9F + this.rand.nextFloat() * 0.3F, -0.01F, 18, 230, this.world, this.prevPosX, this.prevPosY, this.prevPosZ, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, 0);
            anim.framecount = 18;
            ...
            for (int i = 0; i < 5; i++) {
                ...
                GUNParticle part = new GUNParticle(star2, scl, 0.04F, lt, 230, this.world, this.prevPosX, this.prevPosY, this.prevPosZ, (float) this.rand.nextGaussian() / 7.0F, (float) this.rand.nextGaussian() / 13.0F + 0.1F, (float) this.rand.nextGaussian() / 7.0F, 0.8F + this.rand.nextFloat() * 0.2F, 0.2F, 0.8F + this.rand.nextFloat() * 0.2F, true, this.rand.nextInt(360));
        ...
        if (!list.isEmpty()) {
            for (Entity entity : list) {
                if (Team.checkIsOpponent(this.thrower, entity)) {
                    ...
                    Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, false, this, WeaponDamage.soul), parameters.getEnchantedF("damage", might) * this.magicPower, this.getThrower(), entity, true, parameters.getEnchantedF("knockback", impulse));
                    entity.hurtResistantTime = 0;
                    Weapons.setPotionIfEntityLB(entity, PotionEffects.DEMONIC_BURN, parameters.getEnchantedI("potion_time", witchery), parameters.getInt("potion_power"));
                }
            }
        }
    ...
    public void onEntityUpdate() {
        ...
        if (this.world.isRemote && pos1.lengthSquared() > 1.0E-6 && pos2.lengthSquared() > 1.0E-6) {
            BetweenParticle laser = new BetweenParticle(this.world, trace, 0.16F, 230, 1.0F, 0.2F, 0.8F, 0.08F, pos1.distanceTo(pos2), 11, 0.0F, 0.0F, pos1, pos2);
