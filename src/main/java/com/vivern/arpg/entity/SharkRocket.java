...
public class SharkRocket extends EntityThrowable {
    ...
    public void onUpdate() {
        ...
        if (this.ticksExisted > 22) {
            EntityLivingBase target = GetMOP.findNearestEnemy(this.world, this.thrower, this.posX, this.posY, this.posZ, 8.0, EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, this.weaponstack) > 0);
            if (target != null) {
                SuperKnockback.applyMove(this, -0.07F - EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.ACCURACY, this.weaponstack) * 0.01F, target.posX, target.posY + target.height / 3.0F, target.posZ);
            }
        }
    }
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity)) {
                        Weapons.dealDamage(null, Math.max(15.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack) - 1.2F * entity.getDistance(this), 2.0F), this.thrower, entity, true, 2.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) / 1.5F, this.posX, this.posY, this.posZ);
    ...
    public void onEntityUpdate() {
        ...
        if (this.ticksExisted == 20) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.sharkrocket, SoundCategory.AMBIENT, 1.0F, 1.0F, false);
        }
        ...
        if (this.ticksExisted == 22) {
            for (int kf = 0; kf < 5; kf++) {
                this.world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, this.posX, this.posY, this.posZ, this.rand.nextGaussian() / 10.0, this.rand.nextGaussian() / 10.0, this.rand.nextGaussian() / 10.0, new int[0]);
            }
        }
        ...
        this.world.spawnParticle(EnumParticleTypes.FLAME, this.posX, this.posY, this.posZ, gaussianX, gaussianY, gaussianZ, new int[0]);
    }
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 4) {
            ...
            this.world.spawnParticle(EnumParticleTypes.EXPLOSION_HUGE, this.posX, this.posY, this.posZ, 0.0, 0.0, 0.0, new int[0]);

            for (int kf = 0; kf < 10; kf++) {
                this.world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, this.posX, this.posY, this.posZ, this.rand.nextGaussian() / 3.0, this.rand.nextGaussian() / 3.0, this.rand.nextGaussian() / 3.0, new int[0]);
            }
        }
    }
