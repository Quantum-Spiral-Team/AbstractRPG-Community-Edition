...
public class EntitySand extends EntityThrowable {
    ...
    public void onUpdate() {
        ...
        if (!this.world.isRemote) {
            AxisAlignedBB aabbox = new AxisAlignedBB(this.posX - this.scaleaabb, this.posY - this.scaleaabb, this.posZ - this.scaleaabb, this.posX + this.scaleaabb, this.posY + this.scaleaabb, this.posZ + this.scaleaabb);
            this.scaleaabb += 0.05;
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity) && !this.attackedsand.contains(entity)) {
                        ...
                        Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, false, this, WeaponDamage.sand), parameters.getEnchantedF("damage", might) * this.magicPower, this.getThrower(), entity, true, parameters.getEnchantedF("knockback", impulse), this.posX, this.posY, this.posZ);
    ...
    public int getBlockSandTurnType(IBlockState blockState, BlockPos pos) {
        ...
        if (material == Material.ROCK || material == Material.CORAL || material == Material.GROUND || material == Material.CLAY || material == Material.GRASS || material == Material.LEAVES || material == Material.CACTUS || material == Material.PLANTS || material == Material.VINE || material == Material.GOURD || material == Material.WOOD || material == Material.GLASS || material == Material.CIRCUITS || material == Material.PISTON || material == Material.REDSTONE_LIGHT || material == Material.SPONGE || material == Material.WEB || material == Material.CLOTH || material == Material.CARPET) {
            WeaponParameters parameters = WeaponParameters.getWeaponParameters(this.weaponstack.getItem());
    ...
    protected void onImpact(RayTraceResult result) {
        ...
        if (!this.world.isRemote && result.entityHit != null && result.entityHit instanceof EntityFallingBlock && !this.doknock) {
            ...
            SuperKnockback.applyKnockback(block, parameters.getEnchantedF("knockback_to_blocks", impulse), this.getThrower().posX, this.getThrower().posY, this.getThrower().posZ);
        ...
        if (result.typeOfHit == Type.BLOCK) {
            ...
            if (!this.world.isRemote && !this.blockcollided) {
                ...
                if (this.rand.nextFloat() < 0.5F + chargeFullness * 0.5F) {
                    ...
                    if (this.rand.nextFloat() < parameters.getEnchantedF("fallingblock_chance", impulse) && blockState.getBlock() == Blocks.SAND && blockpos.getY() >= 0) {
                        int i = 32;
                        if (!BlockSand.fallInstantly && this.world.isAreaLoaded(blockpos.add(-32, -32, -32), blockpos.add(32, 32, 32))) {
                            if (!this.world.isRemote) {
                                EntityFallingBlock entityfallingblock = new EntityFallingBlock(this.world, blockpos.getX() + 0.5, blockpos.getY() + 0.2, blockpos.getZ() + 0.5, this.world.getBlockState(blockpos));
                                this.world.spawnEntity(entityfallingblock);
                                if (this.world.getBlockState(blockpos.up()).getBlock() == Blocks.SAND && blockpos.up().getY() >= 0) {
                                    if (BlockSand.fallInstantly || !this.world.isAreaLoaded(blockpos.up().add(-32, -32, -32), blockpos.up().add(32, 32, 32))) {
                                        ...
                                        while ((this.world.isAirBlock(blockposss) || BlockSand.canFallThrough(this.world.getBlockState(blockposss))) && blockposss.getY() > 0) {
                                        ...
                                        EntityFallingBlock entityfallingblock2 = new EntityFallingBlock(this.world, blockpos.up().getX() + 0.5, blockpos.up().getY(), blockpos.up().getZ() + 0.5, this.world.getBlockState(blockpos.up()));
                            ...
                            while ((this.world.isAirBlock(blockposs) || BlockSand.canFallThrough(this.world.getBlockState(blockposs))) && blockposs.getY() > 0) {
