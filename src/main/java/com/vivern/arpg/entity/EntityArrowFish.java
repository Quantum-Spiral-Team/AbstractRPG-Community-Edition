    ...
    public void onUpdate() {
        ...
        if (this.watered && !this.world.isRemote) {
            ...
            if (this.target != null) {
                ...
                this.motionY *= 0.6;
                this.motionZ *= 0.6;
                SuperKnockback.applyMove(this, this.target.isPotionActive(MobEffects.GLOWING) ? -0.3F : -0.13F, this.target.posX, this.target.posY + this.target.height / 2.0F, this.target.posZ);
                this.velocityChanged = true;
                if (!this.target.isEntityAlive()) {
                    this.target = null;
                }
            } else if (this.rand.nextFloat() < 0.5F) {
                List<EntityLivingBase> list = GetMOP.getHostilesInAABBto(this.world, this.getPositionVector().add(new Vec3d(this.motionX, this.motionY, this.motionZ).normalize().scale(5.0)), 5.0, 5.0, (EntityLivingBase) this.shootingEntity);
                if (!list.isEmpty()) {
