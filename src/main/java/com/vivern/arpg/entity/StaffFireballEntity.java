    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            ...
            for (int ss = 0; ss < 10; ss++) {
                GUNParticle bigsmoke = new GUNParticle(this.largesmoke, 0.6F + (float) this.rand.nextGaussian() / 2.0F, -0.002F, 54, 60, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 16.0F, (float) this.rand.nextGaussian() / 20.0F, (float) this.rand.nextGaussian() / 16.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
        ...
        if (id == 8) {
            GUNParticle fire2 = new GUNParticle(this.flame, 0.13F + (float) this.rand.nextGaussian() / 15.0F, -0.009F, 10 + this.rand.nextInt(5), 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 1.0F, 0.8F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, this.rand.nextInt(100) - 50);
        ...
        if (!this.world.isRemote) {
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    ...
                    Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, true, false, this, WeaponDamage.explode), parameters.getEnchantedF("damage", might) * this.magicPower * friendlyfire, this.getThrower(), entity, true, parameters.getEnchantedF("knockback", impulse) * friendlyfire, this.posX, this.posY, this.posZ);
            ...
            if (EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, this.weaponstack) > 0) {
                Explosion explosion = new Explosion(this.world, null, this.posX, this.posY, this.posZ, parameters.getFloat("special_explosion_size"), false, true);
