...
public class EntitySunrise extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        ...
        if (id == 5) {
            ...
            for (int ss = 0; ss < 10; ss++) {
                Entity spelll = new GUNParticle(resoue, 0.1F, 0.01F, 10, 240, this.world, this.posX, this.posY, this.posZ, prunex + (float) this.rand.nextGaussian() / 20.0F, pruney + (float) this.rand.nextGaussian() / 20.0F, prunez + (float) this.rand.nextGaussian() / 20.0F, 0.9F + (float) this.rand.nextGaussian() / 10.0F, 0.9F + (float) this.rand.nextGaussian() / 10.0F, 0.8F + (float) this.rand.nextGaussian() / 10.0F, false, 0);
                this.world.spawnEntity(spelll);
            }
        }

        if (id == 18) {
            Entity spelll = new GUNParticle(resoue, 0.1F, -0.01F, 10, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, 0.9F + (float) this.rand.nextGaussian() / 10.0F, 0.9F + (float) this.rand.nextGaussian() / 10.0F, 0.8F + (float) this.rand.nextGaussian() / 10.0F, false, 0);
            this.world.spawnEntity(spelll);
        }
    }
    ...
    protected void onImpact(RayTraceResult result) {
        if (this.to == null && result.entityHit != this.thrower && result.entityHit != null && result.entityHit != this.getThrower() && this.ticksExisted > 1 && this.link) {
            this.link = false;
