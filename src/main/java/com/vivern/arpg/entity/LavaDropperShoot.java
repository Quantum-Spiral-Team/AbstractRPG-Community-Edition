...
public class LavaDropperShoot extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            GUNParticle part = new GUNParticle(tex, 0.05F + this.rand.nextFloat() / 30.0F, 0.025F, 40 + this.rand.nextInt(20), 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 20.0F, (float) this.rand.nextGaussian() / 20.0F, (float) this.rand.nextGaussian() / 20.0F, 1.0F, 1.0F, 1.0F, false, 0, true, 1.5F);
        ...
        if (id == 8) {
            for (int ss = 0; ss < this.rand.nextInt(10) + 4; ss++) {
                GUNParticle part = new GUNParticle(tex, 0.05F + this.rand.nextFloat() / 30.0F, 0.035F, 40 + this.rand.nextInt(20), 240, this.world, this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, 1.0F, 1.0F, 1.0F, false, 0, true, 1.5F);
                part.alphaGlowing = true;
                part.dropMode = true;
                this.world.spawnEntity(part);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && Weapons.canDealDamageTo(result.entityHit) && !this.world.isRemote) {
                ...
                Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.fire), parameters.getEnchantedF("damage", might), this.getThrower(), result.entityHit, true, parameters.getEnchantedF("knockback", impulse));
                result.entityHit.hurtResistantTime = 0;
                result.entityHit.setFire(parameters.getEnchantedI("fire", witchery));
                this.tryPlaceLava(result);
                this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.lava_dropper_impact, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
