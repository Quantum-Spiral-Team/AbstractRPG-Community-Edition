...
public class EntityMagicRocket extends EntityThrowable implements ISynchronizedEntity, IRenderOptions {
        ...
        if (!this.world.isRemote) {
            ...
            if (this.disableMagic > 0) {
                ...
                if (!this.useTarget) {
                    ...
                    if (EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, this.weaponstack) == 0) {
                        ...
                        if (!list.isEmpty()) {
                            for (EntityLivingBase entitylivingbasex : list) {
                                if (entitylivingbasex.isCreatureType(EnumCreatureType.MONSTER, false) && Team.checkIsOpponent(this.thrower, entitylivingbasex) && entitylivingbasex != this.getThrower() && EntitySelectors.NOT_SPECTATING.apply(entitylivingbasex) && entitylivingbasex.getHealth() > 0.0F && GetMOP.thereIsNoBlockCollidesBetween(this.world, GetMOP.entityCenterPos(this), GetMOP.entityCenterPos(entitylivingbasex), null, false)) {
                            ...
                            if (targ == null) {
                                for (EntityLivingBase entitylivingbasexx : list) {
                                    if (Team.checkIsOpponent(this.thrower, entitylivingbasexx) && entitylivingbasexx != this.getThrower() && EntitySelectors.NOT_SPECTATING.apply(entitylivingbasexx) && entitylivingbasexx.getHealth() > 0.0F && GetMOP.thereIsNoBlockCollidesBetween(this.world, GetMOP.entityCenterPos(this), GetMOP.entityCenterPos(entitylivingbasexx), null, false)) {
                    ...
                    this.motionY *= mt;
                    this.motionZ *= mt;
                    SuperKnockback.applyKnockback(this, -acceleration, this.specialTarget.posX, this.specialTarget.posY + this.specialTarget.height / 2.0F, this.specialTarget.posZ);
                    this.velocityChanged = true;
                }
            }
        }
    ...
    public void onClient(double x, double y, double z, double a, double b, double c) {
        for (int ss = 0; ss < 10; ss++) {
            ...
            GUNParticle bigsmoke = new GUNParticle(star, scl, 0.0F, 35, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / delSpd, (float) this.rand.nextGaussian() / delSpd, (float) this.rand.nextGaussian() / delSpd, 0.2F, 0.65F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, this.rand.nextInt(360));
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote) {
                ...
                Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.magic), parameters.getEnchantedF("damage", might) * this.magicPower, this.getThrower(), result.entityHit, true, parameters.getEnchantedF("knockback", impulse), this.posX, this.posY - 1.0, this.posZ);
                result.entityHit.hurtResistantTime = 0;
                this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.hitmagic_a, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
            ...
            if (!this.world.isRemote) {
                if (result.hitVec != null) {
                    ISynchronizedEntity.sendSynchronize(this, 64.0, result.hitVec.x, result.hitVec.y, result.hitVec.z, 0.0, 0.0, 0.0);
                }
    ...
    public void onEntityUpdate() {
        ...
        if (this.world.isRemote && pos1.lengthSquared() > 1.0E-6 && pos2.lengthSquared() > 1.0E-6) {
            BetweenParticle laser = new BetweenParticle(this.world, texture, 0.17F, 240, 0.6F, 0.8F, 1.0F, 0.1F, pos1.distanceTo(pos2), 5, -0.15F, 9999.0F, pos1, pos2);
