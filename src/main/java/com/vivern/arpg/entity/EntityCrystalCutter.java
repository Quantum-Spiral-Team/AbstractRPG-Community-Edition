...
public class EntityCrystalCutter extends EntityThrowable {
    ...
    public void onUpdate() {
        ...
        if (this.ticksExisted > this.livetime && !this.destroying) {
            this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.crystal_cutter_impact, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
        ...
        Vec3d leftVec = GetMOP.pitchYawToVec3D((float) pichyawVec.x, (float) pichyawVec.y + 90.0F).normalize().scale(cutterSizeTo);
        Vec3d rightVec = GetMOP.pitchYawToVec3D((float) pichyawVec.x, (float) pichyawVec.y - 90.0F).normalize().scale(cutterSizeTo);
        ...
        for (Entity entity : list) {
            if (entity != this && Team.checkIsOpponent(this.thrower, entity)) {
                if (this.thrower != null) {
                    Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, false, null, WeaponDamage.plasma), parameters.getEnchantedF("damage", might), this.getThrower(), entity, true, parameters.getEnchantedF("knockback", impulse), this.thrower.posX, this.thrower.posY, this.thrower.posZ);
                }
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 9) {
            for (int ss = 0; ss < 5; ss++) {
                GUNParticle bigsmoke = new GUNParticle(cloud, 0.4F + this.rand.nextFloat() / 6.0F, 0.0F, 30, 200, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 17.0F, (float) this.rand.nextGaussian() / 17.0F, (float) this.rand.nextGaussian() / 17.0F, 1.0F, 0.35F + this.rand.nextFloat() / 10.0F, 0.6F, true, this.rand.nextInt(360));
            ...
            for (int ss = 0; ss < 6; ss++) {
                EntityCubicParticle part = new EntityCubicParticle(shard, 0.005F + this.rand.nextFloat() * 0.003F, 0.017F, 16 + this.rand.nextInt(10), 150, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F + 0.1F, (float) this.rand.nextGaussian() / 14.0F, 1.0F, 0.35F + this.rand.nextFloat() / 10.0F, 0.6F, false, this.rand.nextFloat(), this.rand.nextFloat(), this.rand.nextFloat(), 0.14F, true, 0.0F);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit == null && this.world.getBlockState(result.getBlockPos()).getBlock().getCollisionBoundingBox(this.world.getBlockState(result.getBlockPos()), this.world, result.getBlockPos()) != null && !this.destroying) {
            ...
            if (this.world.getBlockState(rightPos).getBlock().getCollisionBoundingBox(this.world.getBlockState(rightPos), this.world, rightPos) != null || this.world.getBlockState(leftPos).getBlock().getCollisionBoundingBox(this.world.getBlockState(leftPos), this.world, leftPos) != null) {
                this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.crystal_cutter_impact, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
