...
public class EntitySwordGhost extends EntityThrowable {
    ...
    public void onUpdate() {
        ...
        if (targ != null) {
            ...
            this.motionY *= 0.97;
            this.motionZ *= 0.97;
            SuperKnockback.applyKnockback(this, -0.07F, targ.posX, targ.posY + (targ.getEntityBoundingBox().maxY - targ.getEntityBoundingBox().minY) / 2.0, targ.posZ);
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.ghost_sword, SoundCategory.AMBIENT, 0.3F, 0.9F + this.rand.nextFloat() / 5.0F, false);
        }

        if (id == 5) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.magic_d, SoundCategory.AMBIENT, 0.7F, 1.25F + this.rand.nextFloat() / 4.0F, false);

            for (int ss = 0; ss < 6; ss++) {
                GUNParticle bigsmoke = new GUNParticle(this.largesmoke, 0.3F + (float) this.rand.nextGaussian() / 20.0F, 0.0F, 15, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 20.0F, (float) this.rand.nextGaussian() / 25.0F, (float) this.rand.nextGaussian() / 20.0F, 0.3F, 1.0F, 0.7F + (float) this.rand.nextGaussian() / 5.0F, true, this.rand.nextInt(360));
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity)) {
                        Weapons.dealDamage(null, 2.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack), this.thrower, entity, true, 0.3F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) / 1.5F, this.posX, this.posY, this.posZ);
    ...
    public void onEntityUpdate() {
        ...
        if (this.ticksExisted > 2 && this.world.isRemote && this.pos1.lengthSquared() > 1.0E-6 && this.pos2.lengthSquared() > 1.0E-6) {
            BetweenParticle laser = new BetweenParticle(this.world, this.texture, 0.17F, 240, 0.5F, 1.0F, 0.8F, 0.15F, this.pos1.distanceTo(this.pos2), Math.min(5, this.ticksExisted - 2), 0.3F, 1.0F, this.pos1, this.pos2);
