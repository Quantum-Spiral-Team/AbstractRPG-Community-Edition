    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            for (int ss = 0; ss < this.rand.nextInt(8) + 14; ss++) {
                GUNParticle part = new GUNParticle(tex, 0.04F + this.rand.nextFloat() / 30.0F, 0.07F, 20 + this.rand.nextInt(10), -1, this.world, this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ, (float) this.rand.nextGaussian() / 16.0F, (float) this.rand.nextGaussian() / 16.0F + 0.1F, (float) this.rand.nextGaussian() / 16.0F, 0.9F, 0.7F, 1.0F, false, this.rand.nextInt(360), true, 3.0F);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && Weapons.canDealDamageTo(result.entityHit) && !this.world.isRemote) {
                Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.pierce), (3.0F * WeaponParameters.EXLevelSEA_MIDDLE + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack) / 2.0F) * this.magicPower, this.thrower, result.entityHit, true, 1.5F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) / 2.0F, this.posX, this.posY, this.posZ);
