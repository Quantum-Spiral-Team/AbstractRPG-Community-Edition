...
public class FireworkEntity extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 15) {
            ...
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.firework_explode, SoundCategory.AMBIENT, 1.5F, 0.9F + this.rand.nextFloat() / 5.0F, false);
            ...
            for (int i = 0; i < countOfParticles; i++) {
                ...
                GUNParticle particle = new GUNParticle(this.sparkle, 0.07F + (float) this.rand.nextGaussian() / 30.0F, 0.005F, 22 + this.rand.nextInt(6), 240, this.world, this.posX, this.posY, this.posZ, X, Y, Z, red - (float) this.rand.nextGaussian() / 10.0F, green - (float) this.rand.nextGaussian() / 10.0F, blue - (float) this.rand.nextGaussian() / 10.0F, true, 0);
        ...
        if (id == 14) {
            GUNParticle fire = new GUNParticle(this.sparkle, 0.06F + (float) this.rand.nextGaussian() / 30.0F, 0.005F, 7, 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, this.redh - (float) this.rand.nextGaussian() / 10.0F, this.greenh - (float) this.rand.nextGaussian() / 10.0F, this.blueh - (float) this.rand.nextGaussian() / 10.0F, true, 0);
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity)) {
                        ...
                        Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, true, false, this, WeaponDamage.explode), parameters.getEnchantedF("damage", might) + flyBonus, this.getThrower(), entity, true, parameters.getEnchantedF("knockback", impulse), this.posX, this.posY, this.posZ);
