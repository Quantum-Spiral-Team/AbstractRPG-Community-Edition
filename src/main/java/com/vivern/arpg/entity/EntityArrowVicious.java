...
public class EntityArrowVicious extends AbstractArrow {
        ...
        if (this.hasMoreArrows) {
            if (this.ticksExisted >= 3 && !this.inGround && this.pickupStatus == PickupStatus.ALLOWED) {
                ...
                if (!this.world.isRemote) {
                    ...
                    newarrow.shoot(null, -this.rotationPitch, -this.rotationYaw + 3.0F + this.rand.nextInt(3), 0.0F, (float) GetMOP.getSpeed(this), 2.0F);
                    ...
                    newarrow2.shoot(null, -this.rotationPitch, -this.rotationYaw - 3.0F - this.rand.nextInt(3), 0.0F, (float) GetMOP.getSpeed(this), 2.0F);
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 9) {
            for (int i = 0; i < 5; i++) {
                GUNParticle part = new GUNParticle(texture, 0.06F + this.rand.nextFloat() * 0.04F, 0.05F, 10 + this.rand.nextInt(10), 170, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 30.0F + (float) this.motionX, (float) this.rand.nextGaussian() / 30.0F + (float) this.motionY, (float) this.rand.nextGaussian() / 30.0F + (float) this.motionZ, 0.5F, 0.7F - this.rand.nextFloat() * 0.2F, 0.55F - this.rand.nextFloat() * 0.1F, false, this.rand.nextInt(360));
