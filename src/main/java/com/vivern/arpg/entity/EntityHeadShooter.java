...
public class EntityHeadShooter extends EntityThrowable implements ISynchronizedEntity {
    ...
    public void onClient(double x, double y, double z, double a, double b, double c) {
        ...
        for (int ss = 0; ss < 8; ss++) {
            GUNParticle sp = new GUNParticle(drop, 0.04F + this.rand.nextFloat() / 20.0F, 0.03F, 50, -1, this.world, x, y, z, prunex + (float) this.rand.nextGaussian() / 20.0F, pruney + (float) this.rand.nextGaussian() / 20.0F, prunez + (float) this.rand.nextGaussian() / 20.0F, 0.8F, 0.1F, 0.1F, false, (int) Math.round(this.rand.nextGaussian() * 20.0), true, 5.0F);
            sp.dropMode = true;
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote) {
                int damagee = GetMOP.approximatelyEqual(this.posY - result.entityHit.posY, result.entityHit.getEyeHeight() + 0.1F, result.entityHit.height / 5.0) ? 2 : 1;
                Weapons.dealDamage(null, (10 + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack)) * damagee, this.thrower, result.entityHit, true, 1.8F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) / 1.5F, this.thrower.posX, this.thrower.posY, this.thrower.posZ);
    ...
    public void onEntityUpdate() {
        ...
        if (this.world.isRemote) {
            if (this.ticksExisted > 2 && this.pos1.lengthSquared() > 1.0E-6 && this.pos2.lengthSquared() > 1.0E-6) {
                BetweenParticle laser = new BetweenParticle(this.world, this.texture, 0.06F, 240, 1.0F, 0.45F, 0.55F, 0.2F, this.pos1.distanceTo(this.pos2), 4, 0.3F, 2.0F, this.pos1, this.pos2);
            ...
            if (this.ticksExisted == 2) {
                GUNParticle bigsmoke1 = new GUNParticle(redsphere, 0.02F, 0.0F, 8, 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
