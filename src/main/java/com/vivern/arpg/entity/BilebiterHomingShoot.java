...
public class BilebiterHomingShoot extends EntityThrowable {
    ...
    public void onUpdate() {
        ...
        if (EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, this.weaponstack) == 0) {
            ...
            if (!list.isEmpty()) {
                for (EntityLivingBase entitylivingbase : list) {
                    if (Team.checkIsOpponent(this.thrower, entitylivingbase) && entitylivingbase != this.getThrower() && EntitySelectors.NOT_SPECTATING.apply(entitylivingbase)) {
            ...
            if (!list.isEmpty()) {
                for (EntityLivingBase entitylivingbasex : list) {
                    if (entitylivingbasex.isCreatureType(EnumCreatureType.MONSTER, false) && Team.checkIsOpponent(this.thrower, entitylivingbasex) && entitylivingbasex != this.getThrower() && EntitySelectors.NOT_SPECTATING.apply(entitylivingbasex) && entitylivingbasex.getHealth() > 0.0F) {
                ...
                if (targ == null) {
                    for (EntityLivingBase entitylivingbasexx : list) {
                        if (Team.checkIsOpponent(this.thrower, entitylivingbasexx) && entitylivingbasexx != this.getThrower() && EntitySelectors.NOT_SPECTATING.apply(entitylivingbasexx) && entitylivingbasexx.getHealth() > 0.0F) {
        ...
        if (targ != null) {
            ...
            this.motionY *= 0.93;
            this.motionZ *= 0.93;
            SuperKnockback.applyKnockback(this, -0.37F, targ.posX, targ.posY + (targ.getEntityBoundingBox().maxY - targ.getEntityBoundingBox().minY) / 2.0, targ.posZ);
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.bb_impact, SoundCategory.AMBIENT, 1.0F, 0.95F + this.rand.nextFloat() / 10.0F, false);

            for (int ss = 0; ss < 3; ss++) {
                TrailParticle trailpart = new TrailParticle(textur, 0.1F, 0.035F, 25, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 8.0F, (float) this.rand.nextGaussian() / 8.0F + 0.15F, (float) this.rand.nextGaussian() / 8.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(180), true, 1.2F, texture5, 0.09F, 240, 1.0F, 1.0F, 1.0F, 0.15F, 5, 0.3F, 1.0F);
                this.world.spawnEntity(trailpart);
            }

            for (int ss = 0; ss < 8; ss++) {
                Entity bubble = new GUNParticle(texturbubble, 0.09F + (float) this.rand.nextGaussian() / 25.0F, 0.03F, 50 + this.rand.nextInt(20), 200, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 9.0F, (float) this.rand.nextGaussian() / 9.0F + 0.15F, (float) this.rand.nextGaussian() / 9.0F, 1.0F, 0.8F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, false, this.rand.nextInt(180), true, 1.3F);
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity)) {
                        Weapons.dealDamage(null, 5 + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack), this.thrower, entity, true, 0.5F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) / 2.5F, this.posX, this.posY, this.posZ);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.typeOfHit == Type.BLOCK && this.world.getBlockState(result.getBlockPos()).getBlock().getCollisionBoundingBox(this.world.getBlockState(result.getBlockPos()), this.world, result.getBlockPos()) != null) {
    ...
    public void onEntityUpdate() {
        ...
        if (this.ticksExisted > 2 && this.world.isRemote && this.pos1.lengthSquared() > 1.0E-6 && this.pos2.lengthSquared() > 1.0E-6) {
            BetweenParticle laser = new BetweenParticle(this.world, texture5, 0.15F, 240, 1.0F, 1.0F, 1.0F, 0.15F, this.pos1.distanceTo(this.pos2), Math.min(5, this.ticksExisted - 2), 0.3F, 1.0F, this.pos1, this.pos2);
