...
public class EntityTimelessSword extends Entity implements ISynchronizedEntity {
    ...
    public EntityTimelessSword(World worldIn, ItemStack weaponstack, EntityPlayer player, Vec3d to, float damage, boolean crit, float knockback, float yaw, float pitch, float rotation) {
        super(worldIn);
    ...
    public void onUpdate() {
        ...
        if (!this.world.isRemote && this.to != null && this.ticksExisted <= 1) {
            ISynchronizedEntity.sendSynchronize(this, 64.0, this.yaw, this.pitch, this.rotation, this.to.x, this.to.y, this.to.z);
        }
    ...
    public void activate() {
        if (this.weaponstack.getItem() instanceof IWeapon) {
            ...
            boolean dead = true;
            if (this.player != null) {
                dead = sweep((IWeapon) this.weaponstack.getItem(), new Vec3d(this.posX, this.posY, this.posZ), this.to, this.player, this.weaponstack, false, this.rand, this.damage, this.crit, this.knockback, this.yaw, this.pitch, this.rotation) || !reuse;
            }
    ...
    public static boolean sweep(IWeapon iweapon, Vec3d from, Vec3d to, final EntityPlayer player, ItemStack stack, boolean fromSword, Random rand, float damage, boolean crit, float knockback, float yaw, float pitch, float rotation) {
        ...
        boolean ret = false;
        if (!list.isEmpty()) {
            world.playSound(null, vec.x, vec.y, vec.z, Sounds.melee_magic, SoundCategory.PLAYERS, 0.7F, 0.9F + rand.nextFloat() / 5.0F);
            if (crit) {
                world.playSound(null, vec.x, vec.y, vec.z, SoundEvents.ENTITY_PLAYER_ATTACK_CRIT, SoundCategory.PLAYERS, 0.7F, 0.9F + rand.nextFloat() / 5.0F);
            }
            ...
            world.playSound(null, vec.x, vec.y, vec.z, Sounds.melee_miss_magic, SoundCategory.PLAYERS, 0.6F, 0.9F + rand.nextFloat() / 5.0F);
        }

        IWeapon.fireEffect(ItemsRegister.TIMELESS_SWORD, player, world, 64.0, vec.x, vec.y, vec.z, rotation, pitch, yaw, 0.0, 0.0, 0.0);
        return ret;
    }
