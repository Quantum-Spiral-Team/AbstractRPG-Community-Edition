...
public class EntityStingerShard extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 6) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.explode_stinger, SoundCategory.AMBIENT, 0.9F, 0.95F + this.rand.nextFloat() / 10.0F, false);

            for (int ss = 0; ss < 10; ss++) {
                Entity bigsmoke = new GUNParticle(this.largesmoke, 0.4F, 0.0F, 12, 80, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 23.0F, (float) this.rand.nextGaussian() / 27.0F, (float) this.rand.nextGaussian() / 23.0F, 1.0F, 1.0F, 1.0F, true, 0);
                this.world.spawnEntity(bigsmoke);
            }

            for (int ss = 0; ss < 8; ss++) {
                Entity fire = new GUNParticle(this.sparkle, 0.07F + (float) this.rand.nextGaussian() / 30.0F, 0.01F, 10, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F + 0.15F, (float) this.rand.nextGaussian() / 14.0F, 1.0F, 0.8F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, 0);
                this.world.spawnEntity(fire);
            }
        }

        if (id == 7) {
            Entity smoke = new GUNParticle(this.smoketex, 0.16F, 0.0F, 7, 80, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, 0);
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity)) {
                        ...
                        Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, true, false, this, WeaponDamage.explode), parameters.getEnchantedF("damage_shard", might), this.getThrower(), entity, true, parameters.getEnchantedF("knockback_shard", impulse), this.posX, this.posY - 0.1, this.posZ);
