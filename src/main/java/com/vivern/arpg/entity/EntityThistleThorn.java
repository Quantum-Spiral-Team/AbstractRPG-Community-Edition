    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            for (int ss = 0; ss < this.rand.nextInt(4) + 2; ss++) {
                GUNParticle part = new GUNParticle(tex, 0.04F + this.rand.nextFloat() / 30.0F, 0.048F, 40, -1, this.world, this.prevPosX, this.prevPosY, this.prevPosZ, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, 1.0F, 1.0F, 0.55F - this.rand.nextFloat() / 10.0F, false, this.rand.nextInt(360), true, 2.5F);
        ...
        if (id == 5) {
            ...
            GUNParticle part = new GUNParticle(sparkle, 0.025F, 0.001F, 14, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 30.0F, 0.0F, (float) this.rand.nextGaussian() / 30.0F, rr, gg, bb, false, this.rand.nextInt(360), true, 2.0F);
        ...
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && Weapons.canDealDamageTo(result.entityHit) && !this.world.isRemote) {
                if (result.entityHit instanceof EntityLivingBase) {
                    ...
                    if (this.charged) {
                        if (entitylivingbase.isPotionActive(PotionEffects.SPIKED) && this.canMultiple && entitylivingbase.getActivePotionEffect(PotionEffects.SPIKED) instanceof Spiked.SpikedEffect) {
                    ...
                    if (!this.charged && (entitylivingbase.getHealth() <= 0.0F || this.rand.nextFloat() < 0.1F) && !NBTHelper.GetNBTboolean(this.weaponstack, "powered")) {
                        NBTHelper.AddNBTint(this.weaponstack, 1, "charge");
                    }
                }

                Weapons.dealDamage(null, (3.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack)) * this.magicPower, this.thrower, result.entityHit, true, 0.5F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) / 2.0F, this.thrower.posX, this.thrower.posY, this.thrower.posZ);
                result.entityHit.hurtResistantTime = 0;
                this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.thistle_impact, SoundCategory.AMBIENT, 0.6F, 0.9F + this.rand.nextFloat() / 5.0F);
