...
public class EntityAcidFire extends StandardBullet {
    ...
    public void onUpdate() {
        ...
        if (this.getThrower() != null && EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, this.weaponstack) > 0) {
            EntityLivingBase entityLivingBase = GetMOP.findNearestEnemy(this.world, this.getThrower(), this.posX, this.posY + this.height / 2.0F, this.posZ, 2.5, false);
            if (entityLivingBase != null) {
                SuperKnockback.applyMove(this, -this.attractPower, entityLivingBase.posX, entityLivingBase.posY + entityLivingBase.height / 2.0F, entityLivingBase.posZ);
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            for (int ss = 0; ss < this.rand.nextInt(4) + 2; ss++) {
                GUNParticle bigsmoke = new GUNParticle(orb, 0.04F, 0.03F, 80, 240, this.world, this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ, (float) this.rand.nextGaussian() / 10.0F, (float) this.rand.nextGaussian() / 10.0F, (float) this.rand.nextGaussian() / 10.0F, 0.9F + this.rand.nextFloat() / 10.0F, 1.0F - this.rand.nextFloat() / 10.0F, 0.35F, true, this.rand.nextInt(360), true, 3.6F);
    ...
    public void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote) {
                ...
                Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.acid), parameters.getEnchantedF("damage", might) * this.magicPower, this.getThrower(), result.entityHit, true, parameters.getEnchantedF("knockback", impulse), this.posX, this.posY, this.posZ);
                ...
                if (result.entityHit instanceof EntityLivingBase && this.potion != null) {
                    ...
                    PotionEffect baff = new PotionEffect(this.potion, Math.min(duration, parameters.getEnchantedI("potion" + mode + "_time_max", witchery)), parameters.getEnchantedI("potion" + mode + "_power", witchery));
                ...
                this.world.playSound(this.posX, this.posY, this.posZ, Sounds.larva_water_attack, SoundCategory.AMBIENT, 0.55F, 1.0F + this.rand.nextFloat() / 10.0F, false);
    ...
    public void onEntityUpdate() {
        ...
        if (this.ticksExisted > 1 && this.world.isRemote && this.pos1.lengthSquared() > 1.0E-6 && this.pos2.lengthSquared() > 1.0E-6) {
            BetweenParticle laser = new BetweenParticle(this.world, texture, 0.1F, 240, 0.9F, 1.0F, 0.3F, 0.17F, this.pos1.distanceTo(this.pos2), Math.min(4, this.ticksExisted - 2), 0.3F, 2.0F, this.pos1, this.pos2);
