    ...
    public void onUpdate() {
        ...
        if (!this.world.isRemote) {
            if (this.ticksExisted > this.delayBeforeGrow) {
                if (!this.sound) {
                    this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.spike_a, SoundCategory.AMBIENT, 0.9F, 1.9F - (float) this.maxSize + this.rand.nextFloat() / 5.0F);
                    this.sound = true;
                }
                ...
                if (!list.isEmpty()) {
                    for (Entity entity : list) {
                        if (!(entity instanceof LordOfPainSpike) && Team.checkIsOpponent(this.thrower, entity)) {
                            Weapons.dealDamage(null, (10.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack) * 2.0F) * this.magicPower, this.thrower, entity, true, 1.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) / 2.0F, this.posX, this.posY - 10.0, this.posZ);
