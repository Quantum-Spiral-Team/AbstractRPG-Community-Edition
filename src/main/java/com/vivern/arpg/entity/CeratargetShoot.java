...
public class CeratargetShoot extends EntityThrowable implements ISynchronizedEntity {
    ...
    public void onUpdate() {
        ...
        if (!this.impacted) {
            ...
            if (this.impactPos != null) {
                ...
                } else if (!this.impactEntity.isDead && (!(this.impactEntity instanceof EntityLivingBase) || !(((EntityLivingBase) this.impactEntity).getHealth() <= 0.0F))) {
                    this.setPosition(this.impactEntity.posX + this.impactPos.x, this.impactEntity.posY + this.impactPos.y, this.impactEntity.posZ + this.impactPos.z);
                } else {
                    this.expl();
                    this.setDead();
                }
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            AnimatedGParticle anim = new AnimatedGParticle(textureExplode, 0.7F + this.rand.nextFloat() * 0.4F, 0.0F, 8, 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
            anim.framecount = 8;
            anim.alphaGlowing = true;
            anim.randomDeath = false;
            this.world.spawnEntity(anim);
            Vec3d mainDirection = new Vec3d(this.rand.nextGaussian() / 10.0, this.rand.nextGaussian() / 10.0, this.rand.nextGaussian() / 10.0);

            for (int ss = 0; ss < this.rand.nextInt(6) + 10; ss++) {
                GUNParticle part = new GUNParticle(this.rand.nextFloat() < 0.6 ? sparkle4 : sparkle2, 0.03F + this.rand.nextFloat() / 30.0F, 0.025F, 40 + this.rand.nextInt(20), 240, this.world, this.posX, this.posY, this.posZ, (float) (this.rand.nextGaussian() / 16.0 + mainDirection.x), (float) (this.rand.nextGaussian() / 16.0 + mainDirection.y), (float) (this.rand.nextGaussian() / 16.0 + mainDirection.z), 1.0F, 0.75F - this.rand.nextFloat() * 0.2F, 0.3F - this.rand.nextFloat() * 0.15F, true, this.rand.nextInt(360));
    ...
    public void expl() {
        if (!this.world.isRemote) {
            this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.ceratarget_impact, SoundCategory.AMBIENT, 1.2F, 0.9F + this.rand.nextFloat() / 5.0F);
            WeaponParameters parameters = WeaponParameters.getWeaponParameters(this.weaponstack.getItem());
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity) && !(entity instanceof CeratargetShoot) && Weapons.canDealDamageTo(entity)) {
                        ...
                        Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, true, false, this, WeaponDamage.explode), parameters.getEnchantedF("damage", might) * this.magicPower, this.getThrower(), entity, true, parameters.getEnchantedF("knockback", impulse), this.posX, this.posY, this.posZ);
    ...
    protected void onImpact(RayTraceResult result) {
        if (!this.impacted) {
            if (result.entityHit != null) {
                if (Team.checkIsOpponent(this.thrower, result.entityHit) && Weapons.canDealDamageTo(result.entityHit) && !this.world.isRemote) {
                    WeaponParameters parameters = WeaponParameters.getWeaponParameters(this.weaponstack.getItem());
                    int might = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack);
                    int impulse = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack);
                    Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.magic), parameters.getEnchantedF("damage", might) * this.magicPower, this.getThrower(), result.entityHit, true, parameters.getEnchantedF("knockback", impulse));
                    ...
                    this.impactPos = new Vec3d(point.x - result.entityHit.posX, point.y - result.entityHit.posY, point.z - result.entityHit.posZ);
                    ISynchronizedEntity.sendSynchronize(this, 64.0, this.impactPos.x, this.impactPos.y, this.impactPos.z, 1.0, result.entityHit.getEntityId(), 0.0);
                    this.motionX = 0.0;
                    this.motionY = 0.0;
                    this.motionZ = 0.0;
                    this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.normal_projectile, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
                ...
                if (!this.world.isRemote) {
                    ...
                    ISynchronizedEntity.sendSynchronize(this, 64.0, this.impactPos.x, this.impactPos.y, this.impactPos.z, 0.0, 0.0, 0.0);
                    this.motionX = 0.0;
                    this.motionY = 0.0;
                    this.motionZ = 0.0;
                    this.world.setEntityState(this, (byte) 8);
