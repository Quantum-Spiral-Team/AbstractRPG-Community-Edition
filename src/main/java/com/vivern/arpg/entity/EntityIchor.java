...
public class EntityIchor extends StandardBullet {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.ichorhit, SoundCategory.AMBIENT, 1.0F, 1.0F, false);
        }
    }
    ...
    public void onImpact(RayTraceResult result) {
        if (!this.world.isRemote && Team.checkIsOpponent(this.thrower, result.entityHit)) {
            ...
            Weapons.dealDamage(new WeaponDamage(this.weaponStack, this.getThrower(), this, false, true, this, WeaponDamage.acid), parameters.getEnchantedF("damage", might) * this.magicPower, this.getThrower(), result.entityHit, true, parameters.getEnchantedF("knockback", impulse));
            result.entityHit.hurtResistantTime = 0;
            if (result.entityHit instanceof EntityLivingBase) {
                PotionEffect baff = parameters.getPotion("ichor", PotionEffects.ICHOR_CURSE, EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.WITCHERY, this.weaponStack));
    ...
    public void onEntityUpdate() {
        ...
        GUNParticle part = new GUNParticle(sparkle, 0.025F, 0.001F, 25, 240, this.world, this.posX + this.rand.nextGaussian() / 20.0, this.posY + this.rand.nextGaussian() / 20.0, this.posZ + this.rand.nextGaussian() / 20.0, gaussianX, gaussianY, gaussianZ, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
        ...
        if (this.world.isRemote && this.pos1.lengthSquared() > 1.0E-6 && this.pos2.lengthSquared() > 1.0E-6) {
            BetweenParticle laser = new BetweenParticle(this.world, this.texture, 0.25F, 240, 1.0F, 1.0F, 1.0F, 0.0F, this.pos1.distanceTo(this.pos2), 5, -0.15F, 9999.0F, this.pos1, this.pos2);
