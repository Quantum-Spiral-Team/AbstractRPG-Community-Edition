...
public class EntityArrowShell extends AbstractArrow {
    ...
    public void onHit(RayTraceResult raytraceResultIn) {
        ...
        if (!this.world.isRemote && raytraceResultIn != null && speed > 1.0 && this.pickupStatus == PickupStatus.ALLOWED) {
            ...
            if (block) {
                for (int i = 0; i < max; i++) {
                    ShellShard newarrow = new ShellShard(this.world, raytraceResultIn.sideHit == EnumFacing.WEST ? poss.x - blockoffset : poss.x, raytraceResultIn.sideHit == EnumFacing.DOWN ? poss.y - blockoffset : poss.y, raytraceResultIn.sideHit == EnumFacing.NORTH ? poss.z - blockoffset : poss.z);
                    newarrow.motionX = raytraceResultIn.sideHit.getAxis() == Axis.X ? raytraceResultIn.sideHit.getXOffset() * 0.5F * power : (this.rand.nextFloat() - 0.5F) * power;
                    newarrow.motionY = raytraceResultIn.sideHit.getAxis() == Axis.Y ? raytraceResultIn.sideHit.getYOffset() * 0.5F * power : (this.rand.nextFloat() - 0.5F) * power;
                    newarrow.motionZ = raytraceResultIn.sideHit.getAxis() == Axis.Z ? raytraceResultIn.sideHit.getZOffset() * 0.5F * power : (this.rand.nextFloat() - 0.5F) * power;
