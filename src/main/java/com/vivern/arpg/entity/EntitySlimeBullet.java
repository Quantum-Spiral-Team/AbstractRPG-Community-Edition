...
public class EntitySlimeBullet extends StandardBullet {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            ...
            for (int ss = 0; ss < 5; ss++) {
                Entity bubble = new GUNParticle(this.texturbubble, 0.08F + this.rand.nextFloat() / 20.0F, 0.03F, 50 + this.rand.nextInt(20), 200, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 9.0F, (float) this.rand.nextGaussian() / 9.0F + 0.15F, (float) this.rand.nextGaussian() / 9.0F, 0.5F, 0.8F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, false, this.rand.nextInt(180), true, 1.3F);
                this.world.spawnEntity(bubble);
            }

            for (int ss = 0; ss < 4; ss++) {
                GUNParticle bubble = new GUNParticle(this.slime, 0.5F + this.rand.nextFloat() / 3.0F, -0.001F, 10 + this.rand.nextInt(10), -1, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 19.0F, (float) this.rand.nextGaussian() / 22.0F + 0.05F, (float) this.rand.nextGaussian() / 19.0F, 0.55F + (float) this.rand.nextGaussian() / 10.0F, 0.9F + (float) this.rand.nextGaussian() / 10.0F, 0.65F + (float) this.rand.nextGaussian() / 10.0F, true, this.rand.nextInt(180));
    ...
    public void onImpact(RayTraceResult result) {
        if (!this.world.isRemote && !this.impacted && result.entityHit != null && Team.checkIsOpponent(this.thrower, result.entityHit)) {
            ...
            Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.acid), parameters.getEnchantedF("impact_damage", might), this.thrower, result.entityHit, true, parameters.getEnchantedF("knockback", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack)), this.posX, this.posY, this.posZ);
            result.entityHit.hurtResistantTime = 4;
            Weapons.setPotionIfEntityLB(result.entityHit, PotionEffects.SLIME, parameters.getEnchantedI("slime_duration_impact", witchery), Math.round(parameters.getEnchantedF("slime_power", witchery)));
        }
    }
        ...
        if (!list.isEmpty()) {
            for (Entity entity : list) {
                if (Team.checkIsOpponent(this.thrower, entity)) {
                    int might = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack);
                    int witchery = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.WITCHERY, this.weaponstack);
                    Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, false, this, WeaponDamage.acid), parameters.getEnchantedF("damage", might), this.thrower, entity, true, parameters.getEnchantedF("knockback", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack)), this.posX, this.posY, this.posZ);
                    entity.hurtResistantTime = 0;
                    Weapons.setPotionIfEntityLB(entity, PotionEffects.SLIME, parameters.getEnchantedI("slime_duration_explode", witchery), Math.round(parameters.getEnchantedF("slime_power", witchery)));
                }
            }
        }
    ...
    public void onEntityUpdate() {
        if (this.ticksExisted > 2 && this.world.isRemote && this.pos1.lengthSquared() > 1.0E-6 && this.pos2.lengthSquared() > 1.0E-6) {
            BetweenParticle laser = new BetweenParticle(this.world, this.trail, 0.1F, 240, 1.0F, 1.0F, 1.0F, 0.15F, this.pos1.distanceTo(this.pos2), Math.min(2, this.ticksExisted - 2), 0.3F, 1.0F, this.pos1, this.pos2);
