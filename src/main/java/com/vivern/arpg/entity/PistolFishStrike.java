    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            for (int ss = 0; ss < this.rand.nextInt(4) + 2; ss++) {
                GUNParticle part = new GUNParticle(tex, 0.08F + this.rand.nextFloat() / 30.0F, 0.028F, 40, -1, this.world, this.prevPosX, this.prevPosY, this.prevPosZ, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, 1.0F, 0.9F, 0.85F - this.rand.nextFloat() / 10.0F, false, this.rand.nextInt(360), true, 1.5F);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote) {
                ...
                Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.pierce), parameters.getEnchantedF("damage", might), this.getThrower(), result.entityHit, true, parameters.getEnchantedF("knockback", impulse), this.posX, this.posY, this.posZ);
                ...
                if (result.entityHit instanceof EntityLivingBase) {
                    ...
                    if (entitylivingbase.getHealth() <= 0.0F && entitylivingbase.deathTime <= 1 && this.rand.nextFloat() < parameters.getEnchantedF("feed_drop_chance", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.REUSE, this.weaponstack))) {
