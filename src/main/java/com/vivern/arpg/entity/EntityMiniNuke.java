...
public class EntityMiniNuke extends Entity implements ISynchronizedEntity {
    ...
    public void onClient(double x, double y, double z, double a, double b, double c) {
        if (a != 0.0) {
            ...
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.explode, SoundCategory.BLOCKS, (float) (1.0 + c * 0.5), 0.8F + this.rand.nextFloat() * 0.4F, false);
            ...
            for (int ss = 0; ss < 13; ss++) {
                GUNParticle bigsmoke = new GUNParticle(this.largesmoke, 2.0F + (float) this.rand.nextGaussian() / 2.0F, -0.001F, 34, 60, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 7.0F, (float) this.rand.nextGaussian() / 7.0F, (float) this.rand.nextGaussian() / 7.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
                bigsmoke.alphaTickAdding = -0.016F;
                this.world.spawnEntity(bigsmoke);
            }

            for (int ss = 0; ss < 17; ss++) {
                GUNParticle fire = new GUNParticle(this.flame, 1.7F + (float) this.rand.nextGaussian() / 5.0F, -0.005F, 20 + this.rand.nextInt(10), 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 4.0F, (float) this.rand.nextGaussian() / 21.0F, (float) this.rand.nextGaussian() / 4.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
            ...
            for (int ss = 0; ss < 17; ss++) {
                GUNParticle fire = new GUNParticle(this.flame, 2.2F + (float) this.rand.nextGaussian() / 5.0F, 0.02F, 30 + this.rand.nextInt(10), 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 27.0F, this.rand.nextFloat() - 0.05F, (float) this.rand.nextGaussian() / 27.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
            ...
            for (int ss = 0; ss < 10; ss++) {
                GUNParticle fire = new GUNParticle(this.flame, 2.0F + (float) this.rand.nextGaussian() / 5.0F, 0.0F, 20 + this.rand.nextInt(20), 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 4.0F, (float) this.rand.nextGaussian() / 4.0F, (float) this.rand.nextGaussian() / 4.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
                fire.alphaTickAdding = -0.025F;
                fire.alphaGlowing = true;
                this.world.spawnEntity(fire);
            }
            ...
            for (Entity entity : list) {
                if (!this.world.isRemote) {
                    if (!(entity instanceof EntityItem)) {
                        ...
                        Weapons.dealDamage(DamageSource.causeExplosionDamage(this.tntPlacedBy), 10.0F * finalPower, this.tntPlacedBy, entity, true, this.power / 2.5F + finalPower / 3.5F, this.posX, this.posY, this.posZ);
                        entity.hurtResistantTime = 0;
