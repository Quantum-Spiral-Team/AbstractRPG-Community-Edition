...
public class EntityAcidBomb extends Entity implements ISynchronizedEntity {
    ...
    public static ParticleTracker.TrackerSmoothShowHide ssh = new ParticleTracker.TrackerSmoothShowHide(null, new Vec3d[]{new Vec3d(0.0, 7.0, 0.6), new Vec3d(8.0, 12.0, -0.9)});
    ...
    public void onClient(double x, double y, double z, double a, double b, double c) {
        if (a != 0.0) {
            ...
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.frozen_explode, SoundCategory.BLOCKS, (float) (1.0 + c * 0.5), 0.8F + this.rand.nextFloat() * 0.4F, false);

            for (int ss = 0; ss < 2.0 * c; ss++) {
                float size = (float) ((0.3 + this.rand.nextGaussian() / 20.0) * c);
                int lt = 20 + this.rand.nextInt(10);
                GUNParticle bigsmoke = new GUNParticle(splash, (float) ((0.3 + this.rand.nextGaussian() / 20.0) * c), -0.005F, lt, 200, this.world, this.posX, this.posY, this.posZ, (float) (this.rand.nextGaussian() * c / 30.0), (float) (this.rand.nextGaussian() * c / 30.0), (float) (this.rand.nextGaussian() * c / 30.0), 0.75F + this.rand.nextFloat() / 5.0F, 1.0F, 0.65F + this.rand.nextFloat() / 5.0F, true, this.rand.nextInt(360));
            ...
            for (int ss = 0; ss < c; ss++) {
                ...
                GUNParticle bigsmoke = new GUNParticle(bubble, size, -0.01F, lt, 180, this.world, this.posX, this.posY, this.posZ, (float) (this.rand.nextGaussian() * c / 40.0), (float) (this.rand.nextGaussian() * c / 40.0), (float) (this.rand.nextGaussian() * c / 40.0), 0.9F + this.rand.nextFloat() / 10.0F, 1.0F, 0.8F + this.rand.nextFloat() / 10.0F, true, this.rand.nextInt(360));
            ...
            for (int i = 0; i < countOfParticles; i++) {
                ...
                GUNParticle particle = new GUNParticle(tox, 0.2F + (float) this.rand.nextGaussian() / 30.0F, 0.01F, 22 + this.rand.nextInt(10), 200, this.world, x, y, z, X, Y, Z, 0.9F + this.rand.nextFloat() / 10.0F, 1.0F, 0.9F + this.rand.nextFloat() / 10.0F, true, this.rand.nextInt(360));
                particle.alphaGlowing = true;
                this.world.spawnEntity(particle);
            }

            for (int ss = 0; ss < 6; ss++) {
                GUNParticle part = new GUNParticle(sptex, (float) (0.1 * c), 0.0F, 12, 200, this.world, this.posX + this.rand.nextGaussian(), this.posY + this.rand.nextGaussian(), this.posZ + this.rand.nextGaussian(), (float) (this.rand.nextGaussian() * c / 25.0), (float) (this.rand.nextGaussian() * c / 25.0), (float) (this.rand.nextGaussian() * c / 25.0), 0.4F + this.rand.nextFloat() / 4.0F, 1.0F, 0.35F + this.rand.nextFloat() / 4.0F, true, this.rand.nextInt(360));
                part.tracker = ssh;
    ...
    public void explode() {
        ...
        if (!list.isEmpty()) {
            for (Entity entity : list) {
                if (!this.world.isRemote) {
                    float finalPower = (float) Math.max(damageRadius - Weapons.getDistanceToMobHitbox(entity, this.posX, this.posY, this.posZ), 0.0);
                    entity.hurtResistantTime = 0;
                    Weapons.dealDamage(DamageSource.causeExplosionDamage(this.tntPlacedBy), 10.0F * finalPower, this.tntPlacedBy, entity, true, finalPower / 2.0F, this.posX, this.posY, this.posZ);
                    entity.hurtResistantTime = 0;
        ...
        if (!this.world.isRemote) {
            ISynchronizedEntity.sendSynchronize(this, 64.0, this.posX, this.posY + this.height / 2.0F, this.posZ, 0.0, 0.0, this.power);
        }

        if (this.world.isAreaLoaded(this.getPosition().add(-16, -16, -16), this.getPosition().add(16, 16, 16))) {
            for (BlockPos blockpos : GetMOP.getPosesInsideSphere((int) this.posX, (int) this.posY, (int) this.posZ, (int) (this.power * 0.3))) {
            ...
            for (int i = 0; i <= this.power * 3.0F; i++) {
                Vec3d pos2 = new Vec3d(this.posX + this.rand.nextInt(radius + 1) - radius, this.posY + this.rand.nextInt(radius + 1) - radius, this.posZ + this.rand.nextInt(radius + 1) - radius);
                RayTraceResult res = this.world.rayTraceBlocks(pos1, pos2, false);
                if (res != null && res.typeOfHit == Type.BLOCK && res.getBlockPos() != null && Weapons.easyBreakBlockFor(this.world, this.power * 0.5F, res.getBlockPos())) {
                    this.world.setBlockState(res.getBlockPos(), BlocksRegister.FLUID_BIOGENIC_ACID.getDefaultState().withProperty(BlockFluidClassic.LEVEL, 1));
                }
            }
            ...
            if (this.power > 0.0F) {
                ...
                for (int ix = 0; ix < this.power; ix++) {
                    ...
                    AcidBombBlob expl = new AcidBombBlob(this.world, this.tntPlacedBy, this.posX, this.posY + this.height / 2.0F, this.posZ);
                    expl.motionX = X;
                    expl.motionY = Y;
                    expl.motionZ = Z;
        ...
        protected void onImpact(RayTraceResult result) {
            if (result.entityHit != null) {
                if (!this.world.isRemote && !(result.entityHit instanceof AcidBombBlob)) {
                    Weapons.dealDamage(DamageSource.causeExplosionDamage(this.thrower), this.damage, this.thrower, result.entityHit, true, 1.5F, this.posX, this.posY, this.posZ);
                ...
                if (Weapons.easyBreakBlockFor(this.world, 5.0F, result.getBlockPos())) {
                    ...
                    if (GetMOP.collidesWithAnyBlock(this.world, pos) && (this.world.isAirBlock(pos) || Weapons.easyBreakBlockFor(this.world, 5.0F, pos))) {
                        this.world.setBlockState(pos, BlocksRegister.FLUID_BIOGENIC_ACID.getDefaultState());
                    }
                }
