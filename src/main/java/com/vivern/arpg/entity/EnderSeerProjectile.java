...
public class EnderSeerProjectile extends EntityThrowable {
    ...
    public void onUpdate() {
        ...
        this.motionZ /= 1.08;
        if (this.target != null) {
            SuperKnockback.applyMove(this, -0.3F, this.target.posX, this.target.posY + this.target.height / 2.0F, this.target.posZ);
        }
    }
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.ender_seer_projectile, SoundCategory.AMBIENT, 0.6F, 0.9F + this.rand.nextFloat() / 5.0F, false);

            for (int ss = 0; ss < 8; ss++) {
                GUNParticle fire = new GUNParticle(this.texturexpl, 0.2F + (float) this.rand.nextGaussian() / 5.0F, -0.005F, 23 + this.rand.nextInt(10), 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 27.0F, (float) this.rand.nextGaussian() / 27.0F + 0.05F, (float) this.rand.nextGaussian() / 27.0F, 0.8F + (float) this.rand.nextGaussian() / 5.0F, 0.5F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, this.rand.nextInt(100) - 50);
    ...
    protected void onImpact(RayTraceResult result) {
        if ((result.entityHit != null || this.world.getBlockState(result.getBlockPos()).getBlock().getCollisionBoundingBox(this.world.getBlockState(result.getBlockPos()), this.world, result.getBlockPos()) != null) && result.entityHit != this.getThrower() && !this.world.isRemote) {
    ...
    public void onEntityUpdate() {
        ...
        if (this.ticksExisted > 2 && this.world.isRemote && this.pos1.lengthSquared() > 1.0E-6 && this.pos2.lengthSquared() > 1.0E-6) {
            BetweenParticle laser = new BetweenParticle(this.world, this.texture, 0.2F, 240, 0.8F, 0.5F, 1.0F, 0.17F, this.pos1.distanceTo(this.pos2), Math.min(6, this.ticksExisted - 2), 0.3F, 2.0F, this.pos1, this.pos2);
