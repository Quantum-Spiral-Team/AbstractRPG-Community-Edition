...
public class LightningStrike extends Entity implements ISynchronizedEntity {
    ...
    public void onClient(double x, double y, double z, double a, double b, double c) {
        ...
        BetweenParticle laser = new BetweenParticle(this.world, texture, 0.3F + this.rand.nextFloat(), 240, 1.0F, 1.0F, 1.0F, 0.1F, pos1.distanceTo(pos2), 10, 0.0F, 9999.0F, pos1, pos2);
    ...
    public void onUpdate() {
        ...
        if (!this.world.isRemote) {
            ...
            if (this.mainPoint == null) {
                ...
                RayTraceResult result = GetMOP.fixedRayTraceBlocks(this.world, this.thrower, this.raySize, false, this.mainPoint, nextMain, false, true, false);
                if (result != null && result.hitVec != null) {
                    ISynchronizedEntity.sendSynchronize(this, 64.0, this.mainPoint.x, this.mainPoint.y, this.mainPoint.z, result.hitVec.x, result.hitVec.y, result.hitVec.z);
                } else {
                    ISynchronizedEntity.sendSynchronize(this, 64.0, this.mainPoint.x, this.mainPoint.y, this.mainPoint.z, nextMain.x, nextMain.y, nextMain.z);
                }
                ...
                if (this.rand.nextFloat() > this.endChance) {
                    ...
                    if (resultx != null && resultx.typeOfHit != Type.MISS && resultx.typeOfHit != Type.ENTITY) {
                        if (resultx != null && resultx.entityHit != null) {
                            resultx.entityHit.attackEntityFrom(this.thrower == null ? DamageSource.LIGHTNING_BOLT : DamageSource.causeMobDamage(this.thrower), this.damage);
                    ...
                    if (resultx != null && resultx.hitVec != null) {
                        ISynchronizedEntity.sendSynchronize(this, 64.0, vec.x, vec.y, vec.z, resultx.hitVec.x, resultx.hitVec.y, resultx.hitVec.z);
                    } else {
                        ISynchronizedEntity.sendSynchronize(this, 64.0, vec.x, vec.y, vec.z, next.x, next.y, next.z);
                    }
                }
            }
        ...
        if (this.ticksExisted == 1) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.lightning, SoundCategory.WEATHER, this.soundVolume, 0.85F + this.rand.nextFloat() / 4.0F, false);
        }
    }

    public Vec3d nextStrike(Vec3d vec) {
        return new Vec3d(vec.x + this.rand.nextGaussian(), vec.y - this.rand.nextFloat() * this.sizeY, vec.z + this.rand.nextGaussian());
    }
    ...
    public Vec3d nextMainStrike(Vec3d vec) {
        ...
        Vec3d ret = new Vec3d(vec.x + this.rand.nextGaussian() * displMult, vec.y - this.rand.nextFloat() * this.sizeY, vec.z + this.rand.nextGaussian() * displMult);
