...
public class ChlorineCloud extends EntityThrowable {
    ...
    public static ParticleTracker.TrackerSmoothShowHide trssh = new ParticleTracker.TrackerSmoothShowHide(new Vec3d[]{new Vec3d(0.0, 15.0, 0.0666), new Vec3d(20.0, 45.0, -0.04)}, null);
    ...
    public void onEntityUpdate() {
        ...
        if (this.world.isRemote && this.rand.nextFloat() < 0.2F) {
            GUNParticle bigsmoke = new GUNParticle(this.largesmoke, 1.5F + (float) this.rand.nextGaussian() / 3.0F, 3.0E-4F, 45, 110, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 17.0F, (float) this.rand.nextGaussian() / 17.0F, (float) this.rand.nextGaussian() / 17.0F, 0.8F + this.rand.nextFloat() * 0.2F, 0.8F + this.rand.nextFloat() * 0.2F, 0.4F, true, this.rand.nextInt(360), true, 1.0F);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit == null) {
            ...
            if (result != null && result.getBlockPos() != null && block.getCollisionBoundingBox(this.world.getBlockState(result.getBlockPos()), this.world, result.getBlockPos()) != null && block != BlocksRegister.CHLORINE_BELCHER) {
