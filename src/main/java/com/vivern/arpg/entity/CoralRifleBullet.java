...
public class CoralRifleBullet extends StandardBullet implements ISynchronizedEntity, RenderModule.IRenderModuleMulticolored {
    ...
    public void onClient(double x, double y, double z, double a, double b, double c) {
        for (int ss = 0; ss < 3; ss++) {
            ...
            GUNParticle bigsmoke = new GUNParticle(star, scl, 0.0F, 20, 240, this.world, x, y, z, (float) this.rand.nextGaussian() / delSpd, (float) this.rand.nextGaussian() / delSpd, (float) this.rand.nextGaussian() / delSpd, this.getRED(), this.getGREEN(), this.getBLUE(), true, this.rand.nextInt(360));
        ...
        for (int ss = 0; ss < this.rand.nextInt(3) + 2; ss++) {
            GUNParticle part = new GUNParticle(shards[this.rand.nextInt(3)], 0.03F + this.rand.nextFloat() / 35.0F, 0.08F, 30 + this.rand.nextInt(20), -1, this.world, x, y, z, (float) this.rand.nextGaussian() / 13.0F, (float) this.rand.nextGaussian() / 13.0F + 0.1F, (float) this.rand.nextGaussian() / 13.0F, this.getRED(), this.getGREEN(), this.getBLUE(), false, this.rand.nextInt(360), true, 4.0F);
            part.dropMode = true;
            part.rotationPitchYaw = new Vec2f(this.rand.nextInt(360), this.rand.nextInt(360));
            part.tracker = new ParticleTracker.TrackerGlassShard((float) this.rand.nextGaussian() * 2.0F, (float) this.rand.nextGaussian() * 2.0F, (int) this.rand.nextGaussian() * 2, true);
