...
public class StingerBoltEntity extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.explode_stinger, SoundCategory.AMBIENT, 1.0F, 0.95F + this.rand.nextFloat() / 10.0F, false);

            for (int ss = 0; ss < 15; ss++) {
                Entity bigsmoke = new GUNParticle(this.largesmoke, 0.5F, 0.0F, 14, 80, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 20.0F, (float) this.rand.nextGaussian() / 25.0F, (float) this.rand.nextGaussian() / 20.0F, 1.0F, 1.0F, 1.0F, true, 0);
                this.world.spawnEntity(bigsmoke);
            }

            for (int ss = 0; ss < 10; ss++) {
                GUNParticle fire = new GUNParticle(this.sparkle, 0.07F + (float) this.rand.nextGaussian() / 30.0F, 0.01F, 13, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 12.0F, (float) this.rand.nextGaussian() / 12.0F + 0.15F, (float) this.rand.nextGaussian() / 12.0F, 1.0F, 0.8F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, 0);
    ...
    public void expl(RayTraceResult result) {
        if (!this.world.isRemote) {
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity)) {
                        ...
                        Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, true, false, this, WeaponDamage.explode), parameters.getEnchantedF("damage", might), this.getThrower(), entity, true, parameters.getEnchantedF("knockback", impulse));
            ...
            for (int ss = 0; ss < parameters.getEnchantedF("shards", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, this.weaponstack)); ss++) {
                EntityStingerShard entit = new EntityStingerShard(this.world, this.getThrower(), this.rand.nextGaussian() / 3.0, this.rand.nextGaussian() / 3.0, this.rand.nextGaussian() / 3.0, this);
