...
public class CrystalFanShoot extends EntityThrowable {
    ...
    public void onUpdate() {
        ...
        if (this.powerOn && this.ticksExisted == this.randomSound) {
            this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.crystal_fan, SoundCategory.AMBIENT, 0.5F, 0.9F + this.rand.nextFloat() / 5.0F);
        }
    }
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.glass_break, SoundCategory.AMBIENT, 0.55F, 0.95F + this.rand.nextFloat() / 10.0F, false);

            for (int ss = 0; ss < this.rand.nextInt(5) + 4; ss++) {
                GUNParticle part = new GUNParticle(shards[this.rand.nextInt(3)], 0.06F + this.rand.nextFloat() / 30.0F, 0.08F, 30 + this.rand.nextInt(20), -1, this.world, this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ, (float) this.rand.nextGaussian() / 13.0F, (float) this.rand.nextGaussian() / 13.0F + 0.1F, (float) this.rand.nextGaussian() / 13.0F, 0.9F, 0.3F, 1.0F, false, this.rand.nextInt(360), true, 1.2F);
                part.dropMode = true;
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote) {
                Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.shards), this.damage * this.magicPower, this.getThrower(), result.entityHit, true, (this.powerOn ? 2.0F : 0.7F) + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) / 2.0F, this.posX, this.posY, this.posZ);
                result.entityHit.hurtResistantTime = 0;
                if (result.entityHit instanceof EntityLivingBase) {
                    EntityLivingBase entitylivingbase = (EntityLivingBase) result.entityHit;
                    if (entitylivingbase.getHealth() <= 0.0F) {
                        if (!this.powerOn) {
                            if (this.canSpawnBonus && entitylivingbase.deathTime < 1 && this.rand.nextFloat() < 0.2F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.REUSE, this.weaponstack) / 10.0F) {
