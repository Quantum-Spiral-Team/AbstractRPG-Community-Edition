...
public class WandColdWave extends EntityThrowable implements ISynchronizedEntity, RenderModule.IRenderModuleOverride {
    ...
    static ParticleTracker.TrackerSmoothShowHide tssh = new ParticleTracker.TrackerSmoothShowHide(new Vec3d[]{new Vec3d(0.0, 4.0, 0.25), new Vec3d(6.0, 17.0, -0.09)}, null);
    ...
    public void onUpdate() {
        ...
        if (this.ticksExisted < 2 || this.ticksExisted % 10 == 0) {
            ...
            this.bladeNormal = GetMOP.rotateVecAroundAxis(GetMOP.pitchYawToVec3D(this.rotationPitch - 90.0F, -this.rotationYaw), look, (float) Math.toRadians(this.rotationRoll));
        ...
        if (!this.world.isRemote) {
            ...
            if (!this.isDead) {
                ...
                AxisAlignedBB aabb = new AxisAlignedBB(this.min(rightPos.x, leftPos.x, prevRightPos.x, prevLeftPos.x), this.min(rightPos.y, leftPos.y, prevRightPos.y, prevLeftPos.y), this.min(rightPos.z, leftPos.z, prevRightPos.z, prevLeftPos.z), this.max(rightPos.x, leftPos.x, prevRightPos.x, prevLeftPos.x), this.max(rightPos.y, leftPos.y, prevRightPos.y, prevLeftPos.y), this.max(rightPos.z, leftPos.z, prevRightPos.z, prevLeftPos.z));
                aabb = aabb.grow(0.25);

                for (Entity entity : this.world.getEntitiesWithinAABBExcludingEntity(this, aabb)) {
                    if (GetMOP.isBoxInPlane(center, this.bladeNormal, entity.getEntityBoundingBox().grow(0.2F)) && Team.checkIsOpponent(this.thrower, entity) && !this.impacted.contains(entity)) {
                        ...
                        Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, false, damageFrom, WeaponDamage.blade), this.damage * this.magicPower, this.thrower, entity, true, this.knockback, damageFrom.x, damageFrom.y, damageFrom.z);
                        ...
                        Weapons.setPotionIfEntityLB(entity, PotionEffects.COLD_SOUL, parameters.getEnchantedI("potion_time", witchery), parameters.getEnchantedI("potion_power", witchery));
            ...
            GUNParticle bigsmoke = new GUNParticle(snowflake3, scl, 0.0F, lt, 200, this.world, partpos.x, partpos.y, partpos.z, 0.0F, 0.0F, 0.0F, 0.6F + this.rand.nextFloat() * 0.2F, 0.9F + this.rand.nextFloat() / 10.0F, 1.0F, true, this.rand.nextInt(360));
            ...
            bigsmoke = new GUNParticle(mana_flow, scl, 0.0F, lt, 200, this.world, partpos.x, partpos.y, partpos.z, 0.0F, 0.0F, 0.0F, 0.4F + this.rand.nextFloat() * 0.2F, 0.8F + this.rand.nextFloat() * 0.2F, 1.0F, true, this.rand.nextInt(360));
            ...
            if (this.ticksExisted % 2 == 0) {
                ...
                lt = 17;
                bigsmoke = new GUNParticle(largecloud, scl, 0.0F, lt, 200, this.world, partpos.x, partpos.y, partpos.z, 0.0F, 0.0F, 0.0F, 0.4F + this.rand.nextFloat() * 0.2F, 0.7F + this.rand.nextFloat() / 10.0F, 0.8F, true, this.rand.nextInt(360));
                bigsmoke.alpha = 0.0F;
                bigsmoke.alphaGlowing = true;
                bigsmoke.randomDeath = false;
    ...
    public void onClient(double... args) {
        if (args.length == 1) {
            ...
            for (int ss = 0; ss < 21; ss++) {
                ...
                GUNParticle bigsmoke = new GUNParticle(mana_flow, scl, 0.0F, lt, 200, this.world, partpos.x, partpos.y, partpos.z, (float) this.rand.nextGaussian() / 25.0F, (float) this.rand.nextGaussian() / 25.0F, (float) this.rand.nextGaussian() / 25.0F, 0.8F, 0.9F + this.rand.nextFloat() / 10.0F, 1.0F, true, this.rand.nextInt(360));
                bigsmoke.alphaTickAdding = -0.5F / lt;
                bigsmoke.alphaGlowing = true;
                bigsmoke.scaleTickAdding = -scl / lt * 0.95F;
                bigsmoke.randomDeath = false;
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.whispers_blade_impact, SoundCategory.AMBIENT, 1.1F, 0.9F + this.rand.nextFloat() * 0.2F, false);
        }
    }
    ...
    protected void onImpact(RayTraceResult result) {
        if (!this.world.isRemote && result.entityHit == null && this.world.getBlockState(result.getBlockPos()).getBlock().getCollisionBoundingBox(this.world.getBlockState(result.getBlockPos()), this.world, result.getBlockPos()) != null) {
