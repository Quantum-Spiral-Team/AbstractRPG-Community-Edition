...
public class PlasmaPistolShoot extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.plasma_pistol_impact, SoundCategory.AMBIENT, 0.9F, 0.9F + this.rand.nextFloat() / 5.0F, false);
            GUNParticle bigsmoke1 = new GUNParticle(explode, 0.1F, 0.0F, 13, 240, this.world, this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ, 0.0F, 0.0F, 0.0F, 1.0F, 0.9F + this.rand.nextFloat() / 10.0F, 0.9F + this.rand.nextFloat() / 10.0F, true, this.rand.nextInt(360));
        ...
        if (id == 5) {
            GUNParticle fire2 = new GUNParticle(cloud, 0.2F, 0.0F, 11, 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 0.5F, 1.0F, 0.8F + (float) this.rand.nextGaussian() / 5.0F, true, this.rand.nextInt(360));
    ...
    public void expl() {
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity)) {
                        Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.plasma), parameters.getEnchantedF("damage", might), this.getThrower(), entity, true, parameters.getEnchantedF("knockback", impulse), this.posX, this.posY, this.posZ);
        ...
        if (!this.world.isRemote) {
            if (EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, this.weaponstack) > 0) {
                ...
                HostileProjectiles.Plasma plasma = new HostileProjectiles.Plasma(this.world, this.getThrower(), (float) damageRadius * 0.6F, 0.0F, 80, this.posX, this.posY, this.posZ, plasmadamage, plasmaknockback, 2, false, r, g, b);
