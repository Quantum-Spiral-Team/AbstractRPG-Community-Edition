...
public class EntityFiremageSetBonus extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            ...
            for (int ss = 0; ss < 8; ss++) {
                GUNParticle fire = new GUNParticle(this.flame, 0.2F + (float) this.rand.nextGaussian(), -0.005F, 23 + this.rand.nextInt(10), 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 17.0F, (float) this.rand.nextGaussian() / 17.0F + 0.05F, (float) this.rand.nextGaussian() / 17.0F, 1.0F, 0.8F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, this.rand.nextInt(100) - 50);
        ...
        if (id == 8) {
            GUNParticle fire2 = new GUNParticle(this.flame, 0.1F + (float) this.rand.nextGaussian() / 15.0F, -0.009F, 10 + this.rand.nextInt(5), 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 1.0F, 0.8F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, this.rand.nextInt(100) - 50);
    ...
    protected void onImpact(RayTraceResult result) {
        if (Team.checkIsOpponent(this.thrower, result.entityHit) && result.entityHit != null && (result.entityHit instanceof EntityLivingBase || result.entityHit instanceof MultiPartEntityPart) && result.entityHit.isCreatureType(EnumCreatureType.MONSTER, false)) {
            this.expl();
        }
    }
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity)) {
                        Weapons.dealDamage(null, 8.0F * this.magicPower, this.thrower, entity, true, 1.5F, this.posX, this.posY - 0.3, this.posZ);
