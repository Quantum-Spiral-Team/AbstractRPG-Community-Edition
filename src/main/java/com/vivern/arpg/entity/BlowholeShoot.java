...
public class BlowholeShoot extends EntityThrowable implements ISynchronizedEntity {
    ...
    public void onUpdate() {
        ...
        if (this.bubbleSize >= 0.8F) {
            ...
            AxisAlignedBB aabb = new AxisAlignedBB(this.posX - siz, this.posY - siz + this.height / 2.0F, this.posZ - siz, this.posX + siz, this.posY + siz + this.height / 2.0F, this.posZ + siz);
        ...
        if (this.ticksExisted > 80) {
            this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.blowhole_impact, SoundCategory.AMBIENT, 0.8F + this.bubbleSize / 5.0F, 1.25F + this.rand.nextFloat() / 10.0F - this.bubbleSize / 5.0F);
    ...
    public void onClient(double x, double y, double z, double a, double b, double c) {
        if (a == 0.0) {
            ...
            if (this.firstUpdate1 && x >= 0.8) {
                ...
                Minecraft.getMinecraft().getSoundHandler().playSound(new MovingSoundEntity(this, Sounds.blowhole_fly, SoundCategory.PLAYERS, 0.4F + this.bubbleSize / 5.0F, 1.25F + this.rand.nextFloat() / 10.0F - this.bubbleSize / 5.0F, true));
            }
        } else {
            for (int ss = 0; ss < 2.0F + this.bubbleSize; ss++) {
                ...
                GUNParticle bubble = new GUNParticle(waterhammer, 0.04F, -0.001F, lt, 130, this.world, x, y, z, 0.0F, 0.0F, 0.0F, 0.5F - this.rand.nextFloat() / 5.0F, 0.8F - this.rand.nextFloat() / 5.0F, 1.0F - this.rand.nextFloat() / 10.0F, true, this.rand.nextInt(180));
            ...
            for (int i = 0; i < countOfParticles; i++) {
                ...
                GUNParticle particle = new GUNParticle(this.rand.nextFloat() < 0.5F ? bubbleglow1 : bubbleglow2, 0.05F + this.bubbleSize / 30.0F + (float) this.rand.nextGaussian() / 40.0F, 0.025F, 22 + this.rand.nextInt(16), 150, this.world, x + X, y + Y, z + Z, X / reduceSpeed, Y / reduceSpeed, Z / reduceSpeed, 0.5F - this.rand.nextFloat() / 5.0F, 0.8F - this.rand.nextFloat() / 5.0F, 1.0F - this.rand.nextFloat() / 5.0F, true, 0, true, 2.0F);
    ...
    public void onImpact(Entity entity, boolean fromSpecialExplode) {
        ...
        if (Team.checkIsOpponent(this.thrower, entity) && !this.impacted.contains(entity) && !this.world.isRemote) {
            ...
            Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, this.bubbleSize < 0.8, this, WeaponDamage.water), parameters.getEnchantedF("damage", might), this.getThrower(), entity, true, parameters.getEnchantedF("knockback", impulse), this.posX, this.posY, this.posZ);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            ...
            if (result.hitVec != null) {
                ISynchronizedEntity.sendSynchronize(this, 64.0, result.hitVec.x, result.hitVec.y, result.hitVec.z, 1.0, 0.0, 0.0);
            }
    ...
    public boolean specialExplode() {
        if (EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, this.weaponstack) > 0 && this.bubbleSize > 0.8F) {
            ...
            for (int i = 0; i < this.bubbleSize * 4.0F; i++) {
                ...
                HostileProjectiles.Plasma plasma = new HostileProjectiles.Plasma(this.world, this.getThrower(), 0.9F, 0.0F, 100, this.posX + X, this.posY + Y, this.posZ + Z, plasmadamage, plasmaknockback, 1, false, bright * 0.57F, 0.36F + 0.55F * bright, 0.45F + this.rand.nextFloat() * 0.12F + 0.43F * bright);
            ...
            if (this.getThrower() instanceof EntityPlayer) {
                ...
                for (BlockPos blockpos : GetMOP.getBlockPosesCollidesAABB(this.world, aabb.grow(2.0), false)) {
                    if (blockpos.distanceSqToCenter(this.posX, hposY, this.posZ) < radiusSq && Weapons.easyBreakBlockFor(this.world, hardnessBreaks, blockpos)) {
                        Weapons.destroyBlockAsTool(this.world, blockpos, player, "pickaxe", 5, fortune, silktouch);
                    }
                }
            }

            this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.blowhole_special, SoundCategory.AMBIENT, 0.9F + this.bubbleSize / 5.0F, 1.25F + this.rand.nextFloat() / 10.0F - this.bubbleSize / 5.0F);
            this.setDead();
            return true;
        } else {
            return false;
