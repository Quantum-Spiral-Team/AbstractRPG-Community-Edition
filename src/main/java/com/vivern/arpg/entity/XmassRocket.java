    ...
    public void onUpdate() {
        ...
        if (this.world.isRemote) {
            if (this.rand.nextFloat() < Math.max(1.0F - this.ticksExisted / 20.0F, 0.1F)) {
                ...
                GUNParticle part = new GUNParticle(star2, scl, 0.0F, lt, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 60.0F, (float) this.rand.nextGaussian() / 60.0F, (float) this.rand.nextGaussian() / 60.0F, 0.7F + this.rand.nextFloat() * 0.3F, 0.7F + this.rand.nextFloat() * 0.3F, 0.25F, true, this.rand.nextInt(360));
            ...
            this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.xmass_launcher_impact, SoundCategory.AMBIENT, 1.4F, 0.9F + this.rand.nextFloat() / 5.0F);
        ...
        if (args.length == 3 || args.length == 4) {
            ...
            if (!alternative) {
                for (int ss = 0; ss < 50; ss++) {
                    this.world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, x + this.rand.nextDouble() - 0.5, y + this.rand.nextDouble() - 0.5, z + this.rand.nextDouble() - 0.5, this.rand.nextGaussian() / 2.0, this.rand.nextGaussian() / 2.0, this.rand.nextGaussian() / 2.0, new int[]{leavesId});
                }
            }
            ...
            for (int ss = 0; ss < 4; ss++) {
                GUNParticle part1 = new GUNParticle(sparkle3, (0.5F + this.rand.nextFloat() * 0.6F) * scalemult, 0.0F, 5, 240, this.world, x, y, z, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
            ...
            for (int ss = 0; ss < 14; ss++) {
                ...
                float scl = 0.1F + this.rand.nextFloat() * 0.07F;
                GUNParticle part = new GUNParticle(star2, scl, 0.0F, lt, 240, this.world, x, y, z, (float) this.rand.nextGaussian() / 10.0F, (float) this.rand.nextGaussian() / 10.0F, (float) this.rand.nextGaussian() / 10.0F, 0.7F + this.rand.nextFloat() * 0.3F, 0.7F + this.rand.nextFloat() * 0.3F, 0.25F, true, this.rand.nextInt(360));
                part.scaleTickAdding = -scl / lt;
                part.tracker = ptss;
                part.alphaGlowing = true;
                if (alternative) {
            ...
            if (!alternative) {
                GUNParticle part1 = new GUNParticle(circle2, 0.5F + this.rand.nextFloat() * 0.6F, 0.0F, 6, 240, this.world, x, y, z, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 0.8F, true, this.rand.nextInt(360));
    ...
    public void spawnBalls(boolean alternative) {
        ...
        if (alternative) {
            for (int i = 0; i < balls; i++) {
                XmassBall ball = this.thrower == null ? new XmassBall(this.world, this.weaponstack) : new XmassBall(this.world, this.thrower, this.weaponstack);
                Vec3d start = new Vec3d(this.posX + (this.rand.nextDouble() - 0.5) * 0.5, this.posY + (this.rand.nextDouble() - 0.5) * 0.5, this.posZ + (this.rand.nextDouble() - 0.5) * 0.5);
            ...
            for (int i = 0; i < balls; i++) {
                ...
                for (int j = 0; j < 16; j++) {
                    ...
                    Vec3d start = new Vec3d(this.posX + (this.rand.nextDouble() - 0.5) * 0.5, this.posY + (this.rand.nextDouble() - 0.5) * 0.5, this.posZ + (this.rand.nextDouble() - 0.5) * 0.5);
                    Vec3d end = start.add((this.rand.nextDouble() - 0.5) * veclength, (this.rand.nextDouble() - 0.5) * veclength, (this.rand.nextDouble() - 0.5) * veclength);
                    RayTraceResult rayTraceResult = GetMOP.fixedRayTraceBlocks(this.world, this.thrower, 0.1, true, start, end, false, true, false);
                    boolean hitEntity = rayTraceResult != null && rayTraceResult.entityHit != null && Team.checkIsOpponent(this.thrower, rayTraceResult.entityHit);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote) {
                ...
                WeaponParameters parameters = WeaponParameters.getWeaponParameters(this.weaponstack.getItem());
                int impulse = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack);
                Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, true, true, this, WeaponDamage.explode), damage, this.getThrower(), result.entityHit, true, parameters.getEnchantedF("knockback", impulse), this.posX, this.posY, this.posZ);
