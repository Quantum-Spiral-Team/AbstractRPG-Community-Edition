    ...
    public void onClient(double x, double y, double z, double a, double b, double c) {
        if (a != 0.0) {
            ...
            for (int ss = 0; ss < 3.0 * c; ss++) {
                ...
                GUNParticle bigsmoke = new GUNParticle(largecloud, (float) ((0.6 + this.rand.nextGaussian() / 10.0) * c), -0.005F, lt, 240, this.world, this.posX, this.posY, this.posZ, (float) (this.rand.nextGaussian() * c / 20.0), (float) (this.rand.nextGaussian() * c / 20.0), (float) (this.rand.nextGaussian() * c / 20.0), 0.5F + this.rand.nextFloat() / 10.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
            ...
            for (int i = 0; i < countOfParticles; i++) {
                ...
                GUNParticle particle = new GUNParticle(snow, 0.55F + (float) this.rand.nextGaussian() / 30.0F, 0.01F, 22 + this.rand.nextInt(10), 180, this.world, x, y, z, X, Y, Z, 0.75F + this.rand.nextFloat() / 10.0F, 0.9F, 1.0F, false, this.rand.nextInt(360), true, 1.0F);
            ...
            for (int ss = 0; ss < 3; ss++) {
                ...
                GUNParticle part = new GUNParticle(circle, 1.0F, 0.0F, lt, 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 0.35F + this.rand.nextFloat() / 5.0F, 0.8F, 1.0F, true, this.rand.nextInt(360));
        ...
        if (!this.world.isRemote) {
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    float finalPower = Math.max(this.power - (float) Weapons.getDistanceToMobHitbox(entity, this.posX, this.posY, this.posZ), 0.0F);
                    if (entity.isBurning()) {
                        finalPower = (float) (finalPower * 0.7);
                    }

                    entity.hurtResistantTime = 0;
                    Weapons.dealDamage(DamageSource.causeExplosionDamage(this.tntPlacedBy), 20.0F * finalPower, this.tntPlacedBy, entity, true, finalPower / 2.0F, this.posX, this.posY, this.posZ);
                    entity.hurtResistantTime = 0;
                    entity.extinguish();
        ...
        if (this.world.isAreaLoaded(this.getPosition().add(-16, -16, -16), this.getPosition().add(16, 16, 16))) {
            ...
            if (this.power > 3.0F) {
                ...
                for (int ii = 0; ii < i; ii++) {
                    EntityFrostfireExplosive expl = new EntityFrostfireExplosive(this.world, this.posX, this.posY, this.posZ, this.tntPlacedBy, this.power - 2.0F);
