...
public class EntityMinigunIcicle extends EntityThrowable implements ISynchronizedEntity {
    ...
    public void onClient(double x, double y, double z, double a, double b, double c) {
        for (int ss = 0; ss < 2; ss++) {
            GUNParticle bigsmoke = new GUNParticle(this.largecloud, 0.3F + (float) this.rand.nextGaussian() / 14.0F, -0.003F, 20, 240, this.world, x, y, z, (float) this.rand.nextGaussian() / 16.0F, (float) this.rand.nextGaussian() / 16.0F, (float) this.rand.nextGaussian() / 16.0F, 0.45F + this.rand.nextFloat() / 10.0F, 0.8F, 1.0F, true, this.rand.nextInt(360));
        ...
        for (int ss = 0; ss < this.rand.nextInt(2) + 1; ss++) {
            GUNParticle bigsmoke = new GUNParticle(this.snow, 0.2F + (float) this.rand.nextGaussian() / 15.0F, 0.025F, 30, 150, this.world, x, y, z, (float) this.rand.nextGaussian() / 15.0F, (float) this.rand.nextGaussian() / 15.0F, (float) this.rand.nextGaussian() / 15.0F, 0.6F + this.rand.nextFloat() / 10.0F, 0.9F, 1.0F, true, this.rand.nextInt(360), true, 1.6F);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote) {
                ...
                Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.bullet), bdamage, this.getThrower(), result.entityHit, true, parameters.getEnchantedF("knockback", impulse));
