...
public class EnigmateTwinsShoot extends EntityThrowable {
    ...
    public void onUpdate() {
        ...
        if (this.targ == null) {
            if (this.ticksExisted % 5 == 0) {
                EntityLivingBase enemy = GetMOP.findNearestEnemy(this.world, this.thrower, this.posX, this.posY, this.posZ, 7.0, true);
                if (enemy != null) {
                    this.targ = enemy;
                } else {
                    this.expireTicks += 5;
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            AnimatedGParticle anim = new AnimatedGParticle(explode, 0.25F + 0.2F * this.rand.nextFloat(), 0.0F, 20, 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 1.0F - 0.1F * this.rand.nextFloat(), 1.0F - 0.1F * this.rand.nextFloat(), 1.0F, true, this.rand.nextInt(360));
            anim.framecount = 8;
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            ...
            if (this.canImpactTick > this.impactTickNeed && Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote) {
                ...
                Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.thrower, this, false, true, this, WeaponDamage.portal), this.damage * (1.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack) * 0.07F), this.thrower, result.entityHit, true, 0.1F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) * 0.04F);
                result.entityHit.hurtResistantTime = 0;
                this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.enigmate_twins_impact, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
                this.world.setEntityState(this, (byte) 8);
                this.setDead();
            }
        } else if (this.canImpactTick <= 0 && this.world.getBlockState(result.getBlockPos()).getBlock().getCollisionBoundingBox(this.world.getBlockState(result.getBlockPos()), this.world, result.getBlockPos()) != null) {
            this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.enigmate_twins_impact, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
    ...
    public void onEntityUpdate() {
        ...
        if (this.world.isRemote && pos1.lengthSquared() > 1.0E-6 && pos2.lengthSquared() > 1.0E-6) {
            BetweenParticle laser = new BetweenParticle(this.world, texture, 0.04F, 190, 0.85F, 0.2F, 1.0F, 0.1F, pos1.distanceTo(pos2), 5, -0.15F, 9999.0F, pos1, pos2);
