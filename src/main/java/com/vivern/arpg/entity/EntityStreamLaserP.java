...
public class EntityStreamLaserP extends Entity {
    ...
    public EntityStreamLaserP(World worldIn, EntityPlayer player, ResourceLocation texture, float scale, int light, float Red, float Green, float Blue, float alphatime, double distance, int livetime, float animation, float lengh) {
        super(worldIn);
        this.player = player;
        this.scale = scale;
        this.light = light;
    ...
    public EntityStreamLaserP(World worldIn, ResourceLocation texture, float scale, int light, float Red, float Green, float Blue, float alphatime, double distance, int livetime, float animation, float lengh, float rotatPitch, float rotatYaw, int ticksexisted) {
        super(worldIn);
        this.player = null;
        this.scale = scale;
        this.light = light;
    ...
    public Vec3d getPositionEyes(float partialTicks) {
        if (this.useOldPositioning) {
            ...
            return this.player == Minecraft.getMinecraft().player && Minecraft.getMinecraft().getRenderManager().options.thirdPersonView == 0 ? Weapons.getPlayerShootPoint(this.player, this.hand, partialTicks, this.barrelLength, this.shoulders, this.yoffset, false) : Weapons.getPlayerShootPoint(this.player, this.hand, partialTicks, this.barrelLength, this.shoulders, this.yoffset, true);
        }
    }
