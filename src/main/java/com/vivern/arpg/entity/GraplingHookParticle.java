...
public class GraplingHookParticle extends Entity {
    ...
    public GraplingHookParticle(World worldIn, ResourceLocation texture, ResourceLocation texture2, float scale, int light, float Red, float Green, float Blue, float alphatime, double distance, int livetime, float animation, float lengh, int ticksexisted, float pikeLength, float pikeScale, Vec3d from, Vec3d to) {
        super(worldIn);
        this.scale = scale;
        this.light = light;
