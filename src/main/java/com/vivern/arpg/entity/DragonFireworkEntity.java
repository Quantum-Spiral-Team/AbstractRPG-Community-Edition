...
public class DragonFireworkEntity extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 15) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.firework_dragon_explode, SoundCategory.AMBIENT, 2.0F, 0.9F + this.rand.nextFloat() / 5.0F, false);

            for (int ss = 0; ss < 15; ss++) {
                Entity bigsmoke = new GUNParticle(this.largesmoke, 0.9F, 0.0F, 14, 80, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 20.0F, (float) this.rand.nextGaussian() / 20.0F, (float) this.rand.nextGaussian() / 20.0F, 1.0F, 1.0F, 1.0F, true, 0);
                this.world.spawnEntity(bigsmoke);
                GunPEmitter emi = new GunPEmitter(this.sparkle2, 0.3F, 0.06F, 30, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 2.0F, (float) this.rand.nextGaussian() / 2.0F + 1.5F, (float) this.rand.nextGaussian() / 2.0F, 1.0F, 1.0F, 1.0F, true, 1, false, this.largesmoke, 0.45F, 0.0F, 8, 100, 100.0F, 100.0F, 100.0F, 1.0F, 1.0F, 1.0F, true);
            ...
            for (int i = 0; i < countOfParticles; i++) {
                ...
                Entity particle = new GUNParticle(this.sparkle, 0.15F + (float) this.rand.nextGaussian() / 30.0F, 0.005F, 22 + this.rand.nextInt(16), 240, this.world, this.posX, this.posY, this.posZ, X + (float) this.rand.nextGaussian() / 20.0F, Y + (float) this.rand.nextGaussian() / 20.0F, Z + (float) this.rand.nextGaussian() / 20.0F, 1.0F - (float) this.rand.nextGaussian() / 10.0F, 0.5F - (float) this.rand.nextGaussian() / 10.0F, (float) this.rand.nextGaussian() / 10.0F, true, 0);
        ...
        if (id == 14) {
            ...
            for (int ss = 0; ss < 3; ss++) {
                Entity spelll = new GUNParticle(this.sparkle2, 0.07F + (float) this.rand.nextGaussian() / 30.0F, 0.02F, 14, 240, this.world, this.posX, this.posY, this.posZ, prunex + (float) this.rand.nextGaussian() / 15.0F, pruney + (float) this.rand.nextGaussian() / 15.0F, prunez + (float) this.rand.nextGaussian() / 15.0F, 1.0F, 0.8F + (float) this.rand.nextGaussian() / 3.0F, 1.0F, true, 0);
                this.world.spawnEntity(spelll);
            }

            Entity spel = new EntityCubicParticle(this.body, 0.03F, 0.0F, 15, 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(180), this.rand.nextInt(180), this.rand.nextInt(180), 0.0F, false, -0.002F);
    ...
    public void expl() {
        ...
        Explosion explosion = new Explosion(this.world, null, this.posX, this.posY, this.posZ, parameters.getEnchantedF("explode_size", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, this.weaponstack)), true, true);
        ...
        if (!list.isEmpty()) {
            for (Entity entity : list) {
                if (Team.checkIsOpponent(this.getThrower(), entity)) {
                    ...
                    Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, true, false, this, WeaponDamage.explode), parameters.getEnchantedF("damage_dragon", might), this.getThrower(), entity, true, parameters.getEnchantedF("knockback_dragon", impulse), this.posX, this.posY - 1.0, this.posZ);
                    entity.hurtResistantTime = 0;
                } else {
                    int impulse = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack);
                    SuperKnockback.applyKnockback(entity, parameters.getEnchantedF("knockback_dragon", impulse), this.posX, this.posY - 1.0, this.posZ);
                }
            }
        }
