...
public class EntityArcaneExplode extends Entity {
    ...
    public void onUpdate() {
        if (this.spawnparticle && this.world.isRemote) {
            ...
            AnimatedGParticle anim = new AnimatedGParticle(magic_explode, 1.5F, 0.0F, 50, 240, this.world, this.posX, this.posY + this.height / 2.0F, this.posZ, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, 0);
            anim.framecount = 50;
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 9) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.arcane_explode, SoundCategory.AMBIENT, 2.1F, 0.95F + this.rand.nextFloat() / 10.0F, false);
        }

        if (id == 8) {
            for (int i = 0; i < 20; i++) {
                GUNParticle particle = new GUNParticle(star2, 0.1F, 0.003F, 30, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 6.0F, (float) this.rand.nextGaussian() / 6.0F, (float) this.rand.nextGaussian() / 6.0F, 0.15F + this.rand.nextFloat() * 0.1F, 0.3F - this.rand.nextFloat() * 0.2F, 1.0F, true, this.rand.nextInt(360));
            ...
            for (int i = 0; i < 2; i++) {
                GUNParticle particle = new GUNParticle(circle2, 0.1F, 0.0F, 7, 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 0.15F + this.rand.nextFloat() * 0.1F, 0.2F - this.rand.nextFloat() * 0.1F, 1.0F, true, this.rand.nextInt(360));
                particle.alphaGlowing = true;
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    ...
                    if (Team.checkIsOpponent(this.thrower, entity)) {
                        Weapons.dealDamage(damageSource, finaldamage, this.getThrower(), entity, true, this.knockback, this.posX, this.posY, this.posZ);
                        entity.hurtResistantTime = 0;
                    } else if (this.damageTeammates) {
                        Weapons.dealDamage(damageSource, finaldamage / 2.0F, this.getThrower(), entity, true, this.knockback, this.posX, this.posY, this.posZ);
                    }
                }
            }
