...
public class ToxicNukeShard extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            GUNParticle bigsmoke = new GUNParticle(this.largesmoke, 0.9F + (float) this.rand.nextGaussian() / 3.0F, -0.01F, 30, 180, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 10.0F, (float) this.rand.nextGaussian() / 19.0F, (float) this.rand.nextGaussian() / 10.0F, 0.3F, 1.0F, 0.7F + (float) this.rand.nextGaussian() / 5.0F, true, this.rand.nextInt(360), true, 1.0F);
    ...
    public void onUpdate() {
        ...
        if (this.ticksExisted % 10 == 0) {
            ...
            AxisAlignedBB aabbox = new AxisAlignedBB(this.posX - shard_damage_radius, this.posY - shard_damage_radius, this.posZ - shard_damage_radius, this.posX + shard_damage_radius, this.posY + shard_damage_radius, this.posZ + shard_damage_radius);
                ...
                for (EntityLivingBase entitylivingbase : list) {
                    if (Team.checkIsOpponent(this.thrower, entitylivingbase)) {
                        if (!this.world.isRemote) {
                            PotionEffect debaff = new PotionEffect(PotionEffects.TOXIN, parameters.getEnchantedI("toxin_time", witchery), parameters.getEnchantedI("toxin_power", witchery));
                            entitylivingbase.addPotionEffect(debaff);
                        }
                    } else if (!this.world.isRemote) {
                        float friendlyfire = parameters.getFloat("friendlyfire");
                        PotionEffect debaff = new PotionEffect(PotionEffects.TOXIN, (int) (parameters.getEnchantedI("toxin_time", witchery) * friendlyfire), (int) (friendlyfire * parameters.getEnchantedI("toxin_power", witchery)));
                        entitylivingbase.addPotionEffect(debaff);
                    }
                }
            }
