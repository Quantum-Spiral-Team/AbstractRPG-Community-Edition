...
public class CryoDestroyerSpray extends EntityThrowable implements ISynchronizedEntity {
    ...
    public void onClient(double... args) {
        if (args.length == 4) {
            ...
            GUNParticle part = ParticleFastSummon.round2(this.world, frozen_circle, pos, 0.1F, 1.1F, 8 + this.rand.nextInt(4), 170, 1.0F, 1.0F, 1.0F, 0);
            part.snapToFace(EnumFacing.byIndex((int) args[3]));

            for (int ss = 0; ss < 2; ss++) {
                GUNParticle bigsmoke = new GUNParticle(largecloud, 0.4F + (float) this.rand.nextGaussian() / 8.0F, -0.003F, 15 + this.rand.nextInt(10), 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 20.0F, (float) this.rand.nextGaussian() / 20.0F, (float) this.rand.nextGaussian() / 20.0F, 0.75F + this.rand.nextFloat() / 5.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
        ...
        if (!this.world.isRemote) {
            if (result.entityHit != null) {
                if (Team.checkIsOpponent(this.thrower, result.entityHit)) {
                    Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.thrower, this, false, false, this, WeaponDamage.frost), 3.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack) * 0.5F, this.thrower, result.entityHit, true, 0.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) * 0.2F, this.posX, this.posY, this.posZ);
                    result.entityHit.hurtResistantTime = 0;
                    if (result.entityHit instanceof EntityLivingBase) {
                        EntityLivingBase entitylivingbase = (EntityLivingBase) result.entityHit;
                        PotionEffect lastdebaff = Weapons.mixPotion(entitylivingbase, PotionEffects.FREEZING, 20.0F, 1.0F, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumMathOperation.PLUS, Weapons.EnumMathOperation.PLUS, 80, 13);
                ...
                if (result.hitVec != null && result.sideHit != null) {
                    ...
                    ISynchronizedEntity.sendSynchronize(this, 32.0, result.hitVec.x, result.hitVec.y, result.hitVec.z, result.sideHit.getIndex());
                }

                this.world.playSound(this.posX, this.posY, this.posZ, Sounds.cryo_gun_impact, SoundCategory.AMBIENT, 0.7F, 1.1F + this.rand.nextFloat() / 5.0F, false);
                this.setDead();
            }
        }
    }
