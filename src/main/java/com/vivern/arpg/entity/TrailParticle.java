...
public class TrailParticle extends GUNParticle {
    ...
    public TrailParticle(ResourceLocation texture, float scale, float gravity, int livetime, int light, World world, double x, double y, double z, float speedx, float speedy, float speedz, float Red, float Green, float Blue, boolean opa, int rotation, boolean collide, float frictionMultipl, ResourceLocation Ttexture, float Tscale, int Tlight, float TRed, float TGreen, float TBlue, float Talphatime, int Tlivetime, float Tanimation, float Ttexlengh) {
    ...
    public void onEntityUpdate() {
        ...
        if (this.ticksExisted > 2 && this.world.isRemote && this.pos1.lengthSquared() > 1.0E-6 && this.pos2.lengthSquared() > 1.0E-6) {
            BetweenParticle laser = new BetweenParticle(this.world, this.Ttexture, this.Tscale, this.Tlight, this.TRed, this.TGreen, this.TBlue, this.Talphatime, this.pos1.distanceTo(this.pos2), this.Tlivetime, this.Tanimation, this.Ttexlengh, this.pos1, this.pos2);
