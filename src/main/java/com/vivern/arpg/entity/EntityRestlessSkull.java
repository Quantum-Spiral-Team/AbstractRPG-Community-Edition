...
public class EntityRestlessSkull extends EntityThrowable implements ISynchronizedEntity {
    ...
    public void onClient(double x, double y, double z, double a, double b, double c) {
        for (int ss = 0; ss < 8; ss++) {
            GUNParticle bigsmoke = new GUNParticle(largesmoke, 0.3F + (float) this.rand.nextGaussian() / 20.0F, -0.03F, 14, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 20.0F, (float) this.rand.nextGaussian() / 25.0F, (float) this.rand.nextGaussian() / 20.0F, 0.5F, 0.7F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, this.rand.nextInt(360));
        ...
        for (int ss = 0; ss < 3; ss++) {
            TrailParticle trailpart = new TrailParticle(smmsh, 0.08F, -0.02F, 25, 220, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 8.0F, (float) this.rand.nextGaussian() / 8.0F + 0.15F, (float) this.rand.nextGaussian() / 8.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(180), false, 1.0F, texture, 0.08F, 220, 1.0F, 1.0F, 1.0F, 0.1F, 5, -0.15F, 9999.0F);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit)) {
                if (this.active) {
                    if (this.attackDelay <= 0 && !this.world.isRemote) {
                        ...
                        Weapons.dealDamage(null, (9.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack)) * this.magicPower, this.thrower, result.entityHit, true, basekn + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) / 3.0F, this.thrower.posX, this.thrower.posY, this.thrower.posZ);
        ...
        } else if (result != null && result.getBlockPos() != null && this.world.getBlockState(result.getBlockPos()).getBlock().getCollisionBoundingBox(this.world.getBlockState(result.getBlockPos()), this.world, result.getBlockPos()) != null) {
            this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.normal_projectile, SoundCategory.AMBIENT, 0.7F, 0.9F + this.rand.nextFloat() / 5.0F);
            this.ticksNoCollide = 0;
            this.impacts++;
            this.active = true;
            ...
            this.collBlock = true;
            if (!this.powered && !this.world.isRemote && this.impacts > 1 + 2 * EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, this.weaponstack)) {
    ...
    public void onEntityUpdate() {
        ...
        if (this.world.isRemote && pos1.lengthSquared() > 1.0E-6 && pos2.lengthSquared() > 1.0E-6) {
            BetweenParticle laser = new BetweenParticle(this.world, texture, 0.14F, 240, 1.0F, 1.0F, 1.0F, 0.1F, pos1.distanceTo(pos2), 5, -0.15F, 9999.0F, pos1, pos2);
