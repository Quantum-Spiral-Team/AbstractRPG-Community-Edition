...
public class HadronBlasterShoot extends EntityThrowable implements ISynchronizedEntity {
    ...
    public void onClient(double x, double y, double z, double a, double b, double c) {
        ...
        for (int i = 0; i < 3; i++) {
            GUNParticle bigsmoke1 = new GUNParticle(explode, 0.04F, 0.0F, 5, 240, this.world, x, y, z, 0.0F, 0.0F, 0.0F, 1.0F, this.ticksExisted / 20.0F + this.rand.nextFloat() / 4.0F, 0.1F + this.rand.nextFloat() / 10.0F, true, this.rand.nextInt(360));
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (vect.lengthSquared() > 1.0E-6 && newvec.lengthSquared() > 1.0E-6) {
                ...
                BetweenParticle laser = new BetweenParticle(this.world, beam, 0.07F, 240, 1.0F - this.rand.nextFloat() / 10.0F, this.ticksExisted / 20.0F, 0.15F, 0.13F, dto, 7, 0.0F, 5.0F, vect, newvec);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && Weapons.canDealDamageTo(result.entityHit)) {
                ...
                RayTraceResult result2 = GetMOP.fixedRayTraceBlocks(this.world, this.thrower, 0.3, true, new Vec3d(this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ), start.add(this.motionX, this.motionY, this.motionZ), false, false, false);
    ...
    public void expl(Vec3d vect) {
        ...
        AxisAlignedBB axisalignedbb = new AxisAlignedBB(vect.x - damageRadius, vect.y - damageRadius, vect.z - damageRadius, vect.x + damageRadius, vect.y + damageRadius, vect.z + damageRadius);
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity)) {
                        ...
                        boolean ab = Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.laser), parameters.getEnchantedF("damage", might), this.getThrower(), entity, true, parameters.getEnchantedF("knockback", impulse), this.posX, this.posY, this.posZ);
                        ...
                        if (entity instanceof EntityLivingBase) {
                            ...
                            if (ab && this.rand.nextFloat() < reus) {
                                HadronBlasterBonus bonus = new HadronBlasterBonus(this.world, entitylivingbase, this.thrower);
    ...
    public void onEntityUpdate() {
        ...
        if (this.world.isRemote) {
            GUNParticle fire2 = new GUNParticle(cloud, 0.2F, 0.0F, 10, 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 1.0F, this.ticksExisted / 20.0F, 0.1F + (float) this.rand.nextGaussian() / 10.0F, true, this.rand.nextInt(360));
    ...
    public static class HadronBlasterBonus extends Entity implements IRenderOptions {
        ...
        public void onUpdate() {
            ...
            if (this.follow && this.owner != null) {
                if (!this.world.isRemote) {
                    SuperKnockback.applyMove(this, -0.17F, this.owner.posX, this.owner.posY + this.owner.height / 4.0F, this.owner.posZ);
            ...
            if (this.ticksExisted == 2) {
                this.world.playSound(this.posX, this.posY, this.posZ, Sounds.magic_k, SoundCategory.HOSTILE, 0.9F, 0.9F + this.rand.nextFloat() / 5.0F, false);
            }
        ...
        public void onEntityUpdate() {
            if (this.ticksExisted < 50) {
                ...
                if (this.world.isRemote && pos1.lengthSquared() > 1.0E-6 && pos2.lengthSquared() > 1.0E-6) {
                    BetweenParticle laser = new BetweenParticle(this.world, texture, 0.18F, 240, this.randR, this.randG, this.randB, 0.1F, pos1.distanceTo(pos2), 5, -0.15F, 9999.0F, pos1, pos2);
                    laser.alphaGlowing = true;
                    laser.texstart = 0.1F;
                    laser.offset = -0.1F;
