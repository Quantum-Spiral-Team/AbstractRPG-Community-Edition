    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            ...
            for (int ss = 0; ss < 7; ss++) {
                Entity spel = new EntityCubicParticle(this.resou, 0.01F, 0.0F, 20 + this.rand.nextInt(4), 90 + this.rand.nextInt(9), this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 9.0F, (float) this.rand.nextGaussian() / 9.0F, (float) this.rand.nextGaussian() / 9.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextFloat(), this.rand.nextFloat(), this.rand.nextFloat(), 0.05F, true, 0.0F);
                this.world.spawnEntity(spel);
                Entity spelll = new GUNParticle(this.resoup, 0.13F, 0.005F, 20, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 13.0F, (float) this.rand.nextGaussian() / 13.0F, (float) this.rand.nextGaussian() / 13.0F, 0.8F, 0.8F, 0.8F + (float) this.rand.nextGaussian() / 6.0F, false, 0);
                this.world.spawnEntity(spelll);
            }
        }

        if (id == 2) {
            Entity spel = new EntityCubicParticle(this.resou, 0.016F, 0.0F, 12 + this.rand.nextInt(4), 99, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 44.0F, (float) this.rand.nextGaussian() / 44.0F, (float) this.rand.nextGaussian() / 44.0F, 1.0F, 1.0F, 1.0F, true, 1.0F, 1.0F, 1.0F, 0.08F, true, -0.0012F);
            this.world.spawnEntity(spel);
        }
    }
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote) {
                ...
                Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.frost), this.damage * this.magicPower, this.getThrower(), result.entityHit, true, this.knockback);
                ...
                if (result.entityHit instanceof EntityLivingBase) {
                    ...
                    PotionEffect lastdebaff = Weapons.mixPotion(entitylivingbase, PotionEffects.FREEZING, (float) Math.round(potionValue1 * this.magicPower), (float) potionValue2, Weapons.EnumPotionMix.GREATEST, Weapons.EnumPotionMix.GREATEST, Weapons.EnumMathOperation.NONE, Weapons.EnumMathOperation.NONE, 0, 0);
