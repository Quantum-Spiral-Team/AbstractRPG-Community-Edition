...
public class EntityArrowBengal extends AbstractArrow {
    ...
    public void onUpdate() {
        ...
        if (!this.world.isRemote) {
            if (this.inGround && this.timeInGround > 10 && !this.isSparkling && this.pickupStatus != PickupStatus.CREATIVE_ONLY && this.pickupStatus != PickupStatus.DISALLOWED) {
        ...
        if (this.isSparkling) {
            this.timeSparkling++;
            if (this.world.isRemote) {
                Vec3d pointToLights = this.getPositionVector().subtract(GetMOP.pitchYawToVec3D(-this.rotationPitch, -this.rotationYaw).scale((200 - this.timeSparkling) / 200.0 * 0.7));
                if (this.rand.nextFloat() < 0.5F) {
                    GUNParticle part = new GUNParticle(texture, 0.06F + this.rand.nextFloat() * 0.04F, 0.03F, 20 + this.rand.nextInt(10), 240, this.world, pointToLights.x, pointToLights.y, pointToLights.z, (float) this.rand.nextGaussian() / 16.0F, (float) this.rand.nextGaussian() / 16.0F + 0.045F, (float) this.rand.nextGaussian() / 16.0F, 1.0F, 1.0F - this.rand.nextFloat() * 0.1F, 0.7F - this.rand.nextFloat() * 0.3F, true, this.rand.nextInt(360), true, 1.0F);
                    part.alphaGlowing = true;
                    this.world.spawnEntity(part);
                } else {
                    GUNParticle part = new GUNParticle(texture2, 0.01F, 0.0F, 5, 240, this.world, pointToLights.x, pointToLights.y, pointToLights.z, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
                    part.alphaGlowing = true;
                ...
                SparkleSubparticle sparkl = new SparkleSubparticle(pointToLights.x, pointToLights.y, pointToLights.z, 0.01F + this.rand.nextFloat() * 0.01F, 20 + this.rand.nextInt(20), (float) (0.01 * this.rand.nextGaussian()), (float) (0.02 * this.rand.nextGaussian() + 0.01), (float) (0.01 * this.rand.nextGaussian()), 0.0F);
                ...
                for (Entity entity : GetMOP.getEntitiesInAABBof(this.world, this, 1.7, this)) {
                    if (Team.checkIsOpponent(this.shootingEntity, entity)) {
                        Weapons.dealDamage(new WeaponDamage(null, this.shootingEntity, this, false, false, this, WeaponDamage.fire), bengal_damage, this.shootingEntity, entity, true);
