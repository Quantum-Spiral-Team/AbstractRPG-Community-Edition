...
public class BigLightningStrike extends Entity {
    ...
    public void onUpdate() {
        ...
        if (this.deadd) {
            ...
            if (this.model == null) {
                this.model = new LaserModel.LaserModelLinear(lightning4, 240, 2, this.getSegmentWidth(), 0.0F, color, color, 0.0F, 0.0F, this.getSegmentLength(), 0.0F);
            }
            ...
            if (this.mainSegment == null) {
                ...
                if (this.renderbb == null) {
                    this.renderbb = new AxisAlignedBB(this.posX - 100.0, this.posY - 250.0, this.posZ - 100.0, this.posX + 100.0, this.posY + 250.0, this.posZ + 100.0);
                }
            }
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 11 && Minecraft.getMinecraft().player != null) {
            this.world.playSound(this.posX, Minecraft.getMinecraft().player.posY, this.posZ, Sounds.lightning, SoundCategory.WEATHER, 5.0F, 0.85F + this.rand.nextFloat() / 4.0F, false);
            if (this.rand.nextFloat() < 0.3) {
                this.world.playSound(this.posX, Minecraft.getMinecraft().player.posY, this.posZ, Sounds.thunder, SoundCategory.WEATHER, 15.0F, 0.9F + this.rand.nextFloat() / 5.0F, false);
            }
        }
    }
    ...
    public Vec3d nextPos(Vec3d startpos, float horizontality) {
        ...
        Vec3d randDirection = new Vec3d(this.strikerand.nextGaussian() * horizontality, -this.strikerand.nextFloat() * vertically, this.strikerand.nextGaussian() * horizontality).normalize();
    ...
    public static class BigLightningSegment {
        ...
        public float getBrightness() {
            return GetMOP.getFromTo((float) this.ticksExisted, 0.0F, 2.0F) - GetMOP.getFromTo((float) this.ticksExisted, 4.0F, 10.0F) + GetMOP.getFromTo((float) this.ticksExisted, 100.0F, 102.0F) - GetMOP.getFromTo((float) this.ticksExisted, 109.0F, 120.0F);
        }
            ...
            if (!this.generated) {
                ...
                if (!lightningStrike.blockImpacted && this.chanceContinue > 0.0F && lightningStrike.strikerand.nextFloat() < this.chanceContinue) {
                    if (lightningStrike.segmentsCount < BigLightningStrike.getSegmentsLimit()) {
                        this.childs.add(new BigLightningSegment(this.posEnd, lightningStrike.nextPos(this.posEnd, 0.2F), this.age + 1, this.chanceContinue - 0.03F, this));
                        lightningStrike.segmentsCount++;
                    }

                    if (this.age > 2 && lightningStrike.strikerand.nextFloat() < this.chanceContinue && lightningStrike.segmentsCount < BigLightningStrike.getSegmentsLimit()) {
                        this.childs.add(new BigLightningSegment(this.posEnd, lightningStrike.nextPos(this.posEnd, 0.5F), this.age + 1, 0.7F, this));
                        lightningStrike.segmentsCount++;
                    }
                }
