    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            ...
            for (int ss = 0; ss < 5; ss++) {
                ...
                GUNParticle part = new GUNParticle(pixel, 0.0625F, 0.01F, lt, 200, this.world, this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ, (float) this.rand.nextGaussian() / 22.0F, (float) this.rand.nextGaussian() / 22.0F + 0.08F, (float) this.rand.nextGaussian() / 22.0F, (float) col.x, (float) col.y, (float) col.z, false, this.rand.nextInt(360));
            ...
            for (int ss = 0; ss < this.rand.nextInt(3) + 1; ss++) {
                GUNParticle part = new GUNParticle(CrystalFanShoot.shards[this.rand.nextInt(3)], 0.12F + this.rand.nextFloat() * 0.06F, 0.06F, 25 + this.rand.nextInt(10), -1, this.world, this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ, (float) this.rand.nextGaussian() / 15.0F, (float) this.rand.nextGaussian() / 15.0F + 0.1F, (float) this.rand.nextGaussian() / 15.0F, (float) col2.x, (float) col2.y, (float) col2.z, false, this.rand.nextInt(360), true, 1.2F);
                part.dropMode = true;
                part.rotationPitchYaw = new Vec2f(this.rand.nextInt(360), this.rand.nextInt(360));
                part.tracker = new ParticleTracker.TrackerGlassShard((float) this.rand.nextGaussian() * 1.5F, (float) this.rand.nextGaussian() * 1.5F, (int) this.rand.nextGaussian() * 2, false);
                this.world.spawnEntity(part);
            }
        }
    }
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if ((!isEntity || !this.alternative || !(entity instanceof XmassBall)) && Team.checkIsOpponent(this.thrower, entity)) {
                        ...
                        Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, false, this, WeaponDamage.explode), this.damage, this.getThrower(), entity, true, parameters.getEnchantedF("ball_knockback", impulse), this.posX, this.posY, this.posZ);
                        entity.hurtResistantTime = 0;
                    }
                }
            }

            this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.arrow_shell, SoundCategory.AMBIENT, 0.7F, 1.05F + this.rand.nextFloat() / 5.0F);
