...
public class EntityButterfly extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            ...
            Entity spelll = new GUNParticle(resoue, 0.12F, 0.001F, 32, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, 0.7F + (float) this.rand.nextGaussian() / 5.0F, 0.8F + (float) this.rand.nextGaussian() / 8.0F, 0.7F + (float) this.rand.nextGaussian() / 5.0F, false, 0);
        ...
        if (!list.isEmpty()) {
            for (Entity entity : list) {
                if (Team.checkIsOpponent(this.thrower, entity)) {
                    Weapons.dealDamage(null, 3.0F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack) / 2.0F, this.thrower, entity, true, 0.75F + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) / 2.5F, this.posX, this.posY, this.posZ);
