...
public class EntitySnapball extends EntityThrowable implements ISynchronizedEntity {
    ...
    public void onUpdate() {
        ...
        if (this.thrower != null && !this.world.isRemote && !this.powered && !this.picked && this.pickable && this.getDistanceSq(this.thrower) < 2.0) {
            this.picked = true;
            if (this.thrower.getHeldItemMainhand().getItem() == ItemsRegister.SNAP_BALL) {
                ...
                if (this.rand.nextFloat() < parameters.getEnchantedF("reuse_chance", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.REUSE, this.weaponstack))) {
                    this.world.spawnEntity(new EntityItem(this.world, this.posX, this.posY, this.posZ, new ItemStack(ItemsRegister.SNAP_BALL_AMMO)));
                }
            ...
            this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.snap_ball_pick, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
            ...
            if (this.ticksExisted > 80) {
                ...
                if (!list.isEmpty()) {
                    ...
                    for (Entity entity : list) {
                        if (Team.checkIsOpponent(this.thrower, entity) && GetMOP.thereIsNoBlockCollidesBetween(this.world, GetMOP.entityCenterPos(this), GetMOP.entityCenterPos(entity), null, false)) {
                            Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, false, this, WeaponDamage.plasma), parametersx.getEnchantedF("damage_powered", might), this.getThrower(), entity, true);
            ...
            for (AxisAlignedBB aabb : this.world.getCollisionBoxes(this, this.getEntityBoundingBox().grow(5.0))) {
                if (aabb != null) {
                    ...
                    if (dist <= radiusSphere) {
                        ...
                        SuperKnockback.applyMove(this, absdist, collisionpoint.x, collisionpoint.y - this.height / 2.0F, collisionpoint.z);
                    }
                }
            }
        ...
        if (!this.world.isRemote) {
            ISynchronizedEntity.sendSynchronize(this, 64.0, this.posX, this.posY, this.posZ, this.motionX, this.motionY, this.motionZ);
        }
    }
    ...
    public void handleStatusUpdate(byte id) {
        ...
        if (id == 10) {
            ...
            AnimatedGParticle anim = new AnimatedGParticle(blast, 1.7F, 0.0F, 13, 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 0.9F, 1.0F, 0.5F + this.rand.nextFloat() / 10.0F, true, this.rand.nextInt(360));
            anim.framecount = 13;
        ...
        if (id == 11) {
            this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.alert, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
        }
    }
        ...
        if (!this.enablePhysics && result != null) {
            if (result.entityHit != null && this.attackCooldown <= 0) {
                if (Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote) {
                    ...
                    int might = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack);
                    int impulse = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack);
                    Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.bullet), parameters.getEnchantedF("damage", might), this.getThrower(), result.entityHit, true, parameters.getEnchantedF("knockback", impulse), this.thrower.posX, this.thrower.posY, this.thrower.posZ);
                    result.entityHit.hurtResistantTime = 0;
                    this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.snap_ball_impact, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
                    Vec3d vec = new Vec3d(result.entityHit.posX, this.posY, result.entityHit.posZ);
                    AxisAlignedBB bb = this.getEntityBoundingBox().offset(vec.x - this.posX, vec.y - this.posY, vec.z - this.posZ);
            ...
            } else if (result.getBlockPos() != null && this.world.getBlockState(result.getBlockPos()).getBlock().getCollisionBoundingBox(this.world.getBlockState(result.getBlockPos()), this.world, result.getBlockPos()) != null) {
                this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.snap_ball_impact, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
                if (!this.world.isRemote) {
        ...
        if (this.powered) {
            ...
            if (!list.isEmpty()) {
                for (EntityLivingBase entitylivingbase : list) {
                    if (entitylivingbase.isCreatureType(EnumCreatureType.MONSTER, false) && Team.checkIsOpponent(this.thrower, entitylivingbase) && !this.ignore.contains(entitylivingbase) && entitylivingbase != this.getThrower() && EntitySelectors.NOT_SPECTATING.apply(entitylivingbase) && entitylivingbase.getHealth() > 0.0F && GetMOP.thereIsNoBlockCollidesBetween(this.world, GetMOP.entityCenterPos(this), GetMOP.entityCenterPos(entitylivingbase), null, false)) {
                ...
                if (targ == null) {
                    for (EntityLivingBase entitylivingbasex : list) {
                        if (Team.checkIsOpponent(this.thrower, entitylivingbasex) && !this.ignore.contains(entitylivingbasex) && entitylivingbasex != this.getThrower() && EntitySelectors.NOT_SPECTATING.apply(entitylivingbasex) && entitylivingbasex.getHealth() > 0.0F && GetMOP.thereIsNoBlockCollidesBetween(this.world, GetMOP.entityCenterPos(this), GetMOP.entityCenterPos(entitylivingbasex), null, false)) {
    ...
    public void expl(boolean inChangeDerection) {
        WeaponParameters parameters = WeaponParameters.getWeaponParameters(this.weaponstack.getItem());
        if (this.powered && EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.SPECIAL, this.weaponstack) > 0 && this.ticksExisted < parameters.getInt("minimal_special_livetime")) {
                ...
                if (!list.isEmpty()) {
                    ...
                    for (Entity entity : list) {
                        if (Team.checkIsOpponent(this.thrower, entity)) {
                            Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, true, false, this, WeaponDamage.explode), parameters.getEnchantedF("damage_explode", might), this.thrower, entity, true, parameters.getEnchantedF("knockback_explode", impulse), this.posX, this.posY - 0.3, this.posZ);
                            entity.hurtResistantTime = 0;
            ...
            if (!this.world.isRemote) {
                ...
                this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.snap_ball_explode, SoundCategory.AMBIENT, 1.75F, 0.9F + this.rand.nextFloat() / 5.0F);
                this.setDead();
            }
        }
    }
    ...
    public void onEntityUpdate() {
        ...
        if (this.world.isRemote && this.powered && this.ticksExisted % 4 == 0) {
            ...
            AxisAlignedBB axisalignedbb = this.getEntityBoundingBox().expand(damageRadius * 2.0, damageRadius * 2.0, damageRadius * 2.0).offset(-damageRadius, -damageRadius, -damageRadius);
            List<EntityLivingBase> list = this.world.getEntitiesWithinAABB(EntityLivingBase.class, axisalignedbb);
            Vec3d pos1 = new Vec3d(this.posX + this.motionX, this.posY + this.height / 2.0F + this.motionY, this.posZ + this.motionZ);
            if (!list.isEmpty()) {
                for (EntityLivingBase entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity)) {
                        Vec3d pos2 = GetMOP.entityCenterPos(entity);
                        if (GetMOP.thereIsNoBlockCollidesBetween(this.world, pos1, pos2, null, false)) {
                            BetweenParticle laser = new BetweenParticle(this.world, texture, 0.55F, 240, 1.0F, 0.96F, 0.3F, 0.0F, pos1.distanceTo(pos2), 4, 0.16F, 6.0F, pos1, pos2);
