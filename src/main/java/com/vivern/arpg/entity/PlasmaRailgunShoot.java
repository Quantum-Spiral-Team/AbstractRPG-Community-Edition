...
public class PlasmaRailgunShoot extends StandardBullet implements ISynchronizedEntity {
    ...
    public void onClient(double x, double y, double z, double a, double b, double c) {
        for (int ss = 0; ss < 5; ss++) {
            GUNParticle bigsmoke = new GUNParticle(cloud, 0.2F + this.rand.nextFloat() / 8.0F, 0.0F, 30, 220, this.world, x, y, z, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, 0.75F + (float) this.rand.nextGaussian() / 10.0F, 0.45F, 1.0F, true, this.rand.nextInt(360));
            bigsmoke.alphaTickAdding = -0.025F;
            bigsmoke.alphaGlowing = true;
            this.world.spawnEntity(bigsmoke);
        }

        GUNParticle bigsmoke1 = new GUNParticle(explode, 0.1F, 0.0F, 11, 240, this.world, x, y, z, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
    ...
    public void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (Team.checkIsOpponent(this.thrower, result.entityHit) && !this.attacked.contains(result.entityHit) && !this.world.isRemote) {
                ...
                boolean deal = Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.plasma), parameters.getEnchantedF("damage", might) + this.pierces * parameters.getEnchantedF("damage_per_pierces", might), this.getThrower(), result.entityHit, true, parameters.getEnchantedF("knockback", impulse));
                result.entityHit.hurtResistantTime = 0;
                if (result.entityHit instanceof EntityLivingBase) {
                    EntityLivingBase entitylivingbase = (EntityLivingBase) result.entityHit;
                    PotionEffect baff = parameters.getPotion("shock", PotionEffects.SHOCK, EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.WITCHERY, this.weaponstack));
                ...
                this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.plasma_railgun_impact, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
            }
    ...
    public void onEntityUpdate() {
        ...
        if (this.world.isRemote) {
            if (this.ticksExisted > 2 && this.pos1.lengthSquared() > 1.0E-6 && this.pos2.lengthSquared() > 1.0E-6) {
                BetweenParticle laser = new BetweenParticle(this.world, this.texture, 0.05F, 240, 0.92F, 0.8F, 1.0F, 0.09F, this.pos1.distanceTo(this.pos2), 10, 0.3F, 2.0F, this.pos1, this.pos2);
            ...
            if (this.ticksExisted == 2) {
                GUNParticle bigsmoke1 = new GUNParticle(blueexplode, 0.04F, 0.0F, 9, 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, true, this.rand.nextInt(360));
