...
public class SpikedBurst extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            for (int ss = 0; ss < this.rand.nextInt(4) + 2; ss++) {
                GUNParticle part = new GUNParticle(tex, 0.03F + this.rand.nextFloat() / 30.0F, 0.028F, 40, -1, this.world, this.prevPosX, this.prevPosY, this.prevPosZ, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, (float) this.rand.nextGaussian() / 14.0F, 1.0F, 0.9F, 0.85F - this.rand.nextFloat() / 10.0F, false, this.rand.nextInt(360), true, 1.5F);
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if ((this.team == null || this.team.isEmpty() || !this.team.equals(Team.getTeamFor(result.entityHit))) && Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote) {
                Weapons.dealDamage(null, this.damage, this.thrower, result.entityHit, true, 1.0F, this.thrower.posX, this.thrower.posY, this.thrower.posZ);
                result.entityHit.hurtResistantTime = 0;
                this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.bullet, SoundCategory.AMBIENT, 0.4F, 0.9F + this.rand.nextFloat() / 5.0F);
