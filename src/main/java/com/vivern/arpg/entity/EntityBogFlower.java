...
public class EntityBogFlower extends EntityThrowable {
    ...
    public void onUpdate() {
        ...
        if (this.ticksExisted > 40 && !this.isImpacted) {
            this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.bog_flower_impact, SoundCategory.AMBIENT, 0.9F, 0.95F + this.rand.nextFloat() / 10.0F);
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            ...
            for (double i = 0.0; i < doublePi; i += doublePi / 8.0) {
                if (this.rand.nextFloat() < 0.4) {
                    ...
                    GUNParticle fire = new GUNParticle(this.acid, 0.1F + this.rand.nextFloat() / 10.0F, -0.01F, 8 + this.rand.nextInt(10), 240, this.world, newPosX, this.posY + 0.1, newPosZ, speedX + (float) this.rand.nextGaussian() / 25.0F, 0.0F, speedZ + (float) this.rand.nextGaussian() / 25.0F, 1.0F, 0.8F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, this.rand.nextInt(360));
            ...
            GUNParticle bigsmoke = new GUNParticle(this.largesmoke, 1.5F + (float) this.rand.nextGaussian() / 3.0F, -0.009F, 30, 180, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 13.0F, (float) this.rand.nextGaussian() / 19.0F, (float) this.rand.nextGaussian() / 13.0F, 0.5F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, 0.1F, true, this.rand.nextInt(360), true, 1.0F);
        ...
        if (id == 8) {
            ...
            GUNParticle fire2 = new GUNParticle(this.lightroots, sc, 0.0F, lt, 200, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 0.9F + (float) this.rand.nextGaussian() / 5.0F, 0.9F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, this.rand.nextInt(360));
    ...
    protected void onImpact(RayTraceResult result) {
        if (!this.world.isRemote) {
            ...
            if (pos != null && this.world.getBlockState(pos).getBlock().getCollisionBoundingBox(this.world.getBlockState(pos), this.world, pos) != null && !this.isImpacted) {
                this.isImpacted = true;
                this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.bog_flower_impact, SoundCategory.AMBIENT, 0.9F, 0.95F + this.rand.nextFloat() / 10.0F);
            }
        }
            ...
            if (!list.isEmpty()) {
                for (EntityLivingBase entitylivingbase : list) {
                    if (entitylivingbase.getDistance(this) <= damageRadius) {
                        ...
                        if (opon) {
                            PotionEffect baff = new PotionEffect(PotionEffects.TOXIN, Math.round((100 + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack) * 35) * this.magicPower), this.magicPower >= 2.0F ? 1 : 0);
