...
public class EntityThrowedReaper extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 5) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.reaper_impact, SoundCategory.AMBIENT, 0.7F, 0.9F + this.rand.nextFloat() / 5.0F, false);

            for (int ss = 0; ss < 5; ss++) {
                GUNParticle bigsmoke = new GUNParticle(this.largesmoke, 0.3F + (float) this.rand.nextGaussian() / 20.0F, 0.0F, 12, 240, this.world, this.posX, this.posY, this.posZ, (float) this.rand.nextGaussian() / 20.0F, (float) this.rand.nextGaussian() / 25.0F, (float) this.rand.nextGaussian() / 20.0F, 1.0F, 0.7F + (float) this.rand.nextGaussian() / 5.0F, 1.0F, true, this.rand.nextInt(360));
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if (!this.repulse(result.entityHit) && Team.checkIsOpponent(this.thrower, result.entityHit) && Weapons.canDealDamageTo(result.entityHit) && !this.entitycollided) {
                this.expl();
            }
        } else if (this.world.getBlockState(result.getBlockPos()).getBlock().getCollisionBoundingBox(this.world.getBlockState(result.getBlockPos()), this.world, result.getBlockPos()) != null && !this.entitycollided) {
            this.expl();
        }
    }
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity)) {
                        ...
                        Weapons.dealDamage(null, 10.0F + dd + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack) * 1.5F, this.thrower, entity, true, Math.min(this.flyeddist / 3.0F, 3.0F) + EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack) / 1.5F, this.posX, this.posY - 0.3, this.posZ);
    ...
    public void onEntityUpdate() {
        ...
        if (this.ticksExisted > 2 && this.world.isRemote && this.pos1.lengthSquared() > 1.0E-6 && this.pos2.lengthSquared() > 1.0E-6) {
            BetweenParticle laser = new BetweenParticle(this.world, this.texture, 0.3F, 240, 0.8F, 0.5F, 1.0F, 0.17F, this.pos1.distanceTo(this.pos2), Math.min(4, this.ticksExisted - 2), 0.3F, 2.0F, this.pos1, this.pos2);
