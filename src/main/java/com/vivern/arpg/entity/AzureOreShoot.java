...
public class AzureOreShoot extends EntityThrowable {
    ...
    public void onUpdate() {
        ...
        if (this.targ == null) {
            if (this.ticksExisted % 5 == 0) {
                ...
                if (!this.powered) {
                    ...
                    if (!list.isEmpty()) {
                        for (EntityLivingBase entitylivingbase : list) {
                            if (entitylivingbase.isCreatureType(EnumCreatureType.MONSTER, false) && Team.checkIsOpponent(this.thrower, entitylivingbase) && entitylivingbase != this.getThrower() && EntitySelectors.NOT_SPECTATING.apply(entitylivingbase) && entitylivingbase.getHealth() > 0.0F && GetMOP.thereIsNoBlockCollidesBetween(this.world, GetMOP.entityCenterPos(this), GetMOP.entityCenterPos(entitylivingbase), null, false)) {
                        ...
                        if (this.targ == null) {
                            for (EntityLivingBase entitylivingbasex : list) {
                                if (Team.checkIsOpponent(this.thrower, entitylivingbasex) && entitylivingbasex != this.getThrower() && EntitySelectors.NOT_SPECTATING.apply(entitylivingbasex) && entitylivingbasex.getHealth() > 0.0F && GetMOP.thereIsNoBlockCollidesBetween(this.world, GetMOP.entityCenterPos(this), GetMOP.entityCenterPos(entitylivingbasex), null, false)) {
                ...
                if (this.targ == null) {
                    float randommotion = this.powered ? parameters.getEnchantedF("random_motion_powered", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.ACCURACY, this.weaponstack)) : parameters.getEnchantedF("random_motion", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.ACCURACY, this.weaponstack));
            ...
            this.motionY *= 0.92;
            this.motionZ *= 0.92;
            SuperKnockback.applyMove(this, this.homingPower, this.targ.posX, this.targ.posY + this.targ.height / 2.0F, this.targ.posZ);
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            AnimatedGParticle anim = new AnimatedGParticle(explode, 0.5F, -0.01F, 32, 200, this.world, this.posX, this.posY, this.posZ, (this.rand.nextFloat() - 0.5F) / 20.0F, 0.0F, (this.rand.nextFloat() - 0.5F) / 20.0F, 1.0F, 1.0F, 1.0F, true, 0);
            anim.framecount = 16;
    ...
    protected void onImpact(RayTraceResult result) {
        if (result.entityHit != null) {
            if ((!this.powered || result.entityHit.isEntityAlive()) && Team.checkIsOpponent(this.thrower, result.entityHit) && !this.world.isRemote) {
                WeaponParameters parameters = WeaponParameters.getWeaponParameters(this.weaponstack.getItem());
                int might = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.MIGHT, this.weaponstack);
                int impulse = EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.IMPULSE, this.weaponstack);
                Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, false, true, this, WeaponDamage.magic), parameters.getEnchantedF("damage", might) * this.magicPower, this.getThrower(), result.entityHit, true, parameters.getEnchantedF("knockback", impulse), this.prevPosX, this.prevPosY, this.prevPosZ);
                result.entityHit.hurtResistantTime = 0;
                this.world.playSound(null, this.posX, this.posY, this.posZ, Sounds.azure_ore_staff_impact, SoundCategory.AMBIENT, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F);
    ...
    public void onEntityUpdate() {
        if (!this.powered) {
            ...
            if (this.world.isRemote && pos1.lengthSquared() > 1.0E-6 && pos2.lengthSquared() > 1.0E-6) {
                BetweenParticle laser = new BetweenParticle(this.world, texture, 0.12F, 190, 0.0F, 0.04F, 0.8F, 0.1F, pos1.distanceTo(pos2), 7, -0.15F, 9999.0F, pos1, pos2);
