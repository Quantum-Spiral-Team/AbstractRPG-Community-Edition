...
public class EntityLiveHeart extends Entity {
    ...
    public void onEntityUpdate() {
        if (this.world.isRemote) {
            ...
            if (this.rand.nextFloat() < 0.1F) {
                ...
                GUNParticle spelll = new GUNParticle(res, scale, 4.0E-4F, livetime, 160 + this.rand.nextInt(40), this.world, d0 + randispl1, d1 + randispl2, d2 + randispl3, randsp1, randsp2, randsp3, this.getRed(), this.getGreen(), this.getBlue(), false, 45, true, 5.0F);
        ...
        if (this.ticksExisted % 2 == 0) {
            ...
            if (!list.isEmpty()) {
                for (EntityPlayer player : list) {
                    if (player.getMaxHealth() - player.getHealth() > 0.0F && EntitySelectors.NOT_SPECTATING.apply(player) && (this.team.isEmpty() || this.team.equals(Team.getTeamFor(player)))) {
    ...
    public void onCollideWithPlayer(EntityPlayer entityIn) {
        if (this.health != 0.0F && this.ticksExisted > 15 && !entityIn.world.isRemote && entityIn.isEntityAlive() && (this.team.isEmpty() || this.team.equals(Team.getTeamFor(entityIn))) && entityIn.getMaxHealth() - entityIn.getHealth() > 0.0F) {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == -5) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.live_heart, SoundCategory.PLAYERS, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F, false);
