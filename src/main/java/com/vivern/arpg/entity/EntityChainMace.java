...
public class EntityChainMace extends Entity implements ISynchronizedEntity, IProjectile {
    ...
    public Vec3d getChainBindingPoint(float partialTicks, boolean forThirdPerson) {
        ...
        return !this.spinned ? new Vec3d(look.x * 0.6 + offset.x * 0.35 + playerPosX, look.y * 0.6 + playerPosY + (this.thrower.isSneaking() ? 0.9F : 1.25F), look.z * 0.6 + offset.z * 0.35 + playerPosZ) : new Vec3d(look.x * 0.25 + offset.x * 0.4 + playerPosX, look.y * 0.25 + playerPosY + 2.0, look.z * 0.25 + offset.z * 0.4 + playerPosZ);
    }
        ...
        if (this.mace != null) {
            ...
            for (Entity entity1 : list) {
                if (entity1.canBeCollidedWith()) {
                    ...
                    if (vec3d.x >= axisalignedbb.minX && vec3d.x <= axisalignedbb.maxX && vec3d.y >= axisalignedbb.minY && vec3d.y <= axisalignedbb.maxY && vec3d.z >= axisalignedbb.minZ && vec3d.z <= axisalignedbb.maxZ) {
                        float collBS = this.getCollisionBorderSize() + 0.1F;
                        Vec3d decrVector = new Vec3d(vec3d.x - vec3d1.x, vec3d.y - vec3d1.y, vec3d.z - vec3d1.z).normalize();
                        vec3d = new Vec3d(vec3d.x + decrVector.x * collBS, vec3d.y + decrVector.y * collBS, vec3d.z + decrVector.z * collBS);
                    }
