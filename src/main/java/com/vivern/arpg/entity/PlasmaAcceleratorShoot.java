...
public class PlasmaAcceleratorShoot extends EntityThrowable {
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 8) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.plasma_accelerator_impact, SoundCategory.AMBIENT, 1.5F, 0.9F + this.rand.nextFloat() / 5.0F, false);
            ...
            for (int ss = 0; ss < 3 + this.rand.nextInt(3); ss++) {
                TrailParticle trailpart = new TrailParticle(cloud, 0.2F, 0.04F, 25, 240, this.world, this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ, (float) this.rand.nextGaussian() / 5.0F, (float) this.rand.nextGaussian() / 5.0F + 0.15F, (float) this.rand.nextGaussian() / 5.0F, rb, g, rb + (float) this.rand.nextGaussian() / 10.0F, true, this.rand.nextInt(180), true, 1.1F, trail, 0.14F, 240, rb, g, rb + (float) this.rand.nextGaussian() / 10.0F, 0.15F, 5, 0.3F, 1.0F);
        ...
        if (id == 5) {
            GUNParticle fire2 = new GUNParticle(cloud, 0.25F, 0.0F, 15, 240, this.world, this.posX, this.posY, this.posZ, 0.0F, 0.0F, 0.0F, 1.0F, 0.8F + this.rand.nextFloat() / 5.0F, 0.8F + this.rand.nextFloat() / 5.0F, true, this.rand.nextInt(360));
    ...
    public void expl(RayTraceResult result) {
        ...
        if (!this.world.isRemote && result.hitVec != null) {
            ...
            if (!list.isEmpty()) {
                for (Entity entity : list) {
                    if (Team.checkIsOpponent(this.thrower, entity)) {
                        float damage = parameters.getEnchantedF("damage", might) * Math.max(parameters.getFloat("damage_multiplier") - (float) Weapons.getDistanceToMobHitbox(entity.width, entity.height, entity.posX, entity.posY, entity.posZ, result.hitVec.x, result.hitVec.y, result.hitVec.z), 0.1F);
                        entity.hurtResistantTime = 0;
                        Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, true, false, this, WeaponDamage.explode), damage, this.getThrower(), entity, true, parameters.getEnchantedF("knockback", impulse), this.posX, this.posY, this.posZ);
                        entity.hurtResistantTime = 0;
                        DeathEffects.applyDeathEffect(entity, DeathEffects.DE_DISMEMBER, 0.55F);
                    } else {
                        float damage = parameters.getEnchantedF("friendlyfire_damage", might) * Math.max(parameters.getFloat("damage_multiplier") - (float) Weapons.getDistanceToMobHitbox(entity.width, entity.height, entity.posX, entity.posY, entity.posZ, result.hitVec.x, result.hitVec.y, result.hitVec.z), 0.1F) * parameters.getFloat("friendlyfire");
                        entity.hurtResistantTime = 0;
                        Weapons.dealDamage(new WeaponDamage(this.weaponstack, this.getThrower(), this, true, false, this, WeaponDamage.explode), damage, this.getThrower(), entity, true, parameters.getEnchantedF("knockback", impulse), this.posX, this.posY, this.posZ);
                        DeathEffects.applyDeathEffect(entity, DeathEffects.DE_DISMEMBER, 0.55F);
                    }
            ...
            if (this.world.isAreaLoaded(this.getPosition().add(-16, -16, -16), this.getPosition().add(16, 16, 16))) {
                for (BlockPos blockpos : GetMOP.getPosesInsideSphere((int) this.posX, (int) this.posY, (int) this.posZ, parameters.getEnchantedI("radius_blocks", EnchantmentHelper.getEnchantmentLevel(EnchantmentInit.RANGE, this.weaponstack)))) {
                    ...
                    if (Weapons.easyBreakBlockFor(this.world, parameters.getEnchantedF("hardness_breaks", might), blockpos, state)) {
                        if (!state.getBlock().hasTileEntity(state) && !(this.rand.nextFloat() < 0.3F)) {
                            ...
                            SuperKnockback.applyKnockback(entityfallingblock, parameters.getEnchantedF("knockback_to_blocks", impulse), this.posX, this.posY - 0.2, this.posZ);
