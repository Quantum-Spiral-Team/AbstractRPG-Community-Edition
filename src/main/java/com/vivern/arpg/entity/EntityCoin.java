...
public class EntityCoin extends Entity {
    ...
    public void onEntityUpdate() {
        if (this.world.isRemote) {
            ...
            if (this.rand.nextFloat() < 0.1F && Minecraft.getMinecraft().gameSettings.particleSetting < 2) {
                ...
                GUNParticle spelll = new GUNParticle(res, scale, 4.0E-4F, livetime, 160 + this.rand.nextInt(40), this.world, d0 + randispl1, d1 + randispl2, d2 + randispl3, randsp1, randsp2, randsp3, this.getRed(), this.getGreen(), this.getBlue(), false, 0, true, 5.0F);
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 0) {
            this.world.playSound(this.posX, this.posY, this.posZ, Sounds.grap_money, SoundCategory.PLAYERS, 0.8F, 0.9F + this.rand.nextFloat() / 5.0F, false);
        }
    }
