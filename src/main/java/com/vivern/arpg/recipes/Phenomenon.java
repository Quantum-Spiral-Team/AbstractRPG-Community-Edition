...
public abstract class Phenomenon {
    ...
    public void render(RenderTerraformingResearch renderTFR, float posx, float posy, int arrayX, int arrayY, float fullness) {
        int id = this.getId() - 1;
        Phenomenons.getPhenomenonsRenderer().texx = id % 18 * 42;
        Phenomenons.getPhenomenonsRenderer().texy = id / 18 * 42;
        Phenomenons.getPhenomenonsRenderer().render(renderTFR, posx, posy, arrayX, arrayY, 0L, fullness);
    }
    ...
    public TerraformingPlayerCommand getGenerationCommand(TerraformingResearchPuzzle puzzle, Random rand) {
        ...
        if (!list.isEmpty()) {
            ...
            return puzzle.board[this.posX][this.posY].hasWay(direction) ? new TerraformingPlayerCommand(TerraformingPlayerCommand.TRPlayerCommandType.MOVE_SELECTED, this.posX, this.posY, direction) : null;
        }
    }
