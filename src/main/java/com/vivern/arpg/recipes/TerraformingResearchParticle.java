...
public class TerraformingResearchParticle {
    ...
    public void render(RenderTerraformingResearch renderTFR, float boardOffsetX, float boardOffsetY, float partialTicks) {
        ...
        double animAdd = this.animationDelay == 0 ? 0.0 : onepixelY * this.texHeight * (this.frameController.whatFrame(this.ticksExisted / this.animationDelay) % this.animationCycle);
        ...
        bufferbuilder.pos(posx + halfW, posy + halfH, 0.0).tex(u + onepixelX * this.texWidth, v + onepixelY * this.texHeight + animAdd).endVertex();
