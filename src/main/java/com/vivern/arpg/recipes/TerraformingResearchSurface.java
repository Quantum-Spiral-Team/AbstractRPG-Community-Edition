...
public class TerraformingResearchSurface {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (puzzle.checkBounds(xx, yy)) {
                    ...
                    if (this.canExpandOnNull) {
                        if ((surf1 == null || surf1.expansionQuality < this.expansionQuality) && (changes[xx][yy] == null || changes[xx][yy].priority < this.expansionQuality)) {
                            changes[xx][yy] = new TFRSurfaceChange(this, this.expansionQuality);
                        }
                    } else if (surf1 != null && surf1.expansionQuality < this.expansionQuality && (changes[xx][yy] == null || changes[xx][yy].priority < this.expansionQuality)) {
                        changes[xx][yy] = new TFRSurfaceChange(this, this.expansionQuality);
                    }
                }
            }
    ...
    public static boolean hasNear(TerraformingResearchPuzzle puzzle, int x, int y, TerraformingResearchSurface surfacehas) {
        return puzzle.getSurface(x + 1, y, surfacehas.type) == surfacehas || puzzle.getSurface(x - 1, y, surfacehas.type) == surfacehas || puzzle.getSurface(x, y + 1, surfacehas.type) == surfacehas || puzzle.getSurface(x, y - 1, surfacehas.type) == surfacehas;
    }
