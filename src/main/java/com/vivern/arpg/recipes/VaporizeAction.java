        ...
        public void vaporizeTick(World world, BlockPos vaporizerPos, TileAlchemicVaporizer tile, Random rand) {
            if (!world.isRemote) {
                ...
                if (rand.nextFloat() < 0.2F) {
                    ...
                    double x = absPos == 0 ? (rand.nextFloat() < 0.5 ? aabb.minX - 0.05 : aabb.maxX + 0.05) : aabb.minX + (aabb.maxX - aabb.minX) * rand.nextDouble();
                    double y = aabb.minY + (aabb.maxY - aabb.minY) * rand.nextDouble();
                    double z = absPos == 1 ? (rand.nextFloat() < 0.5 ? aabb.minZ - 0.05 : aabb.maxZ + 0.05) : aabb.minZ + (aabb.maxZ - aabb.minZ) * rand.nextDouble();
                    int lt = 50 + rand.nextInt(10) + (rand.nextFloat() < 0.2F ? rand.nextInt(70) : 0);
                    GUNParticle bigsmoke = new GUNParticle(largecloud, 0.1F, -0.002F, lt, 240, world, vaporizerPos.getX() + x, vaporizerPos.getY() + y, vaporizerPos.getZ() + z, (float) (rand.nextGaussian() / 36.0 + (x - 0.5) * 0.07), (float) (rand.nextGaussian() / 36.0 + (y - 0.5) * 0.01), (float) (rand.nextGaussian() / 36.0 + (z - 0.5) * 0.07), 0.0F, 0.0F, 0.0F, true, rand.nextInt(360), true, 1.0F);
                    bigsmoke.alphaGlowing = true;
                    bigsmoke.alphaTickAdding = -1.0F / lt;
                    bigsmoke.scaleTickAdding = 0.02F + rand.nextFloat() * 0.03F;
                    bigsmoke.tracker = new ParticleTracker.TrackerGradient(new Vec3d(0.65F, 0.2F - rand.nextFloat() * 0.2F, 0.45F), new Vec3d(0.0, 0.8F - rand.nextFloat() * 0.55F, 1.0), 0, lt);
                    world.spawnEntity(bigsmoke);
                }

                if (rand.nextFloat() < 0.15F) {
                    world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, vaporizerPos.getX() + 0.4 + rand.nextFloat() * 0.2F, vaporizerPos.getY() + 1.0625, vaporizerPos.getZ() + 0.4 + rand.nextFloat() * 0.2F, (rand.nextFloat() - 0.5F) * 0.01, 0.02, (rand.nextFloat() - 0.5F) * 0.01, new int[0]);
                }
            }
        }
        ...
        public void recursiveGenerate(World world, BlockPos prevPos, EnumFacing prevFacingOpp, Random rand, int[] samples) {
            ...
            if (rand.nextInt(MathHelper.clamp(samples[0], 1, 10)) == 0) {
                ...
                if (stateOn.getMaterial() == Material.AIR) {
                    if (rand.nextFloat() < 0.5F && (world.isSideSolid(newPos.down(), EnumFacing.UP) || world.getBlockState(newPos.down()).getBlock() == BlocksRegister.MANAOIL_SPELEOTHEM)) {
                        world.setBlockState(newPos, BlocksRegister.MANAOIL_SPELEOTHEM.getStateForPlacement(world, newPos, EnumFacing.UP, 0.0F, 0.0F, 0.0F, 0, null, EnumHand.MAIN_HAND));
                        return;
                    }

                    if (world.isSideSolid(newPos.up(), EnumFacing.DOWN) || world.getBlockState(newPos.up()).getBlock() == BlocksRegister.MANAOIL_SPELEOTHEM) {
                        world.setBlockState(newPos, BlocksRegister.MANAOIL_SPELEOTHEM.getStateForPlacement(world, newPos, EnumFacing.DOWN, 0.0F, 0.0F, 0.0F, 0, null, EnumHand.MAIN_HAND));
                        return;
                    }
