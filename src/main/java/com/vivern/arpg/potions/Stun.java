...
public class Stun extends AdvancedPotion {
    ...
    public int getUnstunnableTime(EntityLivingBase entity) {
        if (entity instanceof EntityPlayer) {
            ...
            float entityPower = entity.getMaxHealth() * 0.015F + (entity.width + entity.height) / 2.6F + 4.0F * (float) entity.getEntityAttribute(SharedMonsterAttributes.KNOCKBACK_RESISTANCE).getAttributeValue();
