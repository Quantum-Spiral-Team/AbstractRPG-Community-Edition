...
public class AdvancedPotion extends Potion {
    ...
    public void onApplyEffect(PotionAddedEvent event) {
        if (this.sendToClients && !event.getEntity().world.isRemote) {
            sendPotionPacketToClients(event.getPotionEffect(), 64.0, this.listIndex, event.getPotionEffect().getDuration(), event.getPotionEffect().getAmplifier(), event.getEntity());
        }
    }
    ...
    public static void onLivingAttack(LivingAttackEvent event) {
        ...
        if (!list.isEmpty()) {
            for (PotionEffect effect : list) {
                if (effect.getPotion() instanceof AdvancedPotion && ((AdvancedPotion) effect.getPotion()).onAttackThis(event.getEntityLiving(), event.getSource(), event.getAmount(), effect)) {
                    cancel++;
                }
            }
        }
        ...
        if (event.getSource().getTrueSource() != null && event.getSource().getTrueSource() instanceof EntityLivingBase) {
            ...
            if (!list2.isEmpty()) {
                for (PotionEffect effectx : list2) {
                    if (effectx.getPotion() instanceof AdvancedPotion && ((AdvancedPotion) effectx.getPotion()).onThisAttacks(truesource, event.getEntityLiving(), event.getSource(), event.getAmount(), effectx)) {
                        cancel++;
                    }
                }
            }
    ...
    public static void potionApplicable(PotionApplicableEvent event) {
        if (event.getPotionEffect() != null && event.getPotionEffect().getPotion() instanceof AdvancedPotion && !((AdvancedPotion) event.getPotionEffect().getPotion()).isPotionApplicable(event.getEntityLiving(), event.getPotionEffect())) {
