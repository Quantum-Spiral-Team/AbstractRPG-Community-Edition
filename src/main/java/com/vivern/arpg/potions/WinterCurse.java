...
public class WinterCurse extends AdvancedPotion {
    ...
    public void performEffect(EntityLivingBase entityOnEffect, int amplifier) {
        if (entityOnEffect.ticksExisted % 30 == 0) {
            ...
            if (rand.nextFloat() < 0.2F) {
                entityOnEffect.world.playSound(entityOnEffect.posX, entityOnEffect.posY, entityOnEffect.posZ, Sounds.winter_curse_b, SoundCategory.HOSTILE, 0.8F, 0.9F + rand.nextFloat() / 5.0F, false);
            }
        }
    }
    ...
    public void onThisDeath(LivingDeathEvent event, PotionEffect effect) {
        if (!event.getEntityLiving().world.isRemote && (event.getEntityLiving() instanceof EntityPlayer || event.getEntityLiving() instanceof EntityZombie || event.getEntityLiving() instanceof AbstractSkeleton || event.getEntityLiving() instanceof EntityVillager || event.getEntityLiving() instanceof AbstractIllager || event.getEntityLiving() instanceof EntityWitch)) {
            ...
            EntityLivingBase boss = event.getEntityLiving().world.findNearestEntityWithinAABB(BossWinterFury.class, event.getEntityLiving().getEntityBoundingBox().grow(64.0), event.getEntityLiving());
