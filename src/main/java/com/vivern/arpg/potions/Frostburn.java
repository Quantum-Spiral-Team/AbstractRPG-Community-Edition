...
public class Frostburn extends AdvancedPotion {
    ...
    public static void freezeWhileFrostburning(EntityLivingBase entityOnEffect) {
        if (entityOnEffect.isPotionActive(PotionEffects.FREEZING)) {
            PotionEffect lastdebaff = Weapons.mixPotion(entityOnEffect, PotionEffects.FREEZING, 30.0F, rand.nextFloat() < 0.5 ? 0.0F : 1.0F, Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumPotionMix.ABSOLUTE, Weapons.EnumMathOperation.PLUS, Weapons.EnumMathOperation.PLUS, 100, 0);
            ...
            if (effect != null) {
                PotionEffect lastdebaff = Weapons.mixPotion(entityOnEffect, PotionEffects.FREEZING, 30.0F, (float) (effect.getAmplifier() + 1), Weapons.EnumPotionMix.WITH_MAXIMUM, Weapons.EnumPotionMix.ABSOLUTE, Weapons.EnumMathOperation.PLUS, Weapons.EnumMathOperation.PLUS, 100, 0);
                Freezing.tryPlaySound(entityOnEffect, lastdebaff);
            }
        }
    }
