...
public class WalkingBomb extends AdvancedPotion {
        ...
        if (entityOnEffect.deathTime == 1) {
            ...
            if (world != null && world.isRemote) {
                for (int i = 0; i < Math.min(entityOnEffect.getMaxHealth() / 4.0F, 13.0F); i++) {
                    Entity spel = new EntityCubicParticle(gore, 0.01F + entityOnEffect.height / 100.0F, 0.02F, 20 + rand.nextInt(10), 40 + rand.nextInt(9), world, x, y + 0.5, z, (float) rand.nextGaussian() / 8.0F, (float) rand.nextGaussian() / 20.0F + 0.2F, (float) rand.nextGaussian() / 8.0F, (float) color.x, (float) color.y, (float) color.z, false, rand.nextFloat(), rand.nextFloat(), rand.nextFloat(), 0.05F, true, -3.0E-4F);
                    world.spawnEntity(spel);
                }

                for (int i = 0; i < Math.min(entityOnEffect.getMaxHealth() / 2.0F, 18.0F); i++) {
                    GUNParticle sp = new GUNParticle(drop, 0.07F + rand.nextFloat() / 20.0F, 0.02F, 50, -1, world, x, y + 0.5, z, (float) rand.nextGaussian() / 10.0F, (float) rand.nextGaussian() / 10.0F + 0.3F, (float) rand.nextGaussian() / 10.0F, (float) color.x, (float) color.y, (float) color.z, false, (int) Math.round(rand.nextGaussian() * 20.0), true, 5.0F);
                    sp.dropMode = true;
                    world.spawnEntity(sp);
                    GUNParticle bigsmoke = new GUNParticle(largecloud, 1.1F + (float) rand.nextGaussian() / 8.0F, 0.003F, 20 + rand.nextInt(10), -1, world, x, y + 0.5, z, (float) rand.nextGaussian() / 13.0F, (float) rand.nextGaussian() / 16.0F, (float) rand.nextGaussian() / 13.0F + 0.1F, (float) color.x, (float) color.y, (float) color.z, true, rand.nextInt(360));
    ...
    public void onThisDeath(LivingDeathEvent event, PotionEffect effect) {
        ...
        entityOnEffect.world.playSound(null, x, y, z, Sounds.walking_bomb_explode, SoundCategory.NEUTRAL, 1.2F, 0.8F + rand.nextFloat() / 5.0F);
        AxisAlignedBB axisalignedbb = entityOnEffect.getEntityBoundingBox().expand(damageRadius * 2.0, damageRadius * 2.0, damageRadius * 2.0).offset(-damageRadius, -damageRadius, -damageRadius);
