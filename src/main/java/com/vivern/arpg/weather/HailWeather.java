...
public class HailWeather extends WeatherPhenomenon {
    ...
    public void clientUpdate(double x, double z, World world, double value, double playerY) {
        if (world.isRemote && !Minecraft.getMinecraft().isGamePaused() && this.rand.nextFloat() < value * 0.03) {
            ...
            GUNParticle hailp = new GUNParticle(snowflake, 0.023F + this.rand.nextFloat() / 10.0F, 0.01F + (this.rand.nextFloat() - 0.5F) * 0.016F, 100, light, world, x + this.rand.nextInt(4), Math.min(playerY + 30.0, height), z + this.rand.nextInt(4), (float) (this.rand.nextGaussian() / 30.0), -0.15F, (float) (this.rand.nextGaussian() / 30.0), 0.2F + f1 * 0.8F, 0.4F + f1 * 0.6F, 1.0F, false, this.rand.nextInt(360), true, 5.0F);
            hailp.scaleTickDropAdding = -0.002F;
            hailp.tracker = new ParticleTracker.TrackerMotionVector(Weather.getWindX(x, z), Weather.getWindY(x, z), Weather.getWindZ(x, z), (float) (value / 3.0), (float) (value / 10.0), 100);
    ...
    public void clientChunkUpdate(double x, double z, World world, double value, double playerY) {
        if (world.isRemote && !Minecraft.getMinecraft().isGamePaused() && this.rand.nextFloat() < value * 5.0E-4) {
            CloudSubparticle part = new CloudSubparticle(CloudSubparticle.standartTex, this.rand.nextInt(3), x + this.rand.nextFloat() * 16.0F, height + this.rand.nextGaussian() * 5.0, z + this.rand.nextFloat() * 16.0F, 2.0F + this.rand.nextFloat() * 2.0F, 2.0F + this.rand.nextFloat() * 2.0F, 2.0F + this.rand.nextFloat() * 2.0F, 800 + this.rand.nextInt(1500), 250 + this.rand.nextInt(150));
            part.rotateY = 1.0F;
