...
public class Weather {
    ...
    public static boolean isWeatherGoingAt(World world, WeatherPhenomenon weather, BlockPos pos) {
        if (weather.isGoingInDimension(world.provider.getDimension()) && (weather.anyBiome || weather.isGoingInBiome(world.getBiome(pos)))) {
            double value = weather.noisegenerator3d.getValue(pos.getX() / weather.sizeX, (double) world.getTotalWorldTime() / weather.changeSpeed, pos.getZ() / weather.sizeZ);
    ...
    public static void updateEntity(EntityLivingBase entity) {
        ...
        for (WeatherPhenomenon weather : weatherlist) {
            if (weather.affectEntitys && weather.isGoingInDimension(entity.dimension) && (weather.anyBiome || weather.isGoingInBiome(world.getBiome(entity.getPosition())))) {
                double value = weather.noisegenerator3d.getValue(entity.posX / weather.sizeX, (double) world.getTotalWorldTime() / weather.changeSpeed, entity.posZ / weather.sizeZ);
                if (weather.isGoingOn(value)) {
    ...
    public static void RenderWorld(RenderWorldLastEvent event) {
        ...
        if (player != null) {
            for (WeatherRenderList renderlist : weatherrenderlist) {
                renderlist.render(player.posX, player.posY, player.posZ, player.rotationYaw, player.rotationPitch, event.getPartialTicks());
            }
        }
    }
                        ...
                        for (int bb = -320; bb < 336; bb += 16) {
                            for (int nn = -320; nn < 336; nn += 16) {
                                ...
                                if (weather.anyBiome || weather.isGoingInBiome(player.world.getBiome(new BlockPos(x, 0.0, z)))) {
                                    double value = weather.noisegenerator3d.getValue(x / weather.sizeX, (double) player.world.getTotalWorldTime() / weather.changeSpeed, z / weather.sizeZ);
                                    if (weather.isGoingOn(value)) {
                        ...
                        for (Chunk chunk : chunkProvider.getLoadedChunks()) {
                            for (int bb = 2; bb < 16; bb += 4) {
                                for (int nn = 2; nn < 16; nn += 4) {
                                    ...
                                    double value = weather.noisegenerator3d.getValue(x / weather.sizeX, (double) world.getTotalWorldTime() / weather.changeSpeed, z / weather.sizeZ);
