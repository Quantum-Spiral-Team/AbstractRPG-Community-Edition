        ...
        if (entity != null) {
            ...
            for (int xx = -range; xx <= range; xx += quantityX) {
                ...
                for (int zz = -range; zz <= range; zz += quantityZ) {
                    ...
                    bufferbuilder.pos(x + sin1 + displ1, height1 - ylength1 * 0.5, z).color(lineR * colorReduct1, lineG * colorReduct1, lineB * colorReduct1, 1.0F).endVertex();
                    bufferbuilder.pos(x + sin1 + displ1, height2 + ylength1 * 0.5, z).color(r1, g1, b1, 1.0F).endVertex();
                    bufferbuilder.pos(x + sin2 + displ2, height2 + ylength2 * 0.5, z2).color(r2, g2, b2, 1.0F).endVertex();
                    bufferbuilder.pos(x + sin2 + displ2, height1 - ylength2 * 0.5, z2).color(lineR * colorReduct2, lineG * colorReduct2, lineB * colorReduct2, 1.0F).endVertex();
