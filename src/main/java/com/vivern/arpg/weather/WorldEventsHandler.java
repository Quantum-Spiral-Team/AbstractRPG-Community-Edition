...
public class WorldEventsHandler extends IRenderHandler {
    ...
    public void onUpdate() {
        ...
        if (this.worldProvider.getWorldTime() % startCheckDelay == 0L) {
            ...
            if (this.events.length > 0) {
                for (int ix = this.worldProvider.getWorld().rand.nextInt(this.events.length); k < this.events.length; ix = GetMOP.next(ix, 1, this.events.length)) {
                    WorldEvent event = this.events[ix];
                    if (!event.isStarted && event.canStart() && event.currentCooldown <= 0 && this.worldProvider.getWorld().rand.nextFloat() < event.chanceToStart) {
                        boolean can = true;
    ...
    public static void renderRainSnow(float partialTicks, Random random, float chance, ResourceLocation rainTexture, float alpha, float rainSpeed, float rainSpeedRandomize, float horizontalSpeed, float horizontalSpeedRandomize, @Nullable Predicate<Biome> biomeFilter, @Nullable WorldEvent worldevent) {
        if (rainXCoords == null) {
            initRainCoords();
        }
                ...
                for (int l1 = entityPosX - range; l1 <= entityPosX + range; l1++) {
                    ...
                    if ((nochance || random.nextFloat() < chance) && (biomeFilter == null || biomeFilter.apply(world.getBiome(blockpos$mutableblockpos)))) {
                        ...
                        if (lowestPosY != highstPosY) {
                            ...
                            int j4 = i4 >> 16 & 65535;
                            int k4 = i4 & 65535;
                            bufferbuilder.pos(l1 - d3 + 0.5, highstPosY, k1 - d4 + 0.5).tex(0.0 + texDisplaceX, d8 + texDisplaceY).color(1.0F, 1.0F, 1.0F, f5).lightmap(j4, k4).endVertex();
                            bufferbuilder.pos(l1 + d3 + 0.5, highstPosY, k1 + d4 + 0.5).tex(1.0 + texDisplaceX, d8 + texDisplaceY).color(1.0F, 1.0F, 1.0F, f5).lightmap(j4, k4).endVertex();
                            bufferbuilder.pos(l1 + d3 + 0.5, lowestPosY, k1 + d4 + 0.5).tex(1.0 + texDisplaceX, 16.0 + d8 + texDisplaceY).color(1.0F, 1.0F, 1.0F, f5).lightmap(j4, k4).endVertex();
                            bufferbuilder.pos(l1 - d3 + 0.5, lowestPosY, k1 - d4 + 0.5).tex(0.0 + texDisplaceX, 16.0 + d8 + texDisplaceY).color(1.0F, 1.0F, 1.0F, f5).lightmap(j4, k4).endVertex();
                            if (worldevent != null) {
