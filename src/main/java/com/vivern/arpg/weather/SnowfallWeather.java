...
public class SnowfallWeather extends WeatherPhenomenon {
    ...
    public void clientUpdate(double x, double z, World world, double value, double playerY) {
        if (world.isRemote && !Minecraft.getMinecraft().isGamePaused() && this.rand.nextFloat() < value * 0.04) {
            GUNParticle hailp = new GUNParticle(snow, 2.2F + this.rand.nextFloat(), 0.01F, 110, -1, world, x + this.rand.nextInt(3), playerY + 35.0, z + this.rand.nextInt(3), (float) (this.rand.nextGaussian() / 20.0), -0.1F, (float) (this.rand.nextGaussian() / 20.0), 1.0F, 1.0F, 1.0F, false, this.rand.nextInt(360), true, 2.0F);
            hailp.dropMode = true;
