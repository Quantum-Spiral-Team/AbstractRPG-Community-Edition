...
public abstract class Catalyst {
    ...
    public static void onLivingDrops(LivingDropsEvent event) {
        ...
        if (!entityLiving.world.isRemote && !event.getDrops().isEmpty()) {
            ...
            if (CatalystDroppedLoot.allUsedClasses.contains(clazz)) {
                if (!(entityLiving instanceof EntityBlaze) || event.getSource().getImmediateSource() == null || !(event.getSource().getImmediateSource() instanceof EntitySnowball) && !(event.getSource().getImmediateSource() instanceof CannonSnowball)) {
    ...
    public static class CatalystPotionBottle extends Catalyst {
        ...
        public ItemStack getGoodStackForRender() {
            ...
            int type = current / 3;
            return PotionUtils.addPotionToItemStack(new ItemStack(bottle == 0 ? Items.POTIONITEM : (bottle == 1 ? Items.SPLASH_POTION : Items.LINGERING_POTION)), this.types[type]);
        }
