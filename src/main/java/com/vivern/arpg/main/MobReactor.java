...
public class MobReactor {
    ...
    public MobReactor(int pointsCost, int chance, Class<? extends EntityLiving> clazz, int mobsPerSpawnMin, int mobsPerSpawnMax, GroupSpawnMode mode, int emptyZone, int xzRange, int yRange, boolean canBeOverHead) {
    ...
    public MobReactor(int pointsCost, int chance, EntityEntry entry, int mobsPerSpawnMin, int mobsPerSpawnMax, GroupSpawnMode mode, int emptyZone, int xzRange, int yRange, boolean canBeOverHead) {
        if (entry == null) {
    ...
    public boolean involved(EntityPlayer playerSpawnOn, @Nullable AbstractWorldProvider abstractWorldProvider) {
        if (!this.heightIncludesPlayer || !(playerSpawnOn.posY < this.minWorldHeight) && !(playerSpawnOn.posY > this.maxWorldHeight)) {
            if (this.biomeIncludesPlayer && this.biomeLogic != null && !this.biomeLogic.apply(playerSpawnOn.world.getBiome(playerSpawnOn.getPosition()))) {
                return false;
            } else if (this.survivorGamestyleLevel > 0 && SurvivorGameStyleWatcher.currentWatcher != null && SurvivorGameStyleWatcher.currentWatcher.LEVEL < this.survivorGamestyleLevel) {
                return false;
            } else {
                if (this.worldEventsSpawnLogic != WorldEventsSpawnLogic.NOT_AFFECT) {
                    if (this.worldEventsSpawnLogic == WorldEventsSpawnLogic.ONLY_IN_ASSIGNED && (abstractWorldProvider == null || !abstractWorldProvider.isWorldEventStarted(this.worldEventAssign))) {
                        return false;
                    }
                    ...
                    if (this.worldEventsSpawnLogic == WorldEventsSpawnLogic.ANYTIME_EXCEPT_ASSIGNED && abstractWorldProvider != null && abstractWorldProvider.isWorldEventStarted(this.worldEventAssign)) {
                        return false;
                    }

                    if (this.worldEventsSpawnLogic == WorldEventsSpawnLogic.WHEN_NO_EVENTS && abstractWorldProvider != null && abstractWorldProvider.getWorldEventsHandler() != null) {
                ...
                return !this.enderLogic || !(playerSpawnOn.posX * playerSpawnOn.posX + playerSpawnOn.posZ * playerSpawnOn.posZ < 1000000.0);
            }
        } else {
            return false;
        }
        ...
        if (mobscount > 0) {
            ...
            if (this.mode == GroupSpawnMode.LAND) {
                for (int w = 0; w < 10; w++) {
                    ...
                    if (pos.getY() >= this.minWorldHeight && pos.getY() <= this.maxWorldHeight && this.checkLight(pos, world) && (this.biomeLogic == null || this.biomeLogic.apply(world.getBiome(pos))) && spawn.landMobGroup(world, pos, list)) {
                ...
                for (int wx = 0; wx < 10; wx++) {
                    BlockPos pos = spawn.createRandomPosNearPlayer(player, this.emptyZone, this.xzRange, this.yRange, this.canBeOverHead);
                    if (pos.getY() >= this.minWorldHeight && pos.getY() <= this.maxWorldHeight && this.checkLight(pos, world) && (this.biomeLogic == null || this.biomeLogic.apply(world.getBiome(pos))) && spawn.airborneMobGroup(world, pos, list)) {
                        int mobsspawned = spawn.lastSpawnedMobs;
                        Mana.addSwarmPoints(player, -spawn.lastSpawnedMobs * this.pointsCost);
                        spawn.lastSpawnedMobs = 0;
                        return mobsspawned;
                ...
                for (int wxx = 0; wxx < 10; wxx++) {
                    BlockPos pos = spawn.createRandomPosNearPlayer(player, this.emptyZone, this.xzRange, this.yRange, this.canBeOverHead);
                    if (pos.getY() >= this.minWorldHeight && pos.getY() <= this.maxWorldHeight && this.checkLight(pos, world) && (this.biomeLogic == null || this.biomeLogic.apply(world.getBiome(pos))) && spawn.undergroundMobGroup(world, pos, list)) {
                        int mobsspawned = spawn.lastSpawnedMobs;
                        Mana.addSwarmPoints(player, -spawn.lastSpawnedMobs * this.pointsCost);
                        spawn.lastSpawnedMobs = 0;
                        return mobsspawned;
                ...
                for (int wxxx = 0; wxxx < 10; wxxx++) {
                    BlockPos pos = spawn.createRandomPosNearPlayer(player, this.emptyZone, this.xzRange, this.yRange, this.canBeOverHead);
                    if (pos.getY() >= this.minWorldHeight && pos.getY() <= this.maxWorldHeight && this.checkLight(pos, world) && (this.biomeLogic == null || this.biomeLogic.apply(world.getBiome(pos))) && spawn.waterMobGroup(world, pos, list)) {
                        int mobsspawned = spawn.lastSpawnedMobs;
                        Mana.addSwarmPoints(player, -spawn.lastSpawnedMobs * this.pointsCost);
                        spawn.lastSpawnedMobs = 0;
                        return mobsspawned;
