...
public class ItemsElements {
    ...
    public static ElementsPack getAllElements(Fluid fluid) {
        ...
        if ("water".equals(name)) {
            ...
            return "dissolved_toxinium".equals(name) ? new ElementsPack(new float[]{5.0F, 2.0F, 4.0F, 0.0F, 10.0F, 0.0F, 2.0F, 1.0F, 0.0F, 0.0F, 0.0F, 0.0F}, 0.0F) : EMPTY_ELEMENTS;
        }
    }
    ...
    public static void writeGeneratedItemElements() {
        ...
        for (AssemblyTableRecipe recipe : AssemblyTableRecipesRegister.allRecipes) {
            writeGeneratedForList(recipe.exportInputsAsList(), recipe.craftresult.createStack(), recipe.craftresult2 != null && recipe.craftresult2 != Ingridient.EMPTY ? recipe.craftresult2.createStack() : null);
        }

        for (NetherMelterRecipe recipe : NetherMelterRecipesRegister.allRecipes) {
            if (recipe.output.size() == 1 || recipe.output.size() == 2) {
                writeGeneratedForList(recipe.exportInputsAsList(), recipe.output.get(0).createStack(), recipe.output.size() == 2 ? recipe.output.get(1).createStack() : null);
            }
        }
        ...
        for (AlchemicLabRecipe recipex : AlchemicLabRecipesRegister.allRecipes) {
            if (recipex.output.size() == 1 || recipex.output.size() == 2) {
                if (recipex.costfluidname == null || recipex.costfluidname.isEmpty()) {
                    writeGeneratedForList(recipex.exportInputsAsList(), recipex.output.get(0).createStack(), recipex.output.size() == 2 ? recipex.output.get(1).createStack() : null);
                } else if (FluidRegistry.getFluid(recipex.costfluidname) != null && recipex.amountexist == 0) {
                    writeGeneratedForListAndLiquid(recipex.exportInputsAsList(), new FluidStack(FluidRegistry.getFluid(recipex.costfluidname), recipex.amountcost), recipex.output.get(0).createStack(), recipex.output.size() == 2 ? recipex.output.get(1).createStack() : null);
                }
            }
        }
            ...
            if (recipexx.output.size() == 1 || recipexx.output.size() == 2) {
                if (recipexx.costfluidname1 != null && !recipexx.costfluidname1.isEmpty()) {
                    if (FluidRegistry.getFluid(recipexx.costfluidname1) != null && recipexx.amountexist1 == 0 && recipexx.amountexist2 == 0) {
                        writeGeneratedForListAndLiquid(recipexx.exportInputsAsList(), new FluidStack(FluidRegistry.getFluid(recipexx.costfluidname1), recipexx.amountcost1), recipexx.output.get(0).createStack(), recipexx.output.size() == 2 ? recipexx.output.get(1).createStack() : null);
                    }
                } else if (recipexx.amountexist1 == 0 && recipexx.amountexist2 == 0 && recipexx.amountcost1 == 0 && recipexx.amountcost2 == 0) {
                    writeGeneratedForList(recipexx.exportInputsAsList(), recipexx.output.get(0).createStack(), recipexx.output.size() == 2 ? recipexx.output.get(1).createStack() : null);
                }
            }
        }
    }
    ...
    private static void writeGeneratedForListAndLiquid(List<ItemStack> input, FluidStack inputFluid, ItemStack output, @Nullable ItemStack output2) {
        ...
        if (subtractElementsToGeneratedPack(pack, output2)) {
            ...
            if (pack.howMuchNotNull() > 0) {
                ...
                GUIElementDistributor.writeElementsOfItemToFile(output.getItem(), output.getMetadata(), pack.purity, output.getItem().getHasSubtypes(), "generated", false, pack.elementsAmount);
            }
        }
    }
