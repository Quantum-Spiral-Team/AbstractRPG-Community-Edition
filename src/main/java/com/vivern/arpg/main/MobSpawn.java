...
public abstract class MobSpawn {
    ...
    public static Predicate<Block> toxic = input -> input == BlocksRegister.TOXIC_DIRT || input == BlocksRegister.TOXIC_GRASS || input == BlocksRegister.SLUDGE || input == BlocksRegister.RADIOACTIVE_COBBLESTONE || input == BlocksRegister.RADIOACTIVE_STONE || input == BlocksRegister.JUNK || input == BlocksRegister.SCRAP || input == BlocksRegister.BRICK_SHARDS;
    ...
    public boolean landMobGroup(World world, int baseY, double x, double y, double z, List<EntityLiving> mobs) {
        ...
        while (y > minn) {
            if (world.getBlockState(blockpos$pooledmutableblockpos.setPos(x, y, z)).isSideSolid(world, blockpos$pooledmutableblockpos, EnumFacing.UP) && this.isEnableBlockToSpawnIn(world, blockpos$pooledmutableblockpos.setPos(x, y + 1.0, z))) {
