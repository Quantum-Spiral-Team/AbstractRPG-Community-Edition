...
public class Mana {
    ...
    public static void multipleManaSpeed(EntityPlayer player, float value, float plus) {
        player.getDataManager().set(PropertiesRegistry.MANA_SPEED, player.getDataManager().get(PropertiesRegistry.MANA_SPEED) * value + plus);
    }
    ...
    public static void addRad(EntityPlayer player, int add, boolean fromOutside) {
        ...
        if (!player.world.isRemote && add > 0) {
            ...
            if ((rz <= 380L || add <= 5) && (rz <= 60L || add < 30)) {
                if (rz > 270L) {
                    player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.rad_small, SoundCategory.AMBIENT, 0.7F, 0.9F + player.getRNG().nextFloat() / 5.0F);
                    radTime = player.world.getWorldTime();
                }
            } else {
                player.world.playSound(null, player.posX, player.posY, player.posZ, Sounds.rad, SoundCategory.AMBIENT, 0.7F, 0.9F + player.getRNG().nextFloat() / 5.0F);
                radTime = player.world.getWorldTime();
            }
        }
    }
