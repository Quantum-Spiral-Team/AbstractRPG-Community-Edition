...
public class Shards {
    ...
    public static void onPlayerAttackEntityEvent(AttackEntityEvent event) {
        ...
        if (!event.getEntityLiving().world.isRemote && rand.nextFloat() < 0.3F && event.getTarget() instanceof EntityCreature && !(event.getTarget() instanceof EntityPlayer)) {
    ...
    public static void onCreatureDeath(LivingDropsEvent event) {
        if (!event.getEntityLiving().world.isRemote && event.isRecentlyHit() && (!event.getEntityLiving().isNonBoss() || event.getEntityLiving().getRNG().nextFloat() < AbstractMob.dropShardsChance)) {
