...
public interface IMagicUI {
    ...
    default void onProvideLink(World world, EntityPlayer player, @Nullable BlockPos thispos, @Nullable Entity thisentity, @Nullable BlockPos linkpos, @Nullable Entity linkentity) {
    }

    default void onAcceptLink(World world, EntityPlayer player, @Nullable BlockPos thispos, @Nullable Entity thisentity, @Nullable BlockPos linkpos, @Nullable Entity linkentity) {
    }

    default void open(World world, EntityPlayer player, @Nullable BlockPos pos, @Nullable Entity entity) {
    }

    static void spawnEntityMUIinRound(World world, EntityPlayer player, @Nullable BlockPos pos, @Nullable Entity entitySource, double radius, float yUp, EntityMagicUI... entitymuis) {
        if (pos != null) {
    ...
    static void spawnEntityMUIinRound(World world, EntityPlayer player, @Nullable BlockPos pos, @Nullable Entity entitySource, double radius, float yUp, List<EntityMagicUI> entitymuis) {
        if (pos != null) {
            double x = pos.getX() + 0.5;
            double y = pos.getY();
            double z = pos.getZ() + 0.5;
