...
public class GetMOP {
    ...
    public static final Predicate<IBlockState> SOLID_BLOCKS = input -> input.isFullCube() && input.getMaterial().blocksMovement();
    public static final Predicate<IBlockState> AIR_BLOCKS = input -> input.getBlock() == Blocks.AIR || input.getMaterial() == Material.AIR;
    public static final Predicate<IBlockState> SOLID_NON_PLANTS_BLOCKS = input -> input.isFullCube() && input.getMaterial().blocksMovement() && input.getMaterial() != Material.LEAVES && input.getMaterial() != Material.VINE && input.getMaterial() != Material.CACTUS && input.getMaterial() != Material.PLANTS && input.getMaterial() != Material.WOOD && input.getMaterial() != Material.SNOW;
    public static final Predicate<IBlockState> ALL_BLOCKS = state -> true;
    public static final Predicate<IBlockState> WATER_BLOCKS = input -> input.getBlock() == Blocks.WATER || input.getBlock() == Blocks.FLOWING_WATER;
    public static final Predicate<IBlockState> FLUID_BLOCKS = input -> input.getBlock() instanceof IFluidBlock || input.getBlock() instanceof BlockLiquid;
    ...
    public static Vec3d logicRayTrace(World world, Vec3d from, Vec3d to, Predicate<? super Entity> filterEntityToIgnore, double size, double step, boolean stoponLiquid) {
    ...
    public static Vec3d logicRayTraceIgnoreMobs(World world, Vec3d from, Vec3d to, Predicate<? super IBlockState> filterBlockToIgnore, boolean stoponLiquid) {
    ...
    public static boolean thereIsNoBlockCollidesBetween(World world, Vec3d from, Vec3d to, @Nullable Predicate<? super IBlockState> filter, boolean stopOnLiquid) {
    ...
    public static EntityLivingBase findEntityOnPath(Vec3d start, Vec3d end, EntityLivingBase entity, boolean checkTeam, double size, double step) {
    ...
    public static EntityLivingBase findEntityOnPath(Vec3d start, Vec3d end, World world, Entity shooter, double size, double rayStep, boolean checkTeam) {
        ...
        for (double k = 0.0; k <= 1.0; k += stepIncrement) {
            ...
            AxisAlignedBB aabb = new AxisAlignedBB(curX - halfSize, curY - halfSize, curZ - halfSize, curX + halfSize, curY + halfSize, curZ + halfSize);
    ...
    public static Entity findEntity2OnPath(Vec3d start, Vec3d end, World world, Entity shooter, double size, double rayStep, boolean checkTeam) {
        ...
        for (double k = 0.0; k <= 1.0; k += stepIncrement) {
            ...
            AxisAlignedBB cube = new AxisAlignedBB(curX - halfSize, curY - halfSize, curZ - halfSize, curX + halfSize, curY + halfSize, curZ + halfSize);
    ...
    public static List<Entity> findEntitiesOnPath(Vec3d start, Vec3d end, World world, @Nullable Entity shooter, double size, double raystep) {
        ...
        for (double k = 0.0; k <= 1.0; k += stepIncrement) {
            ...
            AxisAlignedBB cube = new AxisAlignedBB(x - halfSize, y - halfSize, z - halfSize, x + halfSize, y + halfSize, z + halfSize);
    ...
    public static Vec3d findEndCoordOnPath(Vec3d start, Vec3d end, World world, Entity shooter, double size, double raystep, boolean checkTeam) {
        ...
        for (double k = 0.0; k <= 1.0; k += stepIncrement) {
            ...
            AxisAlignedBB cube = new AxisAlignedBB(centerX - halfSize, centerY - halfSize, centerZ - halfSize, centerX + halfSize, centerY + halfSize, centerZ + halfSize);
    ...
    public static Vec3d rotatedPosRayTrace(double blockReachDistance, float partialTicks, EntityLivingBase entity, double size, double step, float rotationPitch, float rotationYaw) {
        return rotatedPosRayTrace(blockReachDistance, partialTicks, entity, size, step, rotationPitch, rotationYaw, false);
    }

    public static Vec3d rotatedPosRayTrace(double blockReachDistance, Vec3d start, EntityLivingBase entity, double size, double step, float rotationPitch, float rotationYaw) {
        Vec3d vec3d1 = pitchYawToVec3D(rotationPitch, rotationYaw);
        Vec3d vec3d2 = start.add(vec3d1.x * blockReachDistance, vec3d1.y * blockReachDistance, vec3d1.z * blockReachDistance);
    ...
    public static Vec3d rotatedPosRayTrace(double blockReachDistance, float partialTicks, EntityLivingBase entity, double size, double step, float rotationPitch, float rotationYaw, boolean checkTeam) {
        Vec3d vec3d = entity.getPositionEyes(partialTicks);
        Vec3d vec3d1 = pitchYawToVec3D(rotationPitch, rotationYaw);
        Vec3d vec3d2 = vec3d.add(vec3d1.x * blockReachDistance, vec3d1.y * blockReachDistance, vec3d1.z * blockReachDistance);
        RayTraceResult raytraceresult = normalizeRayTraceResult(entity.world.rayTraceBlocks(vec3d, vec3d2, false, true, false));
    ...
    @Nullable
    public static RayTraceResult rayTraceBlocks(World world, Vec3d start, Vec3d end, Predicate<? super IBlockState> filter, boolean stopOnLiquid, boolean ignoreNoBox, boolean returnLastUncollidable) {
    ...
    private static boolean shouldCollide(World world, BlockPos pos, IBlockState state, Predicate<? super IBlockState> filter, boolean stopOnLiquid, boolean ignoreNoBox) {
        ...
        for (int x = minX; x < maxX; ++x) {
            for (int z = minZ; z < maxZ; ++z) {
                ...
                if (chunk == null || cx != lastChunkX || cz != lastChunkZ) {
                    if (!world.getChunkProvider().isChunkGeneratedAt(cx, cz))
                        continue;
    ...
    public static List<Entity> entityMopRayTrace(Class<? extends Entity> eclass, double blockReachDistance, float partialTicks, EntityLivingBase entity, double size, float rotationPitch, float rotationYaw) {
        Vec3d vec3d = entity.getPositionEyes(partialTicks);
        Vec3d vec3d1 = pitchYawToVec3D(rotationPitch, rotationYaw);
        Vec3d vec3d2 = vec3d.add(vec3d1.x * blockReachDistance, vec3d1.y * blockReachDistance, vec3d1.z * blockReachDistance);
        RayTraceResult raytraceresult = entity.world.rayTraceBlocks(vec3d, vec3d2, false, true, false);
    ...
    public static EntityLivingBase entityRayTrace(World world, Vec3d start, Vec3d end, double size, double step, Predicate<? super IBlockState> filterBlockToIgnore) {
        RayTraceResult raytraceresult = filterBlockToIgnore == null ? world.rayTraceBlocks(start, end, false, true, false) : rayTraceBlocks(world, start, end, filterBlockToIgnore, false, true, false);
        if (raytraceresult != null) {
            end = new Vec3d(raytraceresult.hitVec.x, raytraceresult.hitVec.y, raytraceresult.hitVec.z);
        }
    ...
    protected static List<Entity> findCustomEntityOnPath(Class<? extends Entity> eclass, Vec3d start, Vec3d end, World world, Entity shooter, double size) {
        AxisAlignedBB traceBox = new AxisAlignedBB(start.x, start.y, start.z, end.x, end.y, end.z).grow(size);
    ...
    @Nullable
    public static BlockTraceResult blockTrace(World world, BlockPos startPos, EnumFacing direction, int distance, @Nullable Predicate<? super IBlockState> filter) {
        IBlockState lastState = world.getBlockState(startPos);
        PooledMutableBlockPos mutablePos = PooledMutableBlockPos.retain();

        try {
            for (int i = 1; i <= distance; i++) {
                mutablePos.setPos(startPos.getX() + direction.getXOffset() * i, startPos.getY() + direction.getYOffset() * i, startPos.getZ() + direction.getZOffset() * i);

                IBlockState currentState = world.getBlockState(mutablePos);

                boolean hit = (filter == null) ? (currentState.getBlock() != lastState.getBlock()) : filter.apply(currentState);
    ...
    public static List<BlockPos> getBlockPosesCollidesAABB(World world, AxisAlignedBB bb, boolean getLiquids) {
        ...
        Iterable<BlockPos.MutableBlockPos> box = BlockPos.getAllInBoxMutable(MathHelper.floor(bb.minX), MathHelper.floor(bb.minY), MathHelper.floor(bb.minZ), MathHelper.ceil(bb.maxX) - 1, MathHelper.ceil(bb.maxY) - 1, MathHelper.ceil(bb.maxZ) - 1);
    ...
    public static RayTraceResult fixedRayTraceBlocks(World world, Entity shooter, double blockReachDistance, double size, boolean checkTeam, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock, float rotationPitch, float rotationYaw) {
        Vec3d vec3d = shooter.getPositionEyes(1.0F);
        Vec3d vec3d1 = pitchYawToVec3D(rotationPitch, rotationYaw);
        Vec3d vec3d2 = vec3d.add(vec3d1.x * blockReachDistance, vec3d1.y * blockReachDistance, vec3d1.z * blockReachDistance);
        return fixedRayTraceBlocks(world, shooter, size, checkTeam, vec3d, vec3d2, stopOnLiquid, ignoreBlockWithoutBoundingBox, returnLastUncollidableBlock);
    }

    public static RayTraceResult fixedRayTraceBlocks(World world, Entity shooter, double blockReachDistance, double size, boolean checkTeam, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock) {
        Vec3d vec3d = shooter.getPositionEyes(1.0F);
    ...
    public static RayTraceResult fixedRayTraceBlocks(World world, Entity shooter, double size, boolean checkTeam, Vec3d start, Vec3d end, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock, @Nullable Predicate<? super IBlockState> filterBlockToIgnore) {
    ...
    public static RayTraceResult fixedRayTraceBlocks(World world, Entity shooter, double size, boolean checkTeam, Vec3d start, Vec3d end, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock) {
        return fixedRayTraceBlocks(world, shooter, size, checkTeam, start, end, stopOnLiquid, ignoreBlockWithoutBoundingBox, returnLastUncollidableBlock, null);
    }
    ...
    public static Vec3d rotateVecAroundAxis(Vec3d vector, Vec3d axisVector, float angle) {
        ...
        return axisVector.scale(axisVector.dotProduct(vector) * (1.0 - cosAngle)).add(axisVector.crossProduct(vector).scale(Math.sin(angle))).add(vector.scale(cosAngle));
    }
    ...
    public static Vec3d rotateVecAroundAxis(Vec3d vector, Vec3d axis, Rotation rotation) {
        ...
        return axis.scale(axis.dotProduct(vector) * (1.0 - c)).add(axis.crossProduct(vector).scale(s)).add(vector.scale(c));
    }
    ...
    public static Vec3d entityCenterPos(Entity entity, float partialTicks) {
        ...
        return new Vec3d(partial(entity.posX, entity.prevPosX, partialTicks), partial(entity.posY + he2, entity.prevPosY + he2, partialTicks), partial(entity.posZ, entity.prevPosZ, partialTicks));
    }
    ...
    public static EntityLivingBase findNearestEnemy(World world, Entity enemyTo, double x, double y, double z, double radius, boolean bestSearch) {
        ...
        Predicate<EntityLivingBase> baseFilter = e -> Team.checkIsOpponent(enemyTo, e) && EntitySelectors.NOT_SPECTATING.apply(e);
        ...
        if (bestSearch) {
            ...
            EntityLivingBase target = entities.stream().filter(monsterFilter::apply).min(Comparator.comparingDouble(e -> e.getDistanceSq(x, y, z))).orElse(null);

            if (target == null) {
                target = entities.stream().filter(aliveFilter::apply).min(Comparator.comparingDouble(e -> e.getDistanceSq(x, y, z))).orElse(null);
            }
            return target;
        }

        return entities.stream().filter(baseFilter::apply).min(Comparator.comparingDouble(e -> e.getDistanceSq(x, y, z))).orElse(null);
    }

    public static List<EntityLivingBase> getHostilesInAABBto(World world, Vec3d forPosition, double radiusXZ, double radiusY, @Nullable EntityLivingBase hostileTo) {
        AxisAlignedBB bb = new AxisAlignedBB(forPosition.x - radiusXZ, forPosition.y - radiusY, forPosition.z - radiusXZ, forPosition.x + radiusXZ, forPosition.y + radiusY, forPosition.z + radiusXZ);
    ...
    public static List<Entity> getEntitiesInAABBof(World world, Entity forPosition, double radius, @Nullable Entity entityExcluding) {
        ...
        AxisAlignedBB bb = new AxisAlignedBB(cenVec3d.x - radius, cenVec3d.y - radius, cenVec3d.z - radius, cenVec3d.x + radius, cenVec3d.y + radius, cenVec3d.z + radius);
    ...
    public static List<Entity> getEntitiesInAABBof(World world, Vec3d forPosition, double radius, @Nullable Entity entityExcluding) {
        AxisAlignedBB bb = new AxisAlignedBB(forPosition.x - radius, forPosition.y - radius, forPosition.z - radius, forPosition.x + radius, forPosition.y + radius, forPosition.z + radius);
        return world.getEntitiesWithinAABBExcludingEntity(entityExcluding, bb);
    }
    ...
    public static List<Entity> getEntitiesInAABBof(World world, BlockPos forPosition, double radius, @Nullable Entity entityExcluding) {
        AxisAlignedBB bb = new AxisAlignedBB(forPosition.getX() - radius, forPosition.getY() - radius, forPosition.getZ() - radius, forPosition.getX() + 1 + radius, forPosition.getY() + 1 + radius, forPosition.getZ() + 1 + radius);
        return world.getEntitiesWithinAABBExcludingEntity(entityExcluding, bb);
    }
    ...
    public static AxisAlignedBB newAABB(Entity center, double radius) {
        return new AxisAlignedBB(center.posX - radius, center.posY - radius, center.posZ - radius, center.posX + radius, center.posY + radius, center.posZ + radius);
    }
    ...
    public static List<Entity> entityUncollidedRayTrace(Class<? extends Entity> eclass, double blockReachDistance, float partialTicks, EntityLivingBase entity, double size, float rotationPitch, float rotationYaw) {
        Vec3d vec3d = entity.getPositionEyes(partialTicks);
    ...
    protected static List<Entity> findUncollidedEntityOnPath(Class<? extends Entity> eclass, Vec3d start, Vec3d end, World world, Entity shooter, double size) {
    ...
    public static @NotNull RayTraceResult rayTrace(World world, double blockReachDistance, EntityLivingBase entity, boolean useLiquids) {
        ...
        Vec3d endPos = startPos.add(lookVec.x * blockReachDistance, lookVec.y * blockReachDistance, lookVec.z * blockReachDistance);
    ...
    public static RayTraceResult normalizeRayTraceResult(@Nullable RayTraceResult sourceResult, double offset) {
        if (sourceResult != null && sourceResult.sideHit != null && sourceResult.hitVec != null) {
            ...
            sourceResult.hitVec = new Vec3d(vec.x + sourceResult.sideHit.getXOffset() * offset, vec.y + sourceResult.sideHit.getYOffset() * offset, vec.z + sourceResult.sideHit.getZOffset() * offset);
        }
    ...
    public static double isPointInPlane(Vec3d planePoint, Vec3d normalVector, Vec3d point) {
        return normalVector.x * (point.x - planePoint.x) + normalVector.y * (point.y - planePoint.y) + normalVector.z * (point.z - planePoint.z);
    }
    ...
    public static float isInCaves(World world, BlockPos pos) {
        ...
        Vec3d[] directions = {new Vec3d(1, 0, 0), new Vec3d(-1, 0, 0),  // East, West
                new Vec3d(0, 0, 1), new Vec3d(0, 0, -1),  // South, North
                new Vec3d(0, 1, 0), new Vec3d(0, -1, 0),  // Up, Down

                new Vec3d(0.707, 0, 0.707), new Vec3d(-0.707, 0, 0.707), new Vec3d(0.707, 0, -0.707), new Vec3d(-0.707, 0, -0.707), new Vec3d(0, 0.707, 0.707), new Vec3d(0, 0.707, -0.707), new Vec3d(0, -0.707, 0.707), new Vec3d(0, -0.707, -0.707), new Vec3d(0.707, 0.707, 0), new Vec3d(-0.707, 0.707, 0), new Vec3d(0.707, -0.707, 0), new Vec3d(-0.707, -0.707, 0)};

        for (int r : radii) {
            for (Vec3d dir : directions) {
                BlockPos checkPos = new BlockPos(pos.getX() + dir.x * r, pos.getY() + dir.y * r, pos.getZ() + dir.z * r);
    ...
    public static class BlockTraceResult {
        ...
        public String toString() {
            return "prevPos: " + this.prevPos + " | pos: " + this.pos + " | prevState: " + this.prevState.getBlock().getRegistryName() + " | impactState: " + this.impactState.getBlock().getRegistryName();
        }
