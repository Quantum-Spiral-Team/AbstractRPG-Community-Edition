...
public class SuperKnockback {
    ...
    public static void applyShieldBlock(EntityLivingBase entity, EntityLivingBase attacker, float basepower, float toSelfPower) {
        ...
        entity.knockBack(attacker, toSelfPower * (1.0F - (float) entity.getEntityAttribute(SharedMonsterAttributes.KNOCKBACK_RESISTANCE).getAttributeValue()), attacker.posX - entity.posX, attacker.posZ - entity.posZ);
        entity.velocityChanged = true;
    }

    public static void shoot(Entity projectile, Entity entityThrower, float rotationPitchIn, float rotationYawIn, float pitchOffset, float velocity, float inaccuracy) {
