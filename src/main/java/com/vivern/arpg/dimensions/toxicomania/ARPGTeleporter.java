...
public class ARPGTeleporter {
    ...
    public ARPGTeleporter(List<BlockPos> membraneConfiguration, List<BlockAtPos> frameConfiguration, List<BlockPos> groundCheck, List<BlockPos> spawnPoints, int dimensionID, IBlockState portalBlock, int portalHeight, boolean setAxisProperty) {
    ...
    public boolean isActivePortalAtPos(World world, BlockPos pos, boolean Zaxis) {
        if (Zaxis) {
            for (BlockAtPos blockp : this.frameConfiguration) {
                ...
                if (stateon.getBlock() != blockp.state.getBlock() || stateon.getBlock().getMetaFromState(stateon) != blockp.state.getBlock().getMetaFromState(blockp.state)) {
                    return false;
                }
            }
            ...
            for (BlockAtPos blockpx : this.frameConfiguration) {
                ...
                IBlockState stateon = world.getBlockState(poss);
                if (stateon.getBlock() != blockpx.state.getBlock() || stateon.getBlock().getMetaFromState(stateon) != blockpx.state.getBlock().getMetaFromState(blockpx.state)) {
                    return false;
                }
            }
            ...
            if (y == blockpos.getY() || this.hasSolidGround(world, pos, zAxis)) {
                ...
                if (canPlacePortal) {
                    for (BlockPos blockpx : this.membraneConfiguration) {
                        BlockPos poss = zAxis ? pos.add(blockpx.getZ(), blockpx.getY(), blockpx.getX()) : pos.add(blockpx);
                        IBlockState stateon = world.getBlockState(poss);
                        if (!stateon.getBlock().isReplaceable(world, poss)) {
                            canPlacePortal = false;
                            break;
                ...
                if (canPlacePortal) {
                    for (BlockAtPos blockpxx : this.frameConfiguration) {
                        BlockPos poss = zAxis ? pos.add(blockpxx.getZ(), blockpxx.getY(), blockpxx.getX()) : pos.add(blockpxx);
            ...
            if (oldDimension != dimensionTo) {
                ...
                if (mainPortalTile.linkedPortal == null) {
                    if (!this.isActivePortalAtPos(worldserver, mainPortalTile.getPos(), Zaxis)) {
                        BlockPos placedPortal = this.canPlaceUnderground ? this.tryPlacePortalUnderground(worldserver, mainPortalTile.getPos(), Zaxis, oldDimension) : this.tryPlacePortal(worldserver, mainPortalTile.getPos(), Zaxis, oldDimension);
                ...
                worldserver.getMinecraftServer().getPlayerList().transferPlayerToDimension(playerMP, dimensionTo, new TeleporterUniversal(worldserver, playerX, playerY, playerZ));
