...
public class LabGenerator {
    ...
    public static void generateLab(World world, BlockPos pos, Random rand, float coloredGlassChance) {
        ...
        for (int i = 0; i < roomcount; i++) {
            makeBigSurfaceRoom(world, pos.up(i * height), xx, zz, height, 2, Math.min(3 + rand.nextInt(4), height - 1), 4 + rand.nextInt(4), 1 + rand.nextInt(6), i == roomcount - 1, rand);
        }
        ...
        for (EnumFacing facing : EnumFacing.HORIZONTALS) {
            ...
            makeSurfaceTunnel(world, pos.add(facing.getXOffset() * (xx - 1) + xoffset, 0, facing.getZOffset() * (zz - 1) + zoffset), facing, 16 + rand.nextInt(10), 2 + rand.nextInt(4), Math.min(sizet + 1 + rand.nextInt(3), height + 1), 3 + rand.nextInt(4), rand, cont, facing.getOpposite());
        }
        ...
        makeUndergroundTunnel(world, stairsPos.add(facing.getXOffset() * 3, 0, facing.getZOffset() * 3), facing, 16 + rand.nextInt(10), 2 + rand.nextInt(4), Math.min(sizet + 1 + rand.nextInt(3), height + 1), 3 + rand.nextInt(4), rand, cont, facing.getOpposite());
    }

    public static void makeBigSurfaceRoom(World world, BlockPos pos, int xsize, int zsize, int ysize, int windowMin, int windowMax, int windowFrequency, int windowSize, boolean roof, Random rand) {
    ...
    public static void makeSurfaceTunnel(World world, BlockPos position, EnumFacing facing, int length, int size, int height, int lampFrequency, Random rand, int continueTunnel, EnumFacing disabledFacing) {
                ...
                for (int xz = -size; xz <= size; xz++) {
                    for (int y = 0; y <= height; y++) {
                        ...
                        if (clmn && xz == 0 && y == height - 1) {
                            world.setBlockState(pos.add(0, y, xz), BlocksRegister.RUST_LAMP.getDefaultState().withProperty(RustLamp.FACING, EnumFacing.DOWN).withProperty(RustLamp.ON, rand.nextFloat() < 0.25F));
                        } else if (y > sheight) {
                ...
                for (int xz = -size; xz <= size; xz++) {
                    for (int yx = 0; yx <= height; yx++) {
                        ...
                        if (clmn && xz == 0 && yx == height - 1) {
                            world.setBlockState(pos.add(xz, yx, 0), BlocksRegister.RUST_LAMP.getDefaultState().withProperty(RustLamp.FACING, EnumFacing.DOWN).withProperty(RustLamp.ON, rand.nextFloat() < 0.25F));
                        } else if (yx > sheight) {
    ...
    public static void makeSurfaceRoom(World world, BlockPos position, int size, int height, Random rand, EnumFacing entryFacing, int continueTunnel, EnumFacing disabledFacing) {
        ...
        if (continueTunnel > 0) {
            ...
            makeSurfaceTunnel(world, position.offset(cfacing, size - 1), cfacing, 8 + rand.nextInt(8), rsize, rheig, 3 + rand.nextInt(4), rand, continueTunnel - 1, disabledFacing);
        }

        decorateSurfaceRoomCenter(world, position.up(), size - 1, rand);
    }

    public static void makeUndergroundTunnel(World world, BlockPos position, EnumFacing facing, int length, int size, int height, int lampFrequency, Random rand, int continueTunnel, EnumFacing disabledFacing) {
        boolean axisX = facing.getAxis() == Axis.X;
        BlockPos pos = position.offset(facing);
        int lem2 = length - 2;
                    ...
                    for (int y = 0; y <= height; y++) {
                        ...
                        if (y > sheight) {
                            ...
                            if (clmn && xz == -size + 1 && y == 3) {
                                world.setBlockState(pos.add(0, y, xz), BlocksRegister.RUST_LAMP.getDefaultState().withProperty(RustLamp.FACING, EnumFacing.SOUTH).withProperty(RustLamp.ON, rand.nextFloat() < 0.25F));
                            } else if (clmn && xz == size - 1 && y == 3) {
                                world.setBlockState(pos.add(0, y, xz), BlocksRegister.RUST_LAMP.getDefaultState().withProperty(RustLamp.FACING, EnumFacing.NORTH).withProperty(RustLamp.ON, rand.nextFloat() < 0.25F));
                            } else if (y == 0) {
                                if (xz == 0) {
                    ...
                    for (int yx = 0; yx <= height; yx++) {
                        ...
                        if (yx > sheight) {
                            ...
                            if (clmn && xz == -size + 1 && yx == 3) {
                                world.setBlockState(pos.add(xz, yx, 0), BlocksRegister.RUST_LAMP.getDefaultState().withProperty(RustLamp.FACING, EnumFacing.EAST).withProperty(RustLamp.ON, rand.nextFloat() < 0.25F));
                            } else if (clmn && xz == size - 1 && yx == 3) {
                                world.setBlockState(pos.add(xz, yx, 0), BlocksRegister.RUST_LAMP.getDefaultState().withProperty(RustLamp.FACING, EnumFacing.WEST).withProperty(RustLamp.ON, rand.nextFloat() < 0.25F));
                            } else if (yx == 0) {
                                if (xz == 0) {
                                    world.setBlockState(pos, glass);
                                    world.setBlockState(pos.down(1), BlocksRegister.RUSTED_PIPE.getDefaultState());
    ...
    public static void makeUndergroundRoom(World world, BlockPos position, int size, int height, Random rand, EnumFacing entryFacing, int continueTunnel, EnumFacing disabledFacing) {
        int height2 = height / 2;
        int sizeRound = (int) (size * 0.5F);
        ...
        if (continueTunnel > 0) {
            ...
            int rsize = Math.round(size / 1.5F) + rand.nextInt(2) - rand.nextInt(3);
            int rheig = Math.max(3, Math.round(height / 1.5F) + rand.nextInt(2) - rand.nextInt(3));
            makeUndergroundTunnel(world, position.offset(cfacing, size - 1), cfacing, 8 + rand.nextInt(8), rsize, rheig, 3 + rand.nextInt(4), rand, continueTunnel - 1, disabledFacing);
        }
    }
