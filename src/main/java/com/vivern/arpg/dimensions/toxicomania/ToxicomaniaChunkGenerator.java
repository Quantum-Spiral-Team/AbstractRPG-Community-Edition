...
public class ToxicomaniaChunkGenerator implements IChunkGenerator {
    ...
    public WorldGenPetroleum petroleumGenerator = new WorldGenPetroleum(BlocksRegister.FLUID_PETROLEUM.getDefaultState(), BlocksRegister.RADIOACTIVE_STONE.getDefaultState(), BlocksRegister.FLUID_NATURAL_GAS.getDefaultState(), 172, WorldGenPetroleum.defaultBlocksToReplace, 16, 10, 22, 0.3F, true).setOptions(0.08, 0.04, 4.0, 2.0, 0.4, 0.6, 0.8, 1.2, 0.2, 0.8);
    public WorldGenPetroleum naturalGasGenerator = new WorldGenPetroleum(BlocksRegister.FLUID_NATURAL_GAS.getDefaultState(), BlocksRegister.RADIOACTIVE_STONE.getDefaultState(), null, 172, WorldGenPetroleum.defaultBlocksToReplace, 20, 9, 21, 0.0F, true).setOptions(0.08, 0.04, 4.0, 2.0, 0.2, 0.5, 1.0, 1.1, 0.14, 0.45);
    public static WorldGenSpread blobs = new WorldGenSpread(BlocksRegister.SLIME_BLOB, 50, 4, 2, null);
    public static Block[] caveJunkPlants = new Block[]{BlocksRegister.MUCOPHILLUS, BlocksRegister.MUCOPHILLUS_BROWN, BlocksRegister.VISCOSA, BlocksRegister.TOXIBERRY_ARCANO};
    ...
    public ToxicomaniaChunkGenerator(World worldIn, long seed) {
        ...
        this.arsenicOreGen = new CustomOreGenerator(BlocksRegister.ARSENIC_ORE.getDefaultState(), 101, BlockMatcher.forBlock(BlocksRegister.RADIOACTIVE_STONE), 4, 10, 6, 10, 50);
        this.toxiniumOreGen = new CustomOreGenerator(BlocksRegister.TOXINIUM_ORE.getDefaultState(), 101, BlockMatcher.forBlock(BlocksRegister.RADIOACTIVE_STONE), 2, 8, 2, 6, 25);
        this.wolframOreGen = new CustomOreGenerator(BlocksRegister.WOLFRAM_ORE.getDefaultState(), 101, BlockMatcher.forBlock(BlocksRegister.RADIOACTIVE_STONE), 2, 6, 8, 5, 55);
        this.lepidoliteOreGen = new CustomOreGenerator(BlocksRegister.LEPIDOLITE_ORE.getDefaultState(), 101, BlockMatcher.forBlock(BlocksRegister.RADIOACTIVE_STONE), 2, 7, 3, 10, 30);
        ...
        if (ForgeEventFactory.onReplaceBiomeBlocks(this, x, z, primer, this.world)) {
            ...
            for (int i = 0; i < 16; i++) {
                for (int j = 0; j < 16; j++) {
                    ...
                    this.generateBiomeTerrain(this.world, this.rand, primer, x * 16 + i, z * 16 + j, this.depthBuffer[j + i * 16], biome, biome instanceof BiomeControlled);
                }
            }
        }
    }
    ...
    public void populate(int x, int z) {
        ...
        if (!flag && this.rand.nextInt(this.settings.lavaLakeChance / 10) == 0 && this.settings.useLavaLakes && TerrainGen.populate(this, this.world, this.rand, x, z, flag, EventType.LAVA)) {
        ...
        if (this.rand.nextInt(15) == 0 && !this.isDungeonNearChunk(this.world, x, z, 10)) {
            ...
            BunkerGenerator.generateBunker(this.world, new BlockPos(i + this.rand.nextInt(16) - disp, 16 + this.rand.nextInt(9), j + this.rand.nextInt(16) - disp), arraySize, 2 + this.rand.nextInt(3), this.rand.nextFloat() < 0.3 ? 2 : 3, this.rand);
            ...
            if (!this.world.getBlockState(pos5).getMaterial().isLiquid() && Math.abs(pos5.getY() - pos1.getY()) < 4 && Math.abs(pos5.getY() - pos2.getY()) < 4 && Math.abs(pos5.getY() - pos3.getY()) < 4 && Math.abs(pos5.getY() - pos4.getY()) < 4) {
                this.dungeons.add(0L | x | (long) z << 32);
            ...
            if (!listposes.isEmpty()) {
                ...
                for (int c = 0; c < maxc; c++) {
                    ...
                    if (finalpos != null) {
                        ...
                        } else if (this.rand.nextFloat() < 0.3F && this.world.getBlockState(finalpos.down()).getBlock() == BlocksRegister.RADIOACTIVE_STONE && this.world.getBlockState(finalpos.up()).getCollisionBoundingBox(this.world, finalpos.up()) == null) {
                            ToxicomaniaMobsPack.PoisonSpitter mob = new ToxicomaniaMobsPack.PoisonSpitter(this.world);
                            mob.setPosition(finalpos.getX() + 0.45 + this.rand.nextFloat() * 0.1, finalpos.getY(), finalpos.getZ() + 0.45 + this.rand.nextFloat() * 0.1);
                ...
                if (rand.nextFloat() < 0.255) {
                    ...
                    for (int yy = 132; yy > 11; yy--) {
                        ...
                        if (state.getBlock() == Blocks.AIR) {
                            ...
                            if (state.getBlock() == BlocksRegister.RADIOACTIVE_STONE && stateup != null && (stateup.getBlock() == blockstalact || stateup.getBlock() == Blocks.AIR) && fpos.getY() < 62) {
                                        ...
                                        if (rand.nextFloat() < 0.9) {
                                            if (rand.nextFloat() < 0.6) {
                                                ...
                                                if (rand.nextFloat() < 0.6 && (state3.getBlock() == blockstalact || state3.getBlock() == Blocks.AIR)) {
                                                    ...
                                                    if (rand.nextFloat() < 0.5 && (state4.getBlock() == blockstalact || state4.getBlock() == Blocks.AIR)) {
                                                        ...
                                                        if (rand.nextFloat() < 0.5 && (state5.getBlock() == blockstalact || state5.getBlock() == Blocks.AIR)) {
    ...
    public static void genNectarFlowerPlace(World world, BlockPos pos, Random rand) {
        ...
        for (int ix = 0; ix <= 30; ix++) {
            ...
            if (world.getBlockState(pos4).getBlock().isReplaceable(world, pos4) && world.getBlockState(pos3).getBlock() != BlocksRegister.CHLORINE_BELCHER && world.getBlockState(pos3).getBlock() != BlocksRegister.SWEET_NECTAR_FLOWER) {
            ...
            if (block == BlocksRegister.RADIOACTIVE_STONE) {
                ...
                if (stone) {
                    if (rand.nextFloat() < 0.3F) {
                        ...
                        world.setBlockState(posf, rand.nextFloat() < 0.76F ? BlocksRegister.LOPPY_TOXISTEM.getDefaultState() : BlocksRegister.LOPPY_TOXIBERRY.getDefaultState());
                    }
                }
            } else {
                stone = false;
    ...
    public static void genCaveJunkPlants(World world, BlockPos pos, Random rand) {
        ...
        for (int ix = 0; ix < 10; ix++) {
            ...
            if (world.getBlockState(fpos).getBlock() == JUNK.getBlock() && world.getBlockState(fpos.up()).getBlock() == Blocks.AIR) {
    ...
    public static void genPoisonPuddle(World world, BlockPos pos, Random rand, Block fluid, boolean lilypad) {
        ...
        for (int i = 0; i < maxi; i++) {
            ...
            if (vlod == BlocksRegister.RADIOACTIVE_STONE && world.getBlockState(downpos.down()).isOpaqueCube() && !GetMOP.collidesWithBlockHorizontal(world, downpos, Blocks.AIR)) {
