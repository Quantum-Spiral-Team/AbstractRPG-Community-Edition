...
public class DimensionDungeon extends WorldProvider {
    ...
    public static void placeDungeonDecor(World worldIn, BlockPos pos, EnumCaveDecorType structure, EnumCaveDecorType structureStyle, Random rand) {
    ...
    public static int getCaveRegionNumberFromCoord(int crX, int crZ) {
        int y = -crX;
        return (int) Math.round(Math.pow(Math.abs(Math.abs(crZ) - Math.abs(y)) + Math.abs(crZ) + Math.abs(y), 2.0) + Math.abs(crZ + y + 0.1) / (crZ + y + 0.1) * (Math.abs(Math.abs(crZ) - Math.abs(y)) + Math.abs(crZ) + Math.abs(y) + crZ - y) + 1.0);
    }
    ...
    public static BlockPos getRelativePosInRegion(BlockPos absolutepos, int crX, int crZ) {
        BlockPos center = getRegionCenterPos(crX, crZ);
        return new BlockPos(absolutepos.getX() - center.getX(), absolutepos.getY() - center.getY(), absolutepos.getZ() - center.getZ());
    }
