...
public class MineshaftGenerator2 {
        ...
        for (int xx = -checkRadius; xx <= checkRadius; xx++) {
            for (int zz = -checkRadius; zz <= checkRadius; zz++) {
                ...
                if (rand.nextFloat() < 2.0F / (level + 30.0F)) {
                    ...
                    MineshaftGenerator2 gen = new MineshaftGenerator2(2 + rand.nextInt(1 + rand.nextInt(1 + rand.nextInt(3))), height, 2 + rand.nextInt(5), 15 + rand.nextInt(45 + level), chunkprimer);
    ...
    public boolean isInGenRange(BlockPos pos) {
        return pos.getX() >= this.genFromX && pos.getX() <= this.genToX && pos.getZ() >= this.genFromZ && pos.getZ() <= this.genToZ;
    }
            ...
            if (this.isInGenRange(fpos1)) {
                ...
                if (i == 0) {
                    ...
                    if (nearDist < 0) {
                        ...
                        this.setBlockState(new BlockPos(fpos1.getX(), fpos1.getY() + nearDist, fpos1.getZ()), BlocksRegister.ROTTEN_PLANKS.getDefaultState());
                    }
                }
            }
        }
    ...
    public void placeLadder(World world, Random rand, BlockPos pos, int size, int height, int downLevels) {
        ...
        if (pos.getY() + downAdd >= 3) {
            ...
            if (this.isInGenRange(posLadder)) {
                for (int ix = 0; ix < downLevels; ix++) {
                    this.setBlockState(posLadder.down(ix * 2), Blocks.LADDER.getDefaultState().withProperty(BlockLadder.FACING, faceLadderOpp));
                    this.setBlockState(posLadder.down(ix * 2 + 1), Blocks.LADDER.getDefaultState().withProperty(BlockLadder.FACING, faceLadderOpp));
                }
        ...
        if (face != null) {
            ...
            if (style == 0) {
                ...
                for (int i = 0; i < decorAmount; i++) {
                    ...
                    BlockPos finalPos = new BlockPos(poss.getX() + randOffset * face.getXOffset() + randOffset2 * faceRopp.getXOffset(), poss.getY() + randOffset3, poss.getZ() + randOffset * face.getZOffset() + randOffset2 * faceRopp.getZOffset());
