        ...
        if (this.mode == DimensionDungeon.EnumCaveDecorType.MAGIC) {
            ...
            if (blockInfoIn.blockState.getBlock() == BlocksRegister.GREEN_GLOWING_MUSHROOM) {
                if (world.rand.nextFloat() < 0.95) {
                    return new BlockInfo(blockInfoIn.pos, BlocksRegister.BLUE_GLOWING_MUSHROOM.getStateFromMeta(BlocksRegister.GREEN_GLOWING_MUSHROOM.getMetaFromState(blockInfoIn.blockState)), blockInfoIn.tileentityData);
                }
            ...
            if (blockInfoIn.blockState.getBlock() == Blocks.STONE_BRICK_STAIRS) {
                return new BlockInfo(blockInfoIn.pos, BlocksRegister.DOLERITE_STAIRS.getStateFromMeta(Blocks.STONE_BRICK_STAIRS.getMetaFromState(blockInfoIn.blockState)), blockInfoIn.tileentityData);
            }

            if (blockInfoIn.blockState.getBlock() == Blocks.STONE_STAIRS) {
                return new BlockInfo(blockInfoIn.pos, BlocksRegister.DOLERITE_STAIRS.getStateFromMeta(Blocks.STONE_STAIRS.getMetaFromState(blockInfoIn.blockState)), blockInfoIn.tileentityData);
            }
        } else if (this.mode == DimensionDungeon.EnumCaveDecorType.CALCITE && (blockInfoIn.blockState.getBlock() == Blocks.STONE || blockInfoIn.blockState.getBlock() == Blocks.COBBLESTONE)) {
