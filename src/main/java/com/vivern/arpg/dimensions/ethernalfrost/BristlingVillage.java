...
public class BristlingVillage {
    ...
    public static Predicate<IBlockState> BLOCKS_TO_COLLIDE = input -> {
        if (input.getBlock() == BlocksRegister.CONIFER_PLANKS) {
            ...
            return input.getMaterial() == Material.SNOW ? input.isFullBlock() : input.getMaterial() != Material.AIR && input.getMaterial() != Material.LEAVES && input.getMaterial() != Material.PLANTS && input.getMaterial() != Material.CACTUS && input.getMaterial() != Material.VINE && input.getMaterial() != Material.WOOD;
        }
    };
    ...
    public World world;
    public IBlockState[] roadBlocks = new IBlockState[]{BlocksRegister.CONIFER_PLANKS.getDefaultState(), Blocks.SNOW.getDefaultState(), BlocksRegister.CONIFER_PLANKS.getDefaultState()};
    public IBlockState[] fluidRoadBlocks = new IBlockState[]{BlocksRegister.CONIFER_PLANKS.getDefaultState(), BlocksRegister.FROZEN_STONE.getDefaultState(), BlocksRegister.BLOCK_BLOCK_HARD.getDefaultState()};
    public Random rand;
    public static NamedInt[] structuresHomes = new NamedInt[]{new NamedInt(":frozen_home_1", 6), new NamedInt(":frozen_home_2", 9), new NamedInt(":frozen_home_3", 6), new NamedInt(":frozen_home_4", 9), new NamedInt(":frozen_home_5", 14), new NamedInt(":frozen_home_6", 8), new NamedInt(":frozen_home_7", 6)};
    public static NamedInt[] structuresRoad = new NamedInt[]{new NamedInt(":christmas_tree_1", 4), new NamedInt(":christmas_tree_2", 5), new NamedInt(":bristling_farm_1", 4), new NamedInt(":bristling_farm_2", 4)};
    public static NamedInt[] structuresLamps = new NamedInt[]{new NamedInt(":bristling_lamp_1", 1), new NamedInt(":bristling_lamp_2", 1), new NamedInt(":bristling_lamp_3", 1), new NamedInt(":bristling_lamp_4", 1)};
    ...
    public void recursiveGenerate(BlockPos pos, Vec3d direction, int last) {
        if (Math.abs(this.center.getX() - pos.getX()) <= this.maxVillageRadius && Math.abs(this.center.getZ() - pos.getZ()) <= this.maxVillageRadius) {
            last--;
        } else {
            last = 0;
        }
            ...
            if (result != null) {
                if ((this.rand.nextFloat() < 0.95F + Debugger.floats[7] || this.noHomesTime > 0) && last > 0) {
                    if (this.isClear(result.pos, 0)) {
                        ...
                        if (this.rand.nextFloat() < 0.35F + Debugger.floats[4]) {
                            ...
                            if ((this.rand.nextFloat() < 0.8 + Debugger.floats[6] || this.noHomesTime > 0) && this.outpathesLast > 0) {
                                ...
                                NamedInt structure = this.rand.nextFloat() < 0.5F ? structuresLamps[this.lampstyle] : structuresRoad[this.rand.nextInt(structuresRoad.length)];
                                ...
                                if (result2 != null && this.isClear(result2.pos, structure.value)) {
                                    ...
                                    GenerationHelper.placeStruct(this.world, result2.pos, this.rand, structure.name, structure.value, 0, rotation, ChestReplacersFrozen.replacerPresentBox);
                    ...
                    if (this.isClear(finalpos, structure.value)) {
                        GenerationHelper.placeStruct(this.world, finalpos, this.rand, structure.name, structure.value, structId == 4 ? -9 : 1, face.getHorizontalIndex(), ChestReplacersFrozen.replacerPresentBox);
    ...
    public void placeFoundation(World world, BlockPos pos, int size) {
        for (int x = pos.getX() - size; x <= pos.getX() + size; x++) {
            for (int z = pos.getZ() - size; z <= pos.getZ() + size; z++) {
                ...
                if (material.getBlock() == BlocksRegister.CONIFER_LOG || material.getBlock() == BlocksRegister.CONIFER_PLANKS || material.getBlock() == BlocksRegister.BLOCK_BLOCK_HARD || material.getBlock() == BlocksRegister.FROZEN_COBBLESTONE || material.getBlock() == Blocks.PLANKS) {
                    generate = true;
                }
