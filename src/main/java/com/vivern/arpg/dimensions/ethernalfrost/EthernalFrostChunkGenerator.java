...
public class EthernalFrostChunkGenerator implements IChunkGenerator {
    ...
    public EthernalFrostChunkGenerator(World worldIn, long seed) {
        ...
        this.snowiceOreGen = new CustomOreGenerator(BlocksRegister.SNOW_ICE.getDefaultState(), 100, BlockMatcher.forBlock(BlocksRegister.GLACIER), 10, 33, 2, 20, 100);
        this.frozenstoneOreGen = new CustomOreGenerator(BlocksRegister.FROZEN_STONE.getDefaultState(), 100, BlockMatcher.forBlock(BlocksRegister.GLACIER), 10, 20, 2, 20, 80);
        this.iceOreGen = new CustomOreGenerator(Blocks.ICE.getDefaultState(), 100, BlockMatcher.forBlock(BlocksRegister.GLACIER), 8, 22, 5, 22, 250);
        this.frozencobbleOreGen = new CustomOreGenerator(BlocksRegister.FROZEN_COBBLESTONE.getDefaultState(), 100, BlockMatcher.forBlock(BlocksRegister.GLACIER), 4, 15, 6, 2, 50);
        this.frozenSlimes = new CustomOreGenerator(BlocksRegister.FROZEN_SLIME.getDefaultState(), 100, BlockMatcher.forBlock(BlocksRegister.GLACIER), 6, 22, 4, 14, 100);
        this.frozenDebrisOreGen = new CustomOreGenerator(BlocksRegister.FROZEN_DEBRIS.getDefaultState(), 100, state -> state.getBlock() == BlocksRegister.FROZEN_STONE || state.getBlock() == BlocksRegister.GLACIER, 1, 4, 8, 2, 17);
    ...
    public void populate(int x, int z) {
        ...
        if (dungeonValue >= 0.0F && dungeonValue < 0.1F) {
            BlockPos castlepos = GetMOP.getTopBlocks(this.world, blockpos.add(this.rand.nextInt(16), 254, this.rand.nextInt(16)), Blocks.SNOW, Blocks.ICE, BlocksRegister.SNOW_ICE, BlocksRegister.GLACIER);
            if (castlepos.getY() > 1) {
        ...
        if (dungeonValue >= 0.63F && dungeonValue < 0.75F && !(biome instanceof FrozenMountain)) {
            BlockPos poshall = GetMOP.getTopBlocks(this.world, blockpos.add(this.rand.nextInt(16), 254, this.rand.nextInt(16)), Blocks.SNOW, Blocks.ICE, Blocks.WATER, BlocksRegister.SNOW_ICE, BlocksRegister.GLACIER);
            if (poshall.getY() > 1) {
                ...
                for (int j3 = 0; j3 < 16; j3++) {
                    ...
                    if (biomee == BiomesRegister.FROZEN_BEACH || biomee == BiomesRegister.EVERFROST_RIVER) {
                        ...
                        if (statDown.isFullCube() && statDown.getBlock() != BlocksRegister.LOOSE_SNOW && statDown.getBlock() != Blocks.SNOW) {
                            this.world.setBlockState(blockpos1, statDown.getBlock() == Blocks.ICE ? BlocksRegister.LOOSE_SNOW.getDefaultState() : Blocks.SNOW_LAYER.getDefaultState(), 2);
                        }
                    }
    ...
    public static boolean checkFlat(World worldIn, BlockPos pos) {
        ...
        return worldIn.getBlockState(posd.west(4)).getBlock() != Blocks.AIR && worldIn.getBlockState(posd.east(4)).getBlock() != Blocks.AIR && worldIn.getBlockState(posd.south(4)).getBlock() != Blocks.AIR && worldIn.getBlockState(posd.north(4)).getBlock() != Blocks.AIR;
    }
    ...
    public static void recursiveGenerateMound(World world, BlockPos startpos, boolean[][][] fillArray, int x, int y, int z, int rotate, Random rand, int[] sizeContainer) {
        if (rand.nextFloat() < 0.8) {
                ...
                for (int rr = 0; rr < 4; rr++) {
                    if (rr != rotate && rr != newr) {
                        if (rand.nextFloat() < 0.25) {
                            ...
                            if (!isOutOfMound(rxx + 1, y, rzx + 1) && !isOutOfMound(rxx - 1, y, rzx - 1) && !fillArray[rxx][y][rzx + 1] && !fillArray[rxx - 1][y][rzx] && !fillArray[rxx][y][rzx] && !fillArray[rxx + 1][y][rzx] && !fillArray[rxx][y][rzx - 1]) {
                                BlockPos rrpos;
                                if (rr == 0) {
            ...
            if (fillArray[x - 1][y][z + 1] || fillArray[x][y][z + 1] || fillArray[x + 1][y][z + 1] || fillArray[x - 1][y][z] || fillArray[x][y][z] || fillArray[x + 1][y][z] || fillArray[x - 1][y][z - 1] || fillArray[x][y][z - 1] || fillArray[x + 1][y][z - 1]) {
                return;
            }

            BlockPos finalpos = startpos.add(x * 7, -y * 15, z * 7);
            GenerationHelper.placeStructMound(world, finalpos, rand, ":mound_room_" + (rand.nextFloat() < 0.3 ? rand.nextInt(3) + 3 : rand.nextInt(2) + 1), 10, 0, rotate);
            fillArray[x][y][z] = true;
    ...
    public static void generateChestAndSpawner(World world, BlockPos pos, int size, Random rand) {
        ...
        GenerationHelper.setChestWithLoot(world, pos.up(), EnumChest.FROZEN, ListLootTable.CHESTS_FROZEN_STRUCTURES, EnumFacing.HORIZONTALS[rand.nextInt(4)]);
