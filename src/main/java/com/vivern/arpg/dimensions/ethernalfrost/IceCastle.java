...
public class IceCastle {
    ...
    public void placeFoundation(World world, BlockPos pos, int size, boolean underground) {
        for (int x = pos.getX() - size; x <= pos.getX() + size; x++) {
            for (int z = pos.getZ() - size; z <= pos.getZ() + size; z++) {
                ...
                if (material.getBlock() == BlocksRegister.BLOCK_BLOCK_HARD || material.getBlock() == Blocks.STONEBRICK || material.getBlock() == BlocksRegister.FROZEN_BRICK_PILASTER) {
                    generate = true;
                }

                if (material.getBlock() == BlocksRegister.FROZEN_VASE || material.getBlock() == BlocksRegister.FROZEN_CHANDELIER || material.getBlock() == BlocksRegister.FROZEN_BRICK_STAIRS) {
                ...
                if (generate) {
                    if (!underground) {
                        for (int y = pos.getY() - 1; y > 1; y--) {
                            ...
                            if (!block.isReplaceable(world, fpos) && !block.isLeaves(getted, world, fpos) && block != Blocks.ICE && block != Blocks.SNOW && block != Blocks.SNOW_LAYER) {
                                break;
                            }
    ...
    public static void placeSpawner(World world, BlockPos pos, boolean parapet, Random random) {
        ...
        DimensionEthernalFrost.setupRandomSpawner(world, world.getTileEntity(pos), parapet ? DimensionEthernalFrost.EnumEverfrostSpawner.ICE_CASTLE_PARAPET : DimensionEthernalFrost.EnumEverfrostSpawner.ICE_CASTLE, random);
    }
    ...
    public static void placeChest(World world, BlockPos pos, Random random, @Nullable IBlockState baseState, boolean rare) {
        if (random.nextFloat() < (rare ? 0.75 : 0.3)) {
            GenerationHelper.setChestWithLoot(world, pos, EnumChest.FROZEN, ListLootTable.CHESTS_ICE_CASTLE_RICH, baseState == null ? EnumFacing.HORIZONTALS[random.nextInt(4)] : baseState.getValue(BlockChest.FACING), ChestLock.WINTER_CURSE);
            ChestReplacersFrozen.posesToSetPuzzle.add(pos);
        } else {
            GenerationHelper.setChestWithLoot(world, pos, EnumChest.FROZEN, ListLootTable.CHESTS_ICE_CASTLE, baseState == null ? EnumFacing.HORIZONTALS[random.nextInt(4)] : baseState.getValue(BlockChest.FACING), ChestLock.WINTER_CURSE);
        }
    }
    ...
    public List<StructPos> generateCastleWalls(World worldIn, Random random, List<BlockPos> roomlist, List<BlockPos> envlist, int y) {
        ...
        for (StructPos stpos : wallslist) {
            ...
            if ((!up || !down || left || right) && (!right || !left || down || up)) {
                ResourceLocation locationw = this.getNearestCollidesCount(worldIn, stpos.blockpos, roomlist) < 2 ? new ResourceLocation("arpg:ice_castle_wall_tower") : new ResourceLocation("arpg:ice_castle_wall_inside");
                if (up && left && !down && !right) {
                    ...
                    this.placeStructure(worldIn, random, new BlockPos(stpos.blockpos.getX(), y, stpos.blockpos.getZ()), new ResourceLocation("arpg:ice_castle_wall_quad"), 4, 3);
                }
            } else {
                this.placeStructure(worldIn, random, new BlockPos(stpos.blockpos.getX(), y, stpos.blockpos.getZ()), new ResourceLocation("arpg:ice_castle_wall"), 4, stpos.rotation);
            }
        }
    ...
    public void generateEnvironment(World worldIn, Random random, List<BlockPos> list, List<BlockPos> roomlist, List<BlockPos> envlist) {
        for (BlockPos pos : roomlist) {
            if (random.nextFloat() < 0.93) {
                ...
                if (this.getNearestCollidesCount(worldIn, posoffset, roomlist) == 0 && this.getNearestCollidesCount(worldIn, posoffset, list) == 0 && this.getNearestCollidesCount(worldIn, posoffset, envlist) == 0) {
    ...
    public void generateRoom(World worldIn, Random random, BlockPos position, int samples, List<BlockPos> list, List<BlockPos> roomlist, List<BlockPos> foundationlist, int generatorRotation) {
    ...
    public void placeOutsideRoomWithRotation(World worldIn, Random random, BlockPos position, int rotation, int mode, List<BlockPos> list, List<BlockPos> foundationlist) {
        WorldServer worldServer = (WorldServer) worldIn;
        MinecraftServer minecraftServer = worldIn.getMinecraftServer();
        TemplateManager templateManager = worldServer.getStructureTemplateManager();
    ...
    public void placeMiniTower(World worldIn, Random random, BlockPos position, PlacementSettings settings) {
        ...
        for (int i = 0; i < random.nextInt(4); i++) {
            templatetower1.addBlocksToWorld(worldIn, position.add(sx * 3 + sxx, updisplace, sz * 3 + szz), ChestReplacersFrozen.replacerIceCastle, settings, 2);
            updisplace += 8;
        }
            ...
            switch (rotat) {
                ...
                default:
                    ...
                    if (random.nextFloat() < 0.3 && rotat != generatorRotation) {
                        ...
                        if (random.nextFloat() < 0.4 && nextpos2.getY() > 19 && !this.listPosMatch(list, nextpos2) && !this.listPosMatch(list, nextpos2.add(0, -9, 0))) {
    ...
    public void generateHallDouble(World worldIn, Random random, BlockPos position, List<BlockPos> list) {
        if (!this.listPosMatch(list, position) && !this.listPosMatch(list, position.add(0, -9, 0))) {
            ...
            MinecraftServer minecraftServer = worldIn.getMinecraftServer();
            TemplateManager templateManager = worldServer.getStructureTemplateManager();
            Template template = templateManager.get(minecraftServer, new ResourceLocation("arpg:ice_castle_underground_double_" + (random.nextInt(2) + 1)));
            PlacementSettings settings = new PlacementSettings();
            int sx = -1;
            int sz = -1;
            int swr = random.nextInt(4);
    ...
    public void generateCorridor(World worldIn, Random random, BlockPos position, int samples, List<BlockPos> list, int generatorRotation) {
        if (!this.listPosMatch(list, position)) {
            ...
            MinecraftServer minecraftServer = worldIn.getMinecraftServer();
            TemplateManager templateManager = worldServer.getStructureTemplateManager();
            Template template = templateManager.get(minecraftServer, new ResourceLocation("arpg:ice_castle_underground_corridor_" + (random.nextInt(2) + 1)));
            PlacementSettings settings = new PlacementSettings();
            int sx = -1;
            int sz = -1;
    ...
    public boolean generateBigRoom(World worldIn, Random random, BlockPos pos, List<BlockPos> list, int generatorRotation) {
        ...
        switch (generatorRotation) {
            ...
            case 3:
                position = position.add(0, 0, -9);
            default:
                if (!this.listPosMatch(list, position) && !this.listPosMatch(list, position.add(9, 0, 0)) && !this.listPosMatch(list, position.add(-9, 0, 0)) && !this.listPosMatch(list, position.add(0, 0, 9)) && !this.listPosMatch(list, position.add(0, 0, -9)) && !this.listPosMatch(list, position.add(9, 0, 9)) && !this.listPosMatch(list, position.add(-9, 0, 9)) && !this.listPosMatch(list, position.add(9, 0, -9)) && !this.listPosMatch(list, position.add(-9, 0, -9))) {
                    list.add(position);
