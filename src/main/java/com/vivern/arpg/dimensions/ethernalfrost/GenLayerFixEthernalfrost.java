...
public class GenLayerFixEthernalfrost {
    ...
    public static GenLayer[] createWorld(long seed) {
        ...
        for (int i = 0; i < BiomesRegister.frozen_mountains_amount - 1; i++) {
            genlayer = new GenLayerMountain(2031L + i, genlayer, Biome.getIdForBiome(BiomesRegister.FROZEN_MOUNTAINS[0 + i]), Biome.getIdForBiome(BiomesRegister.FROZEN_MOUNTAINS[1 + i]));
        }
        ...
        public int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight) {
            ...
            for (int i1 = 0; i1 < areaHeight; i1++) {
                for (int j1 = 0; j1 < areaWidth; j1++) {
                    ...
                    if (k2 != Biome.getIdForBiome(BiomesRegister.FROZEN_MEADOW) && k2 != Biome.getIdForBiome(BiomesRegister.FROZEN_WARM_FOREST) && k2 != Biome.getIdForBiome(BiomesRegister.ICE_HILLS) && k2 != Biome.getIdForBiome(BiomesRegister.FROZEN_FOREST) || k1 != this.neighborId && l1 != this.neighborId && i2 != this.neighborId && j2 != this.neighborId) {
