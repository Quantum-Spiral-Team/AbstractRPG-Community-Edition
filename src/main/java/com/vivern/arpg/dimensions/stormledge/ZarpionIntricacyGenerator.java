    ...
    public void generateIntricacy(World world, Random rand) {
        ...
        for (int xx = -halfSizeInBlocks; xx <= halfSizeInBlocks; xx++) {
            for (int zz = -halfSizeInBlocks; zz <= halfSizeInBlocks; zz++) {
                ...
                if ((Math.abs(xx) > 2 && Math.abs(zz) > 2 || Math.abs(xx) < halfSizeInBlocks - this.corridorWidth * 2 && Math.abs(zz) < halfSizeInBlocks - this.corridorWidth * 2) && (xx == -halfSizeInBlocks || xx == halfSizeInBlocks || zz == -halfSizeInBlocks || zz == halfSizeInBlocks || cells[arrX][arrZ])) {
                ...
                if (pyramidRoofHeight > 0) {
                    this.column(world, inPos.up(7), pyramidRoofHeight, isWall ? BlocksRegister.STORM_MASONRY.getDefaultState() : BlocksRegister.STORM_PLATE.getDefaultState());
                    this.column(world, inPos.down(), -pyramidRoofHeight, isWall ? BlocksRegister.STORM_MASONRY.getDefaultState() : BlocksRegister.STORM_PLATE.getDefaultState());
                }
                        ...
                        if (!this.hasPos(rooms, newpart.pos)) {
                            if (rand.nextFloat() < 0.55F) {
                                ...
                                if (rand.nextFloat() < 0.85) {
                                    GenerationHelper.setChestWithLoot(world, newpart.pos.up(30), EnumChest.STORM, ListLootTable.CHESTS_ZARPION_INTRICACY, face);
        ...
        for (int i = 0; i <= 20; i++) {
            ...
            if (i == 0) {
                ...
                npc.setPosition(this.centerPos.getX() + 0.5 + (rand.nextFloat() - 0.5) * 4.0, this.centerPos.getY() + 3.1 + (rand.nextFloat() - 0.5) * 3.0, this.centerPos.getZ() + 0.5 + (rand.nextFloat() - 0.5) * 4.0);
            }
            ...
            if (!world.collidesWithAnyBlock(npc.getEntityBoundingBox())) {
                ...
                world.setBlockState(new BlockPos(npc.posX + 2.0, this.centerPos.getY() + 2, npc.posZ + 2.0), BlocksRegister.STORM_CONDUCTOR.getDefaultState());
                world.setBlockState(new BlockPos(npc.posX - 2.0, this.centerPos.getY() + 2, npc.posZ + 2.0), BlocksRegister.STORM_CONDUCTOR.getDefaultState());
                world.setBlockState(new BlockPos(npc.posX - 2.0, this.centerPos.getY() + 2, npc.posZ - 2.0), BlocksRegister.STORM_CONDUCTOR.getDefaultState());
                world.setBlockState(new BlockPos(npc.posX + 2.0, this.centerPos.getY() + 2, npc.posZ - 2.0), BlocksRegister.STORM_CONDUCTOR.getDefaultState());
                break;
    ...
    public void placeColumnsAndRacksAtWalls(World world, BlockPos posm, Random rand, EnumFacing facing) {
        ...
        if (result != null) {
            ...
            if (rand.nextFloat() < 0.2F) {
                ...
                GenerationHelper.setChestWithLoot(world, result.pos.offset(facing), EnumChest.STORM, ListLootTable.CHESTS_ZARPION_INTRICACY, facing.getOpposite());
            }
        }
    }
