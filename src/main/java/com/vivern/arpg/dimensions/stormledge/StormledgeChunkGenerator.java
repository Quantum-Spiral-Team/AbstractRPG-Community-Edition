...
public class StormledgeChunkGenerator implements IChunkGenerator {
    ...
    public StormledgeChunkGenerator(World worldIn, long seed) {
        ...
        this.stormsteelOreGen = new CustomOreGenerator(BlocksRegister.ORESTORM_STEEL.getDefaultState(), 104, state -> state.getBlock() == Blocks.STONE || state.getBlock() == BlocksRegister.DOLERITE, 4, 14, 8, 48, 190);
        ...
        if (biome == BiomesRegister.SKY_ISLANDS || biome == BiomesRegister.CRYSTALLIZED_SKY_ISLANDS) {
            ...
            for (int ct = 0; ct < (chanceTrees > 0.0 ? 3 : 1); ct++) {
                if (this.rand.nextFloat() < 0.7 + chanceTrees / 40.0) {
                    BlockPos pos = GetMOP.getTopBlock(this.world, new BlockPos(i + this.rand.nextInt(16) + 8, 254, j + this.rand.nextInt(16) + 8), BlocksRegister.FULMINIFLORA).up();
