...
public class OreGenZinc implements IWorldGenerator {
    ...
    public OreGenZinc(IBlockState oreState, int[] dimensions, Predicate<IBlockState> blocksToReplace, int blockAmountMin, int blockAmountMax, float chancesToSpawn, int minHeight, int maxHeight) {
    ...
    public OreGenZinc(IBlockState oreState, int dimension, Predicate<IBlockState> blocksToReplace, int blockAmountMin, int blockAmountMax, float chancesToSpawn, int minHeight, int maxHeight) {
        this.oreState = oreState;
    ...
    public void runGenerator(IBlockState blockToGen, int blockAmount, float chancesToSpawn, int minHeight, int maxHeight, Predicate<IBlockState> blockToReplace, World world, Random rand, int chunk_X, int chunk_Z) {
        if (minHeight >= 0 && maxHeight <= 256 && minHeight <= maxHeight) {
            ...
            for (float i = chancesToSpawn; i > 0.0F; i--) {
                ...
                for (int r = 0; r < blockAmount; r++) {
                    if (blockToReplace.apply(world.getBlockState(poss))) {
                        if (this.usefalsive && rand.nextFloat() < this.falsivechance) {
                            ...
                            if (rand.nextFloat() < 0.4F) {
                                for (int t = 0; t < 16; t++) {
                                    GenerationHelper.setIfReplaciableOregen(world, poss.add(rand.nextGaussian() * 1.8, 0.0, rand.nextGaussian() * 1.8), blockf, 2, blockToReplace);
                                }
                            } else {
                    ...
                    poss = new BlockPos(MathHelper.clamp(poss.getX() + rand.nextGaussian(), boundXM, boundXP), poss.getY() + rand.nextFloat() * 1.75F, MathHelper.clamp(poss.getZ() + rand.nextGaussian(), boundZM, boundZP));
