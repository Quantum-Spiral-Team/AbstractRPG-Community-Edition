...
public class WorldGenPetroleum implements IWorldGenerator {
    ...
    public static Predicate<IBlockState> defaultBlocksToReplace = apply -> apply.getBlock() != Blocks.BEDROCK && apply.getBlock() != Blocks.BARRIER && apply.getBlock() != Blocks.END_PORTAL_FRAME;
    ...
    public WorldGenPetroleum(IBlockState petrolState, IBlockState wallState, @Nullable IBlockState gasState, int dimension, Predicate<IBlockState> blocksToReplace, int rarity, int radiusMin, int radiusMax, float gasChance, boolean hasGeyser) {
    ...
    public WorldGenPetroleum setOptions(double noiseScaleFrom, double noiseScaleAdd, double boundFrom, double boundAdd, double randMultFrom, double randMultAdd, double noiseMultFrom, double noiseMultAdd, double flattingFrom, double flattingAdd) {
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
        ...
        if (this.dimension == dim && GenerationHelper.canGenerateDungeonAtChunkPos(chunkX, chunkZ, world.getSeed(), this.rarity, 6)) {
            ...
            if (!centerState.getMaterial().isLiquid() && centerState.getMaterial() != Material.AIR) {
                ...
                runGenerator(this.petrolState, this.wallState, hasGas ? this.gasState : this.petrolState, noiseScale, bound, randMult, noiseMult, flatting, this.blocksToReplace, world, random, poss.getX(), poss.getY(), poss.getZ(), radius, this.noisegenerator3d, cracks, gasHeight, this.hasGeyser);
            }
        }
    }

    public static void runGenerator(IBlockState blockToGen, IBlockState wall, IBlockState gas, double noiseScale, double bound, double randMult, double noiseMult, double flatting, Predicate<IBlockState> blockToReplace, World world, Random rand, int x, int y, int z, int radius, NoiseGenerator3D generator3d, int cracks, int gasHeight, boolean hasGeyser) {
        for (int xx = -radius; xx <= radius; xx++) {
            for (int yy = -radius; yy <= radius; yy++) {
                for (int zz = -radius; zz <= radius; zz++) {
                    ...
                    double weight2 = generator3d.getValue(pos.getX() * noiseScale, pos.getY() * noiseScale, pos.getZ() * noiseScale) * noiseMult;
        ...
        for (int xx = -radius; xx <= radius; xx++) {
            for (int yy = -radius; yy <= radius; yy++) {
                for (int zzx = -radius; zzx <= radius; zzx++) {
                    ...
                    if ((state.getBlock() == blockToGen.getBlock() || state.getBlock() == gas.getBlock()) && collidesWithSoftBlock(world, pos, blockToGen.getBlock(), gas.getBlock())) {
    ...
    public static boolean collidesWithSoftBlock(World world, BlockPos pos, Block toGen, Block gas) {
        return isSoft(world.getBlockState(pos.up()), toGen, gas) || isSoft(world.getBlockState(pos.down()), toGen, gas) || isSoft(world.getBlockState(pos.west()), toGen, gas) || isSoft(world.getBlockState(pos.south()), toGen, gas) || isSoft(world.getBlockState(pos.north()), toGen, gas) || isSoft(world.getBlockState(pos.east()), toGen, gas);
    }
