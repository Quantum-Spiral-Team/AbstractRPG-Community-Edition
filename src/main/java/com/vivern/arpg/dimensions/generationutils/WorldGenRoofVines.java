...
public class WorldGenRoofVines implements IWorldGenerator {
    ...
    public WorldGenRoofVines(IBlockState vineState, int chance, int dimension, int minStartY, int maxStartY, int minVineLength, int maxVineLength, int attempts, int attemptsXZSize) {
            ...
            if (topresult != null) {
                ...
                if (this.attempts > 1) {
                    for (int i = 0; i < this.attempts; i++) {
                        GetMOP.BlockTraceResult result2 = GetMOP.blockTrace(world, topresult.pos.add(random.nextGaussian() * this.attemptsXZSize, 2.0, random.nextGaussian() * this.attemptsXZSize), EnumFacing.DOWN, 4, GetMOP.AIR_BLOCKS);
                        if (result2 != null) {
