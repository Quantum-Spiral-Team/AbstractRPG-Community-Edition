...
public class WorldGenAncientRuins extends WorldGenerator {
    ...
    public WorldGenAncientRuins(IBlockState[] bricks, @Nullable IBlockState column, IBlockState columnBorder, IBlockState slab, IBlockState cobble, int ruinsSizeMin, int ruinsSizeMax, int lengthMin, int lengthMax, int bricksHeightMin, int bricksHeightMax, int columnHeightMin, int columnHeightMax) {
        this.bricks = bricks;
        this.column = column;
    ...
    public boolean generate(World world, Random rand, BlockPos position) {
        ...
        this.next(world, rand, position, EnumFacing.HORIZONTALS[rand.nextInt(4)], this.lengthMin + rand.nextInt(this.lengthMax - this.lengthMin + 1), bricksHeight, columnHeight, this.sizelast);
        return true;
    }
                ...
                if (rand.nextFloat() < 0.8) {
                    if (rand.nextFloat() < 0.8) {
                        for (int i = 0; i < Math.max(rand.nextInt(height - y), 1); i++) {
                            if (this.column == null) {
                                this.setBrick(world, rand, GetMOP.getTrueHeight(world, position.add(rand.nextGaussian(), y, rand.nextGaussian())).up(), 0.0F);
                            } else {
                                world.setBlockState(GetMOP.getTrueHeight(world, position.add(rand.nextGaussian(), y, rand.nextGaussian())).up(), this.column, 2);
                            }
                        }
                    }

                    world.setBlockState(GetMOP.getTrueHeight(world, position.add(rand.nextGaussian(), y, rand.nextGaussian())).up(), this.columnBorder, 2);
                }
