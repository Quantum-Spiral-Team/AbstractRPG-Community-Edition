...
public class WorldGenSpread extends WorldGenerator {
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        for (int i = 0; i < this.samples; i++) {
            BlockPos blockpos = position.add(rand.nextInt(this.sizeXZ) - rand.nextInt(this.sizeXZ), this.sizeY == 0 ? 0 : rand.nextInt(this.sizeY) - rand.nextInt(this.sizeY), rand.nextInt(this.sizeXZ) - rand.nextInt(this.sizeXZ));
            if ((this.nocheckAir || worldIn.isAirBlock(blockpos)) && blockpos.getY() < 255 && this.block.canPlaceBlockAt(worldIn, blockpos) && (this.blockGround == null || worldIn.getBlockState(blockpos.down()).getBlock() == this.blockGround)) {
