...
public class WorldGenCoral extends WorldGenerator {
                                    ...
                                    for (EnumFacing facing : EnumFacing.VALUES) {
                                        if (rand.nextFloat() < 0.85F) {
                                            ...
                                            if (block == Blocks.WATER || block == Blocks.FLOWING_WATER) {
                                                worldIn.setBlockState(finalposx, rand.nextFloat() < diversity ? this.randomMinicoral(rand, true, facing) : mainMiniCoral.withProperty(MiniCoral.WET, true).withProperty(MiniCoral.FACING, facing), 2);
                                            } else if (worldIn.isAirBlock(finalposx)) {
                                                worldIn.setBlockState(finalposx, rand.nextFloat() < diversity ? this.randomMinicoral(rand, false, facing) : mainMiniCoral.withProperty(MiniCoral.WET, false).withProperty(MiniCoral.FACING, facing), 2);
                                            }
                                        }
                                    }
                                }
    ...
    public IBlockState randomBlock(Random rand) {
        return this.mainCoralPrioriry != null && rand.nextFloat() > this.diversityModifier ? this.mainCoralPrioriry.getDefaultState() : AquaticaChunkGenerator.corals[rand.nextInt(5)].getDefaultState();
    }
    ...
    public IBlockState randomMainMinicoral(Random rand) {
        if (rand.nextFloat() < 0.04) {
            ...
            return rand.nextFloat() < 0.02 ? AquaticaChunkGenerator.corallimorphas[rand.nextInt(7)].getDefaultState() : AquaticaChunkGenerator.coralsMini[rand.nextInt(7)].getDefaultState();
        }
    }
    ...
    public IBlockState randomMinicoral(Random rand, boolean wet, EnumFacing facing) {
        if (rand.nextFloat() < 0.18 * this.glowingModifier) {
            ...
            return AquaticaChunkGenerator.corallimorphas[rand.nextInt(7)].getDefaultState().withProperty(MiniCoral.WET, wet).withProperty(MiniCoral.FACING, facing);
        } else {
            return rand.nextFloat() < 0.27 ? AquaticaChunkGenerator.coralsBig[rand.nextInt(5)].getDefaultState().withProperty(MiniCoral.WET, wet).withProperty(MiniCoral.FACING, facing) : AquaticaChunkGenerator.coralsMini[rand.nextInt(7)].getDefaultState().withProperty(MiniCoral.WET, wet).withProperty(MiniCoral.FACING, facing);
        }
    }
