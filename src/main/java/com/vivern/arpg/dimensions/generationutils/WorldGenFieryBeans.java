...
public class WorldGenFieryBeans implements IWorldGenerator {
    ...
    public void runGenerator(int height, World world, Random rand, BlockPos pos, boolean checkReplace) {
        ...
        GenerationHelper.placeStruct(world, pos.up(y), rand, ":fiery_beans_tree_" + (rand.nextInt(4) + 1), 5, 0, rand.nextInt(4), ReplaceOnlyReplaceable.instance);
        ...
        for (int y2 = y + 2; y2 > 3; y2--) {
            if (rand.nextFloat() < y2 / 25.0F) {
                ...
                if (world.getBlockState(poss).getBlock().isReplaceable(world, poss) && world.getBlockState(poss.up()).getBlock() != BlocksRegister.FIERY_BEAN_BLOCK) {
                    world.setBlockState(poss, BlocksRegister.FIERY_BEAN_BLOCK.getDefaultState().withProperty(FieryBeanBlock.FACING, f).withProperty(FieryBeanBlock.AGE, rand.nextInt(3)));
                }
            }
        }
    }
