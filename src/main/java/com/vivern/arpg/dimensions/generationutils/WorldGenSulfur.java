...
public class WorldGenSulfur implements IWorldGenerator {
    ...
    public WorldGenSulfur(IBlockState oreState, IBlockState oreState2, float chanceToAir, int[] dimensions, Predicate<IBlockState> blocksToReplace, int blockAmountMin, int blockAmountMax, float chancesToSpawn, int minHeight, int maxHeight) {
    ...
    public WorldGenSulfur(IBlockState oreState, IBlockState oreState2, float chanceToAir, int dimension, Predicate<IBlockState> blocksToReplace, int blockAmountMin, int blockAmountMax, float chancesToSpawn, int minHeight, int maxHeight) {
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
        ...
        for (int succdim : this.dimensions) {
            if (succdim == dim) {
                this.runGenerator(dim, random.nextInt(this.blockAmountMax - this.blockAmountMin + 1) + this.blockAmountMin, this.chancesToSpawn, this.minHeight, this.maxHeight, this.blocksToReplace, world, random, chunkX, chunkZ);
            }
        }
    }

    public void runGenerator(int dim, int blockAmount, float chancesToSpawn, int minHeight, int maxHeight, Predicate<IBlockState> blockToReplace, World world, Random rand, int chunk_X, int chunk_Z) {
    ...
    public static boolean collidesWithBlock(World world, BlockPos pos, Predicate<IBlockState> predicate) {
        return predicate.apply(world.getBlockState(pos.up())) || predicate.apply(world.getBlockState(pos.down())) || predicate.apply(world.getBlockState(pos.west())) || predicate.apply(world.getBlockState(pos.south())) || predicate.apply(world.getBlockState(pos.north())) || predicate.apply(world.getBlockState(pos.east()));
    }
