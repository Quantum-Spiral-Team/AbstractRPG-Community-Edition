...
public class HorribleVillage {
    ...
    public static Predicate<IBlockState> BLOCKS_TO_COLLIDE = input -> input.getBlock() == Blocks.PLANKS || input.getMaterial() != Material.AIR && input.getMaterial() != Material.LEAVES && input.getMaterial() != Material.PLANTS && input.getMaterial() != Material.CACTUS && input.getMaterial() != Material.VINE && input.getMaterial() != Material.SNOW && input.getMaterial() != Material.WOOD;
    ...
    public World world;
    public IBlockState[] roadBlocks = new IBlockState[]{Blocks.DIRT.getStateFromMeta(0), Blocks.DIRT.getStateFromMeta(1), Blocks.DIRT.getStateFromMeta(2), Blocks.GRASS_PATH.getDefaultState(), Blocks.GRASS_PATH.getDefaultState(), Blocks.GRAVEL.getDefaultState()};
    public Random rand;
    public static NamedInt[] structuresHomes = new NamedInt[]{new NamedInt(":horrible_village_tower", 4), new NamedInt(":horrible_village_scaffold", 4), new NamedInt(":horrible_village_home_1", 7), new NamedInt(":horrible_village_home_2", 7), new NamedInt(":horrible_village_home_3", 7), new NamedInt(":horrible_village_horses", 4), new NamedInt(":horrible_village_silverfish", 4), new NamedInt(":horrible_village_shop", 5)};
    public static NamedInt[] structuresRoad = new NamedInt[]{new NamedInt(":horrible_village_lantern", 1), new NamedInt(":horrible_village_well", 2), new NamedInt(":horrible_village_hay", 2), new NamedInt(":horrible_village_haysmall", 1), new NamedInt(":horrible_village_bones", 2), new NamedInt(":horrible_village_coal", 2), new NamedInt(":horrible_village_cart", 3)};
            ...
            if (result != null) {
                if ((this.rand.nextFloat() < 0.95F + Debugger.floats[7] || this.noHomesTime > 0) && last > 0) {
                        ...
                        if (this.rand.nextFloat() < 0.35F + Debugger.floats[4]) {
                            ...
                            if ((this.rand.nextFloat() < 0.8 + Debugger.floats[6] || this.noHomesTime > 0) && this.outpathesLast > 0) {
                                ...
                                if (result2 != null && this.isClear(result2.pos, structure.value)) {
                                    ...
                                    GenerationHelper.placeStruct(this.world, result2.pos, this.rand, structure.name, structure.value, -1, rotation, replacerHorribleVillage);
                    ...
                    if (this.isClear(finalpos, structure.value)) {
                        GenerationHelper.placeStruct(this.world, finalpos, this.rand, structure.name, structure.value, -1, face.getHorizontalIndex(), replacerHorribleVillage);
                ...
                if (x * x + z * z <= sizeSq) {
                    ...
                    if (hasState.getBlock() != Blocks.PLANKS) {
                        if (hasState.getMaterial().isLiquid()) {
                            this.world.setBlockState(poss, this.rand.nextFloat() < 0.15F ? Blocks.DIRT.getStateFromMeta(2) : Blocks.PLANKS.getStateFromMeta(1), 2);
