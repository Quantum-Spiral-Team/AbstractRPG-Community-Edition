...
public class WorldGenMetallicCoral extends WorldGenerator {
    ...
    public IBlockState randomMinicoral(Random rand, boolean wet, EnumFacing facing) {
        if (rand.nextFloat() < 0.02) {
            ...
            return rand.nextFloat() < 0.13 ? BlocksRegister.GORGONARIA_WHITE.getDefaultState().withProperty(MiniCoral.WET, wet).withProperty(MiniCoral.FACING, facing) : AquaticaChunkGenerator.coralsMini[rand.nextInt(7)].getDefaultState().withProperty(MiniCoral.WET, wet).withProperty(MiniCoral.FACING, facing);
        }
    }
