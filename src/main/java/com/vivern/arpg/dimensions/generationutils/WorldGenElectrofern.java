...
public class WorldGenElectrofern extends WorldGenerator {
            ...
            if (length - 3 > 0) {
                ...
                for (int i = length - 3; i > 0; i -= 2) {
                    if (shouldIncrementSize) {
                        this.shootTip(world, position.offset(facing, i), leftDirection, tipSize, leftLeafState, facing.getOpposite(), backState, facing, forwardState);
                    } else {
                        this.shootTip(world, position.offset(facing, i), rightDirection, tipSize, rightLeafState, facing, forwardState, facing.getOpposite(), backState);
                    }
    ...
    public void shootTip(World world, BlockPos position, EnumFacing direction, int length, IBlockState tipState, EnumFacing leftDirection, IBlockState leftState, EnumFacing rightDirection, IBlockState rightState) {
        boolean left = false;
    ...
    public IBlockState rotateFernAround(EnumFacing facing, boolean rotated, Axis axis, boolean clockwise) {
        if (axis == Axis.Y) {
            return facing != EnumFacing.UP && facing != EnumFacing.DOWN ? this.leafState(clockwise ? facing.rotateY() : facing.rotateYCCW(), rotated) : this.leafState(facing, !rotated);
        } else if (axis == Axis.Z) {
