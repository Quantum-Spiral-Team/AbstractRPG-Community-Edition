    ...
    public void generateCrystalIsland(Random rand, Vec3d pos, int crystalType, float scaleFactor) {
        ...
        if (crystalType == 0) {
            ...
            for (int i = 0; i < crystalCount; i++) {
                ...
                Vec3d tip = this.setLargeOctahedronCrystal(pos.add(direction.scale(radius)), direction, blockStone.getDefaultState(), radius, (int) ((this.lengthMin + rand.nextInt(this.lengthRandAdd)) * scaleFactor));
        ...
        if (crystalType == 1) {
            ...
            for (int i = 0; i < crystalCount; i++) {
                ...
                Vec3d directionx = new Vec3d(this.perlin1.getValue(scaledPosX, scaledPosZ), this.perlin2.getValue(scaledPosX, scaledPosZ), this.perlin3.getValue(scaledPosX, scaledPosZ)).normalize();
            ...
            for (int y = 1; radius2 >= 0; y++) {
                for (int x = -radius2; x <= radius2; x++) {
                    for (int z = -radius2; z <= radius2; z++) {
                        this.setBlockState(blockpos$pooledmutableblockpos.setPos(pos.x + x, pos.y + y, pos.z + z), state, 2);
                    }
                }
