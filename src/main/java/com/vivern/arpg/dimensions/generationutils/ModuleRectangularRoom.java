...
public class ModuleRectangularRoom extends Module {
    ...
    public ModuleRectangularRoom(ModularStructureGenerator generator, int radiusX, int radiusZ, int height, int wallThickness, int floorThickness, int roofThickness, boolean floorOverWall, boolean roofOverWall) {
        super(generator);
    ...
    public void generate(BlockPos sourcePos, EnumFacing facing, Module sourceModule, int age) {
        if (!this.generator.cannotGenerate(this, sourcePos, facing, sourceModule, age)) {
            BlockPos center = this.enableOffsetFromSource ? sourcePos.offset(facing, facing.getAxis() == Axis.X ? this.radiusX + 1 : (facing.getAxis() == Axis.Z ? this.radiusZ + 1 : this.height)) : sourcePos;
            ...
            this.generator.addToGenerationQueue(this.onfloorCenterModule, center.up(this.floorThickness), this.onfloorCenterModuleTurnsUP ? EnumFacing.UP : facing, this, age + 1);
            this.generator.addToGenerationQueue(this.onroofCenterModule, center.up(this.height), this.onroofCenterModuleTurnsUP ? EnumFacing.UP : facing, this, age + 1);
    ...
    public AxisAlignedBB getBoundingBox(BlockPos sourcePos, EnumFacing facing, Module sourceModule, int age) {
        BlockPos center = sourcePos.offset(facing, facing.getAxis() == Axis.X ? this.radiusX + 1 : (facing.getAxis() == Axis.Z ? this.radiusZ + 1 : this.height));
        return new AxisAlignedBB((double) center.getX() - this.radiusX, center.getY(), (double) center.getZ() - this.radiusZ, center.getX() + 1 + this.radiusX, center.getY() + this.height, center.getZ() + 1 + this.radiusZ);
    }
