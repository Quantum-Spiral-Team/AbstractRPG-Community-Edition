...
public class OreGenChromium implements IWorldGenerator {
    ...
    public OreGenChromium(IBlockState oreState, int[] dimensions, Predicate<IBlockState> blocksToReplace, int blockAmountMin, int blockAmountMax, int chancesToSpawn, int minHeight, int maxHeight) {
    ...
    public OreGenChromium(IBlockState oreState, int dimension, Predicate<IBlockState> blocksToReplace, int blockAmountMin, int blockAmountMax, int chancesToSpawn, int minHeight, int maxHeight) {
        this.oreState = oreState;
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
        ...
        for (int succdim : this.dimensions) {
            if (succdim == dim) {
                this.runGenerator(this.oreState, random.nextInt(this.blockAmountMax - this.blockAmountMin + 1) + this.blockAmountMin, this.chancesToSpawn, this.minHeight, this.maxHeight, this.blocksToReplace, world, random, chunkX, chunkZ);
            }
        }
    }
            ...
            for (int i = 0; i < chancesToSpawn; i++) {
                ...
                if (!world.isAirBlock(new BlockPos(x, y, z))) {
                    ...
                    if (res != null) {
                        ...
                        if (this.geode && rand.nextFloat() < this.geodechance && this.checkblock(world.getBlockState(res.pos.up())) && this.checkblock(world.getBlockState(res.pos.down())) && this.checkblock(world.getBlockState(res.pos.north())) && this.checkblock(world.getBlockState(res.pos.south())) && this.checkblock(world.getBlockState(res.pos.west())) && this.checkblock(world.getBlockState(res.pos.east())) && this.checkblock(world.getBlockState(res.pos))) {
                ...
                if (!generated) {
                    ...
                    if (!world.isAirBlock(fpos)) {
                        ...
                        if (this.geode && rand.nextFloat() < this.geodechance && this.checkblock(world.getBlockState(fpos.up())) && this.checkblock(world.getBlockState(fpos.down())) && this.checkblock(world.getBlockState(fpos.north())) && this.checkblock(world.getBlockState(fpos.south())) && this.checkblock(world.getBlockState(fpos.west())) && this.checkblock(world.getBlockState(fpos.east())) && this.checkblock(world.getBlockState(fpos))) {
