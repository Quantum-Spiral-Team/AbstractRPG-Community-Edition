...
public class WorldGenMushroomSpawner implements IWorldGenerator {
    ...
    public void generate(Random rand, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
        if (this.dimension == world.provider.getDimension() && rand.nextFloat() < this.chance * 4.0F) {
            ...
            boolean hasBiomeDic = BiomeDictionary.hasType(biome, Type.MUSHROOM) || BiomeDictionary.hasType(biome, Type.DENSE) && BiomeDictionary.hasType(biome, Type.SPOOKY) && BiomeDictionary.hasType(biome, Type.FOREST);
            if (hasBiomeDic || rand.nextFloat() < 0.25F) {
                ...
                if (pos.getY() > 15 && world.getBlockState(pos.up()).getBlock().isReplaceable(world, pos.up()) && !GetMOP.FLUID_BLOCKS.apply(world.getBlockState(pos.up()))) {
                ...
                if (tileentity instanceof TileEntityChest) {
                    ((TileEntityChest) tileentity).setLootTable(ListLootTable.CHESTS_MUSHROOM_TREASURE, GetMOP.getBestRandomBasedOnCoordinates(pos.getX(), pos.getZ(), world.getSeed()).nextLong());
                }
