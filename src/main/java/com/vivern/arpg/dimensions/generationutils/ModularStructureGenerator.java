...
public class ModularStructureGenerator extends WorldGenAdvanced {
    ...
    public static ModularStructureGenerator generator_niveous_hall = new ModularStructureGenerator(generator -> {
        int floorThick = 2;
        int wallThick = 1;
        int roofThick = 2;
        ...
        spawner.interfaceOn = (sourcePos, iblockstate, facing, sourceModule, age) -> DimensionEthernalFrost.setupRandomSpawner(null, generator.getAccess().getTileEntity(sourcePos), DimensionEthernalFrost.EnumEverfrostSpawner.NIVEOUS_HALL, generator.rand);
        ...
        ModulePlaceMultiblock holes = new ModulePlaceMultiblock(generator, true).add(holeSt, 0, -2, 0).add(holeSt, 1, -2, 0).add(holeSt, 1, -2, 1).add(holeSt, 0, -2, 1).add(pilaSt, 0, 6, 0).add(pilaSt, 1, 6, 0).add(pilaSt, 1, 6, 1).add(pilaSt, 0, 6, 1).add(iceeSt, 0, 7, 0).add(iceeSt, 1, 7, 0).add(iceeSt, 1, 7, 1).add(iceeSt, 0, 7, 1);
        ...
        doorSmall.queue = 1;
        int[][] doorProfile2 = new int[][]{{1, 1, 1, 1, 4}, {0, 0, 0, 0, 3}, {0, 0, 0, 0, 3}, {0, 0, 0, 0, 3}, {0, 0, 0, 0, 3}, {0, 0, 0, 0, 3}, {0, 0, 0, 0, 3}, {0, 0, 0, 0, 3}};
        ...
        ModuleIf.IModuleCondition conditionCollideBrick = (msgenerator, sourcePos, facing, sourceModule, age) -> msgenerator.isBoxHasIdAt(sourcePos.offset(facing, 3), 2);
        ModuleIf.IModuleCondition conditionEndOfGeneration = (msgenerator, sourcePos, facing, sourceModule, age) -> !msgenerator.isBoxHasIdAt(sourcePos.offset(facing, 3), 2) && age >= 5;
        perimeterRoomNext.nextRoom = new ModuleArray(generator, perimeterRoomNext, new ModuleIf(generator, doorBig, conditionCollideBrick), new ModuleIf(generator, doorSmall, conditionEndOfGeneration));
    ...
    public void addToGenerationQueue(Module module, BlockPos sourcePos, EnumFacing facing, Module sourceModule, int age) {
        if (module.canBeQueued) {
            ...
            ModularGenerationInstance instance = new ModularGenerationInstance(module, sourcePos, facing, sourceModule, age);
    ...
    public void onEndGenerate(Module module, BlockPos sourcePos, EnumFacing facing, Module sourceModule, int age) {
        ...
        if (module.addHitboxToCollisionList) {
            BoundingBoxWithId boxWithId = new BoundingBoxWithId(module.getBoundingBox(sourcePos, facing, sourceModule, age), module.boundingBoxId);
    ...
    public BoundingBoxWithId getFirstBoxAt(BlockPos pos) {
        for (BoundingBoxWithId hit : this.collisionList) {
            if (hit.aabb.intersects(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 1, pos.getZ() + 1)) {
                return hit;
            }
        }
    ...
    public boolean isBoxHasIdAt(BlockPos pos, int idTo) {
        for (BoundingBoxWithId hit : this.collisionList) {
            if (hit.aabb.intersects(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 1, pos.getZ() + 1) && hit.id == idTo) {
                return true;
            }
        }
