...
public class WorldGenMagmaBloom implements IWorldGenerator {
    ...
    public WorldGenMagmaBloom(IBlockState oreState, int[] dimensions, Predicate<IBlockState> blocksGround, int blockAmountMin, int blockAmountMax, float chancesToSpawn, int minHeight, int maxHeight, int maxMeta) {
    ...
    public WorldGenMagmaBloom(IBlockState oreState, int dimension, Predicate<IBlockState> blocksGround, int blockAmountMin, int blockAmountMax, float chancesToSpawn, int minHeight, int maxHeight, int maxMeta) {
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
        ...
        for (int succdim : this.dimensions) {
            if (succdim == dim) {
                this.runGenerator(this.oreState, random.nextInt(this.blockAmountMax - this.blockAmountMin + 1) + this.blockAmountMin, this.chancesToSpawn, this.minHeight, this.maxHeight, world, random, chunkX, chunkZ);
            }
        }
    }

    public void runGenerator(IBlockState blockToGen, int blockAmount, float chancesToSpawn, int minHeight, int maxHeight, World world, Random rand, int chunk_X, int chunk_Z) {
