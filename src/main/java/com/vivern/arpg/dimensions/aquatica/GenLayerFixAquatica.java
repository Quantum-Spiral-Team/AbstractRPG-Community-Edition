...
public class GenLayerFixAquatica {
    ...
    public static GenLayer[] initializeAllBiomeGenerators(long seed, WorldType worldType, ChunkGeneratorSettings genSettings) {
        ...
        genlayer = new GenLayerRandomArrayPoint(1L, genlayer, Biomes.OCEAN, new Biome[]{BiomesRegister.CORALS, BiomesRegister.SEA_BOTTOM, BiomesRegister.SEAWEED_BAY}, 1);
    ...
    public static class GenLayerRiverMix extends GenLayer {
        ...
        public int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight) {
            ...
            for (int i = 0; i < areaWidth * areaHeight; i++) {
                if (aint[i] == Biome.getIdForBiome(BiomesRegister.CORALS) || aint[i] == Biome.getIdForBiome(BiomesRegister.SEA_BOTTOM) || aint[i] == Biome.getIdForBiome(BiomesRegister.SEAWEED_BAY) || aint[i] == Biome.getIdForBiome(Biomes.OCEAN) || aint[i] == Biome.getIdForBiome(BiomesRegister.SHALLOW) || aint[i] == Biome.getIdForBiome(BiomesRegister.REEF)) {
                    aint2[i] = aint[i];
        ...
        public int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight) {
            ...
            for (int i = 0; i < areaHeight; i++) {
                for (int j = 0; j < areaWidth; j++) {
                    ...
                    } else if (k != Biome.getIdForBiome(Biomes.OCEAN) && k != Biome.getIdForBiome(Biomes.DEEP_OCEAN) && k != Biome.getIdForBiome(Biomes.RIVER) && k != Biome.getIdForBiome(Biomes.SWAMPLAND)) {
