...
public class SunkenTown {
    ...
    public void generateSunkenTown(BlockPos pos) {
        ...
        for (BlockPos posroof : this.freeRoofPoses) {
            if ((this.minimumPlatforms > 0 || this.rand.nextFloat() < 0.25F) && this.isPosFree(posroof.up(22)) && !this.world.getBlockState(posroof.up(12)).getMaterial().blocksMovement()) {
    ...
    public void setPlatform(BlockPos pos) {
        ...
        GenerationHelper.placeStruct(this.world, pos, this.rand, ":sunken_town_platform_" + type, 8, yoffset, this.rand.nextInt(4), ReplacersAquatica.replacerSunkenTown);
    ...
    public void setHall(BlockPos pos) {
        ...
        GenerationHelper.placeStruct(this.world, pos, this.rand, ":sunken_town_hall_" + type, 8, yoffset, this.rand.nextInt(4), ReplacersAquatica.replacerSunkenTown);
    ...
    public void setElevator(BlockPos pos) {
        ...
        int yoffset = 0;
        int type = 1;
        GenerationHelper.placeStruct(this.world, pos, this.rand, ":sunken_town_elevator_" + type, 8, yoffset, this.rand.nextInt(4), ReplacersAquatica.replacerSunkenTown);
        this.decorateWalls(pos);
    ...
    public void setTower(BlockPos pos) {
        ...
        GenerationHelper.placeStruct(this.world, pos, this.rand, ":sunken_town_base_" + type, 8, yoffset, this.rand.nextInt(4), ReplacersAquatica.replacerSunkenTown);
        ...
        for (int i = 0; i < yoffset; i++) {
            ...
            GenerationHelper.placeStruct(this.world, posup, this.rand, ":sunken_town_tube_" + typex, 6, 0, this.rand.nextInt(4), ReplacersAquatica.replacerSunkenTown);
        }

        BlockPos posup = pos.up(yoffset * 4 + 10);
        int typex = this.rand.nextInt(6) + 1;
        GenerationHelper.placeStruct(this.world, posup, this.rand, ":sunken_town_tower_" + typex, 6, 0, this.rand.nextInt(4), ReplacersAquatica.replacerSunkenTown);
        this.decorateWalls(pos);
    ...
    public void setCorridor(BlockPos pos, int type, boolean axisZ) {
        ...
        GenerationHelper.placeStruct(this.world, pos, this.rand, ":sunken_town_corridor_" + type, 9, yoffset, axisZ ? (this.rand.nextFloat() < 0.5F ? 1 : 3) : (this.rand.nextFloat() < 0.5F ? 0 : 2), ReplacersAquatica.replacerSunkenTown);
    ...
    public void buildColumn(BlockPos pos, float radius) {
        ...
        int ybound = result.impactState.getBlock() != BlocksRegister.PEARLESCENT_BRICKS && result.impactState.getBlock() != BlocksRegister.PEARLESCENT_PILASTER && result.impactState.getBlock() != BlocksRegister.PEARLESCENT_STAIRS ? 1 : result.pos.getY();
    ...
    public IBlockState getRandomBrick(boolean usecoralBrick) {
        if (this.rand.nextFloat() < 0.9) {
            ...
            return this.rand.nextFloat() < 0.6 ? BlocksRegister.SHELLROCK.getDefaultState() : BlocksRegister.PEARLESCENT_PILASTER.getDefaultState().withProperty(Pilaster.FACING, EnumFacing.byIndex(this.rand.nextInt(6)));
        }
    }
                ...
                if (x == 9 || x == -9 || z == 9 || z == -9) {
                    for (int y = 0; y <= 8; y++) {
                        if (this.rand.nextFloat() < 0.65) {
                            ...
                            if (state.getBlock() == BlocksRegister.SHELLROCK || state.getBlock() == BlocksRegister.CHALKROCK || state.getBlock() == BlocksRegister.STROMATOLITE) {
                                if (poss.getY() > AquaticaChunkGenerator.sealvl) {
                                    ...
                                    this.world.setBlockState(poss, state.getBlock() == BlocksRegister.STROMATOLITE ? BlocksRegister.CHALKROCK.getDefaultState() : BlocksRegister.STROMATOLITE.getDefaultState());
                                }
                            }
                        }
                    }
