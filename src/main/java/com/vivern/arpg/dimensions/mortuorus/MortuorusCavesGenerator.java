...
public class MortuorusCavesGenerator {
    ...
    public void nextgenerate(BlockPos pos, int lastTunnels, Vec3d mainvec) {
        ...
        this.generateTunnel(pos, 60 + this.rand.nextInt(20), GetMOP.rotateVecAroundAxis(mainvec, axisvec, (100.0F + this.rand.nextInt(41)) * 0.017453F), this.rand.nextFloat() < 0.07F, lastTunnels);
        this.generateTunnel(pos, 60 + this.rand.nextInt(20), GetMOP.rotateVecAroundAxis(mainvec, axisvec, (-100.0F - this.rand.nextInt(41)) * 0.017453F), this.rand.nextFloat() < 0.07F, lastTunnels);
    }
    ...
    public void generateTunnel(BlockPos pos, int length, Vec3d vector, boolean canChangeHeight, int lastTunnels) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (this.rand.nextFloat() < 0.02F) {
                ...
                this.generateBranchA(new BlockPos(posX, posY, posZ), GetMOP.rotateVecAroundAxis(vecto, axisvec, (70.0F + this.rand.nextInt(41)) * (this.rand.nextFloat() < 0.5F ? 0.017453F : -0.017453F)), vecto);
            }
            ...
            for (int z = -sizeXZ; z <= sizeXZ; z++) {
                if (this.isInGenRange(pos.getX() + x, pos.getZ() + z)) {
                    for (int y = -sizeXZ; y <= sizeXZ; y++) {
                        double noiseValue = Math.max(this.noise3d.getValue(x * noiseScaling + pos.getX(), y * noiseScaling + pos.getY(), z * noiseScaling + pos.getZ()) * noiseMultiplier, -1.0);
