...
public class ToxicBarrel extends BlockBlockHard implements IHasSubtypes {
    ...
    public IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer, EnumHand hand) {
        return placer.getHeldItem(hand).getItem() == Item.getItemFromBlock(BlocksRegister.TOXIC_BARREL) ? this.getDefaultState().withProperty(VARIANT, MathHelper.clamp(NBTHelper.GetNBTint(placer.getHeldItem(hand), "type"), 0, 12)) : this.getDefaultState();
    }
