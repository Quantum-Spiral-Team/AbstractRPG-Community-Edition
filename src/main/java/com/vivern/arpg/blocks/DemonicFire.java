        ...
        if (!world.isRemote) {
            if (entityIn instanceof EntityItem) {
                ...
                if (item.getItem().getItem() == Items.ENCHANTED_BOOK) {
                    ...
                    world.playSound(null, item.getPosition(), Sounds.burn, SoundCategory.BLOCKS, 0.8F, 0.9F + world.rand.nextFloat() / 5.0F);
                    item.setDead();
                    EntityItem dust = new EntityItem(item.world, item.posX, item.posY, item.posZ, new ItemStack(ItemsRegister.MAGIC_POWDER, powder));
        ...
        if (DimensionsRegister.teleporterEVERFROST.isReadyToActivate(worldIn, poss, false)) {
            for (BlockPos mpos : DimensionsRegister.teleporterEVERFROST.membraneConfiguration) {
                ...
                if (tile instanceof TilePortal) {
                    ...
                    portalTile.dimensionToTeleport = worldIn.provider.getDimension() == DimensionsRegister.teleporterEVERFROST.dimensionID ? 0 : DimensionsRegister.teleporterEVERFROST.dimensionID;
                }
            }
