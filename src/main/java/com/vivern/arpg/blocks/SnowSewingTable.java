...
public class SnowSewingTable extends Block {
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (!world.isRemote && hand == EnumHand.MAIN_HAND && state.getValue(READY)) {
            ...
            if (block == Blocks.SNOW_LAYER) {
                ...
                world.setBlockToAir(pos.up());
                world.spawnEntity(new EntityItem(world, pos.getX() + 0.5, pos.getY() + 1.15, pos.getZ() + 0.5, new ItemStack(ItemsRegister.SNOW_CLOTH)));
    ...
    public IBlockState getStateFromMeta(int meta) {
        return meta <= 3 ? this.getDefaultState().withProperty(DAMAGE, meta).withProperty(READY, false) : this.getDefaultState().withProperty(DAMAGE, Math.min(meta - 4, 4)).withProperty(READY, true);
    }
