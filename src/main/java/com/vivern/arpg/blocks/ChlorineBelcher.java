...
public class ChlorineBelcher extends BlockBlockHard {
    ...
    public void randomTick(World world, BlockPos pos, IBlockState state, Random random) {
        if (!world.isRemote && RANDOM.nextFloat() < 0.9 && world.isAnyPlayerWithinRangeAt(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, 16.0)) {
    ...
    public boolean canPlaceBlockOnSide(World worldIn, BlockPos pos, EnumFacing side) {
        ...
        return this.canPlaceBlockAt(worldIn, pos) && (bloc == BlocksRegister.TOXIC_GRASS || bloc == BlocksRegister.TOXIC_DIRT || bloc == BlocksRegister.SLUDGE || bloc == BlocksRegister.JUNK || bloc == BlocksRegister.NUCLEAR_WASTE || bloc == BlocksRegister.TOXIBERRY_LOG || bloc == BlocksRegister.TOXIBERRY_LEAVES);
    }
    ...
    public Vec3d getOffset(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
        ...
        if (face.getAxis() == Axis.X) {
            return new Vec3d(((float) (i >> 16 & 15L) / 15.0F - 0.5) * 0.1, ((float) (i >> 20 & 15L) / 15.0F - 1.0) * 0.5, ((float) (i >> 24 & 15L) / 15.0F - 0.5) * 0.5);
        } else if (face.getAxis() == Axis.Y) {
            return new Vec3d(((float) (i >> 16 & 15L) / 15.0F - 0.5) * 0.5, ((float) (i >> 20 & 15L) / 15.0F - 1.0) * 0.1, ((float) (i >> 24 & 15L) / 15.0F - 0.5) * 0.5);
        } else {
            return face.getAxis() == Axis.Z ? new Vec3d(((float) (i >> 16 & 15L) / 15.0F - 0.5) * 0.5, ((float) (i >> 20 & 15L) / 15.0F - 1.0) * 0.5, ((float) (i >> 24 & 15L) / 15.0F - 0.5) * 0.1) : Vec3d.ZERO;
        }
    }
