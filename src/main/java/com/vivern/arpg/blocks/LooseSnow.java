...
public class LooseSnow extends BlockFalling {
    ...
    public AxisAlignedBB getCollisionBoundingBox(IBlockState blockState, IBlockAccess worldIn, BlockPos pos) {
        ...
        return i == 8 ? SNOW_AABB[8] : new AxisAlignedBB(axisalignedbb.minX, axisalignedbb.minY, axisalignedbb.minZ, axisalignedbb.maxX, i * 0.03F, axisalignedbb.maxZ);
    }
    ...
    public void onEndFalling(World worldIn, BlockPos pos, IBlockState placed, IBlockState replaced) {
        if (replaced.getBlock() == BlocksRegister.LOOSE_SNOW) {
            worldIn.setBlockState(pos, placed.withProperty(LAYERS, Math.min(8, replaced.getValue(LAYERS) + placed.getValue(LAYERS))).withProperty(ISFALLING, false));
        } else {
    ...
    private void checkFallable(World worldIn, BlockPos pos) {
        if (worldIn.getBlockState(pos.down()).getBlock() == BlocksRegister.LOOSE_SNOW && worldIn.getBlockState(pos.down()).getValue(LAYERS) < 8 || (worldIn.isAirBlock(pos.down()) || canFallThrough(worldIn.getBlockState(pos.down()))) && pos.getY() >= 0) {
            int i = 32;
            if (fallInstantly || !worldIn.isAreaLoaded(pos.add(-32, -32, -32), pos.add(32, 32, 32))) {
                ...
                EntityFallingBlock entityfallingblock = new EntityFallingBlock(worldIn, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, worldIn.getBlockState(pos).withProperty(ISFALLING, true));
