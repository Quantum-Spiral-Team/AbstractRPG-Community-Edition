...
public class MetallicCoral extends Block implements IBlockHardBreak {
   ...
   public boolean isBlockConnecting(IBlockState state) {
      if (state.getMaterial() == Material.AIR) {
         ...
         return state.getBlock() != Blocks.WATER
                 ...
                 && state.getBlock() != Blocks.FLOWING_LAVA && !(state.getBlock() instanceof IFluidBlock);
      }
   }
   ...
   public AxisAlignedBB getAABB(IBlockState state) {
      ...
      if (state.getValue(NORTH)) index |= BIT_NORTH;
      if (state.getValue(EAST))  index |= BIT_EAST;
      if (state.getValue(SOUTH)) index |= BIT_SOUTH;
      if (state.getValue(WEST))  index |= BIT_WEST;
      if (state.getValue(UPPER)) index |= BIT_UPPER;
      if (state.getValue(DOWN))  index |= BIT_DOWN;
   ...
   public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
      IBlockState state = this.getDefaultState()
              ...
              .withProperty(WET, isInWater(worldIn, pos));
