...
public class BlockSpawner extends BlockBlock implements IBlockHardBreak {
    ...
    public ParticleTracker.TrackerSmoothShowHide ssh = new ParticleTracker.TrackerSmoothShowHide(null, new Vec3d[]{new Vec3d(0.0, 5.0, 0.12), new Vec3d(5.0, 10.0, -0.12)});
    ...
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
        ...
        if (this == BlocksRegister.RUSTED_SPAWNER) {
            ...
            worldIn.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, xx, yy, zz, rand.nextGaussian() / 26.0, rand.nextGaussian() / 26.0, rand.nextGaussian() / 26.0, new int[0]);
            if (rand.nextFloat() < 0.5) {
                GUNParticle spelll = new GUNParticle(circle, 0.05F, 0.0F, 10, 240, worldIn, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, 0.0F, 0.1F, 0.0F, 0.2F + rand.nextFloat() / 10.0F, 0.7F, 1.0F, true, 0);
                spelll.isPushedByLiquids = false;
        ...
        if (this == BlocksRegister.ANCIENT_SPAWNER) {
            ...
            GUNParticle spelll = new GUNParticle(magic_effect_3, 0.3F + rand.nextFloat() * 0.25F, -8.0E-4F - rand.nextFloat() * 6.0E-4F, lt, 240, worldIn, xx, yy, zz, 0.0F, 0.0F, 0.0F, 0.9F + rand.nextFloat() / 10.0F, 0.9F + rand.nextFloat() / 10.0F, 0.9F + rand.nextFloat() / 10.0F, true, 0);
        ...
        if (this == BlocksRegister.AQUATIC_SPAWNER) {
            ...
            GUNParticle spelll = new GUNParticle(mana_flow, 0.25F, 0.0F, livetime, 210, worldIn, d0, d1, d2, 0.0F, 0.0F, 0.0F, 0.5F + rand.nextFloat() * 0.1F, 1.0F, 0.85F + rand.nextFloat() * 0.15F, true, 0);
            spelll.alpha = 1.0F;
        ...
        if (this == BlocksRegister.STORM_SPAWNER) {
            ...
            double d1 = pos.getY() + 0.5;
            double d2 = pos.getZ() + 0.5;
            EntityCubicParticle spel = new EntityCubicParticle(clouds4, 0.032F, 0.0F, 40 + rand.nextInt(10), 240, worldIn, d0, d1, d2, 0.0F, 0.0F, 0.0F, 0.9F + rand.nextFloat() * 0.1F, 0.9F + rand.nextFloat() * 0.1F, 1.0F, true, rand.nextFloat(), rand.nextFloat(), rand.nextFloat(), 0.15F, true, 0.0F);
            ...
            Vec3d pos1 = new Vec3d(pos.getX() + 0.5 + face.getXOffset() * 0.5 + face2.getXOffset() * offset, pos.getY() + rand.nextFloat(), pos.getZ() + 0.5 + face.getZOffset() * 0.5 + face2.getZOffset() * offset);
            Vec3d pos2 = new Vec3d(pos.getX() + 0.5 + face.getXOffset() * 2.5 + face2.getXOffset() * offset * 3.0 + rand.nextGaussian(), pos.getY() - 2, pos.getZ() + 0.5 + face.getZOffset() * 2.5 + face2.getZOffset() * offset * 3.0 + rand.nextGaussian());
            ...
            ParticleFastSummon.coloredLightning(worldIn, 0.03F, 4, 240, 0.9F + rand.nextFloat() * 0.1F, 0.9F + rand.nextFloat() * 0.1F, 1.0F, 5, pos1, pos3, pos1.squareDistanceTo(pos3) > 2.0 ? 0.5F : 0.25F, 0.1F, ParticleFastSummon.rand);
        }
    }
