...
public class StormConductor extends BlockRotatedPillar implements IBlockHardBreak {
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
        ...
        if (axis == 1) {
            ...
            return state.withProperty(FORWARD, stateP.getBlock() == this && stateP.getValue(AXIS) != axis).withProperty(BACKWARD, stateN.getBlock() == this && stateN.getValue(AXIS) != axis);
        } else if (axis == 2) {
    ...
    public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
        if (EnumAxis.fromFacingAxis(facing.getAxis()) == EnumAxis.X) {
            ...
            return EnumAxis.fromFacingAxis(facing.getAxis()) == EnumAxis.Z ? this.getStateFromMeta(meta).withProperty(AXIS, 3) : this.getStateFromMeta(meta).withProperty(AXIS, 0);
        }
    }
