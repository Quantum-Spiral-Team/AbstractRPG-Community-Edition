...
public class BlockFluidSlime extends BlockFluidClassic {
    ...
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
        ...
        if (i > 0 && i < 8 && rand.nextInt(64) == 0) {
            worldIn.playSound(d0 + 0.5, d1 + 0.5, d2 + 0.5, Sounds.slime_fluid, SoundCategory.BLOCKS, rand.nextFloat() * 0.25F + 0.75F, rand.nextFloat() + 0.5F, false);
        }

        if (rand.nextInt(130) == 0 && worldIn.getBlockState(pos.up()).getMaterial() == Material.AIR && !worldIn.getBlockState(pos.up()).isOpaqueCube()) {
            double xx = d0 + rand.nextFloat();
            double yy = d1 + stateIn.getBoundingBox(worldIn, pos).maxY + 0.05;
            double zz = d2 + rand.nextFloat();
            GUNParticle spelll = new GUNParticle(res, 0.11F + rand.nextFloat() / 10.0F, -0.0013F, 30 + rand.nextInt(10), -1, worldIn, xx, yy, zz, 0.0F, 0.0F, 0.0F, 0.9F + rand.nextFloat() / 10.0F, 0.87F + rand.nextFloat() / 10.0F, 1.0F, true, 0);
            spelll.scaleTickAdding = -0.002F;
            spelll.isPushedByLiquids = false;
            worldIn.spawnEntity(spelll);
            worldIn.playSound(xx, yy, zz, SoundEvents.BLOCK_LAVA_POP, SoundCategory.BLOCKS, 0.2F + rand.nextFloat() * 0.2F, 0.9F + rand.nextFloat() * 0.15F, false);
        }
        ...
        if (rand.nextInt(10) == 0 && worldIn.getBlockState(pos.down()).isTopSolid()) {
            ...
            if (!material.blocksMovement() && !material.isLiquid()) {
                ...
                double yy = d1 - 1.05;
                double zz = d2 + rand.nextFloat();
                GUNParticle spelll = new GUNParticle(res, 0.09F + rand.nextFloat() / 20.0F, 3.0E-4F, 30 + rand.nextInt(10), light, worldIn, xx, yy, zz, 0.0F, 0.0F, 0.0F, 0.9F + rand.nextFloat() / 10.0F, 0.87F + rand.nextFloat() / 10.0F, 1.0F, true, 0);
