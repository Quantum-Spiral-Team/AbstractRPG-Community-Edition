...
public class ConiferSapling extends Block implements IGrowable {
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
        return worldIn.getBlockState(pos.down()).getBlock() == Blocks.SNOW ? state.withProperty(SNOWY, true) : state.withProperty(SNOWY, false);
    }
        ...
        if (random.nextFloat() < 0.1) {
            ...
            if (blockd == Blocks.SNOW || blockd == Blocks.GRASS || blockd == Blocks.DIRT || blockd == BlocksRegister.SNOW_ICE) {
                ...
                for (int rt = -4; rt < 14; rt++) {
                    ...
                    if (!worldIn.getBlockState(fpos).getBlock().isReplaceable(worldIn, fpos) && worldIn.getBlockState(fpos).getBlock() != Blocks.SNOW) {
