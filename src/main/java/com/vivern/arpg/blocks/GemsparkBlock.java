...
public class GemsparkBlock extends BlockFalling {
    ...
    public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (!world.isRemote) {
            AxisAlignedBB aabbBig = new AxisAlignedBB(pos.getX() - 0.75, pos.getY(), pos.getZ() - 0.75, pos.getX() + 1.75, pos.getY() + 1.5, pos.getZ() + 1.75);
            AxisAlignedBB aabbSmall = new AxisAlignedBB(pos.getX(), pos.getY() + 1, pos.getZ(), pos.getX() + 1, pos.getY() + 1.5, pos.getZ() + 1);
            ...
            for (EntityItem entityItem : world.getEntitiesWithinAABB(EntityItem.class, aabbSmall)) {
                if (entityItem.getItem() != null && entityItem.getItem().getCount() == 1) {
                    EntitySpellForgeCatalyst forgeCatalyst = new EntitySpellForgeCatalyst(world, entityItem.posX, entityItem.posY, entityItem.posZ, entityItem.getItem(), 20);
            ...
            for (EnumFacing face : EnumFacing.HORIZONTALS) {
                if (rand.nextFloat() < 0.4F) {
                    ...
                    if (iBlockState.getMaterial() != Material.FIRE && !iBlockState.getMaterial().isLiquid() && (Weapons.easyBreakBlockFor(world, 1.0F, posoff, iBlockState) || iBlockState.getMaterial() == Material.WOOD && Weapons.easyBreakBlockFor(world, 10.0F, posoff, iBlockState))) {
