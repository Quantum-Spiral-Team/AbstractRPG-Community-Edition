...
public class ToxicomaniaPortal extends Block {
    ...
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
        if (rand.nextInt(50) == 0) {
            worldIn.playSound(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, Sounds.portal_toxic, SoundCategory.BLOCKS, 0.5F, rand.nextFloat() * 0.4F + 0.8F, false);
        }
    }
    ...
    public IBlockState withRotation(IBlockState state, Rotation rot) {
        return rot != Rotation.NONE && rot != Rotation.CLOCKWISE_180 ? state.withProperty(ROTATE, state.getValue(ROTATE) == EnumAxis.Z ? EnumAxis.X : EnumAxis.Z) : state;
    }
