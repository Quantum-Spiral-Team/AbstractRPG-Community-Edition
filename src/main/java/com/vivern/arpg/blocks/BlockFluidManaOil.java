...
public class BlockFluidManaOil extends BlockFluidClassic {
    ...
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
        ...
        if (rand.nextInt(14) == 0 && worldIn.getBlockState(pos.up()).getMaterial() == Material.AIR && !worldIn.getBlockState(pos.up()).isOpaqueCube()) {
            ...
            GUNParticle spelll = new GUNParticle(star2, scl, -0.0015F, lt, 240, worldIn, xx, yy, zz, (float) rand.nextGaussian() / 50.0F, 0.0F, (float) rand.nextGaussian() / 50.0F, 0.2F + rand.nextFloat() * 0.45F, 0.75F * rand.nextFloat(), 1.0F - rand.nextFloat() * 0.35F, true, rand.nextInt(360));
        ...
        if (rand.nextInt(10) == 0 && worldIn.getBlockState(pos.down()).isTopSolid()) {
            ...
            if (!material.blocksMovement() && !material.isLiquid()) {
                ...
                int lt = 30 + rand.nextInt(30);
                float scl = 0.08F + rand.nextFloat() * 0.08F;
                GUNParticle spelll = new GUNParticle(star2, scl, 8.0E-4F, lt, 240, worldIn, xx, yy, zz, 0.0F, 0.0F, 0.0F, 0.2F + rand.nextFloat() * 0.45F, 0.75F * rand.nextFloat(), 1.0F - rand.nextFloat() * 0.35F, true, rand.nextInt(360));
                spelll.scaleTickAdding = -scl / lt;
                spelll.isPushedByLiquids = false;
                spelll.alphaGlowing = true;
                spelll.alpha = 0.0F;
            ...
            for (EnumFacing facing : EnumFacing.values()) {
                ...
                if (block == Blocks.LAVA || block == Blocks.FLOWING_LAVA) {
                    if (full) {
                        ...
                        world.setBlockState(pos, BlocksRegister.SUMMONED_HELLSTONE.getDefaultState());
                    }
