    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (player.getHeldItem(hand).getItem() == ItemsRegister.BUNKER_KEYCARD) {
            ...
            if (!world.isRemote) {
                ...
                EntityItem it = new EntityItem(world, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, new ItemStack(ItemsRegister.VIRULENT_ROD));
                it.motionX = 0.0;
                it.motionZ = 0.0;
                world.spawnEntity(it);
            } else {
                for (int i = 0; i < 6; i++) {
                    EntityCubicParticle spell = new EntityCubicParticle(resou, 0.027F, 0.03F, 40, 80, world, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, (float) RANDOM.nextGaussian() / 13.0F, (float) RANDOM.nextGaussian() / 13.0F + 0.17F, (float) RANDOM.nextGaussian() / 13.0F, 1.0F, 1.0F, 1.0F, false, RANDOM.nextFloat(), RANDOM.nextFloat(), RANDOM.nextFloat(), 0.08F, true, 0.0F);
                    world.spawnEntity(spell);
                    GUNParticle bigSmoke = new GUNParticle(largesmoke, 0.5F + RANDOM.nextFloat(), 3.0E-4F, 45, 130, world, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, (float) RANDOM.nextGaussian() / 14.0F, (float) RANDOM.nextGaussian() / 23.0F, (float) RANDOM.nextGaussian() / 14.0F, 0.4F + RANDOM.nextFloat() * 0.4F, 0.7F + RANDOM.nextFloat() * 0.3F, 0.7F, true, RANDOM.nextInt(360), true, 1.0F);
