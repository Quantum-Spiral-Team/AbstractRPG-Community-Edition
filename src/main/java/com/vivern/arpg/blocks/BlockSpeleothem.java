...
public class BlockSpeleothem extends Block {
    ...
    public boolean canStay(World world, BlockPos pos, IBlockState state) {
        ...
        if (ths == 0) {
            ...
            if (ths == 2) {
                ...
                if (st1.getBlock() != this && (st1.isSideSolid(world, fpos1, EnumFacing.UP) || st1.getBlockFaceShape(world, fpos1, EnumFacing.UP) == BlockFaceShape.CENTER_BIG || st1.getBlockFaceShape(world, fpos1, EnumFacing.UP) == BlockFaceShape.CENTER_SMALL || st1.getBlockFaceShape(world, fpos1, EnumFacing.UP) == BlockFaceShape.CENTER)) {
                    return true;
                } else if (st2.getBlock() == this || !st2.isSideSolid(world, fpos2, EnumFacing.DOWN) && st2.getBlockFaceShape(world, fpos2, EnumFacing.DOWN) != BlockFaceShape.CENTER_BIG && st2.getBlockFaceShape(world, fpos2, EnumFacing.DOWN) != BlockFaceShape.CENTER_SMALL && st2.getBlockFaceShape(world, fpos2, EnumFacing.DOWN) != BlockFaceShape.CENTER) {
    ...
    public Vec3d getOffset(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
        ...
        if (ths == 6) {
            ...
            return new Vec3d(((float) (i >> 16 & 15L) / 15.0F - 0.5) * 0.4, ((float) (i >> 20 & 15L) / 15.0F - 1.0) * 0.4, ((float) (i >> 24 & 15L) / 15.0F - 0.5) * 0.4);
        } else if (ths == 5) {
            long i = MathHelper.getCoordinateRandom(pos.getX(), pos.getY(), pos.getZ());
    ...
    public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
        if (this.canStay(worldIn, pos, state)) {
            ...
            worldIn.playSound(null, pos.getX(), pos.getY(), pos.getZ(), Sounds.speleothem_crack, SoundCategory.BLOCKS, 0.8F, 0.875F + worldIn.rand.nextFloat() / 4.0F);
            if (worldIn instanceof WorldServer) {
                Vec3d vec = this.getOffset(state, worldIn, pos).add(pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5);
                ((WorldServer) worldIn).spawnParticle(EnumParticleTypes.BLOCK_CRACK, vec.x, vec.y + RANDOM.nextFloat(), vec.z, 12, 0.0, 0.0, 0.0, 0.08, new int[]{Block.getStateId(state)});
            }
        }
