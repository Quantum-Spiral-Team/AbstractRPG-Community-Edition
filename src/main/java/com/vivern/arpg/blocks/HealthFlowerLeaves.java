...
public class HealthFlowerLeaves extends Block implements IPlantable {
    ...
    public boolean canStayAtPos(World worldIn, BlockPos pos) {
        ...
        return HealthFlower.isFertileOre(blockd) || blockd == Blocks.STONE || blockd == Blocks.GRAVEL || blockd == Blocks.COBBLESTONE || blockd == Blocks.MOSSY_COBBLESTONE;
    }
    ...
    public static void growForGeneration(World world, BlockPos pos, Random rand) {
        if (!world.isRemote && rand.nextFloat() < 0.4F && world.isAirBlock(pos.up())) {
            world.setBlockState(pos.up(), BlocksRegister.HEALTH_FLOWER.getDefaultState().withProperty(HealthFlower.AGE, rand.nextInt(3)));
        }
    }
    ...
    public static class HealthFlower extends PlantWithStages {
        ...
        public HealthFlower() {
            super(Material.PLANTS, "health_flower", new Block[]{BlocksRegister.HEALTH_FLOWER_LEAVES}, 0.2F, 0.2F, SoundType.PLANT, 2, new AxisAlignedBB[]{GetMOP.newAABB(10, 7, 0), GetMOP.newAABB(12, 10, 0), GetMOP.newAABB(14, 12, 0)}, 0, 5, 0.3F, "arpg:health_berry,1,1,0");
                        ...
                        for (EnumFacing facing : EnumFacing.HORIZONTALS) {
                            if (rand.nextFloat() < 0.5) {
                                for (int i = 0; i >= -2; i--) {
                                    ...
                                    if (worldIn.isAirBlock(poss) && BlocksRegister.HEALTH_FLOWER_LEAVES.canPlaceBlockAt(worldIn, poss) && isFertileOre(worldIn.getBlockState(poss.down()).getBlock())) {
