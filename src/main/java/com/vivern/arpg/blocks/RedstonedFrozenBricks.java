...
public class RedstonedFrozenBricks extends Block implements IBlockHardBreak {
    ...
    public void crackBlock(World worldIn, BlockPos pos, EnumFacing from) {
        ...
        worldIn.playSound(null, pos.getX(), pos.getY(), pos.getZ(), worldIn.rand.nextFloat() < 0.6 ? SoundEvents.BLOCK_STONE_BREAK : SoundEvents.BLOCK_GRAVEL_BREAK, SoundCategory.BLOCKS, 1.0F, 0.85F + worldIn.rand.nextFloat() / 4.0F);
        if (worldIn instanceof WorldServer) {
            ((WorldServer) worldIn).spawnParticle(EnumParticleTypes.BLOCK_DUST, pos.getX(), pos.getY(), pos.getZ(), 15, -0.5, -0.5, -0.5, 0.08, new int[]{Block.getStateId(BlocksRegister.REDSTONED_FROZEN_BRICKS.getDefaultState())});
        }
