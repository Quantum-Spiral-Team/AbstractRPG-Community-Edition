...
public class FrostfireExplosive extends AbstractBomb {
    ...
    public void onExplosionDestroy(World worldIn, BlockPos pos, Explosion explosionIn) {
        if (!worldIn.isRemote) {
            EntityFrostfireExplosive entitytntprimed = new EntityFrostfireExplosive(worldIn, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5F, explosionIn.getExplosivePlacedBy(), basePower);
    ...
    public void blockexploded(World worldIn, BlockPos pos, EntityLivingBase igniter, Entity entityExplosive) {
        if (!worldIn.isRemote) {
            EntityFrostfireExplosive entitytntprimed = new EntityFrostfireExplosive(worldIn, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5F, igniter, basePower);
            entitytntprimed.fuse = 10 + RANDOM.nextInt(10);
            worldIn.spawnEntity(entitytntprimed);
