...
public class BlockFluidDarkness extends BlockFluidClassic {
    ...
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
        ...
        double d1 = pos.getY();
        double d2 = pos.getZ();
        if (rand.nextInt(80) == 0 && worldIn.getBlockState(pos.up()).getMaterial() == Material.AIR && !worldIn.getBlockState(pos.up()).isOpaqueCube()) {
            double xx = d0 + rand.nextFloat();
            double yy = d1 + stateIn.getBoundingBox(worldIn, pos).maxY + 0.05;
            double zz = d2 + rand.nextFloat();
            GUNParticle spelll = new GUNParticle(res, 0.03F + rand.nextFloat() / 15.0F, -0.0014F, 30 + rand.nextInt(30), -1, worldIn, xx, yy, zz, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, false, 0);
        ...
        if (rand.nextInt(30) == 0 && worldIn.getBlockState(pos.down()).isTopSolid()) {
            ...
            if (!material.blocksMovement() && !material.isLiquid()) {
                ...
                double yy = d1 - 1.05;
                double zz = d2 + rand.nextFloat();
                GUNParticle spelll = new GUNParticle(res, 0.015F + rand.nextFloat() / 20.0F, 5.0E-4F, 30 + rand.nextInt(30), -1, worldIn, xx, yy, zz, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, false, 0);
