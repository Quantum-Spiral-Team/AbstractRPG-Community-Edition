        ...
        if (entityIn instanceof EntityLivingBase && !((EntityLivingBase) entityIn).isEntityUndead()) {
            if (entityIn.ticksExisted % 20 == 0 && !worldIn.isRemote && RANDOM.nextInt(3) == 0) {
                ...
                if (health + 0.5 < ((EntityLivingBase) entityIn).getMaxHealth()) {
                    ...
                    worldIn.playSound(null, pos, Sounds.larva_water_attack, SoundCategory.BLOCKS, RANDOM.nextFloat() * 0.25F + 0.55F, RANDOM.nextFloat() * 0.4F + 0.8F);
                }
            }
    ...
    public void effectattack(World world, double x, double y, double z) {
        ...
        for (int i = 0; i < countOfParticles; i++) {
            ...
            if (world.getBlockState(new BlockPos(x + X * 15.0F, y + Y * 15.0F, z + Z * 15.0F)).getBlock() == this) {
                if (RANDOM.nextBoolean()) {
                    GUNParticle spelll = new GUNParticle(reslarva1, 0.029F + RANDOM.nextFloat() / 20.0F, 0.0F, 30 + RANDOM.nextInt(30), -1, world, x + X * 15.0F, y + Y * 15.0F, z + Z * 15.0F, -X / 2.0F, -Y / 2.0F, -Z / 2.0F, 1.0F, 1.0F, 0.8F + RANDOM.nextFloat() / 5.0F, true, RANDOM.nextInt(360));
                    spelll.alpha = 0.0F;
                    spelll.alphaTickAdding = 0.05F;
                    world.spawnEntity(spelll);
                } else {
                    GUNParticle spelll = new GUNParticle(reslarva2, 0.027F + RANDOM.nextFloat() / 20.0F, 0.0F, 30 + RANDOM.nextInt(30), -1, world, x + X * 15.0F, y + Y * 15.0F, z + Z * 15.0F, -X / 2.0F, -Y / 2.0F, -Z / 2.0F, 1.0F, 1.0F, RANDOM.nextFloat(), true, RANDOM.nextInt(360));
                    spelll.alpha = 0.0F;
                    spelll.alphaTickAdding = 0.05F;
                    world.spawnEntity(spelll);
                }
    ...
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
        ...
        if (i > 0 && i < 8) {
            ...
            if (r < 2) {
                ...
                GUNParticle spelll = new GUNParticle(reslarva1, 0.027F + rand.nextFloat() / 20.0F, (float) rand.nextGaussian() / 8000.0F, 50 + rand.nextInt(30), -1, worldIn, d0 + rand.nextFloat(), d1 + rand.nextFloat(), d2 + rand.nextFloat(), 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, 0.8F + rand.nextFloat() / 5.0F, true, rand.nextInt(360));
                spelll.alpha = 0.0F;
                spelll.alphaTickAdding = 0.05F;
                spelll.isPushedByLiquids = false;
                worldIn.spawnEntity(spelll);
            } else {
                GUNParticle spelll = new GUNParticle(reslarva2, 0.025F + rand.nextFloat() / 20.0F, (float) rand.nextGaussian() / 9500.0F, 50 + rand.nextInt(30), -1, worldIn, d0 + rand.nextFloat(), d1 + rand.nextFloat(), d2 + rand.nextFloat(), 0.0F, 0.0F, 0.0F, 1.0F, 1.0F, rand.nextFloat(), true, rand.nextInt(360));
                spelll.alpha = 0.0F;
                spelll.alphaTickAdding = 0.05F;
                spelll.isPushedByLiquids = false;
                worldIn.spawnEntity(spelll);
